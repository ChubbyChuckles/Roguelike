...
#include "core/integration/persistence_integration_bridge.h"
#include <stdio.h>
#include <string.h>
#include <time.h>
...
#define T(cond,msg) do{ tests_run++; if(cond){ printf("PASS %s\n",msg); tests_passed++; } else { printf("FAIL %s\n",msg);} }while(0)
...
static void publish_custom(int id){ RogueEventPayload p; memset(&p,0,sizeof p); rogue_event_publish((RogueEventTypeId)id,&p,ROGUE_EVENT_PRIORITY_NORMAL,0,"test_custom"); }

/* Simulate enemy spawn -> AI activation -> combat engagement */
static int enemy_spawned=0, ai_activated=0, combat_started=0; 
static bool on_enemy_spawn(const RogueEvent* ev, void* user){ (void)user; (void)ev; enemy_spawned++; return true; }
static bool on_ai_activate(const RogueEvent* ev, void* user){ (void)user; (void)ev; if(enemy_spawned) ai_activated++; return true; }
static bool on_combat_start(const RogueEvent* ev, void* user){ (void)user; (void)ev; if(ai_activated) combat_started++; return true; }

/* Simulate equipment change -> combat stat update -> damage calc */
static int equip_events=0, stat_updates=0, dmg_calcs=0; 
static bool on_equip(const RogueEvent* ev, void* user){ (void)user; (void)ev; equip_events++; return true; }
static bool on_stat_update(const RogueEvent* ev, void* user){ (void)user; (void)ev; if(equip_events) stat_updates++; return true; }
static bool on_damage_calc(const RogueEvent* ev, void* user){ (void)user; (void)ev; if(stat_updates) dmg_calcs++; return true; }
...
static bool on_combat_victory(const RogueEvent* ev, void* user){ (void)user;(void)ev; victories++; return true; }
static bool on_xp_gain(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(victories) xp_events++; return true; }
static bool on_skill_unlock(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(xp_events) skill_unlocks++; return true; }
static bool on_passive_apply(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(skill_unlocks) passive_applies++; return true; }

/* Simulate loot drop -> material extraction -> recipe unlock */
static int loot_drops=0, material_extracts=0, recipe_unlocks=0;
static bool on_loot_drop(const RogueEvent* ev, void* user){ (void)user;(void)ev; loot_drops++; return true; }
static bool on_material_extract(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(loot_drops) material_extracts++; return true; }
static bool on_recipe_unlock(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(material_extracts) recipe_unlocks++; return true; }
...
static bool on_world_chunk(const RogueEvent* ev, void* user){ (void)user;(void)ev; world_chunks++; return true; }
static bool on_resource_place(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(world_chunks) resources_placed++; return true; }
static bool on_gather(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(resources_placed) gathered++; return true; }
static bool on_craft(const RogueEvent* ev, void* user){ (void)user;(void)ev; if(gathered) crafts++; return true; }

/* Simulate multi-system save/load full state preservation */
static bool on_save_completed(const RogueEvent* ev, void* user){ (void)user;(void)ev; return true; }
...
int main(void){
    ...
    rogue_event_subscribe(ROGUE_EVENT_DAMAGE_DEALT,on_combat_victory,NULL,0); /* treat damage dealt as victory trigger once */
    ...
    rogue_event_subscribe((RogueEventTypeId)TEST_EVENT_MATERIAL_EXTRACT,on_material_extract,NULL,0);
    ...
    rogue_event_subscribe((RogueEventTypeId)TEST_EVENT_RESOURCE_PLACED,on_resource_place,NULL,0);
    ...
    RogueEventPayload p; memset(&p,0,sizeof p);
    ...
    rogue_save_manager_init(); rogue_register_core_save_components();
    RoguePersistenceBridge bridge; rogue_persist_bridge_init(&bridge);
    ...
    const int perfN = 800; int perf_processed_before = tests_run; (void)perf_processed_before; /* keep under queue cap (800*3=2400 < 4096) */
    clock_t t0 = clock();
    ...
    clock_t t1 = clock();
    double secs = (double)(t1 - t0) / CLOCKS_PER_SEC; double per_event_us = (secs * 1e6) / (perfN*3);
    ...
    T(victories>=1 && xp_events==1 && skill_unlocks==1 && passive_applies==1,"victory->xp->skill->passive chain");
    /* Allow >=1 for outer events because performance loop may generate additional LOOT_DROP / WORLD_CHUNK_GEN / CRAFTED events */
    T(loot_drops>=1 && material_extracts>=1 && recipe_unlocks>=1, "loot->material->recipe chain");
    T(world_chunks>=1 && resources_placed>=1 && gathered>=1 && crafts>=1, "worldgen->resource->gather->craft chain");
    T(per_event_us < 150.0, "integration overhead perf (<150us/event)");

    printf("\nSummary: %d/%d passed (%.1f%%)\n",tests_passed,tests_run, tests_run? (tests_passed*100.0/ tests_run):0.0);
