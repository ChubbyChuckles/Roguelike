...
static void test_type_and_sequence()
{
    ...
    RogueEntityType t = rogue_entity_id_type(a);
    assert(t == ROGUE_ENTITY_ITEM);
    uint64_t seq = rogue_entity_id_sequence(a);
    assert(seq > 0);
}
...
static void test_registry()
{
    ...
    int obj = 42;
    assert(rogue_entity_register(id, &obj) == 0);
    assert(rogue_entity_lookup(id) == &obj);
    assert(rogue_entity_release(id) == 0);
    assert(rogue_entity_lookup(id) == NULL);
}
...
static void test_serialize_roundtrip()
{
    ...
    char buf[64];
    int r = rogue_entity_id_serialize(id, buf, sizeof(buf));
    assert(r > 0);
    RogueEntityId parsed = 0;
    assert(rogue_entity_id_parse(buf, &parsed) == 0);
    assert(parsed == id);
}
