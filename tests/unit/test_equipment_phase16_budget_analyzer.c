...
static void spawn_item_with_affixes(int def_index, int rarity, int item_level, int prefix_val, int suffix_val, int over_budget_adjust){
    int idx = rogue_items_spawn(def_index, 1, 0.f, 0.f);
    if(idx<0){ printf("spawn failed\n"); exit(1); }
    /* Apply baseline */
    ...
    if(prefix_val>0){ it->prefix_index = 0; it->prefix_value = prefix_val; }
    if(suffix_val>0){ it->suffix_index = 1; it->suffix_value = suffix_val; }
    if(over_budget_adjust>0){ /* push values up to exceed cap intentionally */
...
int main(){
    /* Create a few synthetic item defs & affixes if needed. Assumes at least 1 def & affixes exist; otherwise test cannot proceed. */
    /* We'll just spawn using def 0 multiple times; rarity variations create different caps via rogue_budget_max. */
    /* Layout:
    ...
    */
    ...
    RogueBudgetReport rep; rogue_budget_analyzer_run(&rep);
    ...
    assert_true(rep.bucket_counts[4] >= 1 || rep.bucket_counts[3] >=1, "expected <=100 buckets non-zero");
    ...
    char json[512]; int wrote = rogue_budget_analyzer_export_json(json, sizeof json);
