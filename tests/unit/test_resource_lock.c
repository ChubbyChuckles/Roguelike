...
static void test_mutex_basic(){
    ...
    RogueLockStats st; rogue_mutex_get_stats(m,&st);
...
DWORD WINAPI hold_mutex_thread(LPVOID param){
    ...
    if(rogue_mutex_acquire(m, ROGUE_LOCK_PRIORITY_NORMAL, -1)==0){ InterlockedExchange(&holder_acquired,1); Sleep(50); rogue_mutex_release(m);} return 0; }
#else
#include <pthread.h>
#include <unistd.h>
...
static void *hold_mutex_thread(void *param){
    RogueMutex *m = (RogueMutex*)param;
    if(rogue_mutex_acquire(m, ROGUE_LOCK_PRIORITY_NORMAL, -1)==0){ holder_acquired=1; usleep(50*1000); rogue_mutex_release(m);} return NULL; }
#endif
...
#ifdef _WIN32
    HANDLE th = CreateThread(NULL,0,hold_mutex_thread,m,0,NULL); assert(th);
#else
    pthread_t th; assert(pthread_create(&th,NULL,hold_mutex_thread,m)==0);
#endif
    // Wait until the holder thread definitely acquired the mutex
    int spin=0; int rc_try;
#ifdef _WIN32
    while(holder_acquired==0 && spin<200){ Sleep(1); spin++; }
#else
    while(holder_acquired==0 && spin<200){ usleep(1000); spin++; }
#endif
...
static void test_mutex_ordering(){
    ...
    RogueLockStats st_low; rogue_mutex_get_stats(low,&st_low);
...
static void test_rwlock(){
    ...
    RogueLockStats r,w; rogue_rwlock_get_stats(l,&r,&w);
...
int main(){
    ...
    test_mutex_ordering();
    test_rwlock();
    RogueGlobalLockStats g; rogue_lock_global_stats(&g);
    printf("[resource_lock] mutex=%u rw=%u acq=%llu cont=%llu timeouts=%llu\n", g.mutex_count,g.rwlock_count,(unsigned long long)g.total_acquisitions,(unsigned long long)g.total_contentions,(unsigned long long)g.total_timeouts);
    return 0;
}
