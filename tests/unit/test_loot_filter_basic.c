...
int main(void){
    ...
    if(common<0||epic<0) return fail("defs");
    int inst_common = rogue_items_spawn(common,1,0,0);
    int inst_epic = rogue_items_spawn(epic,1,0,0);
    if(inst_common<0||inst_epic<0) return fail("spawn");
    if(rogue_items_visible_count()!=2) return fail("vis2");
    // Build filter file dynamically
    const char* fname = "loot_filter_test.cfg";
    FILE* f=NULL; f=fopen(fname,"wb"); if(!f) return fail("tmpfile");
    fputs("rarity>=3\n", f); fclose(f);
    if(rogue_loot_filter_reset()!=0) return fail("reset");
    if(rogue_loot_filter_load(fname)<=0) return fail("load");
    rogue_loot_filter_refresh_instances();
    if(rogue_items_visible_count()!=1) return fail("vis1");
    ...
    if(!epic_inst||!com_inst) return fail("inst_null");
    if(epic_inst->hidden_filter) return fail("epic_hidden");
    if(!com_inst->hidden_filter) return fail("common_not_hidden");
    printf("LOOT_FILTER_OK rules=%d visible=%d hidden=%d\n", rogue_loot_filter_rule_count(), rogue_items_visible_count(), rogue_items_active_count()-rogue_items_visible_count());
    return 0;
}
