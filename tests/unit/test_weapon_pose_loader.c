...
int main()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        fprintf(
            f,
            "    "
            "{\"dx\":%d,\"dy\":%d,\"angle\":%d,\"scale\":1.0,\"pivot_x\":0.5,\"pivot_y\":0.5}%s\n",
            i, -i, i * 5, (i == 7) ? "" : " ,");
    }
    fprintf(f, "  ]\n}\n");
    fclose(f);
    if (!rogue_weapon_pose_ensure(0))
    {
        printf("ensure_failed\n");
        return 1;
    }
    ...
    if ((int) fr->dx != 5 || (int) fr->dy != -5 || (int) fr->angle != 25)
    {
        printf("mismatch dx=%.2f dy=%.2f angle=%.2f\n", fr->dx, fr->dy, fr->angle);
        return 1;
    }
