...
int main(void){
    ...
    FILE* f=fopen(in,"wb"); if(!f){ fprintf(stderr,"FAIL open unsorted\n"); return 1; }
    ...
    if(sorted != 3){ fprintf(stderr,"FAIL expected 3 sorted got %d\n", sorted); return 2; }
    rogue_item_defs_reset();
    int added = rogue_item_defs_load_from_cfg(out);
    if(added != 3){ fprintf(stderr,"FAIL expected 3 added got %d\n", added); return 3; }
    int ia = rogue_item_def_index("alpha_item");
    int im = rogue_item_def_index("mid_item");
    int iz = rogue_item_def_index("zeta_item");
    if(!(ia < im && im < iz)){ fprintf(stderr,"FAIL ordering ia=%d im=%d iz=%d\n", ia,im,iz); return 4; }
    FILE* fs=fopen(out,"rb"); if(!fs){ fprintf(stderr,"FAIL reopen sorted\n"); return 5; }
    char first[256]; fgets(first,sizeof first,fs); /* header */ fgets(first,sizeof first,fs); fclose(fs);
    if(strncmp(first,"alpha_item",10)!=0){ fprintf(stderr,"FAIL first data line %s\n", first); return 6; }
