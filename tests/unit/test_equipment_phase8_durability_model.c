...
const RogueItemDef* rogue_item_def_at(int index){ static RogueItemDef defs[2]; defs[0].category=ROGUE_ITEM_WEAPON; defs[0].rarity=0; defs[0].base_value=10; defs[1].category=ROGUE_ITEM_WEAPON; defs[1].rarity=4; defs[1].base_value=10; if(index>=0 && index<2) return &defs[index]; return NULL; }
