...
#endif
    if(!f) return 0;
    struct RogueSaveDescriptor { unsigned version,timestamp,component_mask,section_count; unsigned long long total_size; unsigned checksum; } desc; 
    if(fread(&desc,sizeof desc,1,f)!=1){ fclose(f); return 0; }
    fclose(f); return desc.checksum;
}
...
int main(void){
    ...
    if(rogue_save_manager_save_slot(0)!=0){ printf("ORDER_FAIL save0\n"); return 1; }
    ...
    rogue_save_manager_init();
    rogue_register_core_save_components();
    if(rogue_save_manager_save_slot(1)!=0){ printf("ORDER_FAIL save1\n"); return 1; }
    unsigned c1=read_checksum("save_slot_1.sav");

    if(c0==0 || c1==0){ printf("ORDER_FAIL zero_checksum c0=%u c1=%u\n",c0,c1); return 1; }
    if(c0!=c1){ printf("ORDER_MISMATCH %u %u\n", c0,c1); return 1; }
