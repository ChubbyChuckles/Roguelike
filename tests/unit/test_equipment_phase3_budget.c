...
static void test_budget_validation(void)
{
    ...
    RogueItemInstance* it = (RogueItemInstance*) rogue_item_instance_at(inst);
    it->item_level = 1;
    it->rarity = 1; /* cap = 20 + 1*5 + 1^2*10 = 35; total weight = 3+15=18 -> valid */
    assert(rogue_item_instance_validate_budget(inst) == 0);
    /* Force over budget by manual inflation */
    it->prefix_value = 30;
    it->suffix_value = 30; /* total 60 > 35 */
    assert(rogue_item_instance_validate_budget(inst) < 0);
}
...
static void test_upgrade_within_budget(void)
{
    ...
    it->item_level = 1;
    it->rarity = 1; /* cap 35 */
    int start_val = it->prefix_value;
    assert(start_val == 3);
    rogue_item_instance_upgrade_level(inst, 4,
                                      &rng); /* item_level=5 -> cap = 20 + 5*5 + 1*10 = 55 */
...
int main(void)
{
    ...
    int di = rogue_item_def_index("budget_sword");
    if (di < 0)
    {
        fprintf(stderr, "missing_def\n");
        return 1;
    }
