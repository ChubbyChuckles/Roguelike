...
static int spawn_item(const char* id, const char* prefix, const char* suffix)
{
    ...
    assert(def >= 0);
    int inst_index = rogue_items_spawn(def, 1, 0.f, 0.f);
    assert(inst_index >= 0);
...
static void test_affix_layer_and_fingerprint(void)
{
    ...
    int int_affix = rogue_affix_index("int_flat");
    assert(int_affix >= 0);
    RogueItemInstance* blade_inst = (RogueItemInstance*) rogue_item_instance_at(blade);
    blade_inst->suffix_index = int_affix;
    blade_inst->suffix_value = rogue_affix_at(int_affix)->min_value;
...
/* Equip ordering invariance: equipping items in different orders should yield identical layered
 * totals & fingerprint. */
