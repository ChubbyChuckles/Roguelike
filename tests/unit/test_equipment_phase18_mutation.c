/* Phase 18.5: Mutation / corruption robustness test.
   Goals:
     ...
     4. Corrupt durability numeric (flip digit) produces different state hash (tamper detection
   still meaningful). The test avoids undefined behaviour by always operating within buffer bounds
...
int main(void)
{
    ...
    int b = rogue_items_spawn(1, 1, 0, 0);
    assert(b >= 0);
    assert(rogue_equip_try(ROGUE_EQUIP_RING1, b) == 0);

    char buf[4096];
    int n = rogue_equipment_serialize(buf, (int) sizeof buf);
    assert(n > 0);
    unsigned long long base_hash = rogue_equipment_state_hash();
    assert(base_hash != 0ULL);
