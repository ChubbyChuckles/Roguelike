...
int main(void)
{
    ...
    int id = rogue_skill_register(&cast);
    ...
    rogue_buffs_apply(ROGUE_BUFF_POWER_STRIKE, 25, 1000.0, ctx.now_ms, ROGUE_BUFF_STACK_ADD,
                      0); /* strong haste (clamped at 0.5 factor) */
    assert(rogue_skill_try_activate(id, &ctx) == 1);
