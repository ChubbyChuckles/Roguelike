...
/* Minimal harness: seed recipe registry with a few mock recipes if empty. We assume test
 * environment provides some. */
static int ensure_min_recipes(void)
{
    int n = rogue_craft_recipe_count();
    return n;
}
...
int main(void)
{
    ...
    if (none != 0 && none >= all)
    {
        fprintf(stderr, "P8_FAIL search_filter none=%d all=%d\n", none, all);
        return 10;
    }
    rogue_crafting_ui_set_search("");
    ...
    if (txt <= 0)
    {
        fprintf(stderr, "P8_FAIL text_only_count %d\n", txt);
        return 11;
    }
    if (rogue_crafting_ui_get_batch() != 3)
    {
        fprintf(stderr, "P8_FAIL batch_state %d\n", rogue_crafting_ui_get_batch());
        return 13;
    }
    ...
    if (q != 0)
    {
        fprintf(stderr, "P8_FAIL queue_nonzero %d\n", q);
        return 14;
    }
    ...
    if (g != 0)
    {
        fprintf(stderr, "P8_FAIL gather_overlay_nonzero %d\n", g);
        return 15;
    }
    float risk = rogue_crafting_ui_expected_fracture_damage(10);
    if (risk < 0.1f || risk > 5.0f)
    {
        fprintf(stderr, "P8_FAIL risk_range %.2f\n", risk);
        return 12;
    }
