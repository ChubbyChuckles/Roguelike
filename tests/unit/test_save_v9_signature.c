...
int main(void){
    if(ROGUE_SAVE_FORMAT_VERSION < 9u){ printf("SIG_SKIP v=%u\n", (unsigned)ROGUE_SAVE_FORMAT_VERSION); return 0; }
    ...
    if(rogue_save_manager_save_slot(0)!=0){ printf("SIG_FAIL save\n"); return 1; }
    /* Load (should verify) */
    int rc = rogue_save_manager_load_slot(0);
    if(rc!=0){ printf("SIG_FAIL load rc=%d flags=0x%X\n", rc, rogue_save_last_tamper_flags()); return 1; }
...
#if defined(_MSC_VER)
    FILE* f=NULL; fopen_s(&f,"save_slot_0.sav","r+b");
#else
    FILE* f=fopen("save_slot_0.sav","r+b");
#endif
    if(f){ fseek(f,-1,SEEK_END); int c=fgetc(f); if(c!=EOF){ unsigned char b=(unsigned char)c; b^=0xFF; fseek(f,-1,SEEK_CUR); fwrite(&b,1,1,f);} fclose(f);} else { printf("SIG_FAIL reopen\n"); return 1; }
    rc = rogue_save_manager_load_slot(0);
    if(rc==0){ printf("SIG_FAIL expected tamper rc flags=0x%X\n", rogue_save_last_tamper_flags()); return 1; }
    if((rogue_save_last_tamper_flags() & ROGUE_TAMPER_FLAG_SIGNATURE)==0){ printf("SIG_FAIL no signature flag tf=0x%X rc=%d\n", rogue_save_last_tamper_flags(), rc); return 1; }
