...
/* Minimal synthetic injection by directly populating definitions via loader assumptions: we
 * simulate by constructing temporary CSV in memory written to a file. */
...
int main(void)
{
    ...
    write_cfg("test_items_tmp.cfg");
    int added = rogue_item_defs_load_from_cfg("test_items_tmp.cfg");
    CHECK(added == 3, "added_three");
    int rc = rogue_item_defs_build_index();
    CHECK(rc == 0, "build_index_ok");
    int i_a = rogue_item_def_index_fast("itm_a");
    int i_b = rogue_item_def_index_fast("itm_b");
    int i_c = rogue_item_def_index_fast("itm_c");
