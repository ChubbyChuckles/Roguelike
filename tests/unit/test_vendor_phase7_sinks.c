...
static int g_catalysts = 5;
static int consume_cat(void)
{
    if (g_catalysts <= 0)
        return -1;
    g_catalysts--;
...
static int g_src_mat = 100;
static int consume_src(int c)
{
    if (g_src_mat < c)
        return -1;
    g_src_mat -= c;
...
static int g_dst_mat = 0;
static int grant_dst(int c)
{
    g_dst_mat += c;
    return 0;
}
...
int main()
{
    ...
    int gold_fee = 0;
    int rc = rogue_vendor_upgrade_reroll_affix(0, 1, 0, 10, g_catalysts, consume_cat, spend_gold,
                                               &gold_fee);
    if (rc == 0)
    {
        assert(gold_fee > 0);
        assert(rogue_vendor_sinks_total(ROGUE_SINK_UPGRADE) >= gold_fee);
    }
    ...
    if (rogue_material_count() > 1)
    {
        to_idx = 1;
    }
    int trade_rc = rogue_vendor_material_trade_in(from_idx, to_idx, 24, 20, consume_src, grant_dst,
                                                  spend_gold, &out_count, &trade_fee);
    if (trade_rc == 0)
    {
        assert(out_count > 0);
        assert(trade_fee > 0);
    }
    int grand = rogue_vendor_sinks_grand_total();
    assert(grand >= 0);
