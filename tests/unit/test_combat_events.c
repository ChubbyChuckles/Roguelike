...
static void simulate_to_strike(RoguePlayerCombat* c, float windup_ms){
    ...
    float acc=0; while(acc < windup_ms + 1){ float step=5.0f; if(acc+step>windup_ms+1) step = (windup_ms+1)-acc; rogue_combat_update_player(c, step, 0); acc += step; }
}

int main(){
    RoguePlayerCombat combat; rogue_combat_init(&combat);
    RoguePlayer player; rogue_player_init(&player); player.strength=20; player.base.pos.x=0; player.base.pos.y=0; player.facing=2;
    RogueEnemy enemy; enemy.alive=0; /* no enemy so windows just emit begin/end */
    /* Use light chain index 2 (light_3) multi-window */
    combat.chain_index = 2; combat.archetype = ROGUE_WEAPON_LIGHT; /* fast path: directly strike */
    combat.phase = ROGUE_ATTACK_STRIKE; combat.strike_time_ms = 0; combat.processed_window_mask=0; combat.event_count=0; combat.emitted_events_mask=0;
    ...
    combat.strike_time_ms = 10.0f; rogue_combat_player_strike(&combat,&player,&enemy,0);
    /* Second window active at t=50 */
    combat.strike_time_ms = 50.0f; rogue_combat_player_strike(&combat,&player,&enemy,0);
