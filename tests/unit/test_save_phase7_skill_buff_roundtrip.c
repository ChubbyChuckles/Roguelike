...
/* Minimal harness: initialize skills & buffs, simulate state, save, mutate, load, verify
 * restoration */
...
int main(void)
{
    ...
    /* Ensure at least a few skills exist; if registry empty we fabricate three simple defs inline
     * via public API assumptions */
    ...
    int scount = 0;
    while (rogue_skill_get_def(scount))
        scount++;
    if (scount < 3)
    {
        printf("FAIL: need >=3 skills\n");
        return 1;
    }
    ...
    s0->rank = 3;
    s0->cooldown_end_ms = 12345.0;
    s0->cast_progress_ms = 150.0;
    ...
    /* Apply two buffs */
    double now = 10000.0;
    g_app.game_time_ms = now; /* set global time */
    rogue_buffs_apply(ROGUE_BUFF_POWER_STRIKE, 4, 5000.0, now, ROGUE_BUFF_STACK_ADD,
                      0); /* ends at 15000 */
    rogue_buffs_apply(ROGUE_BUFF_STAT_STRENGTH, 2, 3000.0, now, ROGUE_BUFF_STACK_ADD,
                      0); /* ends at 13000 */

    if (rogue_save_manager_save_slot(0) != 0)
    {
        printf("FAIL: initial save\n");
        return 1;
    }
    ...
    s0->cast_progress_ms = 0;
    s0->channel_end_ms = 0;
    s0->next_charge_ready_ms = 0;
    s0->charges_cur = 0;
    s0->casting_active = 0;
    s0->channel_active = 0;
    s1->rank = 0;
    s1->cooldown_end_ms = 0;
    s1->cast_progress_ms = 0;
    s1->channel_end_ms = 0;
    s1->next_charge_ready_ms = 0;
    s1->charges_cur = 0;
    s1->casting_active = 0;
    s1->channel_active = 0;
    /* Advance time so remaining durations change if we had stored absolutes; using relative we
     * expect restoration roughly preserved independent of current now. */
    ...
    /* Validate buffs roundtrip: we expect two active buffs with magnitudes >= original (stacking
     * may merge types) */
