...
int main(void)
{
    ...
    {
        fprintf(stderr, "SALV_FAIL find swords\n");
        return 10;
    }
    ...
    {
        fprintf(stderr, "SALV_FAIL load dir\n");
        return 11;
    }
    ...
    if (epic < 0)
    {
        fprintf(stderr, "SALV_FAIL epic blade\n");
        return 12;
    }
    ...
    if (gained <= 0)
    {
        fprintf(stderr, "SALV_FAIL no gain\n");
        return 13;
    }
    int after = rogue_inventory_get_count(rogue_item_def_index("primal_shard"));
    if (after <= before)
    {
        fprintf(stderr, "SALV_FAIL inventory not updated\n");
        return 14;
    }
