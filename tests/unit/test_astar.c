...
int main(void){
    ...
    RogueWorldGenConfig cfg = rogue_world_gen_config_build(42u,0,0); if(!rogue_world_generate(&g_app.world_map,&cfg)){ printf("gen_fail\n"); return 2; }
    rogue_vegetation_init(); rogue_vegetation_load_defs("../assets/plants.cfg","../assets/trees.cfg"); rogue_vegetation_generate(0.10f, 444u);
    ...
    float accum=0.0f; for(int i=1;i<path.length;i++){ accum += rogue_nav_tile_cost(path.xs[i],path.ys[i]); if(accum<0){ printf("cost_underflow\n"); return 7; }}
    return 0;
}
