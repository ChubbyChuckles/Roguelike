...
static uint32_t fnv1a32(const void* data, size_t len){
    const unsigned char* p=(const unsigned char*)data; uint32_t h=0x811c9dc5u; for(size_t i=0;i<len;i++){ h ^= p[i]; h *= 16777619u;} return h; }
...
int main(){
    ...
    RogueAIRNG r1, r2; rogue_ai_rng_seed(&r1, 1234); rogue_ai_rng_seed(&r2, 1234);
    for(int i=0;i<16;i++){ assert(rogue_ai_rng_next_u32(&r1)==rogue_ai_rng_next_u32(&r2)); }
