...
int main(void)
{
    ...
    int inst =
        rogue_items_spawn(0, 1, 5.0f, 0.0f); /* spawn outside view radius to test edge notifier */
    int pass = 1;
    pass &= expect(inst >= 0, "spawn inst");
    RogueLootVFXState st;
    pass &= expect(rogue_loot_vfx_get(inst, &st) == 1, "get vfx state");
    ...
    float prev_alpha = st.pulse_alpha;
    pass &= expect(st.pulse_active == 1, "pulse became active by 59s");
