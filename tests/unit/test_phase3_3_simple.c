...
int main(void)
{
    ...
    if (result)
    {
        ...
        /* Test XP award */
        int xp_result = rogue_combat_progression_bridge_award_xp(
            &bridge, ROGUE_XP_SOURCE_DAMAGE_DEALT, 200, 25, 1001);
        if (xp_result)
        {
        ...
        /* Test skill tracking */
        int skill_result =
            rogue_combat_progression_bridge_track_skill_usage(&bridge, 101, 75, 2001);
        if (skill_result)
        {
        ...
        int efficiency_result = rogue_combat_progression_bridge_update_efficiency_metrics(
            &bridge, 5000.0f, 300, 50, 20);
        ...
        int playstyle_result =
            rogue_combat_progression_bridge_analyze_playstyle(&bridge, 70, 60, 40);
        if (playstyle_result)
        {
