...
static int load_items_affixes_materials(void)
{
    ...
    int extra = rogue_item_defs_load_from_cfg("assets/test_items.cfg");
    if (extra <= 0)
        extra = rogue_item_defs_load_from_cfg("../assets/test_items.cfg");
    if (extra <= 0)
        extra = rogue_item_defs_load_from_cfg("../../assets/test_items.cfg");
    if (extra <= 0)
        extra = rogue_item_defs_load_from_cfg("../../../assets/test_items.cfg");
    ...
    if (loaded <= 0)
    {
        fprintf(stderr, "P6_3_LOAD fail item defs (dirs tried=%zu)\n",
    ...
    {
        fprintf(stderr, "P6_3_LOAD fail affix_path\n");
        return 0;
    }
    if (rogue_affixes_load_from_cfg(paff) <= 0)
    {
        fprintf(stderr, "P6_3_LOAD fail affixes\n");
        return 0;
    }
    /* load materials for tier scaling */
    if (rogue_material_registry_load_default() <= 0)
    {
        fprintf(stderr, "P6_3_LOAD fail materials\n");
        return 0;
    }
    return 1;
}
...
int main(void)
{
    if (!load_items_affixes_materials())
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL load_data\n");
        return 10;
    }
    ...
    for (int i = 0; i < 2048; i++)
    {
        ...
        if (!d)
            break;
        if (d->category == ROGUE_ITEM_WEAPON && d->rarity >= 3)
        {
            target = i;
            break;
        }
    ...
    if (target < 0)
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL no_target\n");
        return 11;
    }
    int inst = rogue_items_spawn(target, 1, 0, 0);
    if (inst < 0)
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL spawn\n");
        return 12;
    }
    ...
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL gen_affix\n");
        return 13;
    }
    int cost1 = 0; /* Only reroll prefix to avoid needing enchant material for both */
    if (rogue_item_instance_enchant(inst, 1, 0, &cost1) != 0)
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL enchant1\n");
        return 14;
    }
    int cost2 = 0;
    if (rogue_item_instance_reforge(inst, &cost2) != 0)
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL reforge\n");
        return 15;
    }
    if (cost2 <= cost1)
    {
        fprintf(stderr, "CRAFT_P6_3_FAIL cost_relation c1=%d c2=%d\n", cost1, cost2);
        return 16;
    }
