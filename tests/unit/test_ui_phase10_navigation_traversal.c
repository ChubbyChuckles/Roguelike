...
/* Phase 10.3 Automated navigation traversal test
   Goals:
   - Ensure directional navigation (Right/Left, Down/Up) can visit every focusable widget exactly once before wrap.
   ...
   - Assert unique visit count equals total focusable count (6) and wrap returns to start after 6 moves.
   - Vertical traversal sanity: ensure DOWN moves reach the second row for each column and wrap back. */
...
static int set_contains(int* arr,int count,int v){ for(int i=0;i<count;i++) if(arr[i]==v) return 1; return 0; }

int main(){
    RogueUIContextConfig cfg={0}; cfg.max_nodes=128; cfg.seed=99u; cfg.arena_size=4096;
    ...
    int initial = ui.focus_index; assert(initial>=0);
    rogue_ui_end(&ui);

    /* Horizontal cycle (RIGHT key) */
    int visited[16]; int visit_count=0; memset(visited,-1,sizeof visited);
    for(int step=0; step<6; ++step){
        rogue_ui_begin(&ui,16.0);
        build_buttons(&ui);
        RogueUIInputState in={0}; in.key_right=1; ui.input=in; /* simulate edge press */
        rogue_ui_navigation_update(&ui);
        int idx = ui.focus_index; assert(idx>=0);
        if(step<5){
    ...
    /* Vertical traversal test: For each top-row button force focus and move DOWN once; expect matching bottom-row index (top+3) */
    int vertical_matches=0;
    for(int col=0; col<3; ++col){
        int top_index = col; /* row-major order */
        ui.focus_index = top_index;
        rogue_ui_begin(&ui,16.0); build_buttons(&ui); RogueUIInputState inDown={0}; inDown.key_down=1; ui.input=inDown; rogue_ui_navigation_update(&ui); int bottom_idx=ui.focus_index; rogue_ui_end(&ui);
