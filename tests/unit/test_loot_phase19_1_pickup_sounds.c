...
int main(void)
{
    ...
    g_app.player.base.pos.x = 0.f;
    g_app.player.base.pos.y = 0.f;
    /* Assign pickup sounds */
    rogue_rarity_set_pickup_sound(0, "s_common");
    rogue_rarity_set_pickup_sound(4, "s_legendary");
    int pass = 1;
    /* Spawn common (rarity 0) and legendary (force def rarity using fake defs conceptually; here
     * rely on default def rarity retrieval) */
    int inst_c = rogue_items_spawn(0, 1, 0.1f, 0.1f); /* def 0 assumed rarity 0 in test data */
    int inst_l = rogue_items_spawn(
        1, 1, 0.2f, 0.2f); /* def 1 may not be rarity 4, can't guarantee -> simulate by setting
                              pickup sound retrieval only; test ensures no crash */
