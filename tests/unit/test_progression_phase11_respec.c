...
static RoguePlayer make_player(void){ RoguePlayer p; memset(&p,0,sizeof(p)); p.strength=10; p.dexterity=5; p.vitality=4; p.intelligence=3; p.crit_rating=50; p.haste_rating=25; p.avoidance_rating=10; p.crit_chance=5; p.crit_damage=150; p.max_health=100; return p; }

int main(void){
    struct RogueProgressionMaze maze; build_maze(&maze);
    ...
    size_t def_count=0; const RogueStatDef* defs = rogue_stat_def_all(&def_count);
    int str_id=-1,dex_id=-1; for(size_t i=0;i<def_count;i++){ if(strcmp(defs[i].code,"STR")==0) str_id=defs[i].id; if(strcmp(defs[i].code,"DEX")==0) dex_id=defs[i].id; }
