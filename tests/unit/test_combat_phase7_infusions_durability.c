...
int main(){
    ...
    RoguePlayer player; memset(&player,0,sizeof player); player.team_id=0; player.strength=40; player.dexterity=20; player.intelligence=25; player.facing=2; player.equipped_weapon_id=3; /* focus catalyst int-scaling */ player.combat_stance=0; player.weapon_infusion=3; /* arcane infusion */ player.base.pos.x=0; player.base.pos.y=0;
    RogueEnemy enemy; memset(&enemy,0,sizeof enemy); enemy.alive=1; enemy.team_id=1; enemy.base.pos.x=1.0f; enemy.base.pos.y=0.0f; enemy.health=800; enemy.max_health=800; enemy.facing=1;

    int dmg_arcane = strike_once(&pc,&player,&enemy); if(dmg_arcane<=0){ printf("fail_arcane_base=%d\n", dmg_arcane); return 1; }

    enemy.health=800; player.weapon_infusion=1; /* fire */ int dmg_fire = strike_once(&pc,&player,&enemy); if(!(dmg_fire>0 && dmg_fire!=dmg_arcane)){ printf("fail_fire_variation arc=%d fire=%d\n", dmg_arcane, dmg_fire); return 2; }

    /* Exhaust durability to trigger reduction */
    for(int i=0;i<100;i++){ enemy.health=800; strike_once(&pc,&player,&enemy); enemy.health=800; }
    enemy.health=800; int dmg_low_dur = strike_once(&pc,&player,&enemy); if(!(dmg_low_dur < dmg_arcane)){ printf("fail_durability_scalar orig=%d low=%d\n", dmg_arcane, dmg_low_dur); return 3; }

    printf("phase7_infusions_durability: OK arc=%d fire=%d low_dur=%d\n", dmg_arcane, dmg_fire, dmg_low_dur);
    return 0;
}
