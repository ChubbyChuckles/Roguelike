...
{
    FILE* f = fopen("encounters_phase2.cfg", "wb");
    assert(f);
    fputs("id=0\nname=Test Swarm\ntype=swarm\nmin=3\nmax=6\nelite_spacing=3\nelite_chance=0.1\n\n",
          f);
    fputs("id=1\nname=Test Mixed\ntype=mixed\nmin=4\nmax=7\nelite_spacing=3\nelite_chance=0.2\n\n",
...
static void test_template_selection()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        if (rogue_enemy_integration_choose_template(10, 1, seed + i, &template_id) &&
            template_id == 3)
        {
            boss_count++;
        }
...
static void test_room_encounter_preparation()
{
    ...
    /* Create test room */
    RogueDungeonRoom test_room = {5, 25, 30, 8, 7, ROGUE_DUNGEON_ROOM_ELITE,
                                  0}; /* room id 5, elite tagged */

    RogueRoomEncounterInfo encounter_info;
    int world_seed = 999;
    int region_id = 2;

    assert(rogue_enemy_integration_prepare_room_encounter(&test_room, world_seed, region_id,
                                                          &encounter_info));
    ...
    unsigned int expected_seed = rogue_enemy_integration_encounter_seed((unsigned int) world_seed,
                                                                        region_id, test_room.id, 0);
...
static void test_deterministic_template_selection()
{
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        if (rogue_enemy_integration_choose_template(5, 1, seed + i, &t1) &&
            rogue_enemy_integration_choose_template(5, 1, seed + i + 1000, &t2) && t1 != t2)
        {
            different_count++;
        }
    }
