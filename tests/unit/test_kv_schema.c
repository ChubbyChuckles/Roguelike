...
int main(void){
    ...
    RogueKVFieldValue values[8]; char err[256];
    int errs = rogue_kv_validate(&f,&schema,values,8,err,sizeof err);
    if(errs < 1){ printf("FAIL expected at least 1 error (unknown key)\n"); return 1; }
    int found_hp=0, found_speed=0; for(int i=0;i<3;i++){ if(values[i].present && values[i].def==&defs[0]) found_hp=1; if(values[i].present && values[i].def==&defs[1]) found_speed=1; }
    if(!found_hp || !found_speed){ printf("FAIL required fields missing in values array\n"); return 1; }
