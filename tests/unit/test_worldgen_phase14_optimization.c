...
int main(void){
    ...
    RogueWorldGenBenchmark b1; assert(rogue_worldgen_run_noise_benchmark(64,64,&b1));
    ...
    RogueWorldGenBenchmark b2 = (RogueWorldGenBenchmark){0}; assert(rogue_worldgen_run_noise_benchmark(64,64,&b2));
    /* Basic invariants */
    ...
    if(b2.simd_ms > 0.0){ /* if SIMD measured, expect speedup >= 0.5 (arbitrary sanity bound) */
