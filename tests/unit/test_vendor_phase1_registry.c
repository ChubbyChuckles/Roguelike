...
int main(void){
    ...
    const RogueVendorDef* blacksmith = rogue_vendor_def_find("blacksmith_standard"); if(!blacksmith){ fprintf(stderr,"VENDOR_P1_FAIL find blacksmith_standard\n"); return 9; }
    ...
    if(rogue_negotiation_rule_count() <=0){ fprintf(stderr,"VENDOR_P1_FAIL negotiation count\n"); return 14; }
    printf("VENDOR_PHASE1_REGISTRY_OK vendors=%d policies=%d rep=%d nego=%d\n", rogue_vendor_def_count(), rogue_price_policy_count(), rogue_rep_tier_count(), rogue_negotiation_rule_count());
    return 0;
}
