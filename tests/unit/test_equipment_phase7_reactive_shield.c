...
static void setup_player(RoguePlayer* p){ memset(p,0,sizeof *p); p->health=100; p->max_health=100; p->poise=40; p->poise_max=40; p->guard_meter=50; p->guard_meter_max=50; p->perfect_guard_window_ms=80.0f; }
...
int main(){
    ...
    shield.trigger = ROGUE_PROC_ON_BLOCK; shield.icd_ms=0; shield.duration_ms=1000; shield.magnitude=20; shield.max_stacks=0; shield.stack_rule=ROGUE_PROC_STACK_REFRESH; shield.param=0;
    int id = rogue_proc_register(&shield); assert(id>=0);
    /* Force activate (simulate prior block) with 1 stack */
    rogue_proc_force_activate(id,1,1000);
    int blocked=0, perfect=0; g_player_stat_cache.block_chance=100; g_player_stat_cache.block_value=0; /* force passive block for path */
    int dmg = rogue_player_apply_incoming_melee(&p, 25.0f, 0,-1, 0, &blocked, &perfect);
    /* Block triggers, block_value=0 so raw 25 enters conversion (none) then absorb consumes up to 20 => 5 left */
    assert(dmg == 5);
