...
int main(){
    RogueUIContextConfig cfg={0}; cfg.max_nodes=256; cfg.arena_size=8192; cfg.seed=1234;
    ...
    assert(scroll>=0);
    float y=0; for(int i=0;i<5;i++){ RogueUIRect r={0,y,100,30}; RogueUIRect ar = rogue_ui_scroll_apply(&ui,scroll,r); rogue_ui_button(&ui,ar,"Row",0,0); y+=32.0f; }
    float off0 = rogue_ui_scroll_offset(&ui,scroll); assert(off0==0.0f);
    rogue_ui_end(&ui);
    ...
    y=0; for(int i=0;i<5;i++){ RogueUIRect r={0,y,100,30}; RogueUIRect ar = rogue_ui_scroll_apply(&ui,scroll,r); rogue_ui_button(&ui,ar,"Row",0,0); y+=32.0f; }
    float off1 = rogue_ui_scroll_offset(&ui,scroll); assert(off1>0.0f);
    rogue_ui_end(&ui);
    ...
    frame(&ui,(RogueUIInputState){.mouse_x=10,.mouse_y=10}); ui.time_ms += 250.0; // manually force time
    btn = rogue_ui_button(&ui,(RogueUIRect){0,0,40,20},"Tip",0,0);
    ui.hot_index=btn; int tip_panel = rogue_ui_tooltip(&ui,btn,"Hello",0,0,200); assert(tip_panel>=0);
    rogue_ui_end(&ui);
    ...
    ui.input.key_tab=1; int prev=ui.focus_index;
    rogue_ui_navigation_update(&ui);
