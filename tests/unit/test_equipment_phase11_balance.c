...
static void test_proc_oversaturation(void){
    ...
    for(int i=0;i<25;i++) rogue_equipment_analytics_record_proc_trigger(1);
...
static void test_variant_selection(void){
    rogue_equipment_analytics_reset();
    RogueBalanceParams vA={0}; strcpy(vA.id,"A"); vA.outlier_mad_mult=4; vA.proc_oversat_threshold=10; vA.dr_chain_floor=0.25f;
    RogueBalanceParams vB={0}; strcpy(vB.id,"B"); vB.outlier_mad_mult=6; vB.proc_oversat_threshold=30; vB.dr_chain_floor=0.15f;
    int ia = rogue_balance_register(&vA); assert(ia>=0);
    int ib = rogue_balance_register(&vB); assert(ib>=0 && ib!=ia);
    ...
    for(int i=0;i<cur->proc_oversat_threshold+1;i++) rogue_equipment_analytics_record_proc_trigger(1);
    rogue_equipment_analytics_analyze();
    assert(rogue_equipment_analytics_flag_proc_oversat()==1);
}
...
int main(void){
    ...
    char buf[128]; int n = rogue_equipment_analytics_export_json(buf,sizeof buf); assert(n>0); assert(strstr(buf,"proc_oversaturation"));
