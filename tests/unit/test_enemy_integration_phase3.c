...
static void write_test_encounters_file(void){
    FILE* f = fopen("encounters_phase3.cfg","wb"); assert(f);
    fputs("id=0\nname=Test Swarm\ntype=swarm\nmin=3\nmax=6\nelite_spacing=3\nelite_chance=0.1\n\n",f);
    fputs("id=2\nname=Test Champion\ntype=champion_pack\nmin=2\nmax=4\nelite_spacing=2\nelite_chance=0.5\n\n",f);
    fclose(f);
}
...
static void write_test_modifiers_file(void){
    FILE* f = fopen("modifiers_phase3.cfg","wb"); assert(f);
...
static void test_unit_stats_application(){
    ...
    assert(rogue_enemy_integration_apply_unit_stats(&elite_enemy, &unit, player_level, &mappings[0]));
...
static void test_finalize_spawn(){
    ...
    /* Finalize spawn */
    assert(rogue_enemy_integration_finalize_spawn(&enemy, &unit, &encounter_info, player_level, &mappings[0]));
