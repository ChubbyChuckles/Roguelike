...
#endif
    if (!f)
        return; /* Seek near end to flip a CRC byte (last 40 bytes hold footer & maybe CRC) */
    fseek(f, -20, SEEK_END);
    long pos = ftell(f);
    if (pos > 0)
    {
...
int main(void)
{
    ...
    if (ROGUE_SAVE_FORMAT_VERSION < 8u)
    {
        printf("TAMPER_SKIP v=%u\n", (unsigned) ROGUE_SAVE_FORMAT_VERSION);
        return 0;
    }
    ...
    {
        printf("TAMPER_FAIL save primary\n");
        return 1;
    }
    if (rogue_save_manager_autosave(0) != 0)
    {
        printf("TAMPER_FAIL autosave\n");
        return 1;
    }
    /* corrupt primary */
    ...
    if (rc < 0)
    {
        printf("TAMPER_FAIL recovery rc=%d flags=0x%X\n", rc, rogue_save_last_tamper_flags());
        return 1;
    }
    printf("TAMPER_DBG after load rc=%d flags=0x%X recovery=%d\n", rc,
           rogue_save_last_tamper_flags(), rogue_save_last_recovery_used());
    fflush(stdout);
    if (rc == 0)
    {
        printf("TAMPER_FAIL expected recovery path flags=0x%X\n", rogue_save_last_tamper_flags());
        return 1;
    }
    if (!rogue_save_last_recovery_used())
    {
        printf("TAMPER_FAIL flag not set recovery_used\n");
        return 1;
    }
    unsigned tf = rogue_save_last_tamper_flags(); /* descriptor CRC likely */
    if ((tf & (ROGUE_TAMPER_FLAG_DESCRIPTOR_CRC | ROGUE_TAMPER_FLAG_SECTION_CRC |
               ROGUE_TAMPER_FLAG_SHA256)) == 0)
    {
        printf("TAMPER_FAIL no tamper flag tf=0x%X\n", tf);
        return 1;
    }
