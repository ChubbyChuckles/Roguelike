...
static void test_file_management(void) {
    ...
    // Test adding files
    bool add1 = rogue_dependency_manager_add_file(manager, "assets/items.json", ROGUE_FILE_TYPE_ITEMS, 10);
    TEST_ASSERT(add1 == true, "Add items.json file");
    TEST_ASSERT(manager->graph.node_count == 1, "Node count after adding first file");
    
    bool add2 = rogue_dependency_manager_add_file(manager, "assets/affixes.json", ROGUE_FILE_TYPE_AFFIXES, 5);
    TEST_ASSERT(add2 == true, "Add affixes.json file");
    TEST_ASSERT(manager->graph.node_count == 2, "Node count after adding second file");
    
    bool add3 = rogue_dependency_manager_add_file(manager, "assets/enemies.json", ROGUE_FILE_TYPE_ENEMIES, 15);
    ...
    bool add_duplicate = rogue_dependency_manager_add_file(manager, "assets/items.json", ROGUE_FILE_TYPE_ITEMS, 10);
    ...
    RogueDependencyNode* found2 = rogue_dependency_manager_find_node(manager, "assets/affixes.json");
    ...
    RogueDependencyNode* not_found = rogue_dependency_manager_find_node(manager, "assets/nonexistent.json");
    TEST_ASSERT(not_found == NULL, "Non-existent file returns NULL");
    
    // Test file info updates
    bool update = rogue_dependency_manager_update_file_info(manager, "assets/items.json", 1234567890, "abc123def456");
    ...
    RogueDependencyNode* removed_check = rogue_dependency_manager_find_node(manager, "assets/affixes.json");
    TEST_ASSERT(removed_check == NULL, "Removed file not found");
    
    bool remove_nonexistent = rogue_dependency_manager_remove_file(manager, "assets/nonexistent.json");
...
static void test_dependency_registration(void) {
    ...
    rogue_dependency_manager_add_file(manager, "assets/loot_tables.json", ROGUE_FILE_TYPE_LOOT_TABLES, 20);
    
    // Test adding strong dependencies
    bool dep1 = rogue_dependency_manager_add_dependency(manager, "assets/items.json", "assets/affixes.json", 
                                                       "affix_fire_damage", ROGUE_DEP_TYPE_STRONG, 1, 
                                                       "Items depend on affixes for stat modifiers");
    ...
    bool dep2 = rogue_dependency_manager_add_dependency(manager, "assets/loot_tables.json", "assets/items.json",
                                                       "item_sword_basic", ROGUE_DEP_TYPE_STRONG, 2,
                                                       "Loot tables depend on items for drop definitions");
    ...
    bool dep3 = rogue_dependency_manager_add_dependency(manager, "assets/enemies.json", "assets/loot_tables.json",
                                                       "loot_table_goblin", ROGUE_DEP_TYPE_STRONG, 3,
                                                       "Enemies depend on loot tables for drops");
    ...
    bool weak_dep = rogue_dependency_manager_add_weak_dependency(manager, "assets/enemies.json", "assets/skills.json",
                                                                "skill_fire_breath", "Optional skill reference");
    ...
    bool dup_dep = rogue_dependency_manager_add_dependency(manager, "assets/items.json", "assets/affixes.json",
                                                          "affix_fire_damage", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    ...
    int dep_count = rogue_dependency_manager_get_dependencies(manager, "assets/items.json", deps, 10);
    TEST_ASSERT(dep_count == 1, "Get dependencies for items.json");
    TEST_ASSERT(strcmp(deps[0].target_file, "assets/affixes.json") == 0, "Correct target file in dependency");
    TEST_ASSERT(strcmp(deps[0].reference_key, "affix_fire_damage") == 0, "Correct reference key in dependency");
    TEST_ASSERT(deps[0].type == ROGUE_DEP_TYPE_STRONG, "Correct dependency type");
    
    // Test weak dependency check
    bool is_weak = rogue_dependency_manager_is_weak_dependency(manager, "assets/enemies.json", "assets/skills.json", "skill_fire_breath");
    TEST_ASSERT(is_weak == true, "Correctly identify weak dependency");
    
    bool is_not_weak = rogue_dependency_manager_is_weak_dependency(manager, "assets/items.json", "assets/affixes.json", "affix_fire_damage");
    ...
    int weak_count = rogue_dependency_manager_get_weak_dependencies(manager, "assets/enemies.json", weak_deps, 10);
    TEST_ASSERT(weak_count == 1, "Get weak dependencies for enemies.json");
    TEST_ASSERT(strcmp(weak_deps[0].target_file, "assets/skills.json") == 0, "Correct weak dependency target");
    
    // Test removing dependencies
    bool remove_dep = rogue_dependency_manager_remove_dependency(manager, "assets/enemies.json", "assets/skills.json", "skill_fire_breath");
    ...
    bool remove_nonexistent = rogue_dependency_manager_remove_dependency(manager, "assets/items.json", "assets/nonexistent.json", "fake_ref");
...
static void test_dependency_resolution(void) {
    ...
    rogue_dependency_manager_add_weak_dependency(manager, "assets/enemies.json", "assets/nonexistent.json",
                                                "optional_ref", "Optional reference");
    ...
    RogueDependencyStatus status1 = rogue_dependency_manager_get_dependency_status(manager, "assets/items.json", "assets/affixes.json", "affix_fire");
    TEST_ASSERT(status1 == ROGUE_DEP_STATUS_RESOLVED, "Strong dependency resolved");
    
    RogueDependencyStatus status2 = rogue_dependency_manager_get_dependency_status(manager, "assets/enemies.json", "assets/items.json", "sword_basic");
    TEST_ASSERT(status2 == ROGUE_DEP_STATUS_RESOLVED, "Another strong dependency resolved");
    
    RogueDependencyStatus status3 = rogue_dependency_manager_get_dependency_status(manager, "assets/enemies.json", "assets/nonexistent.json", "optional_ref");
    TEST_ASSERT(status3 == ROGUE_DEP_STATUS_RESOLVED, "Weak dependency resolved (missing target OK)");
    ...
    RogueDependencyStatus missing_status = rogue_dependency_manager_get_dependency_status(manager, "assets/items.json", "assets/missing.json", "missing_ref");
    TEST_ASSERT(missing_status == ROGUE_DEP_STATUS_MISSING, "Missing dependency has correct status");
...
static void test_circular_dependency_detection(void) {
    ...
    rogue_dependency_manager_add_dependency(manager, "assets/a.json", "assets/b.json", "ref1", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    rogue_dependency_manager_add_dependency(manager, "assets/b.json", "assets/c.json", "ref2", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    rogue_dependency_manager_add_dependency(manager, "assets/c.json", "assets/a.json", "ref3", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    
    // Add non-circular dependency: d -> a (should not be part of cycle)
    rogue_dependency_manager_add_dependency(manager, "assets/d.json", "assets/a.json", "ref4", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    ...
    bool a_has_circular = rogue_dependency_manager_has_circular_dependency(manager, "assets/a.json");
    TEST_ASSERT(a_has_circular == true, "File A has circular dependency");
    
    bool d_has_circular = rogue_dependency_manager_has_circular_dependency(manager, "assets/d.json");
    ...
    rogue_dependency_manager_add_dependency(manager, "assets/c.json", "assets/a.json", "ref3_breaker", ROGUE_DEP_TYPE_CIRCULAR_BREAK, 1, NULL);
    
    bool cycles_after_breaker = rogue_dependency_manager_detect_cycles(manager);
    // Note: This might still detect cycles due to the original strong dependency, but the breaker should help in load ordering
...
static void test_load_order_generation(void) {
    ...
    rogue_dependency_manager_add_file(manager, "assets/affixes.json", ROGUE_FILE_TYPE_AFFIXES, 5);
    rogue_dependency_manager_add_file(manager, "assets/items.json", ROGUE_FILE_TYPE_ITEMS, 10);
    rogue_dependency_manager_add_file(manager, "assets/loot_tables.json", ROGUE_FILE_TYPE_LOOT_TABLES, 15);
    rogue_dependency_manager_add_file(manager, "assets/enemies.json", ROGUE_FILE_TYPE_ENEMIES, 20);
    
    rogue_dependency_manager_add_dependency(manager, "assets/items.json", "assets/affixes.json", "affix_fire", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    rogue_dependency_manager_add_dependency(manager, "assets/loot_tables.json", "assets/items.json", "item_sword", ROGUE_DEP_TYPE_STRONG, 2, NULL);
    rogue_dependency_manager_add_dependency(manager, "assets/enemies.json", "assets/loot_tables.json", "loot_goblin", ROGUE_DEP_TYPE_STRONG, 3, NULL);
    ...
    // Verify order: affixes should come before items, items before loot_tables, loot_tables before enemies
    int affix_pos = -1, item_pos = -1, loot_pos = -1, enemy_pos = -1;
    for (int i = 0; i < load_order.file_count; i++) {
        if (strstr(load_order.files[i], "affixes")) affix_pos = i;
        else if (strstr(load_order.files[i], "items")) item_pos = i;
        else if (strstr(load_order.files[i], "loot_tables")) loot_pos = i;
        else if (strstr(load_order.files[i], "enemies")) enemy_pos = i;
    }
    ...
    bool subset_order = rogue_dependency_manager_get_dependency_aware_order(manager, subset_files, 2, ordered_files, &ordered_count);
...
static void test_impact_analysis(void) {
    ...
    bool analyze1 = rogue_dependency_manager_analyze_impact(manager, "assets/affixes.json", &analysis);
    TEST_ASSERT(analyze1 == true, "Analyze impact for affixes");
    TEST_ASSERT(strcmp(analysis.changed_file, "assets/affixes.json") == 0, "Correct changed file in analysis");
    ...
    bool analyze2 = rogue_dependency_manager_analyze_impact(manager, "assets/skills.json", &skills_analysis);
    ...
    TEST_ASSERT(skills_analysis.requires_full_reload == false, "No full reload required for skills");
    
    // Test getting affected files directly
    char affected_files[10][ROGUE_DEP_MAX_PATH_LENGTH];
    int affected_count = rogue_dependency_manager_get_affected_files(manager, "assets/affixes.json", affected_files, 10);
    ...
    int system_count = rogue_dependency_manager_get_dependent_systems(manager, "assets/affixes.json", systems, 10);
...
static void test_graph_validation(void) {
    ...
    bool valid_file = rogue_dependency_manager_validate_file_dependencies(manager, "assets/items.json");
    TEST_ASSERT(valid_file == true, "Valid file dependencies pass validation");
    
    // Add missing dependency to make graph invalid
    rogue_dependency_manager_add_dependency(manager, "assets/items.json", "assets/missing.json", "missing_ref", ROGUE_DEP_TYPE_STRONG, 2, NULL);
    ...
    int unresolved_count = rogue_dependency_manager_get_unresolved_dependencies(manager, unresolved, 10);
    ...
    TEST_ASSERT(strcmp(missing[0].target_file, "assets/missing.json") == 0, "Correct missing dependency identified");
...
static void test_statistics_and_performance(void) {
    ...
    rogue_dependency_manager_get_statistics(manager, &total_deps, &resolved_deps, &failed_deps, &circular_deps);
...
static void test_utility_functions(void) {
    ...
    const char* dep_name1 = rogue_dependency_manager_get_dependency_type_name(ROGUE_DEP_TYPE_STRONG);
    ...
    const char* status_name1 = rogue_dependency_manager_get_dependency_status_name(ROGUE_DEP_STATUS_RESOLVED);
    TEST_ASSERT(strcmp(status_name1, "Resolved") == 0, "Get resolved status name");
    
    const char* status_name2 = rogue_dependency_manager_get_dependency_status_name(ROGUE_DEP_STATUS_MISSING);
...
static void test_error_handling(void) {
    ...
    TEST_ASSERT(rogue_dependency_manager_initialize(null_manager) == false, "Initialize with NULL manager fails");
    TEST_ASSERT(rogue_dependency_manager_find_node(null_manager, "test.json") == NULL, "Find node with NULL manager returns NULL");
    
    // Test valid manager with invalid parameters
    RogueDependencyManager* manager = rogue_dependency_manager_create();
    rogue_dependency_manager_initialize(manager);
    
    bool add_null_file = rogue_dependency_manager_add_file(manager, NULL, ROGUE_FILE_TYPE_ITEMS, 10);
    TEST_ASSERT(add_null_file == false, "Add file with NULL path fails");
    
    bool add_dep_null_source = rogue_dependency_manager_add_dependency(manager, NULL, "target.json", "ref", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    TEST_ASSERT(add_dep_null_source == false, "Add dependency with NULL source fails");
    
    bool add_dep_null_target = rogue_dependency_manager_add_dependency(manager, "source.json", NULL, "ref", ROGUE_DEP_TYPE_STRONG, 1, NULL);
    TEST_ASSERT(add_dep_null_target == false, "Add dependency with NULL target fails");
    
    bool add_dep_null_ref = rogue_dependency_manager_add_dependency(manager, "source.json", "target.json", NULL, ROGUE_DEP_TYPE_STRONG, 1, NULL);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        bool add_result = rogue_dependency_manager_add_file(manager, file_path, ROGUE_FILE_TYPE_OTHER, i);
        TEST_ASSERT(add_result == true, "Add multiple files within limits");
    }
    
    // Test adding many dependencies to one file
    RogueDependencyNode* test_node = rogue_dependency_manager_find_node(manager, "assets/test_0.json");
    if (test_node) {
        for (int i = 1; i < 10 && i < ROGUE_DEP_MAX_DEPENDENCIES; i++) {
            ...
            bool dep_result = rogue_dependency_manager_add_dependency(manager, "assets/test_0.json", target_file, ref_key, ROGUE_DEP_TYPE_STRONG, 1, NULL);
...
static void test_integration_scenarios(void) {
    ...
    // Core data files
    rogue_dependency_manager_add_file(manager, "config/core/affixes.json", ROGUE_FILE_TYPE_AFFIXES, 1);
    rogue_dependency_manager_add_file(manager, "config/core/items.json", ROGUE_FILE_TYPE_ITEMS, 2);
    rogue_dependency_manager_add_file(manager, "config/core/skills.json", ROGUE_FILE_TYPE_SKILLS, 3);
    
    // Game content files
    rogue_dependency_manager_add_file(manager, "config/content/loot_tables.json", ROGUE_FILE_TYPE_LOOT_TABLES, 4);
    rogue_dependency_manager_add_file(manager, "config/content/enemies.json", ROGUE_FILE_TYPE_ENEMIES, 5);
    rogue_dependency_manager_add_file(manager, "config/content/encounters.json", ROGUE_FILE_TYPE_ENCOUNTERS, 6);
    rogue_dependency_manager_add_file(manager, "config/content/biomes.json", ROGUE_FILE_TYPE_BIOMES, 7);
    
    // Set up realistic dependency chain
    rogue_dependency_manager_add_dependency(manager, "config/core/items.json", "config/core/affixes.json", "fire_damage_affix", ROGUE_DEP_TYPE_STRONG, 1, "Items use affixes for properties");
    rogue_dependency_manager_add_dependency(manager, "config/content/loot_tables.json", "config/core/items.json", "basic_sword", ROGUE_DEP_TYPE_STRONG, 1, "Loot tables reference items");
    rogue_dependency_manager_add_dependency(manager, "config/content/enemies.json", "config/content/loot_tables.json", "goblin_loot", ROGUE_DEP_TYPE_STRONG, 1, "Enemies have loot tables");
    rogue_dependency_manager_add_dependency(manager, "config/content/enemies.json", "config/core/skills.json", "fireball", ROGUE_DEP_TYPE_WEAK, 2, "Enemies optionally use skills");
    rogue_dependency_manager_add_dependency(manager, "config/content/encounters.json", "config/content/enemies.json", "goblin_warrior", ROGUE_DEP_TYPE_STRONG, 1, "Encounters reference enemies");
    rogue_dependency_manager_add_dependency(manager, "config/content/biomes.json", "config/content/encounters.json", "forest_encounter", ROGUE_DEP_TYPE_STRONG, 1, "Biomes reference encounters");
    ...
    for (int i = 0; i < game_load_order.file_count; i++) {
        if (strstr(game_load_order.files[i], "affixes")) affix_idx = i;
        else if (strstr(game_load_order.files[i], "items")) item_idx = i;
        else if (strstr(game_load_order.files[i], "loot_tables")) loot_idx = i;
    }
    TEST_ASSERT(affix_idx < item_idx && item_idx < loot_idx, "Game config dependency order is correct");
    
    // Scenario 2: Test impact analysis for core file change
    RogueImpactAnalysis core_impact;
    bool core_analysis = rogue_dependency_manager_analyze_impact(manager, "config/core/affixes.json", &core_impact);
    ...
    bool leaf_analysis = rogue_dependency_manager_analyze_impact(manager, "config/content/biomes.json", &leaf_impact);
