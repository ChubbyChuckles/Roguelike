...
static void test_proc_anomaly_scan(void)
{
    rogue_procs_reset();
    RogueProcDef fast = {0};
    fast.trigger = ROGUE_PROC_ON_HIT;
    fast.icd_ms = 0;
    fast.duration_ms = 0;
    fast.stack_rule = ROGUE_PROC_STACK_IGNORE;
    rogue_proc_register(&fast);
    RogueProcDef slow = {0};
    slow.trigger = ROGUE_PROC_ON_HIT;
    slow.icd_ms = 500;
    slow.duration_ms = 0;
    slow.stack_rule = ROGUE_PROC_STACK_IGNORE;
    rogue_proc_register(&slow);
    /* Simulate 120 hits over 6000ms (~60 hits/minute window smaller but normalized) */
    for (int i = 0; i < 120; i++)
    {
        rogue_procs_event_hit(0);
        rogue_procs_update(50, 50, 100);
    }
    RogueProcAnomaly a[4];
    int found = rogue_integrity_scan_proc_anomalies(
        a, 4, 500.0f); /* absurdly low threshold to catch fast proc */
    assert(found >= 1);
}
...
static void test_chain_and_duplicate_detection(void)
{
    ...
    int b = rogue_items_spawn(0, 1, 0, 0);
    assert(a >= 0 && b >= 0 && a != b);
    assert(rogue_equip_try(ROGUE_EQUIP_WEAPON, a) == 0);
    assert(rogue_equip_try(ROGUE_EQUIP_OFFHAND, b) == 0 ||
           1); /* offhand may fail if weapon two-handed */
    /* Chain mismatch scan should be zero because expected hash recomputed from occupancy equals
     * stored */
    ...
    const RogueItemInstance* it = rogue_item_instance_at(a);
    assert(it);
    ((RogueItemInstance*) it)->equip_hash_chain ^= 0xFFFFu; /* unsafe cast for test */
    RogueItemChainMismatch mm[4];
    int mmc = rogue_integrity_scan_equip_chain_mismatches(mm, 4);
    assert(mmc >= 1);
    /* Duplicate GUID: copy guid of b into a */
    const RogueItemInstance* ib = rogue_item_instance_at(b);
    assert(ib);
...
int main(void)
{
    test_proc_anomaly_scan();
    test_affix_blacklist();
    test_chain_and_duplicate_detection();
    printf("equipment_phase15_replay_auditor_ok\n");
    return 0;
}
