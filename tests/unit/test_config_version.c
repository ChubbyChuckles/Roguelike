...
static void test_event_type_collision_detection(void)
{
    ...
    bool has_collision =
        rogue_event_type_check_collision(0x0C10, collision_info, sizeof(collision_info));
    ...
    has_collision =
        rogue_event_type_check_collision(0x0C11, collision_info, sizeof(collision_info));
    TEST_ASSERT(has_collision == false, "Should not detect collision for unregistered event ID");

    /* Test collision with reserved range */
    has_collision =
        rogue_event_type_check_collision(0x0010, collision_info, sizeof(collision_info));
...
static void test_config_file_validation(void)
{
    ...
    RogueConfigValidationResult result =
        rogue_config_validate_file("non_existent_file.cfg", error_details, sizeof(error_details));
    ...
    if (test_file)
    {
        ...
        /* Test valid file */
        result =
            rogue_config_validate_file("test_config.cfg", error_details, sizeof(error_details));
...
static void test_fuzz_event_type_registration(void)
{
    ...
    for (uint32_t i = 0; i < fuzz_test_count; i++)
    {
        ...
        bool has_collision =
            rogue_event_type_check_collision(event_id, collision_info, sizeof(collision_info));
