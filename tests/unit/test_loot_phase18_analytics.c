...
int main(void)
{
    ...
    int pass = 1;
    pass &= expect(count == ROGUE_LOOT_ANALYTICS_RING_CAP, "ring cap count");
    ...
    {
        char buf[64];
        snprintf(buf, sizeof buf, "rarity count r=%d", r);
        pass &= expect(rc[r] == 120, buf);
    }
    char json[256];
    int jrc = rogue_loot_analytics_export_json(json, sizeof json);
    pass &= expect(jrc == 0, "json export ok");
    pass &= expect(strstr(json, "\"drop_events\":512") != NULL, "json drop_events");
    pass &= expect(strstr(json, "\"rarity_counts\":[120,120,120,120,120]") != NULL,
                   "json rarity counts");
    if (pass)
        printf("OK\n");
    return pass ? 0 : 1;
