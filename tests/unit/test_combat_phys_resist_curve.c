...
static int apply_once(int phys_resist_raw){
    RoguePlayer p; rogue_player_init(&p); p.base.pos.x=0; p.base.pos.y=0; p.facing=2; p.strength=80; /* stronger for higher base damage */
    RoguePlayerCombat c; rogue_combat_init(&c); c.phase=ROGUE_ATTACK_STRIKE; c.archetype=ROGUE_WEAPON_LIGHT; c.chain_index=0; c.strike_time_ms=20.0f;
    RogueEnemy e; e.alive=1; e.base.pos.x=0.9f; e.base.pos.y=0; e.max_health=100000; e.health=100000; e.armor=0; e.resist_physical=phys_resist_raw;
...
int main(){
    ...
    int diff1 = d0 - d30; int diff2 = d30 - d60; int diff3 = d60 - d90;
    assert(diff1 > diff2);
    assert(diff2 >= diff3);
