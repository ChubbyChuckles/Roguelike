...
int main()
{
    ...
    const char* base_json =
        "[ { \"set_id\": 10, \"bonuses\": [ { \"pieces\":2, \"strength\":5 } ] }, { \"set_id\": "
        "20, \"bonuses\": [ { \"pieces\":3, \"dexterity\":4 } ] } ]";
    const char* mod_json =
        "[ { \"set_id\": 10, \"bonuses\": [ { \"pieces\":2, \"strength\":6 } ] }, { \"set_id\": "
        "30, \"bonuses\": [ { \"pieces\":4, \"vitality\":7 } ] } ]"; /* set 20 removed, 30 added, 10
                                                                        changed */
    ck(write_file(base_path, base_json) == 0, "write base");
    ck(write_file(mod_path, mod_json) == 0, "write mod");
    char buf[256];
    int n = rogue_sets_diff(base_path, mod_path, buf, sizeof buf);
    ck(n > 0, "diff success");
