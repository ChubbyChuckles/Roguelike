...
int main(void)
{
    ...
    FILE* f = fopen("unique_tmp_items.cfg", "wb");
    assert(f);
    ...
    int helm = spawn("plain_helm");
    assert(rogue_equip_try(ROGUE_EQUIP_ARMOR_HEAD, helm) == 0);
    rogue_equipment_apply_stat_bonuses(&p);
    rogue_stat_cache_force_update(&p);
    unsigned long long fp2 = rogue_stat_cache_fingerprint();
    assert(fp2 != 0 && fp1 != 0);
    /* Unique layer unchanged by plain helm; fingerprint may change due to armor/resist but unique
     * stats remain */
    assert(g_player_stat_cache.unique_strength == 7);
    ...
    RogueUniqueDef u2 = {0};
#if defined(_MSC_VER)
    strcpy_s(u2.id, sizeof u2.id, "crown_of_wisdom");
    strcpy_s(u2.base_item_id, sizeof u2.base_item_id, "plain_helm");
#else
    strncpy(u2.id, "crown_of_wisdom", sizeof u2.id - 1);
    strncpy(u2.base_item_id, "plain_helm", sizeof u2.base_item_id - 1);
#endif
    u2.intelligence = 6;
    u2.resist_lightning = 8;
