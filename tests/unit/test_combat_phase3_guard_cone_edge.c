...
/* Guard cone edge tolerance: ensure dot slightly below threshold fails, at threshold succeeds, slightly above succeeds. */
int main(){
    RoguePlayer p; rogue_player_init(&p); g_exposed_player_for_stats = p; p.facing = 0; /* facing down (0,1) */
    int ok = rogue_player_begin_guard(&p, 0); assert(ok==1);
    float fdx=0.0f, fdy=1.0f; /* facing vector */
    /* We craft attack dirs with given dot vs facing by rotating slightly. Facing (0,1); vector (sin(theta), cos(theta)) gives dot=cos(theta). */
    ...
    blocked=0; perfect=0;
    int applied_below = rogue_player_apply_incoming_melee(&p,(float)dmg,sinf(theta_below),cosf(theta_below),15,&blocked,&perfect);
    ...
    blocked=0; perfect=0;
    int applied_at = rogue_player_apply_incoming_melee(&p,(float)dmg,sinf(theta_at),cosf(theta_at),15,&blocked,&perfect);
    assert(blocked==1);
    /* Above threshold: also blocked */
    p.guard_active_time_ms = 10.0f;
    blocked=0; perfect=0;
    int applied_above = rogue_player_apply_incoming_melee(&p,(float)dmg,sinf(theta_above),cosf(theta_above),15,&blocked,&perfect);
    assert(blocked==1);
    printf("phase3_guard_cone_edge: OK (below=%d at=%d above=%d)\n", applied_below, applied_at, applied_above);
    return 0;
}
