...
int main(){
    ...
    g_app.viewport_w=320; g_app.viewport_h=240; g_app.show_minimap=1; g_app.show_metrics_overlay=1;
    /* Simulate alerts */
    rogue_alerts_reset();
    ...
    if(g_app.last_alerts_rendered==0){ printf("FAIL expected alerts rendered>0\n"); return 1; }
    /* Toggle minimap off/on and check flag flipping logic in app_step integration (simulate) */
    g_app.show_minimap=0; if(g_app.show_minimap!=0){ printf("FAIL minimap toggle off\n"); return 1; }
    g_app.show_minimap=1; if(!g_app.show_minimap){ printf("FAIL minimap toggle on\n"); return 1; }
    g_app.show_metrics_overlay=1; rogue_metrics_overlay_render(); /* requires renderer to actually draw; here just ensure function callable */
