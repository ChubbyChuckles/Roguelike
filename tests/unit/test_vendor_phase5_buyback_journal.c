...
static void test_depreciation_monotonic(void){
    rogue_vendor_buyback_reset(); now_ms=0; int vendor=1;
    ...
    int before = rogue_vendor_buyback_list(vendor,list,4,now_ms); assert(before==1);
...
static void test_journal_hash_determinism(void){
    rogue_vendor_tx_journal_reset(); now_ms=0; unsigned long long g=0x9999ULL;
    for(int i=0;i<10;i++){ rogue_vendor_tx_journal_record(0,g+i, (i%2)+1, 100+i, 0, 0); }
...
static void test_duplicate_guid_detection(void){
    rogue_vendor_buyback_reset(); now_ms=0; int vendor=2;
