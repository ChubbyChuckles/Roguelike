...
int main()
{
    ...
    enemies[0].alive = 1;
    enemies[0].base.pos.x = 0.8f;
    enemies[0].base.pos.y = 0;
    enemies[0].health = 500;
    enemies[0].max_health = 500;
    /* Advance to light_3 (multi-hit) */
    /* light_1 durations: 110 + 70 + 120 = 300 */
    rogue_combat_update_player(&combat, 0, 1);
    simulate_ms(&combat, 310);
    /* light_2 durations: 95 + 65 + 115 = 275 */
    rogue_combat_update_player(&combat, 0, 1);
    simulate_ms(&combat, 285);
    rogue_combat_update_player(&combat, 0,
                               1); /* attempt to start next, but enforce chain manually for test */
    ...
    printf("combat_multi_hit: OK (initial=%d first=%d second=%d final=%d)\n", health_before,
           after_first, after_second, enemies[0].health);
    return 0;
}
