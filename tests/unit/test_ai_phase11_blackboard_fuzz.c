...
#define MODEL_MAX 32
static ModelEntry model[MODEL_MAX];
static int model_count = 0;
static ModelEntry* model_get(const char* key)
{
    ...
    {
        model[model_count].key = key;
        model[model_count].present = 0;
...
int main(void)
{
    ...
    for (int iter = 0; iter < 5000; ++iter)
    {
        ...
        int op = (r >> 3) %
                 6; /* 0:set_int 1:set_float 2:write_int 3:write_float 4:set_timer 5:set_vec2 */
        int val = (int) (r >> 9) & 0xFF;
        float fval = (float) ((r >> 11) % 1000) / 10.0f; /* 0..99.9 */
        ModelEntry* me = model_get(k);
        assert(me);
        switch (op)
        {
        case 0:
            rogue_bb_set_int(&bb, k, val);
            me->i = val;
            me->present = 1;
            break;
        case 1:
            rogue_bb_set_float(&bb, k, fval);
            me->f = fval;
            me->present = 1;
            break;
        case 2:
        {
            ...
            int before = me->i;
            int changed = 0;
            if (pol == ROGUE_BB_WRITE_SET)
            {
                me->i = val;
                changed = 1;
            }
            else if (pol == ROGUE_BB_WRITE_MAX)
            {
                if (val > me->i)
                {
                    me->i = val;
            ...
            else if (pol == ROGUE_BB_WRITE_MIN)
            {
                if (val < me->i)
                {
                    me->i = val;
            ...
            else if (pol == ROGUE_BB_WRITE_ACCUM)
            {
                me->i += val;
                changed = 1;
            }
        ...
        break;
        case 4:
            rogue_bb_set_timer(&bb, k, (float) (val % 10));
            break;
        case 5:
            rogue_bb_set_vec2(&bb, k, (float) (val % 50), (float) ((val * 3) % 50));
            break;
        }
