...
static int test_disable_behavior(void)
{
    ...
    if (!approx(rogue_enemy_adaptive_scalar(), 1.0f, 0.0001f))
    {
        printf("FAIL adaptive disabled scalar drift\n");
        return 1;
    }
    return 0;
}
...
static int test_upward_pressure(void)
{
    ...
    if (s < 1.01f)
    {
        printf("FAIL upward pressure scalar not increased (%.3f)\n", s);
        return 1;
    }
    if (s > ROGUE_ENEMY_ADAPTIVE_MAX_SCALAR + 0.001f)
    {
        printf("FAIL upward pressure exceeded max (%.3f)\n", s);
        return 1;
    }
    return 0;
}
...
static int test_downward_pressure(void)
{
    ...
    for (int k = 0; k < 14; k++)
    {
        ...
        if (k % 3 == 0)
            rogue_enemy_adaptive_submit_potion_used();
        if (k % 5 == 0)
            rogue_enemy_adaptive_submit_player_death();
        rogue_enemy_adaptive_tick(1.0f);
    }
    float s = rogue_enemy_adaptive_scalar();
    if (s > 0.99f)
    {
        printf("FAIL downward pressure scalar not decreased (%.3f)\n", s);
        return 1;
    }
    if (s < ROGUE_ENEMY_ADAPTIVE_MIN_SCALAR - 0.001f)
    {
        printf("FAIL downward pressure below min (%.3f)\n", s);
        return 1;
    }
    return 0;
}
...
static int test_relaxation(void)
{
    ...
    for (int k = 0; k < 10; k++)
    {
        rogue_enemy_adaptive_submit_kill(2.5f);
        rogue_enemy_adaptive_tick(1.0f);
    }
    if (rogue_enemy_adaptive_scalar() <= 1.0f)
    {
        printf("FAIL pre-relax push failed\n");
        return 1;
    }
    /* Now neutral period */
    ...
    if (!(after < before && after > 1.0f - 0.05f))
    {
        printf("FAIL relaxation not trending toward 1 (before=%.3f after=%.3f)\n", before, after);
        return 1;
    }
    return 0;
}
...
int main(void)
{
    if (test_disable_behavior() != 0)
    {
        printf("stage: disable_behavior failed (scalar=%.3f)\n", rogue_enemy_adaptive_scalar());
        return 1;
    }
    if (test_upward_pressure() != 0)
    {
        printf("stage: upward_pressure failed (scalar=%.3f)\n", rogue_enemy_adaptive_scalar());
        return 1;
    }
    if (test_downward_pressure() != 0)
    {
        printf("stage: downward_pressure failed (scalar=%.3f)\n", rogue_enemy_adaptive_scalar());
        return 1;
    }
    if (test_relaxation() != 0)
    {
        printf("stage: relaxation failed (scalar=%.3f)\n", rogue_enemy_adaptive_scalar());
        return 1;
    }
