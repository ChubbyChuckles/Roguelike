...
static void expect_fail(const char* pattern){
    ...
    if(rc==0){ fprintf(stderr,"expected failure for pattern '%s'\n", pattern?pattern:"(null)"); assert(0); }
}
static void expect_ok(const char* pattern){
    int rc = rogue_runeword_validate_pattern(pattern);
    if(rc!=0){ fprintf(stderr,"expected ok for pattern '%s' rc=%d\n", pattern, rc); assert(0); }
}
...
int main(void){
    ...
    expect_fail("segment__double"); /* empty segment between __ treated as invalid char rule (since second '_' increments segment count with no content) */
    ...
    RogueRuneword rw={0}; strcpy(rw.pattern,"fire_ice"); rw.strength=2; assert(rogue_runeword_register(&rw)>=0);
    RogueRuneword bad={0}; strcpy(bad.pattern,"BAD"); assert(rogue_runeword_register(&bad)<0);
