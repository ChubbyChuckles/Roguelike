...
static void test_quick_actions_basic()
{
    setup();
    ...
    {
        if (rogue_inventory_quick_action_name(i, name, sizeof name) == 0)
        {
            if (strcmp(name, "HiTier") == 0)
                found = 1;
        }
    }
    assert(found);
    ...
    {
        if (rogue_inventory_quick_action_name(i, name, sizeof name) == 0 &&
            strcmp(name, "HiTier") == 0)
        {
            n = rogue_inventory_quick_action_apply(i, ids, 32);
            break;
        }
    }
...
int main()
{
    test_quick_actions_basic();
    printf("inventory_phase4_quick_actions: OK\n");
    return 0;
}
