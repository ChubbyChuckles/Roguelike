...
int main(void)
{
    ...
    g_attack_frame_override = 0;
    rogue_combat_player_strike(&combat, &player, list, 1);
    if (list[0].health != 10)
    {
        printf("frame0 should not damage\n");
        return 1;
    }
    g_attack_frame_override = 1;
    rogue_combat_player_strike(&combat, &player, list, 1);
    if (list[0].health != 10)
    {
        printf("frame1 should not damage\n");
        return 1;
    }
    /* Frame 2 still mask=1? (mask was 0,0,1...) ensure it can damage */
    g_attack_frame_override = 2;
    rogue_combat_player_strike(&combat, &player, list, 1);
    if (list[0].health == 10)
    {
        printf("frame2 should damage\n");
        return 1;
    }
