...
int test_local_get_current_attack_frame(void){ return current_frame; }
...
int main(void){
    ...
    current_frame=0; rogue_combat_player_strike(&combat,&player,list,1); if(list[0].health!=10){ printf("frame0 should not damage\n"); return 1; }
    current_frame=1; rogue_combat_player_strike(&combat,&player,list,1); if(list[0].health!=10){ printf("frame1 should not damage\n"); return 1; }
    /* Frame 2 still mask=1? (mask was 0,0,1...) ensure it can damage */
    current_frame=2; rogue_combat_player_strike(&combat,&player,list,1); if(list[0].health==10){ printf("frame2 should damage\n"); return 1; }
