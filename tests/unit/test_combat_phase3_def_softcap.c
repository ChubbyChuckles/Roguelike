...
int main(){
    RogueEnemy e; memset(&e,0,sizeof e); e.alive=1; e.health=100000; e.max_health=100000;
    /* Case A: moderate defenses below soft cap threshold */
    e.armor = 50; e.resist_physical = 20; int over=0; int raw=400; int dmgA = rogue_apply_mitigation_enemy(&e, raw, ROGUE_DMG_PHYSICAL, &over); assert(dmgA>0);
    /* Case B: very high defenses triggering soft cap. Use much larger armor + resist. */
    e.armor = 600; e.resist_physical = 80; int dmgB = rogue_apply_mitigation_enemy(&e, raw, ROGUE_DMG_PHYSICAL, &over); assert(dmgB>0);
    printf("def_softcap: raw=%d below=%d above=%d\n", raw, dmgA, dmgB);
    /* Soft cap should prevent damage from dropping proportionally as much: ensure dmgB is not less than 5% of raw (cap ~85%). */
