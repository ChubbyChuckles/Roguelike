...
int main(){
    ...
    enemies[0].alive=1; enemies[0].base.pos.x=1.2f; enemies[0].base.pos.y=0.0f; enemies[0].health=200; enemies[0].max_health=200;
    enemies[1].alive=1; enemies[1].base.pos.x=0.0f; enemies[1].base.pos.y=2.5f; enemies[1].health=200; enemies[1].max_health=200;
    enemies[2].alive=1; enemies[2].base.pos.x=-1.5f; enemies[2].base.pos.y=0.0f; enemies[2].health=200; enemies[2].max_health=200;

    rogue_lockon_reset(&player);
    if(!rogue_lockon_acquire(&player,enemies,3)){ printf("fail_acquire\n"); return 1; }
    printf("acquired idx=%d\n", player.lock_on_target_index);
    int initial = player.lock_on_target_index;
    if(!rogue_lockon_cycle(&player,enemies,3,1)){ printf("fail_cycle_first\n"); return 2; }
    printf("cycled first idx=%d\n", player.lock_on_target_index);
    int after_first = player.lock_on_target_index;
    /* Immediate second cycle should be blocked by cooldown */
    if(rogue_lockon_cycle(&player,enemies,3,1)){ printf("fail_cooldown_block_immediate\n"); return 3; }
    printf("cooldown immediate block OK idx=%d\n", player.lock_on_target_index);
    if(player.lock_on_target_index != after_first){ printf("fail_target_changed_on_block\n"); return 4; }
    /* Partial tick (< cooldown) */
    rogue_lockon_tick(&player,100.0f); if(rogue_lockon_cycle(&player,enemies,3,1)){ printf("fail_cooldown_block_partial\n"); return 5; }
    printf("cooldown partial block OK\n");
    /* Complete remaining tick */
    rogue_lockon_tick(&player,90.0f); if(!rogue_lockon_cycle(&player,enemies,3,1)){ printf("fail_cycle_after_cooldown\n"); return 6; }
