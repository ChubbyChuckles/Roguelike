...
static int sample_cap_handler(int def_index, uint64_t add_qty){
    (void)def_index; (void)add_qty; g_handler_invocations++;
    /* Simulate mitigation by removing an existing entry if any beyond index 0. */
    /* For simplicity this test just returns non-zero (abort) on first call, zero on second to exercise retry path. */
    if(g_handler_invocations==1) return -1; /* abort first */
...
static void test_labels(){
    ...
    assert(rogue_inventory_entry_set_labels(5, ROGUE_INV_LABEL_MATERIAL|ROGUE_INV_LABEL_GEAR)==0);
...
static void test_delta_tracking(){
    rogue_inventory_entries_init();
    int def_indices[16]; uint64_t quantities[16];
    /* No changes yet */
    ...
    assert(d==2);
    int seen1=0, seen2=0; for(unsigned i=0;i<d;i++){ if(def_indices[i]==1){ assert(quantities[i]==10); seen1=1;} if(def_indices[i]==2){ assert(quantities[i]==5); seen2=1;} }
    assert(seen1 && seen2);
