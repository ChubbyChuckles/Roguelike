...
int main(void){
    ...
    FILE* f=fopen(tsv,"wb"); if(!f){ fprintf(stderr,"FAIL open tsv\n"); return 1; }
    ...
    if(c != 2){ fprintf(stderr,"FAIL expected 2 converted got %d\n", c); return 2; }
    ...
    if(!def0 || !def1){ fprintf(stderr,"FAIL defs missing after load\n"); return 4; }
    if(def0->rarity != 0){ fprintf(stderr,"FAIL rarity mismatch %d\n", def0->rarity); return 5; }
    remove(tsv); remove(csv);
