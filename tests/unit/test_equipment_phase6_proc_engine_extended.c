...
{
    setup();
    RogueProcDef refresh = {0};
    refresh.trigger = ROGUE_PROC_ON_HIT;
    refresh.icd_ms = 50;
    refresh.duration_ms = 200;
    refresh.stack_rule = ROGUE_PROC_STACK_REFRESH;
    rogue_proc_register(&refresh);
    RogueProcDef stack = {0};
    stack.trigger = ROGUE_PROC_ON_HIT;
    stack.icd_ms = 0;
    stack.duration_ms = 300;
    stack.stack_rule = ROGUE_PROC_STACK_STACK;
    stack.max_stacks = 3;
    rogue_proc_register(&stack);
    RogueProcDef ignore = {0};
    ignore.trigger = ROGUE_PROC_ON_HIT;
    ignore.icd_ms = 0;
    ignore.duration_ms = 300;
    ignore.stack_rule = ROGUE_PROC_STACK_IGNORE;
    rogue_proc_register(&ignore);
    /* Fire rapid hits: 10 hits every 20ms = 200ms total */
    for (int i = 0; i < 10; i++)
    {
        rogue_procs_event_hit(0);
        rogue_procs_update(20, 80, 100);
    }
