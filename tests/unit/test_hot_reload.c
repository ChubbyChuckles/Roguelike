...
#define TEST_FUNCTION(name) printf("\n=== Running %s ===\n", #name); name()
...
static void test_file_watcher(void) {
    ...
    // Test adding watcher
    bool add_result = rogue_hot_reload_add_watcher(system, TEST_WATCH_DIR, false, test_callback, NULL);
...
static void test_notification_system(void) {
    ...
    // Send notifications
    bool notify_result1 = rogue_hot_reload_send_notification(system, "GameSystem", "config.cfg", ROGUE_RELOAD_CHANGE_MODIFIED, NULL);
    TEST_ASSERT(notify_result1 == true);
    TEST_ASSERT(system->notification_count == 1);
    
    bool notify_result2 = rogue_hot_reload_send_notification(system, "AudioSystem", "audio.cfg", ROGUE_RELOAD_CHANGE_CREATED, NULL);
...
static void test_utility_functions(void) {
    ...
    uint64_t files_watched, changes_detected, reloads_successful, reloads_failed, rollbacks_performed;
    rogue_hot_reload_get_statistics(system, &files_watched, &changes_detected, &reloads_successful, &reloads_failed, &rollbacks_performed);
...
static void test_error_handling(void) {
    ...
    // Test invalid paths
    TEST_ASSERT(rogue_hot_reload_add_watcher(system, "/non/existent/path", false, NULL, NULL) == false);
...
static void test_integration_scenarios(void) {
    ...
    rogue_hot_reload_send_notification(system, "GameSystem", config_file, ROGUE_RELOAD_CHANGE_MODIFIED, NULL);
    rogue_hot_reload_process_notifications(system);
