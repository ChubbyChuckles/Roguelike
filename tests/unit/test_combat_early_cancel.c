...
/* Validate early cancel: when hit_confirmed and buffered attack, STRIKE should end earlier than full STRIKE_MS (70ms baseline). */
int main(){
    RoguePlayer player; rogue_player_init(&player); player.base.pos.x=0; player.base.pos.y=0; player.facing=2; player.strength=10;
    RogueEnemy enemy; enemy.alive=1; enemy.base.pos.x=0.9f; enemy.base.pos.y=0.0f; enemy.health=50; enemy.max_health=50;
    RoguePlayerCombat pc; rogue_combat_init(&pc);
    /* Start attack */
    rogue_combat_update_player(&pc, 0, 1); /* press -> windup */
    /* Advance to strike */
    for(int t=0;t<5;t++) rogue_combat_update_player(&pc, 25.0f, 0);
    ...
    if(pc.phase == ROGUE_ATTACK_STRIKE){ printf("early cancel did not transition (phase still STRIKE)\n"); return 1; }
