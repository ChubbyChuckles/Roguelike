...
/* Replay harness: generate constrained inventory twice with same seed context and validate
 * identical snapshot hash. */

static void sort_arrays(int* defs, int* rar, int* prices, int n)
{
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
...
static unsigned int capture_snapshot_hash(const char* vendor_id, unsigned int world_seed, int epoch)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        defs[i] = it ? it->def_index : -1;
        rar[i] = it ? it->rarity : 0;
        prices[i] = it ? it->price : 0;
    }
    sort_arrays(defs, rar, prices, count);
    unsigned int pmh = rogue_vendor_price_modifiers_hash();
    return rogue_vendor_snapshot_hash(defs, rar, prices, count, world_seed, vendor_id,
                                      (unsigned int) epoch, pmh);
}
...
int main()
{
    ...
    const char* vendor_id = "test_vendor";
    unsigned int world_seed = 123456u;
    int epoch = 7;
    ...
    assert(first == second);
    assert(h1 == h2);
    /* Variation test: different epoch -> different hash OR zero inventory when no template (still
     * counts) */
