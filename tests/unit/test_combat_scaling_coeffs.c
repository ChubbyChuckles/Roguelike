...
static int strike_damage_once(RogueWeaponArchetype arch, int str, int dex, int intel){
    RoguePlayer p; rogue_player_init(&p); p.strength=str; p.dexterity=dex; p.intelligence=intel; p.base.pos.x=0; p.base.pos.y=0; p.facing=2; p.crit_chance=0; p.crit_damage=0;
    RoguePlayerCombat c; rogue_combat_init(&c); rogue_combat_set_archetype(&c, arch); c.phase=ROGUE_ATTACK_STRIKE; c.combo=0; /* direct strike */
    RogueEnemy e; e.alive=1; e.base.pos.x=0.8f; e.base.pos.y=0; e.health=10000; e.max_health=10000;
    int before = e.health; rogue_combat_player_strike(&c,&p,&e,1); return before - e.health;
}
