...
int main(void){
    ...
    write_file(gp, "# comment line\nVERSION=-2\n\nWATER_LEVEL=0.40\nNOISE_OCTAVES=7\nNOISE_GAIN=0.50\nNOISE_LACUNARITY=2.10\nRIVER_SOURCES=11\nRIVER_MAX_LENGTH=1300\nCAVE_THRESH=0.65\n");
    ...
    g_app.player.level = 7; g_app.stats_dirty=1; /* mark dirty just in case future gate added */
    rogue_persistence_save_player_stats();
    /* Read back ensure VERSION line present and updated level */
    FILE* f=fopen(ps,"rb"); assert(f); char line[256]; int saw_version=0, saw_level=0; while(fgets(line,sizeof line,f)){ if(strncmp(line,"VERSION=",8)==0) saw_version=1; if(strncmp(line,"LEVEL=7",7)==0) saw_level=1; } fclose(f); assert(saw_version && saw_level);
    /* Gen params save only when dirty */
    remove(gp); /* ensure file absence to detect write */
    g_app.gen_water_level = 0.55f; g_app.gen_params_dirty = 1; /* mark dirty to force write */
    rogue_persistence_save_generation_params_if_dirty();
    f=fopen(gp,"rb"); assert(f); int saw_wl=0; saw_version=0; while(fgets(line,sizeof line,f)){ if(strncmp(line,"VERSION=",8)==0) saw_version=1; if(strncmp(line,"WATER_LEVEL=0.55",16)==0) saw_wl=1; } fclose(f); assert(saw_version && saw_wl);
    return 0;
}
