...
static void assert_true(int c, const char* m){ if(!c){ printf("FAIL:%s\n", m); exit(1);} }

int main(){
    rogue_sets_reset();
    rogue_hot_reload_reset();
    const char* path = "tmp_equipment_sets_hot_reload_hash.json";
    const char* v1 = "[ { \"set_id\": 500, \"bonuses\": [ { \"pieces\":2, \"strength\":1 }, { \"pieces\":3, \"strength\":2 } ] } ]";
    ...
    const char* v2 = "[ { \"set_id\": 500, \"bonuses\": [ { \"pieces\":2, \"strength\":1 }, { \"pieces\":3, \"strength\":3 } ] } ]"; /* second bonus strength 2->3 */
