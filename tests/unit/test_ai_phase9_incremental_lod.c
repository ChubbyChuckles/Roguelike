...
int main(void)
{
    ...
    rogue_ai_lod_set_radius(
        10.0f); /* enemies farther than sqrt(100)=10 tiles get maintenance only */
    /* Setup player position */
    g_app.player.base.pos.x = 0.0f;
    g_app.player.base.pos.y = 0.0f;
    RogueEnemy enemies[8];
    for (int i = 0; i < 8; i++)
    {
        init_enemy(&enemies[i], (float) i * 2.0f, 0.0f);
    }
    ...
    for (int f = 0; f < 16; f++)
    {
        rogue_ai_scheduler_tick(enemies, 8, 0.05f);
    }
    if (enemies[1].base.pos.x >= start_x - 0.0001f)
    {
        printf("AI_INC_FAIL no_progress bucketed enemy start=%.2f cur=%.2f\n", start_x,
               enemies[1].base.pos.x);
        return 1;
    }
    /* Far enemy should not have moved significantly (MoveToPlayer drives toward 0). Allow tiny
     * epsilon. */
    if (enemies[7].base.pos.x < 49.0f)
    {
        printf("AI_INC_FAIL lod_far_moved dist=%f\n", enemies[7].base.pos.x);
        return 1;
    }
    ...
    for (int f = 0; f < 5; f++)
    {
        rogue_ai_scheduler_tick(enemies, 8, 0.1f);
    }
    if (enemies[7].base.pos.x >= far_start)
    {
        printf("AI_INC_FAIL lod_inclusion_no_move\n");
        return 1;
    }
    printf("AI_INC_OK buckets=%d frame=%u moved_near=%.2f far_delta=%.2f\n",
           rogue_ai_scheduler_get_buckets(), rogue_ai_scheduler_frame(),
           start_x - enemies[1].base.pos.x, far_start - enemies[7].base.pos.x);
    return 0;
}
