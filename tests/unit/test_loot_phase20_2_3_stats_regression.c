 ...
 * Strategy:
 *  - Roll a selected loot table many times (100k) capturing rarity outcomes for a target item
 * across its allowed band.
 *  - Compute empirical frequencies; assert each allowed rarity appears and distribution not overly
 * skewed. (Current sampler applies dynamic weights external to this test; baseline uniform expected
 * within band.)
 *  - Tolerance: no rarity frequency may deviate from uniform expectation by more than ABS_TOL (0.15
 * absolute).
 *  - Provides early warning if future weighting logic or pity/floor adjustments inadvertently bias
 * mid-band drastically.
 */
...
int main(void)
{
    ...
    float expected = 1.0f / (float) span;
    ...
    for (int it = 0; it < iterations; ++it)
    {
        ...
        if (rar >= rmin && rar <= rmax)
            counts[rar]++;
    }
    int total = counts[0] + counts[1] + counts[2];
    if (total == 0)
    {
        fprintf(stderr, "FAIL: no rarities sampled\n");
        return 2;
    }
    for (int r = rmin; r <= rmax; ++r)
    {
        ...
        {
            fprintf(stderr,
                    "FAIL: rarity %d frequency %.4f outside tolerance (expected ~%.4f +/- %.2f)\n",
    ...
    for (int r = rmin; r <= rmax; ++r)
    {
        if (counts[r] == 0)
        {
            fprintf(stderr, "FAIL: rarity %d never appeared\n", r);
            return 11;
        }
    }
    printf("loot_stats_regression_ok total=%d f0=%.4f f1=%.4f f2=%.4f\n", total,
           (float) counts[0] / total, (float) counts[1] / total, (float) counts[2] / total);
    return 0;
}
