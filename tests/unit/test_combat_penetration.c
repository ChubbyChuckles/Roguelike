...
/* Minimal stubs (local to this TU to avoid duplicate symbols when linking with core) */
static RoguePlayer g_exposed_player_for_stats = {0};
...
static int expected_after_pen(int raw, int armor, int phys_resist, int pen_flat, int pen_pct)
{
    if (raw < 0)
        raw = 0;
    /* Penetration order in strike code: flat -> percent(of original armor) -> result passed as new
     * armor to mitigation */
    int eff_armor = armor;
    if (pen_flat > 0)
    {
...
int main()
{
    ...
    player.strength = 60; /* produce some base raw */
    player.pen_flat = 5;
    player.pen_percent = 25; /* 5 flat then 25% of original armor */
    RoguePlayerCombat pc;
    rogue_combat_init(&pc);
    pc.phase = ROGUE_ATTACK_STRIKE;
    pc.chain_index = 0;
    pc.archetype = ROGUE_WEAPON_LIGHT;
    pc.strike_time_ms = 20.0f; /* inside first window */
    RogueEnemy enemy;
    enemy.alive = 1;
    ...
    int effective_strength = player.strength;
    float scaled = def->base_damage + (float) effective_strength * def->str_scale +
                   (float) player.dexterity * def->dex_scale +
                   (float) player.intelligence * def->int_scale;
    if (scaled < 1.0f)
        scaled = 1.0f;
    float combo_scale = 1.0f;
    float raw = scaled * combo_scale * 1.0f;
    int raw_i = (int) floorf(raw + 0.5f);
