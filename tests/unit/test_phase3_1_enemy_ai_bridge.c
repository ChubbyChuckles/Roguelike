...
static int test_enemy_spawn_ai_activation(void) {
    ...
    TEST_ASSERT(enemy->ai_intensity == ROGUE_AI_INTENSITY_NORMAL, "AI intensity should be set correctly");
...
static int test_boss_enemy_spawn_intensity(void) {
    ...
    TEST_ASSERT(boss_enemy->ai_intensity == ROGUE_AI_INTENSITY_BOSS, "Boss should get BOSS intensity");
...
static int test_elite_enemy_spawn_intensity(void) {
    ...
    TEST_ASSERT(elite_enemy->ai_intensity == ROGUE_AI_INTENSITY_ELITE, "Elite should get ELITE intensity");
...
static int test_ai_state_synchronization(void) {
    ...
    TEST_ASSERT(bridge.state_sync_requests >= 2, "Sync requests should be tracked"); // spawn + explicit sync
...
static int test_enemy_death_cleanup(void) {
    ...
    TEST_ASSERT(metrics->active_behavior_trees == 0, "Should have 0 active behavior trees after death");
...
static int test_group_creation_and_management(void) {
    ...
    // Create group
    uint32_t group_id = rogue_enemy_ai_bridge_create_group(&bridge, enemy_ids, 4, formation_pattern);
...
static int test_debug_functionality(void) {
    ...
    int chars_written = rogue_enemy_ai_bridge_get_debug_status(&bridge, debug_buffer, sizeof(debug_buffer));
    TEST_ASSERT(chars_written > 0, "Debug status should return formatted text");
    TEST_ASSERT(strstr(debug_buffer, "Enemy-AI Bridge Status") != NULL, "Debug status should contain header");
