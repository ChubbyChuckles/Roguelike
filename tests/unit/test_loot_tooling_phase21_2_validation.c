...
int main(void){
    ...
    if(!f){ fprintf(stderr, "FAIL: could not create temp file\n"); return 1; }
    ...
    int lines[8]; memset(lines,0,sizeof lines);
    int malformed = rogue_item_defs_validate_file(path, lines, 8);
    if(malformed != 1){ fprintf(stderr, "FAIL: expected 1 malformed got %d\n", malformed); return 2; }
    if(lines[0] == 0){ fprintf(stderr, "FAIL: first malformed line not recorded\n"); return 3; }
    /* Clean up */
    remove(path);
