...
int main(){
    ...
    assert(idx_fire>=0 && idx_frost>=0 && idx_arc>=0 && idx_rec>=0 && idx_thp>=0 && idx_thc>=0);
    const RogueAffixDef* a = rogue_affix_at(idx_fire); assert(a && a->min_value>=5 && a->max_value<=15);
