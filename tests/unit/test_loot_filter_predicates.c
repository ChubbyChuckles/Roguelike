...
{
    fprintf(stderr, "PRED_FAIL:%s\n", m);
    return 1;
}

/* Helper to write a temp filter file */
static int write_rules(const char* name, const char* text)
{
    FILE* f = NULL;
    f = fopen(name, "wb");
    if (!f)
...
int main(void)
{
    ...
    if (iron < 0 || steel < 0 || epic < 0 || mat < 0)
        return fail("defs_missing");
    ...
    if (inst_iron < 0 || inst_steel < 0 || inst_epic < 0 || inst_mat < 0)
        return fail("spawn");

    /* Test 1: MODE=ALL rarity>=1 AND category=weapon => hides material only (iron rarity0 <1);
     * steel (1) & epic (3) remain */
    if (!write_rules("flt.cfg", "MODE=ALL\nrarity>=1\ncategory=weapon\n"))
        return fail("write1");
    rogue_loot_filter_reset();
    if (rogue_loot_filter_load("flt.cfg") != 2)
        return fail("load1_count");
    rogue_loot_filter_refresh_instances();
    int vis = rogue_items_visible_count();
    if (vis != 2)
        return fail("vis_all_expected2");
    ...
    rogue_loot_filter_reset();
    if (rogue_loot_filter_load("flt.cfg") != 2)
        return fail("load2_count");
    rogue_loot_filter_refresh_instances();
    vis = rogue_items_visible_count();
    if (vis != 2)
        return fail("vis_any_expected2");

    /* Test 3: MODE=ANY name~sword rarity>=3 => epic_blade passes (rarity>=3) and both swords pass
     * name substring => visible=3 (iron, steel, epic) */
    if (!write_rules("flt.cfg", "MODE=ANY\nname~sword\nrarity>=3\n"))
        return fail("write3");
    rogue_loot_filter_reset();
    if (rogue_loot_filter_load("flt.cfg") != 2)
        return fail("load3_count");
    rogue_loot_filter_refresh_instances();
    vis = rogue_items_visible_count();
    if (vis != 3)
        return fail("vis_any_expected3");
