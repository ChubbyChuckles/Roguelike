...
static void build_graph(RogueUIContext* ui)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        rogue_ui_skillgraph_add(ui, (float) (i * 22), (float) ((i % 4) * 30), i, i % 3, 3,
                                (i % 5) == 0, 0);
    }
}
...
int main()
{
    ...
    rogue_ui_end(&ui);
    printf("FRAME0 emitted=%d pulses=%d spends=%d\n", emitted, ui.skillgraph_pulse_count,
           ui.skillgraph_spend_count);
    fflush(stdout);
    int count1 = 0;
    const RogueUINode* nodes1 = rogue_ui_nodes(&ui, &count1);
    (void) nodes1;
    if (emitted <= 0 || count1 <= 0)
    {
    ...
    for (int f = 0; f < 20; ++f)
    {
        rogue_ui_begin(&ui, 40.0);
        build_graph(&ui);
        emitted = rogue_ui_skillgraph_build(&ui);
        rogue_ui_end(&ui);
    }
    printf("AFTER frames pulses=%d spends=%d\n", ui.skillgraph_pulse_count,
           ui.skillgraph_spend_count);
    fflush(stdout);
    /* After ~600ms pulses should be gone */
    if (ui.skillgraph_pulse_count != 0)
    {
        printf("FAIL pulses not cleared (%d)\n", ui.skillgraph_pulse_count);
        return 1;
    }
    if (ui.skillgraph_spend_count != 0)
    {
        printf("FAIL spend flyouts not cleared (%d)\n", ui.skillgraph_spend_count);
        return 1;
    }
