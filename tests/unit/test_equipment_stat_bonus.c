...
/* Minimal deterministic test: create one weapon instance with agility affix and ensure dex
 * increases after apply. */
int main()
{
    ...
    {
        printf("EQUIP_STAT_FAIL find_items\n");
        return 1;
    }
    int added = rogue_item_defs_load_from_cfg(pitems);
    if (added <= 0)
    {
        printf("EQUIP_STAT_FAIL item_defs count=%d\n", added);
        return 1;
    }
    rogue_affixes_reset();
    char paff[256];
    ...
    {
        printf("EQUIP_STAT_FAIL find_affixes\n");
        return 2;
    }
    if (rogue_affixes_load_from_cfg(paff) <= 0)
    {
        printf("EQUIP_STAT_FAIL affixes\n");
        return 2;
    }
    ...
    if (agility_affix < 0)
    {
        printf("EQUIP_STAT_FAIL no_agility_affix\n");
        return 3;
    }
    it->suffix_index = agility_affix;
    it->suffix_value = 3; /* deterministic value */
    rogue_equip_reset();
    int rc = rogue_equip_try(ROGUE_EQUIP_WEAPON, inst);
    if (rc != 0)
    {
        printf("EQUIP_STAT_FAIL equip rc=%d\n", rc);
        return 4;
    }
    RoguePlayer p;
    rogue_player_init(&p);
    int base_dex = p.dexterity;
    if (base_dex <= 0)
    {
        printf("EQUIP_STAT_FAIL base_dex=%d\n", base_dex);
        return 5;
    }
    rogue_equipment_apply_stat_bonuses(&p);
    if (!(p.dexterity == base_dex + 3))
    {
        printf("EQUIP_STAT_FAIL dex=%d expected=%d\n", p.dexterity, base_dex + 3);
        return 6;
    }
