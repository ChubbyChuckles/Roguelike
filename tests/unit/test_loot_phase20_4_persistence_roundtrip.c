 ...
 * - Generates a batch of items via generation pipeline across multiple loot tables with varying
 * context seeds.
...
{
    int def_index, qty, rarity, pidx, pval, sidx, sval, dcur, dmax, enchant;
} SnapRec;
...
static int snapshot(SnapRec* out, int cap)
{
    ...
    for (int i = 0; i < ROGUE_ITEM_INSTANCE_CAP && count < cap; i++)
    {
        ...
        if (!it || !it->active)
            continue;
        out[count++] =
            (SnapRec){it->def_index,      it->quantity,     it->rarity,       it->prefix_index,
...
int main(void)
{
    ...
    {
        fprintf(stderr, "FAIL: find test_items.cfg\n");
        return 1;
    }
    if (rogue_item_defs_load_from_cfg(pitems) <= 0)
    {
        fprintf(stderr, "FAIL: load test_items.cfg\n");
        return 1;
    }
    ...
    {
        fprintf(stderr, "FAIL: find test_loot_tables.cfg\n");
        return 1;
    }
    if (rogue_loot_tables_load_from_cfg(ptables) <= 0)
    {
        fprintf(stderr, "FAIL: load test_loot_tables.cfg\n");
        return 1;
    }
    rogue_affixes_reset();
    char paff[256];
    ...
    {
        fprintf(stderr, "FAIL: find affixes.cfg\n");
        return 1;
    }
    if (rogue_affixes_load_from_cfg(paff) < 0)
    {
        fprintf(stderr, "FAIL: load affixes.cfg\n");
        return 1;
    }
    ...
    {
        char ej[256];
        if (rogue_save_export_json(0, ej, sizeof ej) == 0)
        {
            fprintf(stderr, "DIAG: early save JSON=%s\n", ej);
        }
    }
    ...
    if (tbl_orc < 0 || tbl_skel < 0)
    {
        fprintf(stderr, "FAIL: loot table indices orc=%d skel=%d\n", tbl_orc, tbl_skel);
        return 3;
    }
    ...
    {
        ctx.enemy_level = (i % 40);
        ctx.player_luck = (i % 15);
        ctx.biome_id = i % 5;
        ctx.enemy_archetype = i % 7;
        ...
        if (rogue_generate_item(tbl, &ctx, &seed, &gi) == 0)
        {
            ...
            {
                int cur, max;
                if (rogue_item_instance_get_durability(gi.inst_index, &cur, &max) == 0 && max > 0)
                {
                    rogue_item_instance_damage_durability(gi.inst_index, (i % 3) == 0 ? 1 : 0);
                }
            }
        }
    }
    ...
    if (active_gen <= 0)
    {
        fprintf(stderr, "FAIL: no active items after generation\n");
        return 4;
    }
    SnapRec before[ROGUE_ITEM_INSTANCE_CAP];
    int bcount = snapshot(before, ROGUE_ITEM_INSTANCE_CAP);
    if (bcount <= 0)
    {
        fprintf(stderr, "FAIL: no items generated bcount=%d\n", bcount);
        return 2;
    }
    ...
    if (save_rc != 0)
    {
        fprintf(stderr, "FAIL: save rc=%d\n", save_rc);
        return 5;
    }
    /* Diagnostic: export JSON summary */
    char json[512];
    int jrc = rogue_save_export_json(0, json, sizeof json);
    if (jrc == 0)
    {
        fprintf(stderr, "DIAG: save JSON=%s\n", json);
    }
    else
    {
        fprintf(stderr, "DIAG: save JSON export failed rc=%d\n", jrc);
    }
    int inv_only_rc = rogue_save_manager_save_slot_inventory_only(0);
    fprintf(stderr, "DIAG: inventory_only_save rc=%d (expect 0)\n", inv_only_rc);
    /* Clear runtime and reload */
    /* For isolation: reset item defs & reload, clear instances via runtime reset (simple way:
     * shutdown+init not exposed here; rely on loading to overwrite) */
    ...
    rogue_loot_tables_reset();
    assert(rogue_loot_tables_load_from_cfg(ptables) > 0);
    rogue_affixes_reset();
    assert(rogue_affixes_load_from_cfg(paff) >= 0);
    /* Load slot */
    /* Load slot (don't rely on assert side effects; ensure executed under Release) */
    int load_rc = rogue_save_manager_load_slot(0);
    if (load_rc != 0)
    {
        fprintf(stderr, "FAIL: load rc=%d\n", load_rc);
        return 6;
    }
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            fprintf(stderr,
                    "  RAW[%d]: def=%d qty=%d rar=%d pidx=%d sidx=%d dur=%d/%d ench=%d active=1\n",
                    i, it->def_index, it->quantity, it->rarity, it->prefix_index, it->suffix_index,
                    it->durability_cur, it->durability_max, it->enchant_level);
        }
        else
        {
            fprintf(stderr, "  RAW[%d]: <inactive>\n", i);
        }
    }

    SnapRec after[ROGUE_ITEM_INSTANCE_CAP];
    int acount = snapshot(after, ROGUE_ITEM_INSTANCE_CAP);
    assert(acount == bcount);

    /* Compare order-insensitive: build simple multiset match counts */
    int matched = 0;
    int used[ROGUE_ITEM_INSTANCE_CAP];
    ...
    for (int i = 0; i < bcount; i++)
    {
        for (int j = 0; j < acount; j++)
            if (!used[j])
            {
                if (memcmp(&before[i], &after[j], sizeof(SnapRec)) == 0)
                {
                    used[j] = 1;
                    matched++;
                    break;
    ...
    if (matched != bcount)
    {
        ...
        for (int i = 0; i < dump; i++)
        {
            ...
            fprintf(stderr, "  B[%d]: def=%d qty=%d rar=%d p(%d,%d) s(%d,%d) dur=%d/%d ench=%d\n",
                    i, r->def_index, r->qty, r->rarity, r->pidx, r->pval, r->sidx, r->sval, r->dcur,
                    r->dmax, r->enchant);
        }
        ...
        if (bcount > 0 && acount > 0)
        {
            ...
            {
                int diff = 0;
                const int* b = (const int*) &before[bi];
                const int* a = (const int*) &after[j];
                for (size_t k = 0; k < sizeof(SnapRec) / sizeof(int); k++)
                    if (b[k] != a[k])
                        diff++;
                if (diff < best_diff)
                {
            ...
            if (best_j >= 0)
            {
                const int* b = (const int*) &before[0];
                const int* a = (const int*) &after[best_j];
                fprintf(stderr, "Field diffs for first before vs after[%d]:\n", best_j);
                const char* names[] = {"def",  "qty",  "rar",  "pidx", "pval",
                                       "sidx", "sval", "dcur", "dmax", "ench"};
                for (size_t k = 0; k < sizeof(SnapRec) / sizeof(int); k++)
                    if (b[k] != a[k])
