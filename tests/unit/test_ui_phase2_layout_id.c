...
int main(){
    ...
    cfg.max_nodes=256; cfg.arena_size=4096; /* seed left 0 */
    RogueUIContext ctx; assert(rogue_ui_init(&ctx,&cfg));
    ...
    RogueUIRect r1; assert(rogue_ui_row_next(&ctx,row,50,30,&r1));
    int b1=rogue_ui_button(&ctx,r1,"BtnA",0x111111u,0xffffffu); assert(b1>=0);
    RogueUIRect r2; assert(rogue_ui_row_next(&ctx,row,60,30,&r2));
    int b2=rogue_ui_button(&ctx,r2,"BtnB",0x222222u,0xffffffu); assert(b2>=0);
    ...
    RogueUIRect c1; assert(rogue_ui_column_next(&ctx,col,80,20,&c1));
    int tgl_state=0; int t1=rogue_ui_toggle(&ctx,c1,"Tog1",&tgl_state,0x0,0x0,0xffffffu); assert(t1>=0);
    RogueUIRect c2; assert(rogue_ui_column_next(&ctx,col,80,20,&c2));
    float val=0.5f; int s1=rogue_ui_slider(&ctx,c2,0,1,&val,0x0,0x0); assert(s1>=0);
