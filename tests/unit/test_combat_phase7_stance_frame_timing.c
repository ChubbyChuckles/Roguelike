...
/* Provide minimal attack def table with distinct startup/recovery so stance modifiers are
 * observable */
...
static const RogueAttackDef g_stance_attack = {0,
                                               "stance_test",
                                               ROGUE_WEAPON_LIGHT,
                                               0,
                                               120,
...
/* Advance update loop until we exit STRIKE -> RECOVER -> IDLE for one attack, capturing phase
 * durations */
static int perform_attack_measure(RoguePlayer* player, int stance, float* out_windup,
                                  float* out_recover)
{
    ...
    g_exposed_player_for_stats.combat_stance = stance;
    RogueEnemy enemy;
    ...
    float wind = 0.0f, rec = 0.0f;
    int in_wind = 0, in_recover = 0;
    if (pc.phase == ROGUE_ATTACK_WINDUP)
        in_wind = 1;
    for (int step = 0; step < 2000; ++step)
    {
        ...
        else if (in_wind && pc.phase != ROGUE_ATTACK_WINDUP)
        {
            in_wind = 0;
        }
        if (pc.phase == ROGUE_ATTACK_STRIKE)
        {
            RoguePlayer pl = *player;
            RogueEnemy earr[1];
            earr[0] = enemy;
            rogue_combat_player_strike(&pc, &pl, earr, 1);
            enemy = earr[0];
        }
        if (pc.phase == ROGUE_ATTACK_RECOVER)
        {
            rec += 5.0f;
            in_recover = 1;
        }
        if (pc.phase == ROGUE_ATTACK_IDLE && rec > 0.0f)
        {
            break;
        }
    }
...
int main()
{
    ...
    rogue_force_attack_active = 1;
    g_attack_frame_override = 3;
    float w_bal = 0, r_bal = 0;
    perform_attack_measure(&player, 0, &w_bal, &r_bal);
    float w_ag = 0, r_ag = 0;
    perform_attack_measure(&player, 1, &w_ag, &r_ag);
    float w_def = 0, r_def = 0;
    perform_attack_measure(&player, 2, &w_def, &r_def);
    /* Expect aggressive shorter windup & recovery vs balanced, defensive longer */
    if (!(w_ag < w_bal * 0.97f))
    {
        printf("fail_aggressive_windup_only w_bal=%.1f w_ag=%.1f\n", w_bal, w_ag);
        return 1;
    }
    if (!(w_def > w_bal * 1.04f))
    {
        printf("fail_defensive_windup_only w_bal=%.1f w_def=%.1f\n", w_bal, w_def);
        return 2;
    }
    printf("phase7_stance_frame_timing: OK w_bal=%.1f w_ag=%.1f w_def=%.1f r_bal=%.1f r_ag=%.1f "
           "r_def=%.1f\n",
           w_bal, w_ag, w_def, r_bal, r_ag, r_def);
    return 0;
}
