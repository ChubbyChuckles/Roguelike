...
typedef struct V1 { int a; } V1;
typedef struct V2 { int a; int b; } V2;
typedef struct V3 { int a; int b; int c; } V3;

static int mig_1_2(void **data, size_t *size, void *user){
    if(*size != sizeof(V1)) return -1;
    V1 *old = (V1*)(*data);
    V2 *n = (V2*)malloc(sizeof(V2)); if(!n) return -1;
    n->a = old->a; n->b = old->a * 2;
    *data = n; *size = sizeof(V2);
    return 0;
}

static int mig_2_3(void **data, size_t *size, void *user){
    if(*size != sizeof(V2)) return -1;
    V2 *old = (V2*)(*data);
    V3 *n = (V3*)malloc(sizeof(V3)); if(!n) return -1;
    n->a = old->a; n->b = old->b; n->c = old->a + old->b;
    *data = n; *size = sizeof(V3);
    return 0;
}
...
static void test_basic_chain(){
    ...
    V1 *obj = (V1*)malloc(sizeof(V1)); obj->a=7; size_t sz=sizeof(V1); void *ptr=obj; 
...
static void test_failure_rollback(){
    ...
    V1 *obj = (V1*)malloc(sizeof(V1)); obj->a=5; size_t sz=sizeof(V1); void *ptr=obj;
    RogueMigrationProgress prog; int rc = rogue_version_migrate("FailType",1,0,&ptr,&sz,&prog);
    assert(rc!=0);
...
int main(){
    ...
    RogueVersioningStats st; rogue_versioning_get_stats(&st);
    printf("[versioning] types=%u migrations=%u executed=%llu steps=%llu failures=%llu\n", st.types_registered, st.migrations_registered,(unsigned long long)st.migrations_executed,(unsigned long long)st.migration_steps,(unsigned long long)st.migration_failures);
    return 0;
}
