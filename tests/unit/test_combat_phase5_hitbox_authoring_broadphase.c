...
static const char* SAMPLE_JSON =
    ...
    " { \"type\":\"projectile_spawn\", \"count\":5, \"ox\":0, \"oy\":0, \"speed\":6, "
    "\"spread\":3.1415926535, \"center\":0 }\n"
    "]";
...
int main()
{
    RogueHitbox boxes[8];
    int count = 0;
    int ok = rogue_hitbox_load_sequence_from_memory(SAMPLE_JSON, boxes, 8, &count);
    assert(ok);
    assert(count == 4);
    ...
    int idx[8];
    int n = rogue_hitbox_collect_point_overlaps(cap, xs, ys, alive, 8, idx, 8);
    /* Points inside: indices 0,1,2,3 (y=0.49 within radius), others out (0.51 outside, 0.6 outside,
     * x=3 outside) */
    assert(n == 4);
    int seen[8] = {0};
    for (int i = 0; i < n; i++)
