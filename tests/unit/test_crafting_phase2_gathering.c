...
int main(void)
{
    ...
    if (rogue_item_defs_load_directory("assets/items") <= 0 &&
        rogue_item_defs_load_directory("../assets/items") <= 0)
    {
        fprintf(stderr, "Could not load item defs directory\n");
        return 2;
    }
    /* Load registry from new materials asset directly to avoid relying on default search during
     * tests. */
    ...
    /* Provide an inline synthetic definition set instead of relying on external file for
     * determinism. */
    const char* cfg_path = "gather_test_nodes.cfg";
    FILE* f = fopen(cfg_path, "wb");
    if (!f)
    {
        fprintf(stderr, "Cannot create temp cfg file\n");
        return 3;
    }
    /* id, material_table, min_roll, max_roll, respawn_ms, tool_req_tier, biome_tags, spawn%, rare%,
     * rare_mult */
    ...
    for (int i = 0; i < 60; i++)
    {
        rogue_gather_update(1.0f);
    }
    rc = rogue_gather_harvest(0, &rng, &mat_def, &qty);
    assert_true(rc == 0, "harvest after respawn");

    /* Determinism: spawn chunk again with same seed should add zero new nodes since we keep global
     * list; but fresh spawn count with same seed increments if we reset - omitted for simplicity */

    printf("CRAFT_P2_OK defs=%d nodes=%d first_qty=%d total_harvests=%llu rare=%llu\n", loaded,
           node_count, qty, rogue_gather_total_harvests(), rogue_gather_total_rare_procs());
    fflush(stdout);
    return 0;
}
