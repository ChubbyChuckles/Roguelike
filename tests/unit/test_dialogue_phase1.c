...
int main(void){
    ...
    if(rogue_ui_init(&ui, &cfg)==0){ printf("FAIL ui init\n"); return 1; }

    rogue_dialogue_reset();
    if(rogue_dialogue_register_from_buffer(7, sample, (int)strlen(sample))!=0){ printf("FAIL register\n"); return 1; }
    if(rogue_dialogue_start(7)!=0){ printf("FAIL start\n"); return 1; }
    const RogueDialoguePlayback* pb = rogue_dialogue_playback();
    if(!pb || pb->line_index!=0){ printf("FAIL playback start index\n"); return 1; }
    ...
    for(int i=0;i<3;i++){
        ...
        rogue_ui_end(&ui);
        if(i<2){ if(rogue_dialogue_advance()!=1){ printf("FAIL advance mid %d\n", i); return 1; } }
