...
static void test_los()
{
    ...
    g_block_tx = 5;
    g_block_ty = 5; /* inject block near middle */
...
{
    RoguePerceptionAgent a = {0};
    a.x = 0;
    a.y = 0;
    a.facing_x = 1;
    a.facing_y = 0; /* facing +X */
    ...
    a.facing_x = 0;
    a.facing_y = 1;
    assert(rogue_perception_can_see(&a, 0, 5, 120.0f, 10.0f, NULL) == 1);
}
...
static void test_threat_accumulation_and_decay()
{
    ...
    a.y = 0;
    a.facing_x = 1;
    a.facing_y = 0; /* see player at +X */
    float visible_gain = 5.0f;
    float decay = 2.0f;
    for (int i = 0; i < 10; i++)
    {
    ...
    for (int i = 0; i < 30; i++)
    {
        rogue_perception_tick_agent(&a, 0.1f, -5.0f, 0.0f, 140.0f, 20.0f, visible_gain, decay,
                                    2.0f);
    }
...
static void test_group_broadcast()
{
    ...
    agents[2].y = 0;
    agents[2].facing_x = 1;
    agents[2].facing_y = 0; /* out of broadcast radius */
    /* Source sees player */
    ...
    {
        agents[0].last_seen_x = 5.0f;
        agents[0].last_seen_y = 0.0f;
        agents[0].has_last_seen = 1;
        agents[0].last_seen_ttl = 2.0f;
