...
int main(){
    ...
    int k=do_strike(&pc,&player,&enemy); (void)k; int dmg_base = 200 - enemy.health; if(dmg_base<=0){ printf("fail_baseline_damage=%d\n", dmg_base); return 1; }

    /* Reset enemy and perform charged attack (simulate hold 800ms) */
    enemy.health=200; pc.phase=ROGUE_ATTACK_IDLE; pc.pending_charge_damage_mult=1.0f; rogue_combat_charge_begin(&pc); for(int t=0;t<8;t++){ rogue_combat_charge_tick(&pc,100,1); }
    /* release */
    rogue_combat_charge_tick(&pc,0,0);
    int k2=do_strike(&pc,&player,&enemy); (void)k2; int dmg_charged = 200 - enemy.health; if(!(dmg_charged > dmg_base*2 -2)){ printf("fail_charged dmg_base=%d charged=%d\n", dmg_base,dmg_charged); return 2; }

    /* Ensure idle state before dodge (strike test left phase STRIKE); reset to idle */
    pc.phase = ROGUE_ATTACK_IDLE; pc.timer=0; pc.stamina = 100.0f; /* full stamina */
    /* Test dodge roll: sufficient stamina & grants i-frames */
    float pre_stam = pc.stamina; if(!rogue_player_dodge_roll(&player,&pc,1)){ printf("fail_dodge_initiate\n"); return 3; }
    if(!(player.iframes_ms >= 399.0f && pc.stamina < pre_stam)){ printf("fail_dodge_effect iframes=%.2f stam=%.2f pre=%.2f\n", player.iframes_ms, pc.stamina, pre_stam); return 4; }
    /* Attempt second immediate dodge with insufficient stamina (drain manually) */
    pc.stamina = 5.0f; if(rogue_player_dodge_roll(&player,&pc,2)){ printf("fail_dodge_should_fail_low_stam\n"); return 5; }

    printf("phase6_charge_and_dodge: OK base=%d charged=%d iframes=%.1f\n", dmg_base,dmg_charged, player.iframes_ms);
    return 0;
}
