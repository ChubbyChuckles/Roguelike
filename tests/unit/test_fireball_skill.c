...
/* Test: passive fire synergy increases fireball damage; projectile spawns and damages enemy
 * correctly. */
...
static RogueSkillDef make_passive_pyro(void)
{
    ...
    d.max_rank = 5;
    d.skill_strength = 0;
    d.base_cooldown_ms = 0.0f;
    d.cooldown_reduction_ms_per_rank = 0.0f;
    d.on_activate = NULL;
    d.is_passive = 1;
    d.tags = ROGUE_SKILL_TAG_FIRE;
    d.synergy_id = ROGUE_SYNERGY_FIRE_POWER;
    d.synergy_value_per_rank = 2;
    return d;
}
static RogueSkillDef make_fireball(void)
{
    ...
    d.max_rank = 5;
    d.skill_strength = 0;
    d.base_cooldown_ms = 6000.0f;
    d.cooldown_reduction_ms_per_rank = 400.0f;
    d.on_activate = NULL;
    d.is_passive = 0;
    d.tags = ROGUE_SKILL_TAG_FIRE;
    d.synergy_id = -1;
    d.synergy_value_per_rank = 0;
    return d;
}
...
int main(void)
{
    ...
    rogue_projectiles_spawn(g_app.player.base.pos.x, g_app.player.base.pos.y, 1, 0,
                            80.0f + 1 * 15.0f, 3500.0f, expected);
    assert(rogue_projectiles_active_count() == 1);
    /* Simulate updates until projectile should hit (distance 3 at ~95 units/s) */
    for (int step = 0; step < 100 && rogue_projectiles_active_count() > 0; ++step)
    {
