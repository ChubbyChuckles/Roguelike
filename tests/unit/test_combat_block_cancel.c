...
static void advance_until_phase(RoguePlayerCombat* c, RogueAttackPhase ph){
    for(int i=0;i<400;i++){ rogue_combat_update_player(c, 1.0f, 0); if(c->phase==ph) return; }
}
...
int main(void){
    ...
    for(int i=0;i<25;i++) rogue_combat_update_player(&c,1.0f,0);
    /* Notify block */
    ...
    for(int i=0;i<40;i++){
        ...
        if(c.phase==ROGUE_ATTACK_RECOVER) break;
    }
