...
#ifdef _WIN32
#include <windows.h>
static DWORD WINAPI stress_thread(LPVOID param)
{
...
#include <pthread.h>
#include <unistd.h>
static void* stress_thread(void* param)
{
    RogueMutex* m = (RogueMutex*) param;
    for (int i = 0; i < 1000; i++)
    {
...
static void test_4_8_1_entity_id_uniqueness()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ids[i] = rogue_entity_id_generate(ROGUE_ENTITY_ENEMY);
        assert(rogue_entity_id_validate(ids[i]));
    }
    for (int i = 0; i < N; i++)
    {
        for (int j = i + 1; j < N; j++)
...
static void test_4_8_5_cow_concurrent()
{
    ...
    const char mod1[] = "HELLO";
    const char mod2[] = "WORLD";
    assert(rogue_cow_write(a, 2, mod1, sizeof(mod1) - 1) == 0);
    assert(rogue_cow_write(b, 10, mod2, sizeof(mod2) - 1) == 0);
    char r1[6] = {0};
    char r2[6] = {0};
    assert(rogue_cow_read(a, 2, r1, 5) == 0);
    assert(rogue_cow_read(b, 10, r2, 5) == 0);
    assert(strcmp(r1, "HELLO") == 0);
    assert(strcmp(r2, "WORLD") == 0);
    char chk[6] = {0};
    assert(rogue_cow_read(a, 10, chk, 5) == 0);
    assert(strcmp(chk, "klmno") == 0);
    assert(rogue_cow_read(b, 2, chk, 5) == 0);
    assert(strncmp(chk, "cdefg", 5) == 0);
    rogue_cow_destroy(a);
    rogue_cow_destroy(b);
}
...
{
    RogueMutex* m = rogue_mutex_create(50, "stress_mtx");
    assert(m);
#define PHASE4_8_THREADS 4
#ifdef _WIN32
    HANDLE th[PHASE4_8_THREADS];
    for (int i = 0; i < PHASE4_8_THREADS; i++)
    {
        th[i] = CreateThread(NULL, 0, stress_thread, m, 0, NULL);
        assert(th[i]);
    }
    for (int i = 0; i < PHASE4_8_THREADS; i++)
    {
    ...
    for (int i = 0; i < PHASE4_8_THREADS; i++)
    {
        pthread_create(&th[i], NULL, stress_thread, m);
    }
    for (int i = 0; i < PHASE4_8_THREADS; i++)
    {
