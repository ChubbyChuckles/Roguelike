...
{
    RogueChunkCoord c = rogue_world_chunk_from_tile(0, 0);
    assert(c.cx == 0 && c.cy == 0);
    c = rogue_world_chunk_from_tile(31, 31);
    assert(c.cx == 0 && c.cy == 0);
    c = rogue_world_chunk_from_tile(32, 0);
    assert(c.cx == 1 && c.cy == 0);
    c = rogue_world_chunk_from_tile(33, 33);
    assert(c.cx == 1 && c.cy == 1);
    int ox, oy;
    rogue_world_chunk_origin(c, &ox, &oy);
    assert(ox == 32 && oy == 32);
}
...
static void test_rng_channel_independence()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        macro_seq[i] = rogue_worldgen_rand_u32(&ctx.macro_rng);
        biome_seq[i] = rogue_worldgen_rand_u32(&ctx.biome_rng);
        micro_seq[i] = rogue_worldgen_rand_u32(&ctx.micro_rng);
    }
    int identical_positions_mb = 0, identical_positions_mm = 0;
    for (int i = 0; i < 4; i++)
    {
        if (macro_seq[i] == biome_seq[i])
            identical_positions_mb++;
        if (macro_seq[i] == micro_seq[i])
            identical_positions_mm++;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        assert(macro_seq[i] == rogue_worldgen_rand_u32(&ctx2.macro_rng));
    }
...
static void test_hash()
{
    ...
    {
        printf("[worldgen_phase1] tilemap_init failed %d x %d\n", cfg.width, cfg.height);
        assert(0);
    }
    ...
    for (int y = 0; y < map.height; y++)
        ...
        {
            unsigned char v = (unsigned char) ((x + y) % 5);
    ...
    printf("[worldgen_phase1] tile pattern nonzero=%d first=%u\n", nonzero,
           (unsigned) map.tiles[0]);
    unsigned long long h1 = rogue_world_hash_tilemap(&map);
    printf("[worldgen_phase1] hash h1=%llu\n", h1);
    unsigned long long h2 = rogue_world_hash_tilemap(&map);
    printf("[worldgen_phase1] hash h2=%llu\n", h2);
    fflush(stdout);
    if (h1 != h2)
    {
        printf("hash mismatch stability\n");
        assert(0);
    }
    map.tiles[0] ^= 1;
    unsigned long long h3 = rogue_world_hash_tilemap(&map);
    printf("[worldgen_phase1] hash h3(after mutate)=%llu\n", h3);
    fflush(stdout);
    if (h3 == h1)
    {
        printf("hash did not change after mutation\n");
        assert(0);
    }
