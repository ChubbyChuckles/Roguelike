...
int main()
{
    ...
    enemy.facing = 1; /* enemy facing left (-x) */

    /* Backstab: place player behind enemy (enemy facing left, player to right of enemy) */
    player.base.pos.x = 1.8f;
    player.base.pos.y = 0.0f; /* ~0.8 right of enemy */
    int bs = rogue_combat_try_backstab(&player, &pc, &enemy);
    printf("after_backstab_attempt bs=%d cooldown=%.1f\n", bs, pc.backstab_cooldown_ms);
    if (!bs)
    {
        printf("fail_backstab_detect pos=(%.2f,%.2f) enemy=(%.2f,%.2f) facing=%d\n",
               player.base.pos.x, player.base.pos.y, enemy.base.pos.x, enemy.base.pos.y,
               enemy.facing);
        return 1;
    }
    ...
    player.base.pos.y = 0.0f; /* move in front for consistent baseline strike */
    int dmg_no_bonus;
    do_strike(&pc, &player, &enemy);
    dmg_no_bonus = 200 - enemy.health;
    if (dmg_no_bonus <= 0)
    {
        printf("fail_base_strike pos=(%.2f,%.2f) enemy_health=%d\n", player.base.pos.x,
               player.base.pos.y, enemy.health);
        return 2;
    }

    /* Parry: activate then register incoming attack */
    rogue_player_begin_parry(&player, &pc);
    printf("after_begin_parry active=%d\n", pc.parry_active);
    if (!rogue_player_is_parry_active(&pc))
    {
        printf("fail_parry_not_active parry_active=%d timer=%.2f\n", pc.parry_active,
               pc.parry_timer_ms);
        return 3;
    }
    /* Attack coming from in front of player (player facing=2 -> +x) */
    int parry_ok = rogue_player_register_incoming_attack_parry(&player, &pc, 1, 0);
    printf("after_register_parry ok=%d riposte_ready=%d window=%.1f iframes=%.1f\n", parry_ok,
           pc.riposte_ready, pc.riposte_window_ms, player.iframes_ms);
    if (!parry_ok)
    {
        printf("fail_parry_register facing=%d active=%d\n", player.facing, pc.parry_active);
        return 4;
    }
    if (!pc.riposte_ready)
    {
        printf("fail_riposte_not_ready\n");
        return 5;
    }

    /* Riposte consumption */
    int riposte = rogue_player_try_riposte(&player, &pc, &enemy);
    if (!riposte)
    {
    ...
    if (pc.riposte_ready)
    {
        printf("fail_riposte_flag_persist\n");
        return 7;
    }
