...
static int section_cb(const struct RogueSaveDescriptor* d, uint32_t id, const void* data,
...
int main(void)
{
    if (ROGUE_SAVE_FORMAT_VERSION < 5u)
    {
        printf("STRINGS_SKIP v=%u\n", (unsigned) ROGUE_SAVE_FORMAT_VERSION);
        return 0;
    }
    ...
    if (a < 0 || b < 0 || c != a)
    {
        printf("STRINGS_FAIL intern logic a=%d b=%d c=%d\n", a, b, c);
        return 1;
    }
    if (rogue_save_manager_save_slot(0) != 0)
    {
        printf("STRINGS_FAIL save\n");
        return 1;
    }
    /* verify string component present via iteration */
    int found = 0;
    int rc = rogue_save_for_each_section(0, section_cb, &found);
    if (rc != 0 || !found)
    {
        printf("STRINGS_FAIL section rc=%d found=%d\n", rc, found);
        return 1;
    }
