...
/* This test fakes a sweep debug frame to validate knockback magnitude application math indirectly.
   ...
   For minimal coupling we'll reimplement the magnitude function analogous to combat code and ensure expected range. */

static float calc_mag(int player_level,int enemy_level,int p_str,int e_str){
    float level_diff = (float)player_level - (float)enemy_level; if(level_diff<0) level_diff=0;
    float str_diff = (float)p_str - (float)e_str; if(str_diff<0) str_diff=0;
