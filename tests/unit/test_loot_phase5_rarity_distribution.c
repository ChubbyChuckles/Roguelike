...
int main(void){
    ...
    for(int si=0; si<spec_count; ++si){
        DistSpec* s = &specs[si];
        int tbl = rogue_loot_table_index(s->table_id); assert(tbl>=0);
        int item_index = rogue_item_def_index(s->item_id); assert(item_index>=0);
        unsigned int seed = 12345u + (unsigned)si * 777u;
        int observed_mask = 0; /* bit i set if rarity (allow_min + i) seen */
        int iterations = 3000; /* chosen to make rare item + rarity band coverage extremely likely */
        for(int it=0; it<iterations; ++it){
            unsigned int local = seed + (unsigned)it * 17u;
            int idef[8]; int qty[8]; int rar[8];
