...
static void reset_player(RoguePlayer* p){ rogue_player_init(p); p->facing=0; p->poise = p->poise_max; p->reaction_type=0; p->reaction_timer_ms=0; }

int main(){
    RoguePlayer p; reset_player(&p); g_exposed_player_for_stats = p;
    /* Light flinch: damage 30 triggers reaction_type=1 */
    int blocked=0, perfect=0; int dmg = rogue_player_apply_incoming_melee(&p,30.0f,0.0f,1.0f,5,&blocked,&perfect);
    assert(dmg==30); assert(p.reaction_type==1 && p.reaction_timer_ms>0);
    /* Advance reaction update until it clears */
    float total=0; while(p.reaction_type!=0 && total<500){ rogue_player_update_reactions(&p,16.0f); total+=16.0f; }
    assert(p.reaction_type==0);
    /* Stagger from poise break: apply large poise damage steps */
    p.poise = 10.0f; p.reaction_type=0; p.reaction_timer_ms=0;
    ...
    p.reaction_type=0; p.reaction_timer_ms=0; p.poise = p.poise_max; /* restore */
    ...
    p.iframes_ms = 200.0f; p.reaction_type=0; p.reaction_timer_ms=0;
