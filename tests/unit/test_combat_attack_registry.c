...
{
    int l = rogue_attack_chain_length(ROGUE_WEAPON_LIGHT);
    assert(l == 3);
    int h = rogue_attack_chain_length(ROGUE_WEAPON_HEAVY);
    assert(h == 2);
    int t = rogue_attack_chain_length(ROGUE_WEAPON_THRUST);
    assert(t == 2);
    int r = rogue_attack_chain_length(ROGUE_WEAPON_RANGED);
    assert(r == 2);
    int s = rogue_attack_chain_length(ROGUE_WEAPON_SPELL_FOCUS);
    assert(s == 1);
}

static void test_bounds_and_clamp()
{
    const RogueAttackDef* d0 = rogue_attack_get(ROGUE_WEAPON_LIGHT, 0);
    assert(d0 && d0->chain_index == 0);
    const RogueAttackDef* d2 = rogue_attack_get(ROGUE_WEAPON_LIGHT, 2);
    assert(d2 && d2->chain_index == 2);
    const RogueAttackDef* d_over = rogue_attack_get(ROGUE_WEAPON_LIGHT, 99);
    assert(d_over && d_over->chain_index == 2); /* clamp */
    const RogueAttackDef* d_neg = rogue_attack_get(ROGUE_WEAPON_LIGHT, -5);
    assert(d_neg && d_neg->chain_index == 0);
}
