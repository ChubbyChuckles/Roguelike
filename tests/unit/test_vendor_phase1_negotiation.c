...
{
    /* We cannot easily inject alternate file without filesystem write override; rely on existing
       loader. Instead verify there are no duplicate IDs among loaded negotiation rules. */
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = i + 1; j < n; j++)
        {
            ...
            {
                printf("NEG_RULE_COLLISION id=%s indexes=%d,%d\n", a->id, i, j);
                return 0;
            }
        }
    }
...
int main(void)
{
    ...
    {
        printf("NEG_RULE_FAIL load_all\n");
        return 1;
    }
    if (!simulate_collision_duplicate_rule())
        return 2;
    if (!validate_ranges())
        return 3;
    printf("VENDOR_PHASE1_NEGOTIATION_OK rules=%d rep=%d vendors=%d policies=%d\n",
           rogue_negotiation_rule_count(), rogue_rep_tier_count(), rogue_vendor_def_count(),
           rogue_price_policy_count());
    return 0;
}
