...
{
    RogueUIContext ctx;
    RogueUIContextConfig cfg = {.max_nodes = 8, .seed = 42u, .arena_size = 1024};
...
{
    RogueUIContext ctx;
    RogueUIContextConfig cfg = {.max_nodes = 4, .seed = 99u, .arena_size = 2048};
    assert(rogue_ui_init(&ctx, &cfg));
    struct SimDummy
    {
        int hp;
    ...
    size_t snap_size = 0;
    const void* snap = rogue_ui_simulation_snapshot(&ctx, &snap_size);
...
{
    RogueUIContext ctx;
    RogueUIContextConfig cfg = {.max_nodes = 2, .seed = 1u, .arena_size = 32};
    assert(rogue_ui_init(&ctx, &cfg));
