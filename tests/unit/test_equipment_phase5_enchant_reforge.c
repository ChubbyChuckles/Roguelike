...
int main(void)
{
    ...
    if (orb < 0 || hammer < 0)
    {
        ...
        if (extra <= 0)
            extra = rogue_item_defs_load_from_cfg("../assets/test_materials.cfg");
        /* materials optional file */
    }
    /* If still missing, we cannot proceed; skip gracefully */
    orb = rogue_item_def_index("enchant_orb");
    hammer = rogue_item_def_index("reforge_hammer");
    if (orb < 0)
    {
        printf("skipped_enchant_reforge_missing_materials\n");
        return 0;
    }
    ensure_materials();
    ...
    int old_prefix = it->prefix_index;
    int old_suffix = it->suffix_index;
    int cost = -1;
    int rc = 0;
    rc = rogue_item_instance_enchant(inst, 1, 0, &cost);
    assert(rc == 0 && cost > 0 && it->prefix_index != old_prefix && it->suffix_index == old_suffix);
    old_prefix = it->prefix_index;
    old_suffix = it->suffix_index;
    cost = -1;
    rc = rogue_item_instance_enchant(inst, 1, 1, &cost);
    assert(rc == 0 && it->prefix_index != old_prefix && it->suffix_index != old_suffix);
    int level_before = it->item_level;
    int sockets_before = it->socket_count;
    int rarity_before = it->rarity;
    cost = -1;
    rc = rogue_item_instance_reforge(inst, &cost);
    assert(rc == 0 && it->item_level == level_before && it->socket_count == sockets_before &&
           it->rarity == rarity_before);
