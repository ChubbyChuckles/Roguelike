...
int main(void)
{
    ...
    int r = rogue_equipment_deserialize(legacy);
    assert(r == 0);
    /* Ensure items were spawned and remapped to modern slots */
    int w = rogue_equip_get(ROGUE_EQUIP_WEAPON);
    assert(w >= 0);
    int head = rogue_equip_get(ROGUE_EQUIP_ARMOR_HEAD);
    assert(head >= 0);
    int chest = rogue_equip_get(ROGUE_EQUIP_ARMOR_CHEST);
    assert(chest >= 0);
    ...
    char cur[1024];
    int n = rogue_equipment_serialize(cur, (int) sizeof cur);
    assert(n > 0);
