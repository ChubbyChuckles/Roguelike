...
int main(){
    ...
    RogueSandboxScript s; ck(rogue_script_load(path,&s)==0, "load ok");
    ck(s.instr_count==3, "instr count");
    unsigned int h1 = rogue_script_hash(&s); ck(h1!=0, "hash nonzero");
    int str=0,dex=0,vit=0,intel=0,armor=0; int rf=0,rc=0,rl=0,rp=0,rs=0,rph=0;
    ...
    RogueSandboxScript s2; ck(rogue_script_load(path,&s2)!=0, "bad rejected");
    remove(path);
