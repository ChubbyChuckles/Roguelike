...
static void fake_world(int w, int h){
    g_app.world_map.width = w; g_app.world_map.height = h;
    g_app.world_map.tiles = (unsigned char*)malloc((size_t)w*h);
    if(!g_app.world_map.tiles){ printf("alloc fail\n"); exit(1);}    
    for(int y=0;y<h;y++) for(int x=0;x<w;x++){ g_app.world_map.tiles[y*w+x] = 0; }
}
...
static void build_lut_manual(void){
    ...
    if(!g_app.tile_sprite_lut){ printf("lut alloc fail\n"); exit(1);}    
    for(size_t i=0;i<total;i++){ g_app.tile_sprite_lut[i] = (const void*)0x1; }
...
int main(void){
    ...
    if(g_app.tile_sprite_lut[0] != first_ptr){ printf("ensure mutated existing lut unexpectedly\n"); return 1; }
    rogue_tile_sprite_cache_free();
    if(g_app.tile_sprite_lut || g_app.tile_sprite_lut_ready || g_app.tileset_loaded){ printf("cache not freed cleanly\n"); return 1; }
    free(g_app.world_map.tiles); g_app.world_map.tiles=NULL;
    return 0;
}
