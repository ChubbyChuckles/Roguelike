...
int main(void)
{
    ...
    for (int i = 0; i < n; i++)
        rogue_loot_stats_record_rarity(seq[i]);
    int counts[5];
    rogue_loot_stats_snapshot(counts);
    if (counts[0] != 3 || counts[1] != 2 || counts[2] != 2 || counts[3] != 1 || counts[4] != 4)
        return fail("counts_initial");
    /* Overwrite ring fully to test eviction */
    for (int i = 0; i < ROGUE_LOOT_STATS_CAP; i++)
        rogue_loot_stats_record_rarity(4);
    int after[5];
    rogue_loot_stats_snapshot(after);
    if (after[4] != ROGUE_LOOT_STATS_CAP)
        return fail("overwrite_fill");
