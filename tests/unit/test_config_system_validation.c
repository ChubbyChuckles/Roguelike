...
#define PERF_TIMER_END(operation)                                                                  \
    do                                                                                             \
    {                                                                                              \
...
static const char* mock_cfg_data = "# Test CFG file for migration testing\n"
                                   "item {\n"
                                   "    id = 2001\n"
                                   "    name = \"Steel Sword\"\n"
                                   "    type = weapon\n"
...
static bool test_schema_validation_accuracy(void)
{
    ...
    /* Test 1: Valid JSON should pass validation */
    {
        ...
        bool valid =
            (strlen(mock_json_valid) > 0 && strstr(mock_json_valid, "\"version\"") != NULL);
...
static bool test_cfg_to_json_migration_integrity(void)
{
    ...
    {
        bool has_all_required_fields = strstr(mock_cfg_data, "id = 2001") != NULL &&
                                       strstr(mock_cfg_data, "name = \"Steel Sword\"") != NULL &&
                                       strstr(mock_cfg_data, "type = weapon") != NULL &&
                                       strstr(mock_cfg_data, "damage = 35") != NULL;
        TEST_ASSERT(has_all_required_fields,
                    "All CFG data fields should be preserved during migration");
    }
...
static bool test_hot_reload_functionality(void)
{
    ...
    /* Test 6: State consistency after reload */
    {
        ...
        bool type1_exists =
            rogue_event_type_check_collision(3200, collision_info, sizeof(collision_info));
        bool type2_exists =
            rogue_event_type_check_collision(3201, collision_info, sizeof(collision_info));
        TEST_ASSERT(type1_exists && type2_exists,
                    "Registered data should persist through hot-reload");
    }
...
static bool test_full_system_reload(void)
{
    ...
    /* Test 4: Inter-system consistency during reload */
    {
        ...
        TEST_ASSERT(consistency_maintained,
                    "Inter-system consistency should be maintained during reload");
    }
    ...
    /* Test 6: Post-reload validation */
    {
        ...
        char collision_info[256];
        bool type1_exists =
            rogue_event_type_check_collision(3300, collision_info, sizeof(collision_info));
        bool type2_exists =
            rogue_event_type_check_collision(3301, collision_info, sizeof(collision_info));
        bool type3_exists =
            rogue_event_type_check_collision(3302, collision_info, sizeof(collision_info));
...
static bool test_rapid_configuration_changes(void)
{
    ...
    /* Test 2: Rapid event type registration/deregistration */
    {
        ...
        TEST_ASSERT(successful_operations >= 90,
                    "Rapid configuration changes should succeed reliably");
    }
    ...
    /* Test 7: System recovery after stress */
    {
        ...
        bool recovery_successful =
            rogue_event_type_register_safe(3999, "RECOVERY_TEST", __FILE__, __LINE__);
