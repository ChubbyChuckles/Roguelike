...
int main(void){
    RoguePlayer p; rogue_player_init(&p); p.base.pos.x=0; p.base.pos.y=0; p.facing=2;
    ...
    for(int i=0;i<200;i++){
        ...
        if(c.phase==ROGUE_ATTACK_STRIKE) break;
    }
    ...
    for(int t=0;t<200; t++){
        if(t==60){ /* around 60ms into strike, buffer next */
            rogue_combat_update_player(&c, 0.0f, 1);
        }
        rogue_combat_update_player(&c, 1.0f, 0);
        if(c.phase==ROGUE_ATTACK_RECOVER){ transitioned_early = 1; break; }
    }
