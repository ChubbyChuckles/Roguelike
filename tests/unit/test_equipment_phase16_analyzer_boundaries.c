...
static void test_empty_inventory(){
    ...
    char buf[128]; int n = rogue_budget_analyzer_export_json(buf,sizeof buf);
...
static void test_extreme_item_level(){
    rogue_budget_analyzer_reset();
    int idx = rogue_items_spawn(0,1,0,0); assert_true(idx>=0, "spawn item");
    const RogueItemInstance* itc = rogue_item_instance_at(idx); RogueItemInstance* it=(RogueItemInstance*)itc; it->item_level = 500; it->rarity=4; it->prefix_index=0; it->prefix_value = 1000; it->suffix_index=-1; it->suffix_value=0;
    RogueBudgetReport rep; rogue_budget_analyzer_run(&rep);
