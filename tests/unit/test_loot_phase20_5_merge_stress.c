...
int main(void){
    rogue_item_defs_reset(); char pitems[256];
    ...
    if(load_items <= 0){ fprintf(stderr,"FAIL: load test_items.cfg rc=%d path=%s\n", load_items, pitems); return 2; }

    rogue_loot_tables_reset(); char ptables[256];
    ...
    if(load_tables <= 0){ fprintf(stderr,"FAIL: load test_loot_tables.cfg rc=%d path=%s\n", load_tables, ptables); return 4; }
    ...
    if(gold_index < 0){ fprintf(stderr,"FAIL: gold_coin index <0 (item defs not loaded?) count=%d\n", item_count); return 5; }
    ...
    for(int i=0;i<attempts;i++){
        ...
        int inst = rogue_items_spawn(gold_index, 1, ox, oy); if(inst>=0) spawned++; else {
            /* first few diagnostics */
            if(i<3) fprintf(stderr,"DIAG: spawn_fail iter=%d def_index=%d\n", i, gold_index);
        }
    }
    ...
    if(active > ROGUE_ITEM_INSTANCE_CAP){ fprintf(stderr,"FAIL: active exceeds cap %d>%d\n", active, ROGUE_ITEM_INSTANCE_CAP); return 6; }
    int merges = spawned - active; if(merges <= 0){ fprintf(stderr,"FAIL: no merges occurred spawned=%d active=%d (gold_index=%d)\n", spawned, active, gold_index); return 7; }
    float merge_ratio = (float)merges / (float)spawned;
    if(merge_ratio < 0.20f){ fprintf(stderr,"FAIL: insufficient merge ratio %.2f spawned=%d active=%d merges=%d\n", merge_ratio, spawned, active, merges); return 8; }
