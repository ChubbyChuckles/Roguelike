...
int main(void)
{
    ...
    if (write_cfg(path) != 0)
    {
        fprintf(stderr, "FAIL: cfg write\n");
        return 1;
    }
    if (rogue_affixes_reset() != 0)
    {
    ...
    if (len < 0)
    {
        fprintf(stderr, "FAIL: export\n");
        return 4;
    }
    if (strstr(buf, "flat_dmg") == NULL || strstr(buf, "agi_boost") == NULL)
    {
        fprintf(stderr, "FAIL: missing ids in json %s\n", buf);
        return 5;
    }
    if (strstr(buf, "\"min\":1") == NULL || strstr(buf, "\"max\":5") == NULL)
    {
        fprintf(stderr, "FAIL: missing min/max\n");
        return 6;
    }
    remove(path);
