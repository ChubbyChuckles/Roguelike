 ...
 * flash, then adding gold and confirming purchase */
int main(void)
{
    ...
    /* Simulate pressing return twice: first opens modal, second attempts buy (insufficient) -> sets
     * flash timer */
    if (g_app.vendor_confirm_active)
    {
        fprintf(stderr, "VT_FAIL unexpected active start\n");
        return 14;
    }
    /* Open modal */
    ...
    if (!g_app.vendor_confirm_active)
    {
        fprintf(stderr, "VT_FAIL modal not active\n");
        return 15;
    }
    /* Attempt accept with insufficient funds */
    if (rogue_econ_gold() >= g_app.vendor_confirm_price)
    {
        fprintf(stderr, "VT_FAIL gold unexpectedly sufficient\n");
        return 16;
    }
    {
        if (rogue_econ_gold() >= g_app.vendor_confirm_price)
        ...
        {
            g_app.vendor_insufficient_flash_ms = 480.0;
        }
    }
    if (g_app.vendor_insufficient_flash_ms <= 0)
    {
        fprintf(stderr, "VT_FAIL no flash set\n");
        return 17;
    }
    ...
    if (rogue_econ_gold() < g_app.vendor_confirm_price)
    {
        fprintf(stderr, "VT_FAIL still insufficient after add\n");
        return 18;
    }
