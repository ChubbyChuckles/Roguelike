...
static void inject(RogueUIContext* ctx, float mx, float my, int down, int pressed, int released,
                   char ch, int back)
{
...
{
    RogueUIContext ctx;
    RogueUIContextConfig cfg = {.max_nodes = 64, .seed = 1u, .arena_size = 8192};
    assert(rogue_ui_init(&ctx, &cfg));
    int toggle_state = 0;
    float slider_val = 5.0f;
    char textbuf[32] = "";

    /* Frame 1: initial build, click button */
    rogue_ui_begin(&ctx, 16.6);
    inject(&ctx, 10, 10, 0, 0, 0, 0, 0);
    int btn = rogue_ui_button(&ctx, (RogueUIRect){0, 0, 80, 20}, "Click", 0x303030FFu, 0xFFFFFFFFu);
    assert(btn == 0);
    int tog = rogue_ui_toggle(&ctx, (RogueUIRect){0, 25, 80, 20}, "Tog", &toggle_state, 0x800000FFu,
                              0x008000FFu, 0xFFFFFFFFu);
    assert(tog == 1);
    int sld = rogue_ui_slider(&ctx, (RogueUIRect){0, 50, 100, 12}, 0.0f, 10.0f, &slider_val,
                              0x202020FFu, 0x00FFFFFFu);
    assert(sld == 2);
    int tin = rogue_ui_text_input(&ctx, (RogueUIRect){0, 70, 120, 20}, textbuf, 32, 0x101010FFu,
                                  0xFFFFFFFFu);
    assert(tin == 3);
    rogue_ui_end(&ctx);
    ...
    rogue_ui_toggle(&ctx, (RogueUIRect){0, 25, 80, 20}, "Tog", &toggle_state, 0x800000FFu,
                    0x008000FFu, 0xFFFFFFFFu);
    rogue_ui_slider(&ctx, (RogueUIRect){0, 50, 100, 12}, 0.0f, 10.0f, &slider_val, 0x202020FFu,
                    0x00FFFFFFu);
    rogue_ui_text_input(&ctx, (RogueUIRect){0, 70, 120, 20}, textbuf, 32, 0x101010FFu, 0xFFFFFFFFu);
    rogue_ui_end(&ctx);
    assert(toggle_state == 0); /* no release yet */
