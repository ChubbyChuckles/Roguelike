...
static const RogueAttackDef g_stub_attack = {
    ...
    0.0f, /* int_scale */
    1, /* num_windows */
    { {0,80,0,1.0f,0,0,0,0} }, /* windows (start_ms,end_ms,flags,damage_mult,bleed_build,frost_build,start_frame,end_frame) */
    0.0f, /* poise_cost */
    0,    /* cancel_flags */
...
const RogueAttackDef* rogue_attack_get(int arch,int ci){(void)arch;(void)ci; return &g_stub_attack;}
int rogue_attack_chain_length(int arch){(void)arch; return 1;}
...
int main(){
    ...
    enemies[0].alive=1; enemies[0].team_id=0; enemies[0].base.pos.x=1.0f; enemies[0].base.pos.y=0; enemies[0].health=100; enemies[0].max_health=100;
    enemies[1].alive=1; enemies[1].team_id=1; enemies[1].base.pos.x=1.0f; enemies[1].base.pos.y=0; enemies[1].health=100; enemies[1].max_health=100;
    int kills = rogue_combat_player_strike(&pc,&player,enemies,2); (void)kills;
    if(enemies[0].health!=100){ printf("fail_friend_fire health=%d\n", enemies[0].health); return 1; }
    if(enemies[1].health==100){ printf("fail_enemy_untouched\n"); return 2; }
