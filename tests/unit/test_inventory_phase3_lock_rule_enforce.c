...
static void test_lock_prevents_salvage_drop(){
    rogue_inventory_entries_init(); rogue_inv_tags_init(); rogue_inv_tag_rules_clear();
...
static void test_rule_determinism(){
    ...
    const char* tags[4]; int count = rogue_inv_tags_list(20,tags,4);
    assert(count==2);
    unsigned seenA=0,seenB=0; for(int i=0;i<count;i++){ if(tags[i] && tags[i][0]=='A') seenA=1; if(tags[i] && tags[i][0]=='B') seenB=1; }
    assert(seenA && seenB);
}

int main(){ test_lock_prevents_salvage_drop(); test_rule_determinism(); printf("inventory_phase3_lock_rule_enforce: OK\n"); return 0; }
