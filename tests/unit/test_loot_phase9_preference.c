...
/* Test 9.3 player preference: pickups of one category should reduce its preference factor relative to untouched. */

int main(void){
    rogue_adaptive_reset();
    int catA = 0; int catB = 1;
    /* Simulate base observation so adaptive recompute has data */
    for(int i=0;i<10;i++){ rogue_adaptive_record_item(catA); rogue_adaptive_record_item(catB); }
    ...
    for(int i=0;i<50;i++){ rogue_adaptive_record_pickup(catA); }
    rogue_adaptive_recompute();
