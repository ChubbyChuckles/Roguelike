...
{
    RogueUIContext ctx;
    RogueUIContextConfig cfg = {.max_nodes = 16, .seed = 123u};
    ...
    fflush(stdout);
    int p = rogue_ui_panel(&ctx, (RogueUIRect){0, 0, 100, 50}, 0xFF00FFFFu);
    assert(p == 0);
    printf("STEP panel ok idx=%d\n", p);
    fflush(stdout);
    int t = rogue_ui_text(&ctx, (RogueUIRect){2, 4, 96, 12}, "Hello", 0xFFFFFFFFu);
    assert(t == 1);
...
{
    RogueUIContext a, b;
    RogueUIContextConfig cfg = {.max_nodes = 4, .seed = 999u};
    printf("RNG init A\n");
    fflush(stdout);
    assert(rogue_ui_init(&a, &cfg));
    assert(rogue_ui_init(&b, &cfg));
    printf("RNG init both ok\n");
    fflush(stdout);
    unsigned int seq_a[3];
    unsigned int seq_b[3];
    for (int i = 0; i < 3; i++)
    {
        seq_a[i] = rogue_ui_rng_next(&a);
        seq_b[i] = rogue_ui_rng_next(&b);
    }
    for (int i = 0; i < 3; i++)
    {
