...
int main(void)
{
    ...
    if (!expect(vr == 0, "server verify match"))
        return 1;
    int vrb = rogue_loot_server_verify(3, seed, 3, ids, qty, rar, h ^ 0x1);
    if (!expect(vrb == 1, "server verify mismatch"))
        return 2;
    ...
    for (int i = 0; i < 5; i++)
    {
        rogue_loot_anomaly_record(5, high_roll_rar);
    }
    if (!expect(rogue_loot_anomaly_flag() == 1, "anomaly flagged"))
        return 3;
    ...
    for (int i = 0; i < 20; i++)
    {
        rogue_loot_anomaly_record(5, normal_rar);
    }
    if (!expect(rogue_loot_anomaly_flag() == 0, "no anomaly"))
        return 4;
