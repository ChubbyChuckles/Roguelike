...
static int g_gold=50000; static int spend_gold(int amt){ if(amt<0) return -1; if(g_gold<amt) return -2; g_gold-=amt; return 0; }
static int unlocked_count=0; static void on_unlocked(int idx){ unlocked_count++; }
...
int main(){
    ...
    if(rogue_craft_recipe_count()>0){
        ...
        int gold_after = g_gold; rc = rogue_vendor_purchase_recipe_unlock(0, 1000, spend_gold, on_unlocked); assert(rc==0); assert(g_gold==gold_after);
    }
    /* Scarcity record simulation */
    rogue_vendor_scarcity_record(0, 5); rogue_vendor_scarcity_record(0, -2); assert(rogue_vendor_scarcity_score(0)==3);
