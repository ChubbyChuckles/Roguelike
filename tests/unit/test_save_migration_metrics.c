/* Simplified metrics test: validates baseline (no migration needed) metrics remain zero and non-negative timings */
...
int main(void){
    ...
    if(rogue_save_manager_load_slot(0)!=0){ printf("MIG_METRIC_FAIL load\n"); return 1; }
    if(rogue_save_last_migration_failed()){ printf("MIG_METRIC_FAIL unexpected_failed_flag\n"); return 1; }
    if(rogue_save_last_migration_steps()!=0){ printf("MIG_METRIC_FAIL steps=%d expected0\n", rogue_save_last_migration_steps()); return 1; }
