...
static int iter_cb(const struct RogueSaveDescriptor* d, uint32_t id, const void* data,
...
int main(void)
{
    if (ROGUE_SAVE_FORMAT_VERSION < 7u)
    {
        printf("INTEG_SKIP v=%u\n", (unsigned) ROGUE_SAVE_FORMAT_VERSION);
        return 0;
    }
    ...
    if (save_rc != 0)
    {
        printf("INTEG_FAIL save\n");
        return 1;
    }
    unsigned char digest[32];
    memcpy(digest, rogue_save_last_sha256(), 32);
    /* Load triggers verification and populates last sha again */
    int lrc = rogue_save_manager_load_slot(0);
    printf("load rc=%d\n", lrc);
    if (lrc != 0)
    {
        printf("INTEG_FAIL load rc=%d\n", lrc);
        return 1;
    }
    unsigned char digest2[32];
    memcpy(digest2, rogue_save_last_sha256(), 32);
    if (memcmp(digest, digest2, 32) != 0)
    {
        printf("INTEG_FAIL digest mismatch\n");
        return 1;
    }
    ...
    if (rc != 0 || section_counter == 0)
    {
        printf("INTEG_FAIL iter rc=%d cnt=%d\n", rc, section_counter);
        return 1;
    }
    char hex[65];
    rogue_save_last_sha256_hex(hex);
    if (strlen(hex) != 64)
    {
        printf("INTEG_FAIL hex len\n");
        return 1;
    }
