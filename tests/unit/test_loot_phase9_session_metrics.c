...
int main(void){
    ...
    /* Record 30 drops: rarities distributed 10 common (0), 8 uncommon (1), 6 rare (2), 4 epic (3), 2 legendary (4) */
    for(int i=0;i<10;i++) rogue_metrics_record_drop(0);
    for(int i=0;i<8;i++) rogue_metrics_record_drop(1);
    for(int i=0;i<6;i++) rogue_metrics_record_drop(2);
    for(int i=0;i<4;i++) rogue_metrics_record_drop(3);
    for(int i=0;i<2;i++) rogue_metrics_record_drop(4);
    /* Record 12 pickups (subset) */
    for(int i=0;i<12;i++) rogue_metrics_record_pickup(i%5);
    double iph=0.0; double rph[5]; memset(rph,0,sizeof rph);
