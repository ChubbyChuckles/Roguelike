...
static int spawn_item(const char* id, const char* affix){
    ...
    int inst = rogue_items_spawn(def,1,0,0); assert(inst>=0);
    if(affix){ int ai=rogue_affix_index(affix); assert(ai>=0); RogueItemInstance* it=(RogueItemInstance*)rogue_item_instance_at(inst); it->prefix_index=ai; it->prefix_value=rogue_affix_at(ai)->min_value; }
    return inst;
}

static void test_resist_softcaps(void){
    RoguePlayer player={0}; player.max_health=100; player.strength=5; player.dexterity=5; player.vitality=5; player.intelligence=5; player.crit_chance=5; player.crit_damage=150;
    ...
    assert(g_player_stat_cache.resist_lightning<=85 && g_player_stat_cache.resist_lightning>=75); /* diminished from 80 */
