...
static RoguePlayer make_player(void){ RoguePlayer p; memset(&p,0,sizeof(p)); p.strength=12; p.dexterity=8; p.vitality=10; p.intelligence=4; p.crit_rating=120; p.haste_rating=60; p.avoidance_rating=30; p.crit_chance=12; p.crit_damage=160; p.max_health=120; return p; }
