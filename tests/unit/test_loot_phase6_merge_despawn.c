...
int main(void)
{
    ...
    int coin = rogue_item_def_index("gold_coin");
    assert(coin >= 0);
    int a = rogue_items_spawn(coin, 5, 10.0f, 10.0f);
    assert(a >= 0);
    int b = rogue_items_spawn(coin, 7, 10.2f, 10.1f);
    assert(b >= 0);
    ...
    for (int i = 0; i < g_app.item_instance_cap; i++)
        ...
        {
            active++;
            total_qty += g_app.item_instances[i].quantity;
        }
    assert(total_qty == 12);
    assert(active <= 2);
