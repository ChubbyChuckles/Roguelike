...
int main(void){
    ...
    RogueSkillDef s = {0};
    s.name="Whirl"; s.max_rank=1; s.base_cooldown_ms=50; s.cooldown_reduction_ms_per_rank=0; s.on_activate=cb_dummy; s.is_passive=0; s.action_point_cost=30; s.resource_cost_mana=0; s.max_charges=0;
    ...
    if(g_app.player.action_points >= 30){ assert(rogue_skill_try_activate(id,&ctx)==1); }
