...
int main(void)
{
    ...
    g_app.player.mana = 0; /* no mana cost path */
    ...
    s.cooldown_reduction_ms_per_rank = 0;
    s.on_activate = cb_dummy;
    s.is_passive = 0;
    s.action_point_cost = 30;
    s.resource_cost_mana = 0;
    s.max_charges = 0;
    ...
    int regen_ap_mid = g_app.player.action_points;
    assert(regen_ap_mid > after_first);
    ...
    printf("AP_ECON_OK start=%d after_first=%d regen_mid=%d after_second=%d uses=%d ap_spent=%d\n",
           start_ap, after_first, regen_ap_mid, after_second, rogue_skill_get_state(id)->uses,
           rogue_skill_get_state(id)->action_points_spent_session);
    fflush(stdout);
