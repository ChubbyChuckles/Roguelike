...
/* Minimal hyper armor simulation: manually toggle active state before incoming hit and ensure poise doesn't drop. */
int main(){
    RoguePlayer p; rogue_player_init(&p); g_exposed_player_for_stats = p; p.poise = p.poise_max; p.facing=0; /* down */
    ...
    assert(applied==0 || applied==50); /* either perfect guard zero (if guard active) or damage; poise unaffected */
