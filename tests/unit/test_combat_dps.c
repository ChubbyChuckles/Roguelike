...
int main(){
    ...
    RogueEnemy* e = rogue_test_spawn_hostile_enemy(0.3f, 0.0f); /* closer to ensure within 0.36f dist2 radius */
    assert(e);
    ...
    while(sim_ms < 8000.0){
        ...
        if(dt_sec <= 0) dt_sec = 0.001; /* safety */
    ...
    last_health = h; /* update baseline each frame */
        if(((int)sim_ms)%500==0){
            float dx = e->base.pos.x; float dy = e->base.pos.y; float dist2 = dx*dx + dy*dy;
            printf("tick ms=%.0f cooldown=%.1f dist2=%.3f hp=%d events=%d\n", sim_ms, e->attack_cooldown_ms, dist2, h, damage_events); fflush(stdout);
        }
    }

    /* Expect at least one damage event */
    if(damage_events==0){
    printf("no enemy attacks registered: start_hp=%d end_hp=%d enemy_pos=(%.2f,%.2f) cooldown=%.1f dist2=%.3f\n", start_health, last_health, e->base.pos.x, e->base.pos.y, e->attack_cooldown_ms, (e->base.pos.x*e->base.pos.x + e->base.pos.y*e->base.pos.y)); fflush(stdout);
        rogue_app_shutdown();
        return 1;
    }
