...
int main(void)
{
    ...
    {
        enemies[i].alive = 1;
        enemies[i].base.pos.y = 0;
        enemies[i].health = 10;
        enemies[i].max_health = 10;
    }
    /* Place all enemies in a semicircle around slight forward arc so new perpendicular limit does
     * not exclude them when facing right */
    enemies[0].base.pos.x = 0.6f;
    enemies[0].base.pos.y = -0.2f;
    enemies[1].base.pos.x = 0.8f;
    enemies[1].base.pos.y = 0.0f;
    enemies[2].base.pos.x = 0.6f;
    enemies[2].base.pos.y = 0.2f;
    enemies[3].base.pos.x = 0.9f;
    enemies[3].base.pos.y = 0.0f;
    ...
    if (damaged == 0)
    {
        printf("no enemies damaged on hit frame\n");
        return 1;
    }
    (void) kills;
    return 0;
