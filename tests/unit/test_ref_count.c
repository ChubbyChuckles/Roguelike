...
typedef struct TestObj { int value; struct TestObj *child; } TestObj;

static void testobj_dtor(void *p){ TestObj *o=(TestObj*)p; o->value = -999; }

// Edge enumerator: expose child pointer
static size_t enum_edges(void *obj, void **out_children, size_t max){ if(max==0) return 0; TestObj *o=(TestObj*)obj; if(o->child){ out_children[0]=o->child; return 1; } return 0; }

static void test_basic(){
    TestObj *a = (TestObj*)rogue_rc_alloc(sizeof(TestObj), 1, testobj_dtor);
    a->value = 42; a->child=NULL;
...
static void test_graph_and_snapshot(){
    ...
    char buf[512]; size_t dot_sz = rogue_rc_generate_dot(NULL,0); assert(dot_sz>0);
    size_t w = rogue_rc_generate_dot(buf,sizeof(buf)); assert(w<sizeof(buf));
    // snapshot
    size_t snap_needed = rogue_rc_snapshot(NULL,0); assert(snap_needed>0);
    char snap[256]; rogue_rc_snapshot(snap,sizeof(snap));
    // validate contains both ids
    uint64_t pid = rogue_rc_get_id(parent); uint64_t cid = rogue_rc_get_id(child);
    ...
    assert(strstr(buf,needle1)); assert(strstr(buf,needle2));
    assert(strstr(snap,needle1)); assert(strstr(snap,needle2));
    // release order
...
static void test_validation_and_leaks(){
    ...
    RogueRcStats st; rogue_rc_get_stats(&st); assert(st.live_objects>=1);
...
int main(){
    ...
    RogueRcStats st; rogue_rc_get_stats(&st);
    printf("[ref_count] total_allocs=%llu total_frees=%llu live=%llu peak=%llu\n",(unsigned long long)st.total_allocs,(unsigned long long)st.total_frees,(unsigned long long)st.live_objects,(unsigned long long)st.peak_live);
    return 0;
}
