...
static void test_dedup(){
    ...
    const size_t pg=32; const size_t total = pg*3;
    char buf[96]; /* 32*3 */
    for(size_t i=0;i<pg;i++){ buf[i]='A'; buf[pg+i]='A'; buf[2*pg+i]='B'; }
    RogueCowBuffer *b = rogue_cow_create_from_bytes(buf,total,pg);
    assert(b);
    // force write on 3rd page to ensure uniqueness
    char x='B'; assert(rogue_cow_write(b,2*pg,&x,1)==0);
...
static void test_serialize(){
    ...
    assert(b);
    size_t need = rogue_cow_serialize(b,NULL,0); assert(need==100);
    unsigned char out[128]; memset(out,0,sizeof(out));
    size_t ret = rogue_cow_serialize(b,out,sizeof(out)); assert(ret==100);
    assert(memcmp(out,data,100)==0);
    RogueCowBuffer *c = rogue_cow_deserialize(out,100,chunk);
    assert(c);
    unsigned char chk[100]; assert(rogue_cow_read(c,0,chk,100)==0);
    assert(memcmp(chk,data,100)==0);
    rogue_cow_destroy(b); rogue_cow_destroy(c);
}
...
int main(){
    ...
    test_dedup();
    test_serialize();
    RogueCowStats st; rogue_cow_get_stats(&st);
    printf("[cow] buffers=%llu pages=%llu cows=%llu copies=%llu dedup=%llu linear=%llu\n",(unsigned long long)st.buffers_created,(unsigned long long)st.pages_created,(unsigned long long)st.cow_triggers,(unsigned long long)st.page_copies,(unsigned long long)st.dedup_hits,(unsigned long long)st.serialize_linearizations);
    return 0;
}
