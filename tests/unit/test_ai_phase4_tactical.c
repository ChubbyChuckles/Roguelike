...
static void test_strafe_action()
{
    ...
    RogueBTNode* strafe =
        rogue_bt_action_strafe("strafe", "player_pos", "agent_pos", "strafe_left", 4.0f, 0.5f);
    RogueBehaviorTree* tree = rogue_behavior_tree_create(strafe);
    float elapsed = 0.0f;
    RogueBTStatus st = ROGUE_BT_RUNNING;
    while (st == ROGUE_BT_RUNNING && elapsed < 2.0f)
    {
        st = rogue_behavior_tree_tick(tree, &bb, 0.1f);
        elapsed += 0.1f;
    }
    assert(st == ROGUE_BT_SUCCESS);
    bool left = false;
    rogue_bb_get_bool(&bb, "strafe_left", &left);
    assert(left == true); /* flipped after completion */
...
static void test_flank_and_regroup()
{
    ...
    rogue_bb_set_vec2(&bb, "agent_pos", 0.0f, 0.0f);
    RogueBTNode* flank =
        rogue_bt_tactical_flank_attempt("flank", "player_pos", "agent_pos", "flank_point", 2.0f);
    ...
    RogueBBVec2 flank_pt;
    assert(rogue_bb_get_vec2(&bb, "flank_point", &flank_pt));
    ...
    float elapsed = 0.0f;
    RogueBTStatus rs = ROGUE_BT_RUNNING;
    while (rs == ROGUE_BT_RUNNING && elapsed < 3.0f)
    {
        rs = rogue_behavior_tree_tick(regroup_tree, &bb, 0.1f);
        elapsed += 0.1f;
    }
...
static void test_cover_seek()
{
    ...
    RogueBTNode* cover = rogue_bt_tactical_cover_seek(
        "cover", "player_pos", "agent_pos", "rock_pos", "cover_point", "in_cover", 0.6f, 6.0f);
    RogueBehaviorTree* tree = rogue_behavior_tree_create(cover);
    assert(rogue_behavior_tree_tick(tree, &bb, 0.016f) == ROGUE_BT_SUCCESS);
    bool in_cover = false;
    rogue_bb_get_bool(&bb, "in_cover", &in_cover);
    assert(in_cover);
    rogue_behavior_tree_destroy(tree);
}
