...
int main(void)
{
    ...
    {
        fprintf(stderr, "item defs load fail\n");
        return 2;
    }
    ...
    {
        fprintf(stderr, "materials registry load fail\n");
        return 2;
    }
    ...
    {
        fprintf(stderr, "no materials in registry\n");
        return 3;
    }
    int mat0 = 0;
    ...
    int avg = rogue_material_quality_average(mat0);
    assert_true(avg >= 10 && avg <= 25, "avg within expected bounds");
    float bias = rogue_material_quality_bias(mat0);
    assert_true(bias >= 0.0f && bias <= 1.0f, "bias bounds");
    /* Force multiple refinements to exercise failure & crit statistically */
    for (int i = 0; i < 10; i++)
    {
        rogue_material_quality_add(mat0, 10, 30);
        unsigned int before_rng = rng;
        rc = rogue_material_refine(mat0, 10, 20, 30, &rng, &produced, &crit);
        assert_true(rc == 0 || rc == -3, "refine rc range");
    }
    printf("CRAFT_P3_OK mat=%d total=%d avg=%d bias=%.2f\n", mat0,
           rogue_material_quality_total(mat0), rogue_material_quality_average(mat0),
           rogue_material_quality_bias(mat0));
    return 0;
}
