...
int main(){
    ...
    RoguePlayer player; rogue_player_init(&player);
    float dt = 30.0f; /* ms per tick */
    int presses = 0; int transitions=0; int lastPhase=-1;
    for(int frame=0; frame<80; ++frame){
        int press = 0;
        /* Press at start and near expected recover end to buffer next */
        if(frame==0 || frame==15 || frame==30 || frame==45 || frame==60) { press=1; presses++; }
        rogue_combat_update_player(&pc, dt, press);
        if(pc.phase != lastPhase){ transitions++; lastPhase = pc.phase; }
    }
    if(pc.combo < 2){ printf("expected combo to advance, combo=%d\n", pc.combo); return 1; }
    if(transitions < 5){ printf("too few transitions=%d (buffering failed)\n", transitions); return 1; }
    printf("buffer chain test ok combo=%d transitions=%d presses=%d\n", pc.combo, transitions, presses);
    return 0;
}
