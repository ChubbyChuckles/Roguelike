...
int main()
{
    ...
    int dmg_balanced = strike_once(&pc, &player, &enemy);
    if (dmg_balanced <= 0)
    {
        printf("fail_balanced_damage=%d\n", dmg_balanced);
        return 1;
    }

    enemy.health = 500;
    player.combat_stance = 1; /* aggressive */
    int dmg_aggr = strike_once(&pc, &player, &enemy);
    if (!(dmg_aggr > (int) (dmg_balanced * 1.10f)))
    {
        printf("fail_aggressive_scaling base=%d aggr=%d\n", dmg_balanced, dmg_aggr);
        return 2;
    }
    ...
    enemy.health = 500;
    int dmg_fam = strike_once(&pc, &player, &enemy);
    if (!(dmg_fam > dmg_balanced))
    {
        printf("fail_familiarity_bonus base=%d fam=%d\n", dmg_balanced, dmg_fam);
        return 3;
    }

    /* Durability decreases */
    float dur_before = rogue_weapon_current_durability(0);
    if (dur_before <= 0)
        dur_before = 100.0f;
    strike_once(&pc, &player, &enemy);
    float dur_after = rogue_weapon_current_durability(0);
    if (!(dur_after < dur_before))
    {
        printf("fail_durability_drop before=%.2f after=%.2f\n", dur_before, dur_after);
        return 4;
    }

    printf("phase7_weapon_stance_familiarity: OK base=%d aggr=%d fam=%d dur_before=%.1f "
           "dur_after=%.1f\n",
