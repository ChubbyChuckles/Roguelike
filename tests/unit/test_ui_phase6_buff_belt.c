...
int main()
{
    ...
    if (belt.count != 2)
    {
        printf("FAIL expected 2 buffs got %d\n", belt.count);
        return 1;
    }
    if (belt.icons[0].magnitude + belt.icons[1].magnitude != 8)
    {
        printf("FAIL magnitudes mismatch\n");
        return 1;
    }
    /* Advance time past first buff only */
    now = 3200.0;
    g_app.game_time_ms = now;
    rogue_buffs_update(now);
    rogue_hud_buff_belt_refresh(&belt, now);
    if (belt.count != 1)
    {
        printf("FAIL expected 1 buff after expiry got %d\n", belt.count);
        return 1;
    }
    if (belt.icons[0].type != ROGUE_BUFF_POWER_STRIKE)
    {
        printf("FAIL remaining buff wrong type %d\n", belt.icons[0].type);
        return 1;
    }
