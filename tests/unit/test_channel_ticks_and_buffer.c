/* Test channel multi-tick scheduler (every 250ms) and that a second activation during casting waits. */
#include <assert.h>
#include <stdio.h>
#include <string.h>
...
static int cb_tick(const RogueSkillDef* def, struct RogueSkillState* st, const struct RogueSkillCtx* ctx){ (void)def;(void)st;(void)ctx; g_ticks++; return 1; }

static void advance_time(double start_ms, double target_ms){
    for(double t=start_ms; t<=target_ms; t+=16.0){ rogue_skills_update(t); }
}

int main(void){
    rogue_skills_init();
    g_app.talent_points = 1;
    RogueSkillDef chan; memset(&chan,0,sizeof chan); chan.name="Beam"; chan.max_rank=1; chan.base_cooldown_ms=0; chan.on_activate=cb_tick; chan.cast_type=2; chan.cast_time_ms=800.0f; /* duration 800ms -> expect ~1 immediate + ticks at 250,500,750 */
