...
static RogueSkillDef make_def(int id, int ring)
{
    ...
    d.icon = "i";
    d.max_rank = 3;
    d.skill_strength = ring;
    d.base_cooldown_ms = 0;
    return d;
}
...
int main(void)
{
    ...
    defs[1] = make_def(1, 2);
    defs[2] = make_def(2, 3);
    defs[3] = make_def(3, 0); /* wildcard */
    for (int i = 0; i < skills; i++)
        rogue_skill_register(&defs[i]);
    ...
    for (int i = 0; i < maze.node_count; i++)
    {
        if (assigned[i] < 0)
        {
            fprintf(stderr, "FAIL node %d unassigned\n", i);
            return 2;
        }
    }
    ...
    for (int i = 0; i < maze.node_count; i++)
    {
        ...
        if (sid >= 0)
        {
            RogueSkillDef* def = &defs[sid];
            if (def->skill_strength > 0 && def->skill_strength != maze.nodes[i].ring &&
                !(def->skill_strength > maze.rings && maze.nodes[i].ring == maze.rings))
            {
                fprintf(stderr, "FAIL ring mismatch node=%d ring=%d skill_ring=%d\n", i,
                        maze.nodes[i].ring, def->skill_strength);
                return 3;
            }
        }
    }
