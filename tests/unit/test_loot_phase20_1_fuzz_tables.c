...
static int rrange(int lo, int hi)
{
    if (hi <= lo)
        return lo;
...
{
    const char* set = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-";
    return set[r32() % (unsigned) strlen(set)];
}

static void write_temp(const char* path, const char* data)
{
    FILE* f = NULL;
#if defined(_MSC_VER)
    fopen_s(&f, path, "wb");
...
static void build_item_defs_for_refs(void)
{
    ...
    if (added <= 0)
    {
        fprintf(stderr, "FAIL: fuzz items load %d\n", added);
        exit(2);
    }
...
static void fuzz_tables(void)
{
    ...
    char big[32768];
    size_t pos = 0;
    int lines = 1200; /* many lines to exercise caps */
    for (int i = 0; i < lines && pos + 200 < sizeof big; ++i)
    {
        ...
        if (form == 0)
        {
            ...
            pos += (size_t) snprintf(big + pos, sizeof big - pos, "TAB%d,%d,%d,", i, rrange(0, 2),
                                     rrange(0, 4));
            for (int e = 0; e < entries; e++)
            {
                ...
                int qmin = rrange(-2, 5);
                int qmax =
                    qmin + rrange(0, 5) - (r32() & 1 ? rrange(0, 3) : 0); /* sometimes reversed */
                int rmin = (r32() & 3) ? rrange(-2, 4) : -1;
                int rmax = rmin + rrange(0, 3) - (r32() & 1 ? rrange(0, 2) : 0);
                pos += (size_t) snprintf(big + pos, sizeof big - pos, "ITM%d,%d,%d,%d,%d,%d", itm,
                                         w, qmin, qmax, rmin, rmax);
                if (e < entries - 1)
        ...
        else if (form == 3)
        {
            /* Extremely long id */
            pos += (size_t) snprintf(big + pos, sizeof big - pos,
                                     "LONG%08XNAME,%d,%d,ITM1,1,1,1,1,-1,-1\n", r32(), 1, 1);
        }
        else if (form == 4)
        {
            /* Random chars */
            int len = rrange(5, 40);
            for (int k = 0; k < len && pos + 1 < sizeof big; ++k)
            {
                big[pos++] = rchar();
            }
            if (pos + 1 < sizeof big)
    ...
    if (added < 0)
    {
        fprintf(stderr, "FAIL: loader returned error\n");
        exit(3);
    }
    if (rogue_loot_tables_count() > ROGUE_MAX_LOOT_TABLES)
    {
        fprintf(stderr, "FAIL: table cap exceeded %d>%d\n", rogue_loot_tables_count(),
                ROGUE_MAX_LOOT_TABLES);
        exit(4);
    }
    if (rogue_loot_tables_count() > 0)
    {
        ...
        if (t && t->entry_count > ROGUE_MAX_LOOT_ENTRIES)
        {
            fprintf(stderr, "FAIL: entry cap exceeded %d>%d\n", t->entry_count,
                    ROGUE_MAX_LOOT_ENTRIES);
            exit(5);
        }
    }
}
