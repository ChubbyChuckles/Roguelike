...
int main(void)
{
    ...
    maze.base.rings = 2;
    maze.base.node_count = 4;
    maze.base.nodes = (RogueSkillMazeNode*) malloc(sizeof(RogueSkillMazeNode) * 4);
    for (int i = 0; i < 4; i++)
    {
        maze.base.nodes[i].x =
            (float) ((i < 2) ? 40 * cos(i * 3.14159f) : 80 * cos(i * 3.14159f * 0.5f));
        maze.base.nodes[i].y =
            (float) ((i < 2) ? 40 * sin(i * 3.14159f) : 80 * sin(i * 3.14159f * 0.5f));
        maze.base.nodes[i].ring = (i < 2) ? 0 : 1;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        maze.meta[i].ring = maze.base.nodes[i].ring;
        maze.meta[i].level_req = 1;
    ...
    float layers[16];
    int lc = rogue_progression_maze_layers(&maze, layers, 16);
    if (lc != rogue_progression_maze_total_rings(&maze))
    {
        printf("layer_count_mismatch %d %d\n", lc, rogue_progression_maze_total_rings(&maze));
        return 2;
    }
    for (int i = 1; i < lc; i++)
    {
        ...
        {
            printf("non_monotonic\n");
            return 3;
        }
    }
    /* Project nodes */
    for (int i = 0; i < maze.base.node_count; i++)
    {
        float r, t;
        if (!rogue_progression_maze_project(&maze, i, &r, &t))
        {
            printf("proj_fail\n");
            return 4;
        }
    ...
    char buf[2048];
    int w = rogue_progression_maze_ascii_overview(&maze, buf, sizeof buf, 48, 16);
    if (w < 0)
    {
        printf("ascii_fail\n");
        return 6;
