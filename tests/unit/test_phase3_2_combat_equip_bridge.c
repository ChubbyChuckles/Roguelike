...
static int test_bridge_initialization() {
    ...
    TEST_ASSERT(bridge.initialized == false, "Bridge should be marked as uninitialized after shutdown");
}
...
static int test_durability_damage_taken() {
    ...
    int result = rogue_combat_equip_bridge_on_damage_taken(&bridge, player, damage_amount, damage_type);
    ...
    TEST_ASSERT(bridge.durability_event_count == 0, "Event queue should be cleared after processing");
...
static int test_enchantment_application() {
    ...
    int result = rogue_combat_equip_bridge_apply_enchantments(&bridge, player, 
...
static int test_enchantment_effects_triggering() {
    ...
    int result = rogue_combat_equip_bridge_trigger_enchantment_effects(&bridge, enchant_trigger, context_data);
...
static int test_equipment_upgrade_notification() {
    ...
    int result = rogue_combat_equip_bridge_on_equipment_upgraded(&bridge, slot, old_item_id, new_item_id);
