...
int main(void){
    ...
    for(int i=0;i<count;i++){
        if(strcmp(types[i].id,"goblin_grunt")==0 || strcmp(types[i].id,"orc_grunt")==0 || strcmp(types[i].id,"skeleton_grunt")==0){
            found_grunt=1; /* basic grunt style enemy */
        }
        if(strcmp(types[i].id,"goblin_elite")==0 || strcmp(types[i].id,"orc_warrior")==0 || strcmp(types[i].id,"skeleton_warrior")==0){
