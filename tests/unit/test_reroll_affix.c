...
static int spend_gold(int amount)
{
    if (amount <= 0)
        return 0;
    if (rogue_econ_gold() < amount)
        return -1;
    rogue_econ_add_gold(-amount);
    return 0;
}

static int reroll_affixes(int inst_index, unsigned int* rng_state, int rarity)
{
    return rogue_item_instance_generate_affixes(inst_index, rng_state, rarity);
}
...
int main(void)
{
    ...
    {
        fprintf(stderr, "REROLL_FAIL find affixes\n");
        return 5;
    }
    rogue_affixes_reset();
    if (rogue_affixes_load_from_cfg(paff) <= 0)
    {
        fprintf(stderr, "REROLL_FAIL load affixes\n");
        return 6;
    }
    char pitems[256];
    if (!rogue_find_asset_path("items/swords.cfg", pitems, sizeof pitems))
    {
        fprintf(stderr, "REROLL_FAIL find swords\n");
        return 10;
    }
    ...
    {
        fprintf(stderr, "REROLL_FAIL load dir\n");
        return 11;
    }
    ...
    if (sword < 0)
    {
        fprintf(stderr, "REROLL_FAIL iron_sword\n");
        return 12;
    }
    ...
    if (inst < 0)
    {
        fprintf(stderr, "REROLL_FAIL spawn\n");
        return 13;
    }
    /* Generate initial affixes so reroll has something to change */
    const RogueItemInstance* it0 = rogue_item_instance_at(inst);
    unsigned int seed0 = rng_state;
    rogue_item_instance_generate_affixes(inst, &seed0, 3);
    /* seed some materials + gold */
    int dust = rogue_item_def_index("arcane_dust");
    if (dust < 0)
    {
        fprintf(stderr, "REROLL_FAIL dust missing\n");
        return 14;
    }
    rogue_inventory_add(dust, 10);
    ...
    int rc = rogue_craft_reroll_affixes(inst, elevated_rarity, dust, 5, 100, inv_get, inv_consume,
                                        spend_gold, reroll_affixes, &rng_state);
    const RogueItemInstance* it_after_dbg = rogue_item_instance_at(inst);
    fprintf(stderr, "DEBUG after reroll rc=%d new_pref=%d\n", rc,
            it_after_dbg ? it_after_dbg->prefix_index : -999);
    if (rc != 0)
    {
        fprintf(stderr, "REROLL_FAIL api rc=%d inst=%d rarity=%d before_pref=%d after_pref=%d\n",
                rc, inst, it->rarity, before_prefix,
                it_after_dbg ? it_after_dbg->prefix_index : -999);
        return 15;
    }
    const RogueItemInstance* it2 = rogue_item_instance_at(inst);
    if (it2->prefix_index < 0)
    {
        fprintf(stderr, "REROLL_FAIL no prefix_after\n");
        return 16;
    }
    if (rogue_inventory_get_count(dust) != 5)
    {
        fprintf(stderr, "REROLL_FAIL material not consumed\n");
        return 17;
    }
    if (rogue_econ_gold() != 900)
    {
        fprintf(stderr, "REROLL_FAIL gold not spent\n");
        return 18;
    }
