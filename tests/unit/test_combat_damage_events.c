...
static int last_event_index(void){ int idx = g_damage_event_head - 1; if(idx < 0) idx += ROGUE_DAMAGE_EVENT_CAP; return idx; }

int main(){
    int start_head = g_damage_event_head;
    RoguePlayer player; rogue_player_init(&player); player.strength=40; player.base.pos.x=0; player.base.pos.y=0; player.facing=2;
    RoguePlayerCombat pc; rogue_combat_init(&pc); pc.phase=ROGUE_ATTACK_STRIKE; pc.archetype=ROGUE_WEAPON_LIGHT; pc.chain_index=2; /* light_3 two-window */
    RogueEnemy enemy; enemy.alive=1; enemy.base.pos.x=1.0f; enemy.base.pos.y=0; enemy.health=500; enemy.max_health=500; enemy.armor=0; enemy.resist_physical=0; enemy.resist_fire=0; enemy.resist_frost=0; enemy.resist_arcane=0; enemy.resist_bleed=0; enemy.resist_poison=0;
    /* First window (0-36ms) */
    pc.strike_time_ms = 10.0f; rogue_combat_player_strike(&pc,&player,&enemy,1);
    /* Second window (36-75ms) */
    pc.strike_time_ms = 50.0f; rogue_combat_player_strike(&pc,&player,&enemy,1);
