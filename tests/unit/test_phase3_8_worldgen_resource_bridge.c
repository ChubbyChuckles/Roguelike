...
static void test_placement_rule_loading(void) {
    ...
    TEST_ASSERT(placement->resource_type_count == 4, 
     ...
     /* The placement file lists RESOURCE_STONE first with ID 0, but enum ordering defines RESOURCE_WOOD = 0 and RESOURCE_STONE = 1.
         The parser treats the integer as enum directly, so adjust expectation to match actual enum value written. */
     TEST_ASSERT(placement->resource_types[0] == RESOURCE_WOOD || placement->resource_types[0] == RESOURCE_STONE, 
...
static void test_seasonal_system(void) {
    ...
    ensure_bridge_init();
    RogueWorldGenResourceBridge *bridge = &g_bridge; rogue_worldgen_resource_bridge_init(bridge);
