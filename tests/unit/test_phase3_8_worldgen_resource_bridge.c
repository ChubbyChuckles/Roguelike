...
static void ensure_bridge_init(void)
{
    if (!g_bridge_initialized)
    {
        rogue_worldgen_resource_bridge_init(&g_bridge);
...
#define TEST_ASSERT_FLOAT_EQ(a, b, tolerance, test_name)                                           \
    do                                                                                             \
    {                                                                                              \
        tests_run++;                                                                               \
...
static bool create_test_placement_file(const char* filename)
{
    ...
    if (fopen_s(&file, filename, "w") != 0)
        return false;
    /* Use enum ordering: 0 WOOD, 1 STONE, 2 METAL_ORE, 6 FOOD */
    fprintf(file, "0,0.8\n"); /* RESOURCE_WOOD, weight 0.8 */
    fprintf(file, "1,0.6\n"); /* RESOURCE_STONE, weight 0.6 */
...
static void test_bridge_initialization(void)
{
    ...
    ensure_bridge_init();
    RogueWorldGenResourceBridge* bridge = &g_bridge;

    TEST_ASSERT(rogue_worldgen_resource_bridge_init(bridge), "Bridge initialization succeeds");

    TEST_ASSERT(bridge->initialized == true, "Bridge initialized flag set");

    TEST_ASSERT(bridge->enabled == true, "Bridge enabled flag set");

    TEST_ASSERT(bridge->total_node_count == 0, "Initial node count is zero");

    TEST_ASSERT(bridge->seasonal_system.current_season == SEASON_SPRING,
                "Default season is spring");

    TEST_ASSERT(bridge->quality_system.world_generation_seed == 12345, "Default quality seed set");
...
static void test_placement_rule_loading(void)
{
    ...
    TEST_ASSERT(create_test_placement_file(test_file), "Test placement file created");
    ...
    TEST_ASSERT(placement->placement_rules_loaded == true, "Placement rules marked as loaded");

    TEST_ASSERT(placement->resource_type_count == 4, "Correct number of resource types loaded");

    /* The placement file lists RESOURCE_STONE first with ID 0, but enum ordering defines
       RESOURCE_WOOD = 0 and RESOURCE_STONE = 1. The parser treats the integer as enum directly, so
       adjust expectation to match actual enum value written. */
    TEST_ASSERT(placement->resource_types[0] == RESOURCE_WOOD ||
                    placement->resource_types[0] == RESOURCE_STONE,
                "First resource type parsed");
    ...
    TEST_ASSERT(
        rogue_worldgen_resource_bridge_load_placement_rules(NULL, BIOME_FOREST, test_file) == false,
        "Loading fails with null bridge");

    TEST_ASSERT(rogue_worldgen_resource_bridge_load_placement_rules(bridge, MAX_BIOME_TYPES,
                                                                    test_file) == false,
                "Loading fails with invalid biome type");

    TEST_ASSERT(rogue_worldgen_resource_bridge_load_placement_rules(bridge, BIOME_FOREST,
                                                                    "nonexistent.cfg") == false,
...
static void test_node_placement(void)
{
    ...
    TEST_ASSERT(nodes_placed > 0, "Nodes were placed in region");

    TEST_ASSERT(bridge->total_node_count == nodes_placed, "Total node count matches placed nodes");

    /* Verify node properties */
    if (bridge->total_node_count > 0)
    {
        ResourceNode* first_node = &bridge->resource_nodes[0];
        TEST_ASSERT(first_node->is_active == true, "Placed node is active");

        TEST_ASSERT(first_node->region_id == 0, "Node has correct region ID");

        TEST_ASSERT(first_node->biome_type == BIOME_FOREST, "Node has correct biome type");
    ...
    /* Test invalid parameters */
    TEST_ASSERT(rogue_worldgen_resource_bridge_place_nodes(NULL, 0, BIOME_FOREST, 0, 0, 100, 100) ==
                    0,
                "Placement fails with null bridge");

    TEST_ASSERT(
        rogue_worldgen_resource_bridge_place_nodes(bridge, 64, BIOME_FOREST, 0, 0, 100, 100) == 0,
...
static void test_fertility_management(void)
{
    ...
    TEST_ASSERT(abundance->regeneration_rate > 1.5f, "Regeneration rate boosted by fertility");

    /* Test abundance calculation */
    float abundance_value =
        rogue_worldgen_resource_bridge_get_resource_abundance(bridge, 5, RESOURCE_STONE);
    TEST_ASSERT(abundance_value >= 1.0f, "Abundance calculation returns valid value");
    ...
    TEST_ASSERT_FLOAT_EQ(
        rogue_worldgen_resource_bridge_get_resource_abundance(NULL, 5, RESOURCE_STONE), 0.0f, 0.01f,
...
static void test_abundance_scaling(void)
{
    ...
    float low_abundance =
        rogue_worldgen_resource_bridge_get_resource_abundance(bridge, 0, RESOURCE_STONE);
    float normal_abundance =
        rogue_worldgen_resource_bridge_get_resource_abundance(bridge, 1, RESOURCE_STONE);
    float high_abundance =
        rogue_worldgen_resource_bridge_get_resource_abundance(bridge, 2, RESOURCE_STONE);

    TEST_ASSERT(low_abundance < normal_abundance, "Low fertility gives lower abundance");

    TEST_ASSERT(normal_abundance < high_abundance, "High fertility gives higher abundance");
...
static void test_seasonal_system(void)
{
    ...
    TEST_ASSERT(bridge->seasonal_system.current_season == SEASON_SUMMER, "Current season updated");

    /* Test seasonal availability setup */
    float season_modifiers[SEASON_COUNT] = {1.0f, 1.2f, 0.8f,
                                            0.6f}; /* Spring, Summer, Fall, Winter */
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_seasonal_availability(
                    bridge, RESOURCE_FOOD, SEASON_SUMMER, season_modifiers),
                "Seasonal availability added");

    TEST_ASSERT(bridge->seasonal_system.availability_count == 1, "Availability count incremented");

    SeasonalResourceAvailability* availability = &bridge->seasonal_system.availabilities[0];
    TEST_ASSERT(availability->resource_type == RESOURCE_FOOD, "Resource type set correctly");

    TEST_ASSERT(availability->peak_season == SEASON_SUMMER, "Peak season set correctly");
    ...
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_seasonal_availability(
                    NULL, RESOURCE_FOOD, SEASON_SUMMER, season_modifiers) == false,
...
static void test_quality_system_initialization(void)
{
    ...
    TEST_ASSERT(bridge->quality_system.world_generation_seed == 54321, "World seed set correctly");
    ...
    TEST_ASSERT_FLOAT_EQ(total_probability, 1.0f, 0.01f, "Tier probabilities sum to 1.0");
...
static void test_quality_calculation(void)
{
    ...
    if (nodes_placed > 0)
    {
        ...
        uint32_t quality =
            rogue_worldgen_resource_bridge_calculate_resource_quality(bridge, 0, RESOURCE_STONE);

        TEST_ASSERT(quality >= 1 && quality <= 100, "Quality value within valid range");

        /* Quality should be consistent for same node */
        uint32_t quality2 =
            rogue_worldgen_resource_bridge_calculate_resource_quality(bridge, 0, RESOURCE_STONE);
        TEST_ASSERT(quality == quality2, "Quality calculation is consistent");

        /* Test different nodes should potentially have different quality */
        if (nodes_placed > 1)
        {
            uint32_t quality3 = rogue_worldgen_resource_bridge_calculate_resource_quality(
                bridge, 1, RESOURCE_STONE);
            /* Note: Due to randomness, we can't guarantee different quality, but we test the
             * function works */
            TEST_ASSERT(quality3 >= 1 && quality3 <= 100, "Second node quality within valid range");
        }
    }

    /* Test invalid parameters */
    TEST_ASSERT(
        rogue_worldgen_resource_bridge_calculate_resource_quality(NULL, 0, RESOURCE_STONE) == 1,
        "Quality calculation returns minimum with null bridge");

    TEST_ASSERT(
        rogue_worldgen_resource_bridge_calculate_resource_quality(bridge, 999, RESOURCE_STONE) == 1,
...
static void test_depletion_cycle_setup(void)
{
    ...
    RogueWorldGenResourceBridge* bridge = &g_bridge;
    rogue_worldgen_resource_bridge_init(bridge);
    /* Ensure at least one resource node exists before setting up depletion cycle */
    const char* placement_file = "test_depletion_cycle_rules.cfg";
    create_test_placement_file(placement_file);
    rogue_worldgen_resource_bridge_load_placement_rules(bridge, BIOME_FOREST, placement_file);
    uint32_t nodes_placed = rogue_worldgen_resource_bridge_place_nodes(bridge, 0, BIOME_FOREST,
                                                                       0.0f, 0.0f, 200.0f, 200.0f);
    TEST_ASSERT(nodes_placed > 0, "Depletion setup: nodes placed for cycle");
    ...
    TEST_ASSERT(bridge->depletion_cycle_count == 1, "Depletion cycle count incremented");

    ResourceDepletionCycle* cycle = &bridge->depletion_cycles[0];
    TEST_ASSERT(cycle->node_id == 0, "Cycle node ID set correctly");

    TEST_ASSERT(cycle->max_capacity == 100, "Max capacity set correctly");

    TEST_ASSERT(cycle->current_capacity == 100, "Initial capacity equals max");

    TEST_ASSERT(cycle->regeneration_rate == 5, "Regeneration rate set correctly");

    TEST_ASSERT(cycle->is_depleted == false, "Initial depletion state is false");

    TEST_ASSERT(cycle->can_regenerate == true, "Regeneration enabled by default");
...
static void test_resource_harvesting(void)
{
    ...
    /* Prepare node for harvesting */
    const char* placement_file = "test_harvest_rules.cfg";
    create_test_placement_file(placement_file);
    rogue_worldgen_resource_bridge_load_placement_rules(bridge, BIOME_FOREST, placement_file);
    uint32_t nodes_placed = rogue_worldgen_resource_bridge_place_nodes(bridge, 0, BIOME_FOREST,
                                                                       0.0f, 0.0f, 200.0f, 200.0f);
    TEST_ASSERT(nodes_placed > 0, "Harvest: nodes placed for depletion cycle");
    ...
    ResourceDepletionCycle* cycle = &bridge->depletion_cycles[0];
    TEST_ASSERT(cycle->current_capacity == 80, "Capacity reduced after harvest");
    ...
    TEST_ASSERT(cycle->current_capacity == 0, "Node fully depleted");

    TEST_ASSERT(cycle->is_depleted == true, "Depletion flag set");
...
static void test_resource_regeneration(void)
{
    ...
    /* Prepare node for regeneration */
    const char* placement_file = "test_regen_rules.cfg";
    create_test_placement_file(placement_file);
    rogue_worldgen_resource_bridge_load_placement_rules(bridge, BIOME_FOREST, placement_file);
    uint32_t nodes_placed = rogue_worldgen_resource_bridge_place_nodes(bridge, 0, BIOME_FOREST,
                                                                       0.0f, 0.0f, 200.0f, 200.0f);
    TEST_ASSERT(nodes_placed > 0, "Regeneration: nodes placed for depletion cycle");
    ...
    ResourceDepletionCycle* cycle = &bridge->depletion_cycles[0];
    TEST_ASSERT(cycle->is_depleted == true, "Node is depleted before regeneration");
    ...
    TEST_ASSERT(cycle->current_capacity > 0, "Capacity increased after regeneration");
...
static void test_rare_event_spawning(void)
{
    ...
    TEST_ASSERT(rogue_worldgen_resource_bridge_spawn_rare_event(bridge, RARE_EVENT_CRYSTAL_BLOOM, 5,
                                                                100.0f, 200.0f),
                "Rare event spawn successful");

    TEST_ASSERT(bridge->event_system.event_count == 1, "Event count incremented");

    TEST_ASSERT(bridge->event_system.active_event_count == 1, "Active event count incremented");

    RareResourceEvent* event = &bridge->event_system.events[0];
    TEST_ASSERT(event->event_type == RARE_EVENT_CRYSTAL_BLOOM, "Event type set correctly");

    TEST_ASSERT(event->region_id == 5, "Event region ID set correctly");

    TEST_ASSERT_FLOAT_EQ(event->world_x, 100.0f, 0.01f, "Event X position set correctly");

    TEST_ASSERT_FLOAT_EQ(event->world_y, 200.0f, 0.01f, "Event Y position set correctly");

    TEST_ASSERT(event->is_active == true, "Event is active");

    TEST_ASSERT(event->bonus_resource_type == RESOURCE_RARE_CRYSTALS,
                "Bonus resource type matches event type");

    TEST_ASSERT(event->bonus_yield > 0, "Bonus yield is positive");

    TEST_ASSERT(event->bonus_quality_multiplier >= 1.0f, "Quality multiplier is at least 1.0");

    /* Test invalid parameters */
    TEST_ASSERT(rogue_worldgen_resource_bridge_spawn_rare_event(NULL, RARE_EVENT_CRYSTAL_BLOOM, 5,
                                                                100.0f, 200.0f) == false,
                "Event spawn fails with null bridge");

    TEST_ASSERT(rogue_worldgen_resource_bridge_spawn_rare_event(bridge, RARE_EVENT_TYPE_COUNT, 5,
                                                                100.0f, 200.0f) == false,
...
static void test_rare_event_processing(void)
{
    ...
    /* Spawn an event */
    rogue_worldgen_resource_bridge_spawn_rare_event(bridge, RARE_EVENT_METAL_VEIN_DISCOVERY, 3,
                                                    50.0f, 75.0f);

    TEST_ASSERT(bridge->event_system.active_event_count == 1, "One active event before processing");
    ...
    TEST_ASSERT(event->is_active == false, "Event marked as inactive after expiration");

    TEST_ASSERT(bridge->event_system.active_event_count == 0, "No active events after expiration");
...
static void test_discovery_location_management(void)
{
    ...
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_discovery_location(bridge, 300.0f, 400.0f, 7,
                                                                      RESOURCE_GEMS, 5),
                "Discovery location added successfully");

    TEST_ASSERT(bridge->discovery_system.location_count == 1, "Location count incremented");

    ResourceDiscoveryLocation* location = &bridge->discovery_system.locations[0];
    TEST_ASSERT_FLOAT_EQ(location->world_x, 300.0f, 0.01f, "Location X coordinate set correctly");

    TEST_ASSERT_FLOAT_EQ(location->world_y, 400.0f, 0.01f, "Location Y coordinate set correctly");

    TEST_ASSERT(location->region_id == 7, "Location region ID set correctly");

    TEST_ASSERT(location->hidden_resource_type == RESOURCE_GEMS,
                "Hidden resource type set correctly");

    TEST_ASSERT(location->discovery_difficulty == 5, "Discovery difficulty set correctly");

    TEST_ASSERT(location->has_been_discovered == false, "Location initially undiscovered");

    TEST_ASSERT(location->discovery_radius > 0.0f, "Discovery radius is positive");

    /* Test invalid parameters */
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_discovery_location(NULL, 300.0f, 400.0f, 7,
                                                                      RESOURCE_GEMS, 5) == false,
                "Adding location fails with null bridge");

    TEST_ASSERT(rogue_worldgen_resource_bridge_add_discovery_location(bridge, 300.0f, 400.0f, 64,
                                                                      RESOURCE_GEMS, 5) == false,
                "Adding location fails with invalid region ID");

    TEST_ASSERT(rogue_worldgen_resource_bridge_add_discovery_location(
                    bridge, 300.0f, 400.0f, 7, RESOURCE_TYPE_COUNT, 5) == false,
...
static void test_discovery_attempts(void)
{
    ...
    rogue_worldgen_resource_bridge_add_discovery_location(bridge, 100.0f, 100.0f, 0,
                                                          RESOURCE_MAGICAL_ESSENCE, 3);

    ResourceDiscoveryLocation* location = &bridge->discovery_system.locations[0];
    uint32_t discovery_id;

    /* Test discovery attempt from far away (should fail) */
    TEST_ASSERT(rogue_worldgen_resource_bridge_attempt_discovery(bridge, 500.0f, 500.0f, 100,
                                                                 &discovery_id) == false,
                "Discovery fails when player is too far away");

    /* Test discovery attempt with insufficient skill (should fail) */
    TEST_ASSERT(rogue_worldgen_resource_bridge_attempt_discovery(bridge, 100.0f, 100.0f, 5,
                                                                 &discovery_id) == false,
    ...
    {
        discovered = rogue_worldgen_resource_bridge_attempt_discovery(bridge, 100.0f, 100.0f, 100,
                                                                      &discovery_id);
    }

    if (discovered)
    {
        TEST_ASSERT(discovery_id == 0, "Discovery ID matches first location");

        TEST_ASSERT(location->has_been_discovered == true, "Location marked as discovered");

        TEST_ASSERT(bridge->discovery_system.discovered_count == 1, "Discovered count incremented");
    }
    ...
    /* Test invalid parameters */
    TEST_ASSERT(rogue_worldgen_resource_bridge_attempt_discovery(NULL, 100.0f, 100.0f, 100,
                                                                 &discovery_id) == false,
                "Discovery fails with null bridge");

    TEST_ASSERT(rogue_worldgen_resource_bridge_attempt_discovery(bridge, 100.0f, 100.0f, 100,
                                                                 NULL) == false,
...
static void test_bridge_update_functionality(void)
{
    ...
    /* Test normal update */
    TEST_ASSERT(rogue_worldgen_resource_bridge_update(bridge, 0.016f), "Bridge update successful");
...
static void test_metrics_collection(void)
{
    ...
    /* Run update to increment total_operations */
    rogue_worldgen_resource_bridge_update(bridge, 0.016f);

    /* Get metrics */
    WorldGenResourceBridgeMetrics metrics = rogue_worldgen_resource_bridge_get_metrics(bridge);

    TEST_ASSERT(metrics.abundance_calculations > 0, "Abundance calculations recorded");

    TEST_ASSERT(metrics.seasonal_updates > 0, "Seasonal updates recorded");
    ...
    TEST_ASSERT(null_metrics.total_operations == 0, "Null bridge returns empty metrics");
...
static void test_operational_status(void)
{
    ...
    ensure_bridge_init();
    RogueWorldGenResourceBridge* bridge = &g_bridge; /* Intentionally not re-init to test states */
...
static void test_spatial_queries(void)
{
    ...
    if (nodes_placed > 0)
    {
        ...
        uint32_t found_nodes = rogue_worldgen_resource_bridge_get_nodes_in_radius(
            bridge, 50.0f, 50.0f, 100.0f, node_ids, 10);

        TEST_ASSERT(found_nodes <= nodes_placed, "Found nodes doesn't exceed total nodes");

        TEST_ASSERT(found_nodes <= 10, "Found nodes doesn't exceed maximum requested");

        /* Verify node IDs are valid */
        for (uint32_t i = 0; i < found_nodes; i++)
        {
            TEST_ASSERT(node_ids[i] < bridge->total_node_count, "Found node ID is valid");
        }
    }

    /* Test invalid parameters */
    uint32_t dummy_ids[1];
    TEST_ASSERT(rogue_worldgen_resource_bridge_get_nodes_in_radius(NULL, 50.0f, 50.0f, 100.0f,
                                                                   dummy_ids, 1) == 0,
                "Spatial query fails with null bridge");

    TEST_ASSERT(rogue_worldgen_resource_bridge_get_nodes_in_radius(bridge, 50.0f, 50.0f, 100.0f,
                                                                   NULL, 1) == 0,
...
static void test_complete_workflow_integration(void)
{
    ...
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_seasonal_availability(
                    bridge, RESOURCE_FOOD, SEASON_SUMMER, seasonal_mods),
    ...
    uint32_t nodes_placed = rogue_worldgen_resource_bridge_place_nodes(bridge, 0, BIOME_FOREST,
                                                                       0.0f, 0.0f, 200.0f, 200.0f);
    TEST_ASSERT(nodes_placed > 0, "Workflow: Resource nodes placed");
    ...
    if (nodes_placed > 0)
    {
        uint32_t quality =
            rogue_worldgen_resource_bridge_calculate_resource_quality(bridge, 0, RESOURCE_STONE);
        TEST_ASSERT(quality >= 1 && quality <= 100, "Workflow: Resource quality calculated");
    }

    /* 8. Spawn rare event */
    TEST_ASSERT(rogue_worldgen_resource_bridge_spawn_rare_event(bridge, RARE_EVENT_ANCIENT_GROVE, 0,
                                                                100.0f, 100.0f),
                "Workflow: Rare event spawned");

    /* 9. Add discovery location */
    TEST_ASSERT(rogue_worldgen_resource_bridge_add_discovery_location(bridge, 150.0f, 150.0f, 0,
                                                                      RESOURCE_RARE_CRYSTALS, 4),
    ...
    TEST_ASSERT(true,
                "Workflow: Regeneration processing completed"); /* May or may not regenerate */
    ...
    float abundance =
        rogue_worldgen_resource_bridge_get_resource_abundance(bridge, 0, RESOURCE_FOOD);
    TEST_ASSERT(abundance > 0.0f, "Workflow: Resource abundance calculated");

    /* 14. Check spatial queries */
    uint32_t node_ids[5];
    uint32_t found_nodes = rogue_worldgen_resource_bridge_get_nodes_in_radius(
        bridge, 100.0f, 100.0f, 150.0f, node_ids, 5);
    ...
    WorldGenResourceBridgeMetrics metrics = rogue_worldgen_resource_bridge_get_metrics(bridge);
    TEST_ASSERT(metrics.total_operations > 0, "Workflow: Operations metrics collected");
    ...
    printf("   Placed %u nodes, spawned %llu rare events\n", nodes_placed,
           (unsigned long long) metrics.rare_events_spawned);
    printf("   Processed %llu abundance calculations, %llu seasonal updates\n",
           (unsigned long long) metrics.abundance_calculations,
           (unsigned long long) metrics.seasonal_updates);
