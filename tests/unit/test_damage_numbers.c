...
int main()
{
    RogueAppConfig cfg = {"DMGNUM",      320, 180, 320, 180, 0, 0, 0, 1, ROGUE_WINDOW_WINDOWED,
                          {0, 0, 0, 255}};
    ...
    if (mid_count != start_count)
    { /* numbers persist during idle frames; start_count already reflects creation */
    }
    ...
    if (end_count != 0)
    {
        printf("damage numbers did not fully decay start=%d end=%d\n", start_count, end_count);
        rogue_app_shutdown();
        return 1;
    }
    printf("damage number lifecycle ok (start=%d mid=%d end=%d)\n", start_count, mid_count,
           end_count);
    rogue_app_shutdown();
    return 0;
}
