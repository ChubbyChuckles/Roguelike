...
int main(){
    ...
    unsigned int now=0; unsigned int seed=12345u;
    ...
    for(int i=0;i<15;i++){ rogue_vendor_offers_roll(seed+2+i, now + 11*60*1000 + (unsigned int)(i*10), 0); }
    int after_expire = rogue_vendor_offers_count();
    assert(after_expire >=0 && after_expire <= ROGUE_VENDOR_OFFER_SLOT_CAP);
    rogue_vendor_offers_reset();
    int rare_hits=0; for(int i=0;i<80;i++){ rogue_vendor_offers_roll(seed+100+i, now, 1); for(int j=0;j<rogue_vendor_offers_count();j++){ const RogueVendorSpecialOffer* o=rogue_vendor_offer_get(j); if(o && o->rarity==4) rare_hits++; } }
    assert(rare_hits>0);
