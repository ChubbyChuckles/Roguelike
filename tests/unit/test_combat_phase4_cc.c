...
int rogue_vegetation_tile_blocking(int x, int y)
{
    (void) x;
    (void) y;
    return 0;
...
int main()
{
    ...
    g_app.run_speed = 2.0f;
    g_app.walk_speed = 1.0f;
    g_app.dt = 16.0f;
    g_app.world_map.width = 2;
    g_app.world_map.height = 2;
    g_app.world_map.tiles = tiles_stub;
    g_app.tile_size = 16;
    g_app.viewport_w = 64;
    g_app.viewport_h = 64;
    g_app.player_state = 2;
    float start_x = g_app.player.base.pos.x;
    /* Apply slow 50% */
    g_app.player.cc_slow_ms = 100.0f;
    g_app.player.cc_slow_pct = 0.5f;
    g_app.player_state = 2; /* running */
    rogue_player_controller_update();
    float moved_slow = g_app.player.base.pos.x -
                       start_x; /* zero because no input, we test speed scaling indirectly */
    ...
    g_app.player.cc_stun_ms = 0.0f;
    g_app.player.cc_disarm_ms = 200.0f;
    ...
    g_app.player.cc_disarm_ms = 0.0f;
    g_app.player.cc_root_ms = 200.0f;
    rogue_combat_update_player(&pc, 16.0f, 1);
