...
static RogueJsonValue* create_json_string(const char* value) { return json_create_string(value); }

static RogueJsonValue* create_json_integer(int64_t value) { return json_create_integer(value); }

static RogueJsonValue* create_json_object(void) { return json_create_object(); }
...
static void free_json_value(RogueJsonValue* json) { json_free(json); }
...
static void test_field_validation_required_fields(void)
{
    ...
    RogueSchemaField* field =
        rogue_schema_add_field(&schema, "required_field", ROGUE_SCHEMA_TYPE_STRING);
...
static void test_field_validation_string_constraints(void)
{
    ...
    RogueSchemaField* field =
        rogue_schema_add_field(&schema, "constrained_string", ROGUE_SCHEMA_TYPE_STRING);
...
static void test_field_validation_integer_constraints(void)
{
    ...
    RogueSchemaField* field =
        rogue_schema_add_field(&schema, "constrained_int", ROGUE_SCHEMA_TYPE_INTEGER);
...
static void test_schema_builder_functions(void)
{
    ...
    /* Test adding fields */
    RogueSchemaField* field1 =
        rogue_schema_add_field(&schema, "test_string", ROGUE_SCHEMA_TYPE_STRING);
    assert(field1 != NULL);
    assert(strcmp(field1->name, "test_string") == 0);
    assert(field1->type == ROGUE_SCHEMA_TYPE_STRING);
    assert(schema.field_count == 1);

    RogueSchemaField* field2 =
        rogue_schema_add_field(&schema, "test_int", ROGUE_SCHEMA_TYPE_INTEGER);
    assert(field2 != NULL);
    ...
    for (int i = 0; i < ROGUE_SCHEMA_MAX_FIELDS + 10; i++)
    {
        ...
        RogueSchemaField* field =
            rogue_schema_add_field(&schema, field_name, ROGUE_SCHEMA_TYPE_STRING);

        if (i < ROGUE_SCHEMA_MAX_FIELDS - 2)
        { /* -2 because we already added 2 fields */
            assert(field != NULL);
        }
        else
        {
...
static void test_comprehensive_validation(void)
{
    ...
    RogueSchemaField* name_field =
        rogue_schema_add_field(&schema, "name", ROGUE_SCHEMA_TYPE_STRING);
    ...
    RogueSchemaField* description_field =
        rogue_schema_add_field(&schema, "description", ROGUE_SCHEMA_TYPE_STRING);
    ...
    RogueSchemaField* value_field =
        rogue_schema_add_field(&schema, "value", ROGUE_SCHEMA_TYPE_INTEGER);
