...
/* Rely on core library's global state & derived stat routines; avoid local redefinitions that
 * caused duplicate symbols. */
...
int main(void)
{
    ...
    if (rogue_vegetation_tree_count() == 0)
    {
        printf("no_trees\n");
        return 2;
    }
    ...
    e->alive = 1;
    e->type_index = 0;
    e->base.pos.x = (float) tree_tx - 1.0f;
    e->base.pos.y = (float) tree_ty;
    e->anchor_x = e->base.pos.x;
    e->anchor_y = e->base.pos.y;
    e->patrol_target_x = (float) tree_tx;
    e->patrol_target_y = (float) tree_ty;
    g_app.enemy_count = 1;
    ...
    int exi = (int) (e->base.pos.x + 0.5f);
    int eyi = (int) (e->base.pos.y + 0.5f);
    if (exi == tree_tx && eyi == tree_ty)
    {
        printf("enemy_entered_tree\n");
        return 4;
    }
    g_app.player.base.pos.x = e->base.pos.x;
    g_app.player.base.pos.y = e->base.pos.y; /* Overlap */
    ...
    if (d2 < 0.30f * 0.30f - 0.00005f)
    {
        printf("player_overlap\n");
        return 5;
    }
    return 0;
}
