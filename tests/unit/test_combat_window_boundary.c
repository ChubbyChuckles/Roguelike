...
int main()
{
    ...
    RogueEnemy enemy;
    setup_enemy(&enemy, 1.0f);

    /* Just before first window start (negative time) expect no damage */
    combat.strike_time_ms = -0.1f;
    int hp = enemy.health;
    rogue_combat_player_strike(&combat, &player, &enemy, 1);
    assert(enemy.health == hp);
    /* At first window start (0) -> damage */
    combat.strike_time_ms = 0.0f;
    rogue_combat_player_strike(&combat, &player, &enemy, 1);
    int after_w0 = enemy.health;
    assert(after_w0 < hp);
    /* At exact overlap boundary (40ms) both window0 (still active) & window1 (starts) but should
     * only process new window1 since window0 already processed; damage increases. */
    combat.strike_time_ms = 40.0f;
