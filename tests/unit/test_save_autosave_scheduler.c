...
int main(void){
    ...
    if(rogue_save_autosave_count()<4){ printf("AUTO_FAIL count=%u\n", (unsigned)rogue_save_autosave_count()); return 1; }
    printf("AUTO_OK count=%u last_bytes=%u ms=%.2f\n", (unsigned)rogue_save_autosave_count(), (unsigned)rogue_save_last_save_bytes(), rogue_save_last_save_ms());
    return 0; }
