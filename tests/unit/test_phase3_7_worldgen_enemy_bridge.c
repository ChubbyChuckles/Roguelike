...
#define ASSERT_FLOAT_EQ(a, b, tolerance, message)                                                  \
    ASSERT_TRUE(fabs((a) - (b)) < (tolerance), message)
...
static void test_biome_encounter_management(void)
{
    ...
    bool selection_result = rogue_worldgen_enemy_bridge_get_biome_encounter(
        &bridge, BIOME_FOREST, 5, &enemy_id, &enemy_level);
    ...
    ASSERT_TRUE(enemy_level >= 1 && enemy_level <= 15,
                "Selected enemy level should be in valid range");

    /* Test encounter selection for invalid level */
    bool invalid_selection = rogue_worldgen_enemy_bridge_get_biome_encounter(
        &bridge, BIOME_FOREST, 50, &enemy_id, &enemy_level);
    ASSERT_FALSE(invalid_selection, "Encounter selection should fail for invalid level");

    /* Test loading non-existent file */
    bool invalid_load =
        rogue_worldgen_enemy_bridge_load_biome_encounters(&bridge, BIOME_DESERT, "nonexistent.cfg");
...
static void test_enemy_level_scaling(void)
{
    ...
    ASSERT_FLOAT_EQ(scaling->difficulty_rating, 2.0f, 0.01f,
                    "Difficulty rating should be set correctly");
    ...
    uint32_t scaled_level =
        rogue_worldgen_enemy_bridge_get_scaled_enemy_level(&bridge, 5, original_level);
    ASSERT_TRUE(scaled_level >= 1, "Scaled level should be at least 1");
    ASSERT_TRUE(scaled_level != original_level,
                "Scaled level should differ from original with 2.0x difficulty");
...
static void test_seasonal_enemy_variations(void)
{
    ...
    ASSERT_EQ(bridge.seasonal_system.current_season, SEASON_WINTER,
              "Current season should be winter");

    /* Test adding seasonal variations */
    bool variation_result = rogue_worldgen_enemy_bridge_add_seasonal_variation(
        &bridge, 100, SEASON_WINTER, 2.0f, 1.5f, 1.2f);
    ...
    rogue_worldgen_enemy_bridge_add_seasonal_variation(&bridge, 101, SEASON_SUMMER, 0.5f, 0.8f,
                                                       0.9f);
    rogue_worldgen_enemy_bridge_add_seasonal_variation(&bridge, 102, SEASON_SPRING, 1.5f, 1.0f,
                                                       1.1f);
...
static void test_enemy_pack_size_scaling(void)
{
    ...
    ASSERT_FLOAT_EQ(pack_scaling->danger_rating, 2.5f, 0.01f,
                    "Danger rating should be set correctly");
    ...
    /* Test invalid region */
    bool invalid_pack_scaling =
        rogue_worldgen_enemy_bridge_set_region_pack_scaling(&bridge, 999, 1.0f);
...
static void test_enemy_environmental_modifiers(void)
{
    ...
    bool modifier_result = rogue_worldgen_enemy_bridge_add_biome_modifier(
        &bridge, BIOME_VOLCANIC, ENEMY_MOD_FIRE_RESISTANT, 0.8f, 1.5f);
    ...
    ASSERT_EQ(mod_system->modifiers[0].modifier_type, ENEMY_MOD_FIRE_RESISTANT,
              "Modifier type should match");
    ASSERT_FLOAT_EQ(mod_system->modifiers[0].activation_chance, 0.8f, 0.01f,
                    "Activation chance should match");

    /* Test adding multiple modifiers */
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_VOLCANIC, ENEMY_MOD_HEALTH_BOOST,
                                                   0.6f, 1.3f);
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_VOLCANIC, ENEMY_MOD_DAMAGE_BOOST,
                                                   0.4f, 1.2f);
    ASSERT_EQ(mod_system->modifier_count, 3, "Should have three modifiers");

    /* Test applying environmental modifiers */
    uint32_t applied_modifiers =
        rogue_worldgen_enemy_bridge_apply_environmental_modifiers(&bridge, BIOME_VOLCANIC, 200);
    ...
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_TUNDRA, ENEMY_MOD_ICE_RESISTANT,
                                                   0.9f, 2.0f);
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_SWAMP, ENEMY_MOD_POISON_IMMUNE,
                                                   0.7f, 1.0f);
    ...
    /* Test invalid biome */
    bool invalid_modifier = rogue_worldgen_enemy_bridge_add_biome_modifier(
        &bridge, (RogueBiomeType) 999, ENEMY_MOD_ARMORED, 0.5f, 1.0f);
...
static void test_enemy_spawn_density_control(void)
{
    ...
    ASSERT_FLOAT_EQ(density->base_spawn_density, 1.5f, 0.01f,
                    "Base spawn density should be set correctly");
    ASSERT_EQ(density->max_concurrent_enemies, 25,
              "Max concurrent enemies should be set correctly");
    ...
    ASSERT_TRUE(density->respawn_rate_modifier < 1.0f,
                "Respawn rate should be reduced when near capacity");
...
static void test_enemy_migration_patterns(void)
{
    ...
    bool migration_result =
        rogue_worldgen_enemy_bridge_add_migration_route(&bridge, 30, 31, enemy_types, 3, 0.8f);
    ...
    ASSERT_FLOAT_EQ(route->migration_trigger_threshold, 0.8f, 0.01f,
    ...
    bool invalid_migration =
        rogue_worldgen_enemy_bridge_add_migration_route(&bridge, 999, 31, enemy_types, 3, 0.8f);
...
static void test_bridge_update_and_performance(void)
{
    ...
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_FOREST, ENEMY_MOD_HEALTH_BOOST,
                                                   0.5f, 1.2f);
    ...
    ASSERT_TRUE(final_metrics.avg_processing_time_us >= 0.0,
                "Average processing time should be non-negative");
    ASSERT_TRUE(final_metrics.total_operations >= updated_metrics.total_operations,
...
static void test_complete_workflow_integration(void)
{
    ...
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_FOREST, ENEMY_MOD_HEALTH_BOOST,
                                                   0.3f, 1.1f);
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_DESERT, ENEMY_MOD_FIRE_RESISTANT,
                                                   0.8f, 1.5f);
    rogue_worldgen_enemy_bridge_add_biome_modifier(&bridge, BIOME_DESERT, ENEMY_MOD_SPEED_BOOST,
                                                   0.6f, 1.3f);
    ...
    bool forest_encounter = rogue_worldgen_enemy_bridge_get_biome_encounter(
        &bridge, BIOME_FOREST, 5, &forest_enemy_id, &forest_enemy_level);
    bool desert_encounter = rogue_worldgen_enemy_bridge_get_biome_encounter(
        &bridge, BIOME_DESERT, 5, &desert_enemy_id, &desert_enemy_level);
    ...
    uint32_t forest_scaled_level =
        rogue_worldgen_enemy_bridge_get_scaled_enemy_level(&bridge, 0, 5);
    uint32_t desert_scaled_level =
        rogue_worldgen_enemy_bridge_get_scaled_enemy_level(&bridge, 1, 5);
    ...
    ASSERT_TRUE(desert_scaled_level >= forest_scaled_level,
                "Desert should generally have higher scaled levels");
    ...
    uint32_t forest_modifiers =
        rogue_worldgen_enemy_bridge_apply_environmental_modifiers(&bridge, BIOME_FOREST, 100);
    uint32_t desert_modifiers =
        rogue_worldgen_enemy_bridge_apply_environmental_modifiers(&bridge, BIOME_DESERT, 100);
    ...
    ASSERT_TRUE(post_migration_desert <= pre_migration_desert,
                "Desert should lose enemies via migration");
    ASSERT_TRUE(post_migration_forest >= pre_migration_forest,
                "Forest should gain enemies via migration");
