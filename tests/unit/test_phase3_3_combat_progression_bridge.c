...
extern int rogue_equip_get(int slot)
{
    (void) slot;
    return 1;
}
...
int test_combat_xp_distribution()
{
    ...
    TEST_ASSERT(bridge.current_difficulty_multiplier == 1.0f,
                "Default difficulty multiplier should be 1.0");

    /* Test XP award from damage dealt */
    int xp_result = rogue_combat_progression_bridge_award_xp(&bridge, ROGUE_XP_SOURCE_DAMAGE_DEALT,
                                                             200, 25, 1001);
    ...
    int xp_result2 = rogue_combat_progression_bridge_award_xp(
        &bridge, ROGUE_XP_SOURCE_ENEMY_DEFEATED, 0, 50, 1002);
    ...
    int xp_result3 = rogue_combat_progression_bridge_award_xp(
        &bridge, ROGUE_XP_SOURCE_DIFFICULTY_BONUS, 100, 75, 1003);
...
int test_skill_usage_tracking()
{
    ...
    int mastery_result = rogue_combat_progression_bridge_get_skill_mastery_progress(
        &bridge, 101, &progress, &required);
...
int test_passive_skill_effects()
{
    ...
    int activate_result =
        rogue_combat_progression_bridge_activate_passive_skill(&bridge, 201, 0, 1.25f, 10000);
    ...
    int effects_applied = rogue_combat_progression_bridge_apply_passive_effects(
        &bridge, &damage_mod, &defense_mod, &speed_mod);
    ...
    rogue_combat_progression_bridge_activate_passive_skill(&bridge, 202, 1, 0.8f,
                                                           5000); /* Defense reduction */
    TEST_ASSERT(bridge.active_passives_count == 2, "Should have 2 active passives");

    damage_mod = defense_mod = speed_mod = 1.0f;
    effects_applied = rogue_combat_progression_bridge_apply_passive_effects(
        &bridge, &damage_mod, &defense_mod, &speed_mod);
...
int test_achievement_system()
{
    ...
    int recent_count =
        rogue_combat_progression_bridge_get_recent_achievements(&bridge, recent_achievements, 5);
    TEST_ASSERT(recent_count > 0, "Should have recent achievements");
    TEST_ASSERT(strlen(recent_achievements[0].achievement_name) > 0,
                "Achievement should have a name");
...
int test_playstyle_analysis()
{
    ...
    RogueCombatPlaystyle initial_style =
        rogue_combat_progression_bridge_get_detected_playstyle(&bridge, &initial_confidence);
    ...
    {
        int analysis_result =
            rogue_combat_progression_bridge_analyze_playstyle(&bridge, 70, 80, 30);
    ...
    RogueCombatPlaystyle detected_style =
        rogue_combat_progression_bridge_get_detected_playstyle(&bridge, &confidence);
    TEST_ASSERT(bridge.playstyle_profile.combat_sessions_analyzed >= 10,
                "Should have analyzed multiple sessions");
    ...
    for (int i = 0; i < 12; i++)
    {
        rogue_combat_progression_bridge_analyze_playstyle(&bridge2, 40, 20,
                                                          60); /* Low risk, high tactical */
    }
...
int test_efficiency_metrics()
{
    ...
    int score_result =
        rogue_combat_progression_bridge_get_efficiency_score(&bridge, &initial_score, &detailed);
    ...
    int update_result =
        rogue_combat_progression_bridge_update_efficiency_metrics(&bridge, 5000.0f, 300, 50, 20);
    TEST_ASSERT(update_result == 1, "Efficiency update should succeed");
    TEST_ASSERT(bridge.metrics.efficiency_calculations_performed == 1,
                "Should have performed 1 calculation");
...
int test_death_penalty()
{
    ...
    TEST_ASSERT(bridge.death_penalty_state.equipment_durability_loss > 0,
                "Should have durability loss");
    ...
    int info_result =
        rogue_combat_progression_bridge_get_death_penalty_info(&bridge, &penalty_info);
    ...
    TEST_ASSERT(penalty_info.xp_penalty_percentage >= 5.0f,
                "XP penalty percentage should be at least 5%");
    TEST_ASSERT(penalty_info.skill_penalty_duration_ms == 300000,
                "Skill penalty duration should be 5 minutes");
    ...
    TEST_ASSERT(bridge.death_penalty_state.xp_penalty_amount >= prev_xp_penalty,
                "Higher level should have higher or equal penalty");
...
int test_combat_progression_integration()
{
    ...
    rogue_combat_progression_bridge_activate_passive_skill(&bridge, 401, 0, 1.15f,
                                                           0); /* Permanent damage boost */
    ...
    int effects = rogue_combat_progression_bridge_apply_passive_effects(&bridge, &dmg_mod, &def_mod,
                                                                        &spd_mod);
    ...
    TEST_ASSERT(bridge.efficiency_metrics.overall_efficiency_score > 0,
...
int test_combat_progression_performance()
{
    ...
    for (int i = 0; i < 100; i++)
    {
        rogue_combat_progression_bridge_award_xp(&bridge, ROGUE_XP_SOURCE_DAMAGE_DEALT, 100 + i,
                                                 20 + (i % 30), 4000 + i);
        rogue_combat_progression_bridge_track_skill_usage(&bridge, 500 + (i % 10), 70 + (i % 30),
                                                          4000 + i);

        if (i % 10 == 0)
        {
            rogue_combat_progression_bridge_analyze_playstyle(&bridge, 50 + (i % 40), 40 + (i % 50),
                                                              30 + (i % 60));
            rogue_combat_progression_bridge_update_efficiency_metrics(
                &bridge, 3000.0f + (i * 10), 150 + i, 30 + (i % 20), i % 10);
        }
    }

    TEST_ASSERT(bridge.metrics.xp_distributions_processed >= 100,
                "Should process many XP distributions");
    TEST_ASSERT(bridge.metrics.skill_usage_events_processed >= 100,
                "Should process many skill events");
    TEST_ASSERT(bridge.metrics.avg_processing_time_ms >= 0,
...
int test_combat_progression_debug()
{
    ...
    int result =
        rogue_combat_progression_bridge_award_xp(NULL, ROGUE_XP_SOURCE_DAMAGE_DEALT, 100, 10, 5001);
    ...
    result = rogue_combat_progression_bridge_apply_passive_effects(&bridge, NULL, &modifiers[1],
                                                                   &modifiers[2]);
    ...
    { /* More than max capacity */
        rogue_combat_progression_bridge_award_xp(&bridge, ROGUE_XP_SOURCE_DAMAGE_DEALT, 50, 10,
                                                 6000 + i);
    }
