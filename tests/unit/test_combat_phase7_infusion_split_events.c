...
int main(){
    ...
    RoguePlayer player; memset(&player,0,sizeof player); player.team_id=0; player.strength=30; player.dexterity=10; player.intelligence=10; player.facing=2; player.equipped_weapon_id=0; player.combat_stance=0; player.weapon_infusion=1; /* fire */ player.base.pos.x=0; player.base.pos.y=0;
    RogueEnemy enemy; memset(&enemy,0,sizeof enemy); enemy.alive=1; enemy.team_id=1; enemy.base.pos.x=1.0f; enemy.base.pos.y=0.0f; enemy.health=600; enemy.max_health=600; enemy.facing=1; enemy.resist_fire=50; /* strong fire resist to force visible split */

    /* First strike with fire infusion: expect lower effective damage vs high fire resist than pure physical would produce */
    int dmg_fire = strike_once(&pc,&player,&enemy); printf("dmg_fire=%d\n", dmg_fire); if(dmg_fire<=0){ printf("fail_fire_dmg=%d\n", dmg_fire); return 1; }
    /* Reset enemy and remove infusion to compare */
    enemy.health=600; player.weapon_infusion=0; int dmg_phys = strike_once(&pc,&player,&enemy); printf("dmg_phys=%d\n", dmg_phys); if(dmg_phys<=0){ printf("fail_phys_dmg=%d\n", dmg_phys); return 2; }
    /* Because high fire resist mitigates infused portion, fire build should be noticeably lower (at least 5% lower) */
    if(dmg_fire < dmg_phys){
        printf("phase7_infusion_split_events: OK fire=%d phys=%d\n", dmg_fire, dmg_phys); fflush(stdout);
        return 0;
    }
