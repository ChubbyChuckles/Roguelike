...
/* Minimal helper spawning a dummy item definition & instance if available; fallback no-op if API
 * absent. */
...
int main(void)
{
    ...
    {
        extern void rogue_stat_cache_force_update(
            const void* p); /* player pointer unused placeholder */
        rogue_stat_cache_force_update(NULL);
    }
    char snap2[128];
    int n2 = rogue_equipment_snapshot_export(snap2, sizeof snap2);
    ck(n2 > 0, "snapshot export post-mutate");
