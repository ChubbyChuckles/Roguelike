...
int main(void)
{
    ...
    a.max_rank = 5;
    a.skill_strength = 0;
    a.base_cooldown_ms = 2000.0f;
    a.cooldown_reduction_ms_per_rank = 200.0f;
    RogueSkillDef b = {0};
    b.id = -1;
    b.name = "Beta";
    b.icon = "icon_b";
    b.max_rank = 3;
    b.skill_strength = 0;
    b.base_cooldown_ms = 3000.0f;
    b.cooldown_reduction_ms_per_rank = 300.0f;
    ...
    if (rogue_skill_rank_up(idA) != 1)
        return 2;
    if (rogue_skill_rank_up(idA) != 2)
        return 3;
    if (rogue_skill_rank_up(idB) != 1)
        return 4;
    ...
    if (g_app.talent_points != saved_tp)
    {
        printf("tp mismatch %d vs %d\n", g_app.talent_points, saved_tp);
        return 6;
    }
    ...
    if (!sa || !sb)
    {
        printf("missing states\n");
        return 7;
    }
    if (sa->rank != 2 || sb->rank != 1)
    {
        printf("rank mismatch %d %d\n", sa->rank, sb->rank);
        return 8;
    }
    /* Clean */
    rogue_skills_shutdown();
