...
static const char* sample = "narrator|Hello ${player_name}! Seed=${run_seed}.\n"
...
int main(void)
{
    ...
    if (rogue_dialogue_register_from_buffer(55, sample, (int) strlen(sample)) != 0)
    {
        printf("FAIL register\n");
        return 1;
    }
    if (rogue_dialogue_start(55) != 0)
    {
        printf("FAIL start\n");
        return 1;
    }
    ...
    if (strstr(buf, "Aria") == NULL)
    {
        printf("FAIL player_name replace got: %s\n", buf);
        return 1;
    }
    if (strstr(buf, "4242") == NULL)
    {
        printf("FAIL run_seed replace got: %s\n", buf);
        return 1;
    }
    if (strstr(buf, "${player_name}") != NULL)
    {
        printf("FAIL token not replaced\n");
        return 1;
    }
    ...
    if (strstr(buf, "${does_not_exist}") == NULL)
    {
        printf("FAIL unknown token should remain literal\n");
        return 1;
    }
