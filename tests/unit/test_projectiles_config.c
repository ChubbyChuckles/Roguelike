...
static void write_temp(const char* path, const char* contents){ FILE* f=fopen(path,"wb"); assert(f); fputs(contents,f); fclose(f); }
...
int main(void){
    ...
    mkdir("assets", 0777);
#endif
    write_temp("assets/projectiles.cfg","IMPACT_LIFE_MS=260\nSHARD_COUNT_HIT=10\nSHARD_COUNT_EXPIRE=6\nSHARD_LIFE_MIN_MS=340\nSHARD_LIFE_VAR_MS=120\nSHARD_SPEED_MIN=2.5\nSHARD_SPEED_VAR=3.5\nSHARD_SIZE_MIN=4.0\nSHARD_SIZE_VAR=3.0\nGRAVITY=0.2\n");
    ...
    write_temp("assets/projectiles.cfg","IMPACT_LIFE_MS=300\nSHARD_COUNT_HIT=8\nSHARD_COUNT_EXPIRE=5\nSHARD_LIFE_MIN_MS=400\nSHARD_LIFE_VAR_MS=100\nSHARD_SPEED_MIN=3.0\nSHARD_SPEED_VAR=2.0\nSHARD_SIZE_MIN=5.0\nSHARD_SIZE_VAR=1.0\nGRAVITY=0.25\n");
