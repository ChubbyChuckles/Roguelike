...
int main(void){
    ...
    if(loaded <= 0){ fprintf(stderr,"ITEM_LOAD_FAIL\n"); return 11; }
    ...
    if(gold_idx<0 || bandage_idx<0){ fprintf(stderr,"ITEM_INDEX_FAIL\n"); return 12; }
    const RogueItemDef* gold = rogue_item_def_at(gold_idx); const RogueItemDef* band = rogue_item_def_at(bandage_idx);
    if(!gold || !band){ fprintf(stderr,"ITEM_PTR_FAIL\n"); return 13; }
    int cat0 = gold->category; int cat1 = band->category; if(cat0==cat1){ fprintf(stderr,"CATEGORY_EQUAL_FAIL\n"); return 14; }
    ...
    if(!(f0 > 0.4f && f0 < 1.4f)){ fprintf(stderr,"FACTOR_RANGE_FAIL f0=%f\n", f0); return 20; }
    if(!(f1 > 0.4f && f1 < 1.4f)){ fprintf(stderr,"FACTOR_RANGE_FAIL f1=%f\n", f1); return 21; }
    ...
    if(!(f0b <= f0 + 0.20f)){ fprintf(stderr,"SKEW_UPPER_FAIL f0b=%f f0=%f\n", f0b, f0); return 22; }
    if(!(f1b >= f1 - 0.20f)){ fprintf(stderr,"SKEW_LOWER_FAIL f1b=%f f1=%f\n", f1b, f1); return 23; }
    ...
    if(!(f1c <= f1b + 0.20f)){ fprintf(stderr,"REBALANCE_FAIL f1c=%f f1b=%f\n", f1c, f1b); return 24; }
