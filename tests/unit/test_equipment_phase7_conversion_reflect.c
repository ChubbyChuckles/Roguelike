...
static void setup_player(RoguePlayer* p){ memset(p,0,sizeof *p); p->health=100; p->max_health=100; p->poise=40; p->poise_max=40; p->guard_meter=60; p->guard_meter_max=60; p->perfect_guard_window_ms=120.0f; }
...
int main(void){
    RoguePlayer p; setup_player(&p); reset_cache();
    ...
    g_player_stat_cache.thorns_percent = 25; g_player_stat_cache.thorns_cap = 12;
    /* Disable block chance to isolate conversion */
    g_player_stat_cache.block_chance = 0; g_player_stat_cache.block_value = 0;
    int blocked=0, perfect=0; int dmg = rogue_player_apply_incoming_melee(&p, 100.0f, 0,-1, 0, &blocked, &perfect);
    /* Damage should remain 100 (conversion conserves) */
    assert(dmg == 100);
    /* Now test conversion cap: push over 95% attempted */
    reset_cache(); setup_player(&p);
    g_player_stat_cache.phys_conv_fire_pct = 70; g_player_stat_cache.phys_conv_frost_pct=40; g_player_stat_cache.phys_conv_arcane_pct=10; /* 120 => capped to 95 */
    ...
    reset_cache(); setup_player(&p); p.guard_meter=10.0f; p.guarding=0; g_player_stat_cache.guard_recovery_pct=50; /* +50% regen */
