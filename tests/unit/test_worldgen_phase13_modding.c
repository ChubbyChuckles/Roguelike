...
static void write_file(const char* path, const char* text){ FILE* f=fopen(path,"wb"); assert(f); fwrite(text,1,strlen(text),f); fclose(f); }
...
int main(void){
    ...
    write_file("temp_pack_phase13/example.biome.cfg",
        "name=Example\n"
    ...
    char err[256]; RogueDescriptorPackMeta meta; int r = rogue_pack_load_dir("temp_pack_phase13",1,&meta,err,sizeof(err));
    assert(r==ROGUE_PACK_LOAD_OK); assert(meta.schema_version==1); assert(rogue_pack_active_schema_version()==1);

    /* Validate active pack */
    assert(rogue_pack_validate_active()==1);

    /* Summary string */
    char summary[128]; rogue_pack_summary(summary,sizeof(summary)); assert(strstr(summary,"schema=1"));
    ...
    write_file("temp_pack_phase13/second.biome.cfg",
        "name=Second\n"
        "tile_weight_GRASS=1.0\n"
    ...
    assert(r==ROGUE_PACK_LOAD_OK); assert(rogue_pack_active_schema_version()==1);
