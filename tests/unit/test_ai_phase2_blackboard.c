...
int main(){
    ...
    RogueBBVec2 v; assert(rogue_bb_get_vec2(&bb, "last_player_pos", &v));
    assert(v.x==10.f && v.y==20.f);
    assert(rogue_bb_set_timer(&bb, "alert_timer", 1.0f));
    float t=0.f; assert(rogue_bb_get_timer(&bb, "alert_timer", &t) && t==1.0f);

    // Write policies
    assert(rogue_bb_set_int(&bb, "score", 5));
    assert(rogue_bb_write_int(&bb, "score", 3, ROGUE_BB_WRITE_MAX)); // 5 -> stays 5
    int iv=0; assert(rogue_bb_get_int(&bb, "score", &iv) && iv==5);
    ...
    float fv=0.f; assert(rogue_bb_get_float(&bb, "threat", &fv) && fv==1.0f);
