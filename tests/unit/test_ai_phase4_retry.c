...
static RogueBTStatus tick_flaky(RogueBTNode* node, RogueBlackboard* bb, float dt){
    (void)dt; FlakyConfig* cfg = (FlakyConfig*)node->user_data;
    int val=0; rogue_bb_get_int(bb, cfg->counter_key, &val);
...
static void test_retry_success_path(){
    ...
    RogueBTStatus st;
    int ticks=0; do { st = rogue_behavior_tree_tick(tree,&bb,0.016f); ticks++; assert(ticks<10); } while(st == ROGUE_BT_RUNNING);
    assert(st == ROGUE_BT_SUCCESS);
    int attempts=0; rogue_bb_get_int(&bb, "attempts", &attempts);
...
static void test_retry_exhaustion(){
    ...
    int attempts=0; rogue_bb_get_int(&bb, "attempts2", &attempts);
