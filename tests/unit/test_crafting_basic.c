...
int main(void)
{
    ...
    {
        fprintf(stderr, "CRAFT_FAIL find materials\n");
        return 10;
    }
    ...
    {
        fprintf(stderr, "CRAFT_FAIL load dir\n");
        return 11;
    }
    ...
    if (dust < 0 || shard < 0)
    {
        fprintf(stderr, "CRAFT_FAIL mat defs\n");
        return 12;
    }
    if (rogue_material_tier(dust) != 1)
    {
        fprintf(stderr, "CRAFT_FAIL dust tier\n");
        return 13;
    }
    if (rogue_material_tier(shard) != 3)
    {
        fprintf(stderr, "CRAFT_FAIL shard tier\n");
        return 14;
    }
    /* Fake recipe: create primal_shard from arcane_dust x5 */
    const char* recipe_path = "tmp_recipe.cfg";
    FILE* f = fopen(recipe_path, "wb");
    if (!f)
    {
        fprintf(stderr, "CRAFT_FAIL tmp open\n");
        return 15;
    }
    fprintf(f, "dust_to_shard,primal_shard,1,arcane_dust:5,\n");
    fclose(f);
    if (rogue_craft_load_file(recipe_path) <= 0)
    {
        fprintf(stderr, "CRAFT_FAIL load recipe\n");
        return 16;
    }
    const RogueCraftRecipe* r = rogue_craft_find("dust_to_shard");
    if (!r)
    {
        fprintf(stderr, "CRAFT_FAIL find recipe\n");
        return 17;
    }
    rogue_inventory_add(dust, 5);
    if (rogue_craft_execute(r, inv_get, inv_consume, inv_add) != 0)
    {
        fprintf(stderr, "CRAFT_FAIL exec\n");
        return 18;
    }
    if (rogue_inventory_get_count(shard) < 1)
    {
        fprintf(stderr, "CRAFT_FAIL no shard\n");
        return 19;
    }
    printf("CRAFT_OK recipes=%d shard=%d\n", rogue_craft_recipe_count(),
           rogue_inventory_get_count(shard));
    return 0;
}
