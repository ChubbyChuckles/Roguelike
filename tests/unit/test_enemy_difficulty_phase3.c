...
static void test_compose_swarm()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        int r = rogue_encounter_compose(0, 10, 10, 0, 1234u + i, &comp);
        assert(r == 0);
        assert(comp.unit_count >= 6 && comp.unit_count <= 8);
        for (int u = 0; u < comp.unit_count; u++)
        {
            assert(comp.units[u].level == 10);
        }
    }
}
...
static void test_compose_boss()
{
    RogueEncounterComposition comp;
    memset(&comp, 0, sizeof comp);
    int r = rogue_encounter_compose(1, 20, 20, 0, 999u, &comp);
    assert(r == 0);
