...
int main(void)
{
    ...
    int ring = rogue_items_spawn(1, 1, 0, 0);
    assert(ring >= 0);
    rogue_equip_try(ROGUE_EQUIP_RING1, ring);
    /* Serialize */
    char buf[2048];
    int n = rogue_equipment_serialize(buf, (int) sizeof buf);
    assert(n > 0);
    assert(strstr(buf, "EQUIP_V1"));
    /* Expect new tokens present */
    assert(strstr(buf, " SET ") != NULL);
    assert(strstr(buf, " RW ") != NULL);
    unsigned long long h1 = rogue_equipment_state_hash();
    assert(h1 != 0ULL);
    /* Clear equips */
    rogue_equip_unequip(ROGUE_EQUIP_WEAPON);
    rogue_equip_unequip(ROGUE_EQUIP_RING1);
    /* Deserialize */
    int r = rogue_equipment_deserialize(buf);
    assert(r == 0);
    /* Re-hash */
    ...
    {
        char legacy[2048];
        strncpy(legacy, buf, sizeof legacy);
        legacy[sizeof legacy - 1] = '\0';
        char* p = legacy;
        while ((p = strstr(p, " SET ")))
        {
            char* nl = strchr(p, '\n');
            if (!nl)
                break; /* remove until newline */
            memmove(p, nl, strlen(nl) + 1);
        }
        p = legacy;
        while ((p = strstr(p, " UNQ ")))
        {
            char* nl = strchr(p, '\n');
            if (!nl)
                break;
    ...
    {
        char tamper[2048];
        strncpy(tamper, buf, sizeof tamper);
        tamper[sizeof tamper - 1] = '\0';
        char* d = strstr(tamper, " DUR ");
        if (d)
        { /* find a digit after */
            while (*d && *d != '\n' && (*d < '0' || *d > '9'))
                d++;
