...
int main(void){
    ...
    int inst = rogue_items_spawn(sword,1,0,0); if(inst<0) return fail("spawn");
    unsigned int seed=99u; if(rogue_item_instance_generate_affixes(inst,&seed,3)!=0) return fail("gen");
    ...
    int found=0; for(int i=0;i<g_app.item_instance_cap;i++) if(g_app.item_instances[i].active){
        ...
        if(ri->def_index==before.def_index && ri->prefix_index==before.prefix_index && ri->prefix_value==before.prefix_value && ri->suffix_index==before.suffix_index && ri->suffix_value==before.suffix_value){ found=1; break; }
    }
    if(!found) return fail("roundtrip");
    printf("AFFIX_PERSIST_ROUNDTRIP_OK prefix=%d pv=%d suffix=%d sv=%d\n", before.prefix_index,before.prefix_value,before.suffix_index,before.suffix_value);
    return 0;
}
