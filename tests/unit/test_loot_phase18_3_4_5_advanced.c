...
int main(void){
    ...
    int pass=1; int i=0;
    for(;i<60;i++) rogue_loot_analytics_record(i,0,(double)i);
    for(;i<75;i++) rogue_loot_analytics_record(i,1,(double)i);
    for(;i<85;i++) rogue_loot_analytics_record(i,2,(double)i);
    for(;i<95;i++) rogue_loot_analytics_record(i,3,(double)i);
    for(;i<100;i++) rogue_loot_analytics_record(i,4,(double)i);
    int flags[5]; int any = rogue_loot_analytics_check_drift(flags);
    pass &= expect(!any, "no drift expected initial");

    /* Force drift: flood rarity 4 */
    for(int k=0;k<50;k++) rogue_loot_analytics_record(200+k,4,200.0+k);
    any = rogue_loot_analytics_check_drift(flags);
    pass &= expect(any, "drift expected");
    pass &= expect(flags[4]==1, "legendary drift flag");

    /* Session summary */
    RogueLootSessionSummary sum; rogue_loot_analytics_session_summary(&sum);
    ...
    for(int y=0;y<ROGUE_LOOT_HEAT_H;y++){
        for(int x=0;x<ROGUE_LOOT_HEAT_W;x++){
            if((x+y)%17==0) rogue_loot_analytics_record_pos(300 + y*ROGUE_LOOT_HEAT_W + x, (x+y)%5, 4000.0 + x + y, x,y);
        }
    }
    int heat_checks=0; for(int y=0;y<ROGUE_LOOT_HEAT_H;y++){ for(int x=0;x<ROGUE_LOOT_HEAT_W;x++){ if((x+y)%17==0){ heat_checks += rogue_loot_analytics_heat_at(x,y)==1; } } }
    pass &= expect(heat_checks>0, "heat points recorded");
    char csv[4096]; int csv_rc = rogue_loot_analytics_export_heatmap_csv(csv,sizeof csv); pass &= expect(csv_rc==0, "heatmap csv export");
