...
int main(void)
{
    ...
    g_app.player.xp = 321;
    g_app.player.health = 77;
    g_app.talent_points = 5;
    /* Minimal skill registration stub (simulate two skills) */
    struct RogueSkillDef d0 = {0};
    d0.name = "TestSkillA";
    d0.max_rank = 5;
    rogue_skill_register(&d0);
    struct RogueSkillDef d1 = {0};
    d1.name = "TestSkillB";
    d1.max_rank = 3;
    rogue_skill_register(&d1);
    ...
    s1->rank = 1;
    s1->cooldown_end_ms = 0.0;

    /* Inventory item instances */
    rogue_items_init_runtime();
    int inst = rogue_items_spawn(2, 4, 0, 0);
    if (inst >= 0)
    {
        g_app.item_instances[inst].rarity = 2;
        g_app.item_instances[inst].prefix_index = 1;
        g_app.item_instances[inst].prefix_value = 5;
    }
    ...
    /* Vendor */
    g_app.vendor_seed = 4242u;
    g_app.vendor_time_accum_ms = 1500.0;
    g_app.vendor_restock_interval_ms = 30000.0;

    /* World meta */
    g_app.pending_seed = 999u;
    g_app.gen_water_level = 0.42;
    g_app.gen_cave_thresh = 0.33;
    ...
    FILE* fchk = NULL;
    fchk = fopen("save_slot_0.sav", "rb");
    if (fchk)
    {
        struct RogueSaveDescriptor raw;
        if (fread(&raw, sizeof raw, 1, fchk) == 1)
        {
            printf("RAW_DESC v=%u sections=%u mask=0x%X size=%llu crc=0x%X\n", raw.version,
                   raw.section_count, raw.component_mask, (unsigned long long) raw.total_size,
                   raw.checksum);
        }
        fclose(fchk);
    }
    ...
    g_app.player.xp = 0;
    g_app.player.health = 1;
    g_app.talent_points = 0;
    s0->rank = 0;
    s0->cooldown_end_ms = 0;
    s1->rank = 0;
    s1->cooldown_end_ms = 0;
    ...
    g_app.vendor_seed = 0;
    g_app.vendor_time_accum_ms = 0;
    g_app.vendor_restock_interval_ms = 0;
    g_app.pending_seed = 0;
    g_app.gen_water_level = 0;
    g_app.gen_cave_thresh = 0;
    ...
    /* Assertions */
    assert(g_app.player.level == 7);
    assert(g_app.player.xp == 321);
    assert(g_app.player.health == 77);
    assert(g_app.talent_points == 5);
    s0 = (struct RogueSkillState*) rogue_skill_get_state(0);
    s1 = (struct RogueSkillState*) rogue_skill_get_state(1);
    assert(s0->rank == 3 && (int) s0->cooldown_end_ms == 1234);
    assert(s1->rank == 1);
    int inv_count = 0;
    ...
    assert(g_app.vendor_seed == 4242u && (int) g_app.vendor_time_accum_ms == 1500 &&
           (int) g_app.vendor_restock_interval_ms == 30000);
    assert(g_app.pending_seed == 999u);

    printf("SAVE_ROUNDTRIP_OK lvl=%d skill0=%d inv=%d buff=%d seed=%u\n", g_app.player.level,
           s0->rank, inv_count, rogue_buffs_get_total(ROGUE_BUFF_POWER_STRIKE), g_app.vendor_seed);
    return 0;
}
