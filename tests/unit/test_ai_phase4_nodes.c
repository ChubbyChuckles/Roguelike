...
/* Scorer helpers */
static float score_low(struct RogueBlackboard* bb, void* ud)
{
    (void) bb;
    (void) ud;
    return 1.0f;
}
static float score_high(struct RogueBlackboard* bb, void* ud)
{
    (void) bb;
    (void) ud;
    return 5.0f;
...
static void test_conditions_actions()
{
    ...
    RogueBTNode* visible = rogue_bt_condition_player_visible("vis", "player_pos", "agent_pos",
                                                             "agent_facing", 140.0f, 20.0f);
    RogueBTNode* seq = rogue_bt_sequence("seq");
    rogue_bt_node_add_child(seq, visible);
    RogueBTNode* move =
        rogue_bt_action_move_to("move", "player_pos", "agent_pos", 10.0f, "reached");
    ...
    for (int i = 0; i < max_ticks; i++)
    {
        ...
        bool r = false;
        rogue_bb_get_bool(&bb, "reached", &r);
        if (r)
        {
            reached = 1;
            break;
...
static void test_cooldown_retry()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        assert(rogue_behavior_tree_tick(tree, &bb, 0.05f) == ROGUE_BT_FAILURE);
    }
    /* Advance enough time */
    for (int i = 0; i < 10; i++)
    {
