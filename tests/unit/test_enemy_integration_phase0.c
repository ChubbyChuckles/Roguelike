...
/* Minimal fake app state population using already loaded enemy types via app_init would be ideal,
 * but here we directly build mapping on existing g_app state after app initialization in runtime
 * tests. For unit test simplicity we fabricate two type defs locally. */
...
static void fabricate_types()
{
    ...
    strncpy_s(g_app.enemy_types[0].name, sizeof g_app.enemy_types[0].name, "Goblin Grunt",
              _TRUNCATE);
#else
...
#endif
    g_app.enemy_types[0].group_min = 2;
    g_app.enemy_types[0].group_max = 3;
    ...
    strncpy_s(g_app.enemy_types[1].name, sizeof g_app.enemy_types[1].name, "Goblin Elite",
              _TRUNCATE);
#else
...
#endif
    g_app.enemy_types[1].group_min = 1;
    g_app.enemy_types[1].group_max = 1;
    g_app.enemy_types[1].tier_id = 2;
...
{
    fabricate_types();
    RogueEnemyTypeMapping maps[8];
    int count = 0;
    int ok = rogue_enemy_integration_build_mappings(maps, 8, &count);
    assert(ok && count == 2);
    assert(rogue_enemy_integration_validate_unique(maps, count));
    int idx0 = rogue_enemy_integration_find_by_type(0, maps, count);
    int idx1 = rogue_enemy_integration_find_by_type(1, maps, count);
...
static void test_spawn_apply()
{
    RogueEnemy e;
    memset(&e, 0, sizeof e);
    e.type_index = 1; /* elite */
    RogueEnemyTypeMapping maps[8];
    int count = 0;
    rogue_enemy_integration_build_mappings(maps, 8, &count);
    int idx = rogue_enemy_integration_find_by_type(1, maps, count);
    assert(idx >= 0);
    int player_level = 10;
...
int main(void)
{
    test_mappings();
    test_spawn_apply();
    printf("OK test_enemy_integration_phase0\n");
    return 0;
}
