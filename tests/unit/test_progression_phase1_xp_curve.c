...
static void test_catchup(void){
    int median = 80;
    float m_equal = rogue_xp_catchup_multiplier(80, median); assert(m_equal == 1.0f);
    float m_lower = rogue_xp_catchup_multiplier(60, median); assert(m_lower > 1.0f);
    float m_far = rogue_xp_catchup_multiplier(10, median); assert(m_far > m_lower);
    float m_cap = rogue_xp_catchup_multiplier(-100, median); (void)m_cap; /* function normalizes */
}
