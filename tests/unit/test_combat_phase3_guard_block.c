...
static void init_player(RoguePlayer* p){ rogue_player_init(p); p->facing = 0; /* facing down */ }

int main(){
    RoguePlayer p; init_player(&p);
    ...
    /* Incoming attack from front (up towards player is (0,-1) so enemy below player -> attack_dir=(0,-1) giving dot with facing (0,1) = -1; we instead simulate enemy above player attacking downward -> dir (0,1) */
    int dmg_full = 100;
    int applied = rogue_player_apply_incoming_melee(&p, (float)dmg_full, 0.0f, 1.0f, 0, &blocked, &perfect);
    assert(blocked==1);
    ...
    blocked=0; perfect=0;
    int applied2 = rogue_player_apply_incoming_melee(&p, (float)dmg_full, 0.0f, 1.0f, 0, &blocked, &perfect);
    ...
    blocked=0; perfect=0;
    int applied3 = rogue_player_apply_incoming_melee(&p, (float)dmg_full, 0.0f, -1.0f, 0, &blocked, &perfect);
