...
static void test_rule_application_basic(){
    ...
    /* We cannot assert actual rarity without accessing item defs; this ensures no crash and tag attempt executed on pickup. */
    rogue_inventory_register_pickup(5,1);
    rogue_inv_tag_rules_apply_def(5);
}

static void test_persistence(){
    rogue_inventory_entries_init(); rogue_inv_tags_init(); rogue_inv_tag_rules_clear(); rogue_register_core_save_components();
    ...
    const RogueInvTagRule* r0=rogue_inv_tag_rules_get(0); const RogueInvTagRule* r1=rogue_inv_tag_rules_get(1);
    assert(r0 && r1);
}

int main(){ test_rule_application_basic(); test_persistence(); printf("inventory_phase3_rules: OK\n"); return 0; }
