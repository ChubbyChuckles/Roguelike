...
int main(){
    srand(2222);
    RoguePlayer p; rogue_player_init(&p); p.facing=2; p.dexterity = 80; p.strength = 50; /* raise base stats for non-zero damage */
    p.team_id = 0; /* ensure player on team 0 */
    /* Elevate crit chance for deterministic observation: original 15 was too low for sampling window under some RNG seeds. */
    p.crit_chance = 100; p.crit_damage = 120; /* 2.20x multiplier, force cap */
    p.pen_flat = 0; p.pen_percent = 0;
    RoguePlayerCombat combat; rogue_combat_init(&combat); combat.phase = ROGUE_ATTACK_STRIKE; combat.strike_time_ms = 10.0f; combat.combo=1; /* inside window with combo ensures some base damage */
    combat.archetype = ROGUE_WEAPON_LIGHT; combat.chain_index = 0; /* explicit for clarity */

    RogueEnemy e; e.alive=1; e.base.pos.x=0.6f; e.base.pos.y=0; e.health=100000; e.max_health=100000; e.armor=15; e.resist_physical=30; e.resist_fire=0; e.resist_frost=0; e.resist_arcane=0; e.resist_bleed=0; e.resist_poison=0; e.team_id = 1; /* hostile team */
    int trials = 3000; int crit_seen=0; int pre_ge_post=0; int equal_count=0; int raw_unchanged=0;
    ...
    for(int t=0;t<trials;t++){
        /* Pre-mitigation mode */
    g_crit_layering_mode = 0; reset_damage_events(); e.health = e.max_health; combat.processed_window_mask=0; combat.hit_confirmed=0; if(t==0) combat.force_crit_next_strike=1; int kills = rogue_combat_player_strike(&combat,&p,&e,1); (void)kills;
        ...
        if(t==0){ combat.force_crit_next_strike = 1; }
    ...
    if(ev_post.crit || ev_pre.crit){ crit_seen=1; }
    if(t<3){ printf("DIAG t=%d precrit=%d postcrit=%d pre_raw=%d post_raw=%d pre_mitig=%d post_mitig=%d\n", t, ev_pre.crit, ev_post.crit, ev_pre.raw_damage, ev_post.raw_damage, ev_pre.mitigated, ev_post.mitigated); }
    if(ev_pre.crit && ev_post.crit){
        if(ev_pre.raw_damage == ev_post.raw_damage) raw_unchanged++;
        /* Legacy (mode 0) applies crit pre-mitigation -> armor reduces enlarged value -> mitigated may be higher OR lower than post depending on curve; accept any difference but require at least some trials where they differ */
    ...
    if(pre_ge_post==0){ printf("no differing mitigated values observed (unexpected)\n"); return 1; }
