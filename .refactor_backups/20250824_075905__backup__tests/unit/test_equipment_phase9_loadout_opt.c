...
#include "../../src/util/loadout_optimizer.h"
#include <assert.h>
#include <stdio.h>
#include <string.h>
...
static int first_two_weapon_defs(int* out_a, int* out_b)
{
    int found = 0;
...
static void test_snapshot_compare(void)
{
    ensure_item_content();
    rogue_equip_reset();
    RogueLoadoutSnapshot a, b;
    int w1 = -1, w2 = -1;
    int got = first_two_weapon_defs(&w1, &w2);
    assert(got == 2);
    int instA = rogue_items_spawn(w1, 1, 0, 0);
    assert(instA >= 0);
    assert(rogue_equip_try(ROGUE_EQUIP_WEAPON, instA) == 0);
    rogue_stat_cache_mark_dirty();
    rogue_stat_cache_force_update(&g_exposed_player_for_stats);
    assert(rogue_loadout_snapshot(&a) == 0);
    int instB = rogue_items_spawn(w2, 1, 0, 0);
    assert(instB >= 0);
    assert(rogue_equip_try(ROGUE_EQUIP_WEAPON, instB) == 0);
    rogue_stat_cache_mark_dirty();
    rogue_stat_cache_force_update(&g_exposed_player_for_stats);
    assert(rogue_loadout_snapshot(&b) == 0);
    int changed[ROGUE_EQUIP_SLOT_COUNT] = {0};
    int diff = rogue_loadout_compare(&a, &b, changed);
    assert(diff == 1);
    assert(changed[ROGUE_EQUIP_WEAPON] == 1);
}
...
static void fabricate_progression(void)
{
    int w1 = -1, w2 = -1;
    first_two_weapon_defs(
        &w1, &w2); /* spawn several of higher rarity weapon (w2) to give optimizer choices */
    for (int i = 0; i < 3; i++)
    {
        rogue_items_spawn(w2, 1, 0, 0);
    }
}
...
static void test_optimizer(void)
{
    ...
    fabricate_progression();
    int w1 = -1, w2 = -1;
    first_two_weapon_defs(&w1, &w2);
    int inst_base = rogue_items_spawn(w1, 1, 0, 0);
    assert(inst_base >= 0);
    assert(rogue_equip_try(ROGUE_EQUIP_WEAPON, inst_base) == 0);
    rogue_stat_cache_mark_dirty();
    rogue_stat_cache_force_update(&g_exposed_player_for_stats);
    int base_dps = g_player_stat_cache.dps_estimate;
    int base_ehp = g_player_stat_cache.ehp_estimate;
    int improvements = rogue_loadout_optimize(50, base_ehp - 10);
    assert(improvements >= 0);
    assert(g_player_stat_cache.dps_estimate >= base_dps);
    int used = 0, cap = 0, hits = 0, inserts = 0;
    rogue_loadout_cache_stats(&used, &cap, &hits, &inserts);
    assert(cap == 256);
    assert(inserts > 0);
}
...
int main(void)
{
    ...
    test_optimizer();
    printf("equipment_phase9_loadout_opt_ok\n");
    return 0;
}
