...
int main(void)
{
    ...
    if (sword < 0)
        return fail("sword_idx");
    int inst = rogue_items_spawn(sword, 1, 0.0f, 0.0f);
    if (inst < 0)
        return fail("spawn");
    ...
    int dmin = rogue_item_instance_damage_min(inst);
    int dmax = rogue_item_instance_damage_max(inst);
    const RogueItemDef* def = rogue_item_def_at(it->def_index);
    if (!(dmin >= def->base_damage_min && dmax >= def->base_damage_max))
        return fail("damage_bounds");
    /* Determinism check: recreate with same seed and expect same affix indices/values */
    rogue_items_shutdown_runtime();
    rogue_items_init_runtime();
    int inst2 = rogue_items_spawn(sword, 1, 0, 0);
    if (inst2 < 0)
        return fail("spawn2");
    unsigned int seed2 = 777u;
    if (rogue_item_instance_generate_affixes(inst2, &seed2, 3) != 0)
        return fail("gen2");
    const RogueItemInstance* it2 = rogue_item_instance_at(inst2);
    if (!it2)
        return fail("inst2_ptr");
    if (it->prefix_index != it2->prefix_index || it->prefix_value != it2->prefix_value)
        return fail("det_prefix");
    if (it->suffix_index != it2->suffix_index || it->suffix_value != it2->suffix_value)
        return fail("det_suffix");
