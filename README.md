...
- Simple word-run RLE compression (opt-in via `compression_level > 0`) stores a secondary buffer; format tag = 1 (future codecs reserved).
  - Compression analytics: the loader now records `compressed_size`, `memory_footprint`, and a derived `compression_ratio` (compressed/raw). You can request AUTO selection by setting `compression_level = -1`; it will choose between no compression and RLE based on size benefit per frame.
