...
**Phase 4.2: Shared Memory Pool System** - Complete ✅
Implemented layered allocator stack: (1) fixed-size block pools for tiny/small objects (32/64/128/256/512B) with per-page bitmaps; (2) buddy allocator (default 1MB arena, power-of-two blocks 32B..1MB) for medium/large allocations; (3) slab allocator for homogeneous objects (up to 64 classes, 8–32 objs/page, ctor/dtor hooks) enabling high locality and O(1) alloc/free. Added cross-cutting diagnostics: stats snapshot (`rogue_memory_pool_get_stats`), heuristic recommendations, fragmentation estimate (largest free vs total free), live & peak byte tracking, shrink (reclaim fully free slab pages), validation invariants, and human-readable dump. Thread safety optional (SDL mutex). Unit test `test_memory_pool` exercises fixed pool churn (300 tiny alloc/free), multi-size buddy allocations (70K & 130K), slab allocation/free cycles with shrink, stats/recommendations, and validation. This establishes a deterministic shared allocator substrate for subsequent reference counting, cache layers, and copy-on-write phases.
...
**Phase 4.3: Cache Management & Invalidation** - Complete ✅
Implemented multi-level cache hierarchy (`cache_system.c/.h`) with L1 (hot), L2 (warm), L3 (cold) open‑addressing hash tables (power-of-two capacities, linear probing). Features: size‑based placement + optional level hint; automatic promotion to hotter level on access (`rogue_cache_get`) and manual promotion API; targeted invalidation (`rogue_cache_invalidate`) and global flush (`rogue_cache_invalidate_all`); deterministic preload pipeline (`rogue_cache_preload`) accepting loader callback (cache assumes ownership on success); full iteration (`rogue_cache_iterate`) and human-readable dump (`rogue_cache_dump`). Integrated lightweight RLE compression (byte,run pairs) with configurable threshold (`rogue_cache_set_compress_threshold`) tracking compressed entry count and bytes saved. Statistics snapshot (`rogue_cache_get_stats`) exposes per-level hits, misses, evictions (simple linear-scan eviction when logical capacity reached), invalidations, promotions, preload operations, plus compression metrics. Unit test `test_cache_system` validates insertion/update, retrieval and promotion behavior, miss accounting, preload batch ingestion, iteration early-exit semantics, compression path (repetitive large buffers), targeted & full invalidation, and stat consistency under MSVC warnings-as-errors policy.
**Phase 4.5: Copy-on-Write & Data Sharing** - Complete ✅
Implemented page‑granular copy‑on‑write buffer (`cow.c/.h`) leveraging intrusive ref‑counted pages from Phase 4.4. Clones share pages; first write to a shared page triggers duplication (tracked via `cow_triggers` / `page_copies`). Open‑addressing hash dedup pass merges identical unmodified pages, reducing memory and bumping `dedup_hits`. Exposes diagnostics: stats snapshot (`rogue_cow_get_stats`) with buffers/pages/cow_triggers/page_copies/dedup_hits/serialize_linearizations and human dump (`rogue_cow_dump`) printing per‑page refcounts. Serialization/deserialization linearizes logical buffer contents for persistence (Phase 4.5.7). Unit test `test_cow` validates clone isolation, dedup effectiveness, and serialization round‑trip under MSVC.
**Phase 4.6: Data Structure Versioning & Migration** - Complete ✅
Introduced generic versioning registry (`versioning.c/.h`) with per-type current version numbers and chained N→N+1 migration functions. `rogue_version_migrate` automatically targets the registered current version when target=0, applying each step atomically; on any failure the original buffer & size are restored (rollback). Progress struct reports total/completed steps, last transition, and failure point. Statistics track registered types, migrations, executions, steps, and failures; `rogue_versioning_dump` prints registry & step graph. Unit test `test_versioning` exercises successful multi-step upgrade (v1→v3), verifies transformed field derivations, inspects progress metrics, and validates rollback on injected failing migration, ensuring resilience.
...
**Phase 5.1–5.2: Snapshot & Differential Synchronization System** - Complete ✅
Introduced lightweight per‑system snapshot registry (`snapshot_manager.c/.h`) enabling read‑only state captures with monotonic versioning, FNV1a‑64 hashing for integrity, and cached latest snapshot per system (timestamped). Differential synchronization: contiguous byte‑range diff (coalesced spans, size growth handled), delta build/apply APIs with version & bounds validation, dependency graph + topological plan (delta ordering), optional circular replay log (debug/replay scaffolding), stats (captures, failures, bytes stored, delta bytes, bytes saved, apply/build failure counters) and dump diagnostics. Unit test `test_snapshot_manager` exercises registration, capture version enforcement, delta round‑trip, dependency ordering, reset & replay log enable path. Establishes substrate for upcoming multi‑system transactions (Phase 5.3) and rollback/recovery.

**Phase 5.3: Multi-System Transaction Framework** - Complete ✅
Added lightweight 2‑phase commit transaction manager (`transaction_manager.c/.h`) with participant registry (callbacks: prepare/commit/abort, version read), per‑transaction isolation (Read Committed / Repeatable Read with version snapshot validation), timeouts producing TIMED_OUT state, explicit abort & rollback (invokes participant abort hooks, tracks rollback_invocations), and optional transition log ring (state changes with timestamps & participant bitmask). Stats expose lifecycle counts (started, committed, aborted, prepare failures, isolation violations, timeouts, rollbacks, peak active). Deterministic unit test `test_transaction_manager` validates commit success, isolation read path, timeout handling via injected logical clock, abort semantics, and logging enablement.

**Phase 5.4: Rollback & Recovery (In Progress)**
**Phase 5.5: State Validation & Integrity (Completed)**
Implemented `state_validation_manager` providing: per-system validator & optional repair callbacks, cross-system rule registry, scheduling (interval + trigger + immediate run), incremental hash-based skip optimization (reuses latest snapshot hash), severity-typed results (OK/WARN/CORRUPT), automatic repair attempts with success tracking, event log ring capturing each validation (system id / severity / code / message / repair outcome), stats snapshot & dump API. Unit test `test_state_validation_manager` exercises scheduling, warning & corruption detection+repair, and incremental skip flow.
Foundation implemented and expanded: snapshot restoration hook (`restore` callback + `rogue_snapshot_restore`) plus `rollback_manager.c/.h` offering per‑system configurable ring of checkpoints storing either FULL copies or DELTA entries (range diff vs previous FULL) to reduce memory (bytes_saved_via_delta tracked). Public APIs: configure, capture (single/multi), step_back(N), latest, purge, partial multi‑system rollback (`rogue_rollback_partial`), auto rollback for participant (`rogue_rollback_auto_for_participant`), stats snapshot, dump (version history), and per‑operation event log (`rogue_rollback_events_get`) with impact metadata (from_version, to_version, bytes, delta flag, auto flag). Delta path selects smaller of full or encoded diff. Stats now include systems/bytes rewound. Tests: `test_rollback_manager` (multi‑capture, step-back, purge, partial rollback) and `test_rollback_events` (auto rollback + event recording). Remaining: automatic invocation from transaction abort path (explicit API currently) and future stress/perf harness.

**Phase 5.6: Deadlock Detection & Prevention (Completed)**
Added `deadlock_manager` delivering reactive detection & resolution complementing preventative lock ordering (Phase 4.7). Each registered resource maintains current holder (`holder_tx`) and FIFO wait queue (bounded) while each transaction tracks the resource it's blocked on plus bitset of held resources (O(1) membership). A wait‑for graph is implicit (waiting tx -> holder of desired resource). Detection (`rogue_deadlock_tick`) performs bounded DFS from each waiting transaction, capturing first discovered cycle (path up to 16 tx ids). Victim heuristic selects highest tx id (approximate youngest) minimizing wasted prior work; installed abort callback allows integration with the transaction manager (future: direct `rogue_tx_abort` invocation) and releases all held resources promoting next waiters. Stats track registrations, acquisitions (immediate grants), waits, deadlocks_detected, victims_aborted, releases, wait_promotions, ticks. A cycle log ring (sequence id, path, victim) plus dump API aids debugging & future visualization. Unit test `test_deadlock_manager` constructs a 3‑way cycle (T1 holds R1 waits R2, T2 holds R2 waits R3, T3 holds R3 waits R1) asserting detection, victim abort callback firing, and absence of residual cycles post-resolution.

**Phase 5.7: Concurrency Control & Thread Safety (Completed/Partial)**
New cross‑platform threading helpers and a lightweight thread pool were added:
• Threading primitives (`src/core/integration/threading.c/.h`): RogueSem (SDL semaphore), RogueCond (mutex+cond wrapper), and reusable RogueBarrier with generation counter. Unit test `test_thread_barrier` prints BARRIER_OK.
• Thread pool (`src/core/integration/thread_pool.c/.h`): SDL threads + semaphore signaling and a lock‑free SPSC task ring using SDL atomics. Supports priority control (`rogue_thread_pool_set_priority`), debug yield knob for race hunting, and stats snapshot (`rogue_thread_pool_get_stats`) exposing pending, peak queue depth, worker wakeups, tasks submitted/executed. Unit test `test_thread_pool` prints THREAD_POOL_OK.
Build targets: `test_thread_barrier`, `test_thread_pool` (Windows/MSBuild generator uses `/m` for parallel builds). These additions underpin safe multi‑threaded slices and will be expanded with additional lock‑free structures in future phases.
...
* Audio/VFX Phase 4.2 (Hot reload + validation): Added `rogue_vfx_config_watch(path)` to auto-reload VFX CSV on change via shared hot-reload utility. Exposed `rogue_vfx_last_cfg_error_count`/`rogue_vfx_last_cfg_error_get` for tool/UI surfacing of parse/load issues. Unit test `test_audio_vfx_phase4_2_hot_reload_and_validation` passes; audio_vfx suite 12/12 in Debug with SDL2 (`ctest -j8 -R audio_vfx`).
* Audio/VFX Phase 4.1 (VFX Authoring Config Loader): Added CSV-based loader `rogue_vfx_load_cfg` (id, layer, lifetime_ms, world_space, emit_hz, particle_lifetime_ms, max_particles). Registers VFX and applies emitter settings; parser is MSVC-safe (no non-portable calls). New unit test `test_audio_vfx_phase4_1_vfx_config_loader` passes; audio_vfx suite now 11/11 in Debug with SDL2 (`ctest -j8 -R audio_vfx`).
* Audio/VFX Phase 3.6 (Screen vs World Transforms): Added camera-based transform path. New APIs: `rogue_vfx_set_camera(cam_x, cam_y, pixels_per_world)` and `rogue_vfx_particles_collect_screen(float* out_xy, uint8_t* out_layers, int max)`. World-space particles transform by camera offset and scale; screen-space particles are emitted as-is. Unit test `test_audio_vfx_phase3_6_screen_vs_world` passes; audio_vfx suite now 10/10 passing in Debug with SDL2 and parallel tests (`ctest -j8 -R audio_vfx`).
* Audio/VFX Phase 3.2 (Particles Core): Added per-VFX emitter config (emit Hz, particle lifetime, per-instance cap), global particle pool (1024), fractional emission accumulator, and strict expiry (> lifetime). New unit test `test_audio_vfx_phase3_2_particles_core` validates emission/expiry behavior (50 Hz, 200 ms lifetime, cap=10) with expected counts 5→10→5. Built Debug with SDL2 using `--parallel 8`; all audio_vfx tests passing.
* Audio/VFX Phase 3.4 (Layer Ordering Helper): Introduced `rogue_vfx_particles_collect_ordered` to return active particle layers in canonical BG→MID→FG→UI order for renderer iteration. Added unit test `test_audio_vfx_phase3_4_particle_layer_order`; audio_vfx suite now 9/9 passing under Debug with SDL2 (`ctest -j8 -R audio_vfx`).
* Audio/VFX Phase 3.1 (VFX Foundations): Added minimal VFX subsystem with registry (id→layer/world_space/lifetime), pooled active instances (cap 256), global timescale and freeze controls, per-layer active counts, and dispatcher hook via public spawn API. New unit test `test_audio_vfx_phase3_1_vfx_foundations` validates spawn via FX event, count/peek helpers, freeze/time scaling, and expiry. Built Debug with SDL2 using parallel -j8; audio_vfx suite now 7/7 passing.
* Audio/VFX Phase 2.2–2.7 (Foundations): Implemented audio mixer (master/category gains + mute), deterministic variation selection, simple voice cap, and positional attenuation stub with listener controls. Added audio registry helpers and a pure math testing hook. Unit tests: mixer gain, mute, positional falloff, and registry clear — all passing in Debug with SDL2 (-j8).
* Skills System Phase 1B (Talents v2 – serialization + deterministic hash): Upgraded talents serialization to a versioned format with varint ranks and backward-compatible v1 loader (unlocked→rank=1). Hash now includes version and non-zero ranks, stable regardless of unlock order. Added unit test `test_talents_phase1b_ranks_and_hash`; all talents tests pass in Debug with SDL2 using parallel builds (-j8).
...
* AI Phase 6.3 (Combat Behaviour Extensions): Added opportunistic attack node `rogue_bt_action_opportunistic_attack` that succeeds when the target is in a recovery window and (optionally) within max range; on success can reset a cooldown timer. Unit test `test_ai_phase6_3_opportunistic` passes in Debug with SDL2 and parallel builds.
* AI Phase 6.4 (Combat Behaviour Extensions): Added kiting logic node `rogue_bt_action_kite_band` to maintain a preferred distance band [min,max] from the target — moves away when too close, toward when too far, returns SUCCESS when inside the band. Unit test `test_ai_phase6_4_kiting` passes (Debug, SDL2, `--parallel 8`).
* AI Phase 6.5 (Combat Behaviour Extensions): Implemented focus fire coordination with threat leader broadcast and TTL decay. Added `rogue_bt_tactical_focus_broadcast_if_leader` and `rogue_bt_tactical_focus_decay`. Unit test `test_ai_phase6_5_focus` passes (Debug, SDL2, `--parallel 8`).
* AI Phase 6.6 (Combat Behaviour Extensions): Added finisher/execute action `rogue_bt_action_finisher_execute` that triggers below an HP% threshold and within range, with optional cooldown reset. Unit test `test_ai_phase6_6_finisher` passes (Debug, SDL2, `--parallel 8`).
* AI Phase 6.7 (Combat Behaviour Extensions): Added decorators `rogue_bt_decorator_reaction_delay` (gates child until reaction time elapses) and `rogue_bt_decorator_aggression_gate` (requires minimum aggression scalar). Unit test `test_ai_phase6_7_decorators` passes (Debug, SDL2, `--parallel 8`).
* AI Phase 5 (Movement & Pathing): Implemented flow field precomputation and local avoidance utilities. New modules `ai/pathing/flow_field.{c,h}` (reverse Dijkstra, per-cell distance and step) and `ai/pathing/local_avoidance.{c,h}` (perpendicular detour when forward is blocked). Unit tests `test_ai_phase5_2_flow_field` and `test_ai_phase5_3_local_avoid` pass in Debug with SDL2 and `--parallel 8`.
* AI Phase 5 (Movement & Pathing): Implemented path smoothing API and stuck detection decorator. New navigation function `rogue_nav_path_simplify` collapses collinear segments while preserving endpoints; AI decorator `rogue_bt_decorator_stuck_detect` flags low movement over a window using blackboard timer/vec2. Unit tests `test_ai_phase5_path_smoothing_and_stuck` and `test_ai_phase5_stuck_detect` pass (Debug, SDL2, --parallel 8).
...
* Combat: Familiarity progression now validated by unit tests. We reset the per-strike hit mask when invoked directly by tests and suppress knockback during forced-attack harness runs, ensuring repeated stationary hits accumulate usage and cap at +10% as intended (`test_combat_phase7_familiarity_cap`).
...
* Integration Phase 5.8 (Tests): Implemented synchronization validation tests covering snapshots, deltas, transactions, rollback, and multi-system integration. Fixed snapshot pointer lifetime by introducing per-system double-buffered snapshots; improved delta build/apply; aligned rollback semantics (step_back progression, full/delta storage) and expanded system ID range. All Phase 5.8 tests pass. Builds use SDL2 with parallel compilation by default.
...
* UPDATE: Integration Phase 3.6 (Character Progression ↔ Skills):
  * 3.6.1–3.6.3 implemented: centralized XP award events, first-unlock gating (level >= 5*skill_strength) with SKILL_UNLOCKED emission, and passive maze threshold enforcement (heap OOB fix in meta allocation).
  * 3.6.4 implemented: skill effect scaling with progression stats — Fireball damage now includes INT scaling (0.25x total INT) plus FIRE_POWER synergy; fireball cooldown observes effective CDR via progression caps. New unit test `test_progression_phase3_6_effect_scaling` prints PH3_6_EFFECT_SCALING_OK.
  * 3.6.5 implemented: mastery bonus scalar integrated into active skill damage path (e.g., Fireball). Multiplicative bracketed scalar (1.01–1.20) composed after base+synergy+attributes with stable rounding. New unit test `test_progression_phase3_6_mastery_integration` prints PH3_6_MASTERY_INTEG_OK.
  * 3.6.6–3.6.7 implemented: specialization subsystem with two paths per skill — POWER (+10% damage) and CONTROL (−10% cooldown). Added APIs to choose/get/respec and scalar queries, integrated into `rogue_damage_fireball` and `rogue_cooldown_fireball_ms`. Respec consumes attribute respec tokens. New unit test `test_progression_phase3_6_specialization` validates damage/cooldown effects and respec flow (token PH3_6_SPECIALIZATION_OK).
...
* AI Phase 7 (Group Tactics & Coordination): Implemented squad metadata node, role assignment with weight fallback, surround slot assignment, retreat condition, and stagger-by-index decorator. Consolidated unit test `test_ai_phase7_group_tactics` passes (Debug, SDL2, parallel -j8).
...
Audio & VFX – Phase 1–2 minimal + 1.5 compaction (new)
- Added `src/audio_vfx/effects.[ch]` implementing a simple event bus (double-buffer) and deterministic dispatch ordering (priority,type,id,seq) with per-frame digest.
- Frame compaction merges identical events (type+priority+id) within a frame and exposes `repeats` for intensity-aware dispatch; digest includes repeat count.
- Minimal AudioRegistry with SDL_mixer-backed chunk loading; `LEVELUP` sound registered from `assets/sounds.cfg` and routed via the FX bus on level-up.
- `app_step` begins/ends FX frame and dispatches before present. Unit tests `test_audio_vfx_phase1_2` and `test_audio_vfx_phase1_5_compaction` validate ordering, digest determinism, and compaction semantics.
...
* Phase 3.11 Integration Bridge Validation (Complete): `test_phase3_11_integration_bridges` now exercises all six synthetic multi-system flows (enemy→AI→combat; equipment→stat→damage; victory→xp→skill→passive; loot→material→recipe unlock; worldgen→resource→gather→craft; multi-system save completion) plus an integration overhead performance benchmark (2400 events processed at ~93 µs/event vs <150 µs target). Roadmap items 3.11.1–3.11.7 marked Done. Assertions for loot/material/recipe & worldgen/resource/gather/craft chains use >=1 guards to remain robust under added benchmark event publications.
...
* Phase 4.4 Reference Counting & Lifecycle: Introduced core intrusive ref counting (`ref_count.c/h`) with strong/weak counters (stdatomic or Windows Interlocked), weak handles + upgrade, automatic destructor call on last strong, memory free when both counts hit zero, live list & leak stats/dump, pluggable edge enumerators enabling DOT graph export, textual snapshot (id type strong weak) for tooling/persistence scaffolding, validation helper, and unit test `test_ref_count` covering retain/release, weak upgrade success & failure, graph DOT & snapshot presence, leak dump smoke, and invariant validation.
