...
* Phase 9 Performance & Virtualization (Partial): Virtual list range & emission helpers, union dirty rectangle tracking, perf timing (frame/update/render) + budget API, headless render hook capturing timings. Further work pending: per-phase CPU split, glyph cache, inventory integration, regression baselines.
