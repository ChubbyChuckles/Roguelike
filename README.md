...
* Phase 6.1 / 6.3 (Initial Advanced Offense/Defense): Added charged attack subsystem (hold to build damage multiplier curve up to 2.5x over 800ms charge, with overcharge clamp) and dodge roll granting 400ms i-frames + stamina cost (18) + slight poise restore; single-use charge multiplier consumed on next strike. Covered by `test_combat_phase6_charge_and_dodge` validating baseline vs charged damage scaling, multiplier consumption, stamina deduction, i-frame grant, and low-stamina failure path. Future Phase 6 items (backstab, parry→riposte, guard break follow-up, projectile deflect) pending.
* Phase 6.4 / 6.5 (Positional & Reactive Windows): Implemented backstab detection (`rogue_combat_try_backstab`) using rear-cone dot check (<= -0.70) within 1.5 tiles & 650ms cooldown (damage multiplier application to integrate later). Added parry → riposte chain: `rogue_player_begin_parry` opens 160ms parry window; successful directional parry (lenient dot >=0.10) grants 350ms i-frames + 650ms riposte window (`rogue_player_try_riposte`). Unit test `test_combat_phase6_parry_backstab_riposte` covers backstab detection, parry success, and riposte consumption. Guard-break follow-up API scaffolded (pending trigger & crit wiring).
* Phase 6.6 / 6.7 (Guard Break & Projectile Deflect – Baseline): Added guard-break bonus scaffold (`rogue_player_set_guard_break`, `rogue_player_consume_guard_break_bonus`) and projectile deflection (`rogue_player_try_deflect_projectile`) active during parry/riposte windows, reflecting projectiles toward current facing. Aerial landing lag placeholder field integrated (landing lag extends recovery). Unit test `test_combat_phase6_aerial_deflect_guardbreak` validates guard-break consume & deflect direction. Further integration (actual crit modifier, projectile entity interaction, full airborne state machine) pending later phases.
