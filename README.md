...
* Multi-hit active windows (Phase 1A.2 partial): data-driven window array per attack with per-window cancel flag override, per-window damage_mult scalar, status buildup scaffold fields, and duplicate hit prevention mask; example: light_3 split into two sequential pulses (validated by `test_combat_multi_hit`).
