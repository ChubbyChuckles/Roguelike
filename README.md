...
* Phase 4.1 Inventory Grid (Minimal Slice): Added `rogue_ui_inventory_grid` helper producing a container panel plus one panel per visible slot (count overlay text for occupied slots). Deterministic serialization confirmed by new unit test `test_ui_phase4_inventory`. Advanced features (scroll virtualization, drag & drop, stack split modal) intentionally deferred to avoid partial dead code; they'll follow in 4.2–4.3.
