...
* JSON Migration Phase 2.3.4 (Biomes & Vegetation – partial): Introduced C-only JSON loaders/validators for biomes (`world_gen_biome_json.c/.h`) and vegetation (`vegetation_json.c/.h`). Biomes: normalized tile weights, built transition/compatibility matrix, and validated encounter tables and balance ranges. Vegetation: parsed plants/trees arrays and registered defs via new internal helper. Tests `test_worldgen_phase3_4_biome_json` and `test_vegetation_json` pass (Debug, SDL2, -j8). Next up: resource node JSON migration and asset dependency validation.
