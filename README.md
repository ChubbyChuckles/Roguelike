...
* NEW (Persistence Phase 3 early tooling): Lightweight JSON export (`rogue_save_export_json`) listing descriptor + section ids/sizes and targeted component reload API (`rogue_save_reload_component_from_slot`) for fast iteration while debugging a single subsystem.
