...
* Inventory System Phase 4 (in progress): Added query expression parser (rarity/qty/tag/equip_slot/affix_weight/quality/durability_pct/category) with AND/OR + comparison operators (now with basic diagnostics hook), trigram fuzzy search index with lazy build + incremental dirty-bit rebuild path, stable multi-key composite sort (rarity, qty, name, category with descending markers via `-key`), saved searches registry (persisted via save component id=12) plus quick-apply helper API, query result LRU cache (32 entries, invalidated on inventory or instance mutation) with hit/miss stats, and mutation hook wiring to quantity & instance changes. Tests: `test_inventory_phase4_query_sort`, `test_inventory_phase4_saved_searches`, `test_inventory_phase4_saved_searches_persist`, `test_inventory_phase4_query_cache`. Pending: UI quick action bar binding for saved searches, parser fuzz harness, cache hit ratio threshold test, leak/perf guards.
