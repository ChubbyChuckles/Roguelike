...
* Phase 7.1 / 7.3 / 7.4 / 7.5 / 7.6 (Weapons, Stances, Infusions, Familiarity, Durability): Weapon table (`weapons.c`) with base + STR/DEX/INT scaling, stamina & poise multipliers, durability max. Stance modifiers now also adjust frame data (agg -5% windup/-3% recovery, def +6% windup/+8% recovery) and stamina costs. Infusion system (`infusions.c`) adds Fire/Frost/Arcane/Bleed/Poison variants converting portions of base damage to elemental types and adding status buildup (bleed/poison) with scaling multipliers. Familiarity grants up to +10% damage (usage based). Durability now reduces weapon contribution below 50% linearly to 70% at 0 and affects poise damage. Tests: `test_combat_phase7_weapon_stance_familiarity` (stance diff, familiarity, durability tick) and `test_combat_phase7_infusions_durability` (infusion variance + low durability scalar).
