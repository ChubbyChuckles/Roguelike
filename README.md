...
* Multi-hit active windows (Phase 1A.2 partial): data-driven window array per attack with per-window cancel flag override, per-window damage_mult scalar, status buildup scaffold fields, duplicate hit prevention mask; example: light_3 split into two sequential pulses (`test_combat_multi_hit`). Per-window begin/end animation events emitted (`test_combat_events`). Added helper `rogue_attack_window_frame_span` to translate authored ms windows into animation frame indices for forthcoming external frame data tooling.
