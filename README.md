...
* NEW: Crafting & Gathering Phase 8 – UI / UX Layer slice: Added `crafting_ui.[ch]` providing recipe panel (search filter, missing-resource dim, upgrade tag heuristic), enhancement risk preview (expected fracture damage), material ledger panel (avg quality + bias), accessibility text-only fallback mode, and unit test `test_crafting_phase8_ui` validating search persistence, text-only mode, and risk function bounds.
