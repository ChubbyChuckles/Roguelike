...
* Phase 17 Performance & Memory (Initial): Added `loot_perf.c/h` module providing object pool for affix roll scratch buffers with metrics (acquires, releases, peak usage). Introduced SIMD (SSE2) accelerated weight summation path with scalar fallback; performance counters distinguish simd vs scalar sums. Metrics API (`rogue_loot_perf_get`) and test `test_loot_phase17_perf` validate pool lifecycle, counter increments, and accumulated total weights. Future work: integrate timing around real affix roll pipeline and cache-friendly item definition indexing.
