...
### World Generation Phase 14 (Optimization & Memory – Initial Slice)
Introduces performance scaffolding and memory pooling groundwork:
* 14.1 Transient Arena Allocator: Added linear bump arena (`RogueWorldGenArena`) with create/destroy/alloc/reset/usage APIs. Integrated into macro generation temporary buffers replacing four heap allocations (continent/elevation/temperature/moisture) when a global arena is provided (`rogue_worldgen_set_arena`). Fallback to malloc retained for standalone calls (tests unchanged).
* 14.2 SIMD Toggle & Batched Noise Path: Exposed optimization toggle (`rogue_worldgen_enable_optimizations(simd,parallel)`). Implemented SIMD stub path batching 4 samples at a time for value noise / fbm while still invoking scalar noise per lane to preserve deterministic output; future work will substitute true vectorized kernels + erosion SIMD.
* 14.3 Parallel Placeholder: Parallel flag plumbed (no threading yet). Planned use: continent mask + river carving segmentation with deterministic partition ordering.
* 14.4 Benchmark Harness: New API `rogue_worldgen_run_noise_benchmark(w,h,&bench)` returns scalar vs SIMD timings, sample count, and computed speedup (scalar_ms / simd_ms). Provides foundation for future regression guard (baseline file + tolerance) not yet persisted.
* 14.5 Unit Test: `test_worldgen_phase14_optimization` exercises toggling SIMD, runs benchmark twice (scalar then SIMD-enabled), asserts sample counts, scalar timing >0, and minimum speedup bound when SIMD timing captured. Determinism maintained since SIMD path currently delegates to scalar noise internally.
Roadmap updated: 14.1 and 14.5 marked Done (initial); 14.2 & 14.4 Partial; 14.3 pending. Next steps: adopt arena across caves/dungeon/temp queues, implement real SSE2/AVX2 kernels (guarded by feature detection macros), add erosion SIMD, introduce thread pool for macro passes, and persist benchmark baselines in CI for variance alerting.
