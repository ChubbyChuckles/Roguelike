...
* Inventory System Phase 4 (in progress): Added query expression parser (rarity/qty/tag/equip_slot/affix_weight/quality/durability_pct/category) with AND/OR + comparison operators, trigram fuzzy search index with lazy build + rebuild API, multi-key composite sort (rarity, qty, name, category with descending markers via `-key`) using reverse-pass qsort chaining, and saved searches registry (persisted via new save component id=12) with tests (`test_inventory_phase4_query_sort`, `test_inventory_phase4_saved_searches`, `test_inventory_phase4_saved_searches_persist`). Pending: stable secondary ordering guarantees, quick action bar UI binding for saved searches, wiring mutation hook for incremental fuzzy re-index, result cache + LRU, parser fuzz harness, performance & memory leak guards.
