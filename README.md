...
* Phase 22.1–22.3 Security: Added `loot_security.c/h` providing `rogue_loot_roll_hash` (FNV1a-32 over table/seed/results) for verification logs, optional seed obfuscation helper (`rogue_loot_obfuscate_seed` with enable flag) and lightweight config file hash snapshot + verify APIs (`rogue_loot_security_snapshot_files`, `rogue_loot_security_verify_files`). Unit test `test_loot_phase22_security` validates deterministic hashing, obfuscation toggle, and snapshot/verify path.
