...
* Phase 7.1 / 7.3 / 7.5 / 7.6 (Initial Weapons & Stances): Added in-memory weapon table (`weapons.c`) with base damage + per-stat scaling + stamina/poise multipliers + durability max. Integrated stance modifiers (aggressive/defensive/balanced) affecting damage, stamina, poise damage scalars; familiarity system accumulates usage-based bonus up to +10%; durability decrements per hit (effect on damage pending). Test `test_combat_phase7_weapon_stance_familiarity` validates stance differential, familiarity growth, durability decrease.
