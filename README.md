...
* Phase 16 Multiplayer / Personal Loot (Initial Slice): Added per-item ownership field `owner_player_id` for instanced drops and a loot mode enum (shared vs personal) with APIs `rogue_loot_set_mode` / `rogue_loot_get_mode`. Pickup loop enforces personal ownership gating (non-owner items ignored). Introduced assignment API `rogue_loot_assign_owner`, basic trade request `rogue_loot_trade_request` (ownership transfer validation stub), and placeholder need/greed roll function `rogue_loot_need_greed_roll`. Unit test `test_loot_phase16_personal_mode` verifies shared pickup vs protected personal loot and trade transfer enabling pickup. Future work: full need/greed multi-player aggregation, anti-duplication safeguards, richer trade validation (distance, cooldown, logging), and network authority integration.
