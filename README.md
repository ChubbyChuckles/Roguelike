...
* Vendor System Phase 2 (partial – 2.1 & 2.2 complete): Added inventory template registry (`inventory_templates.json`) mapping vendor archetype → category & rarity weight arrays with loader module `vendor_inventory_templates.[ch]`. Implemented deterministic inventory seed composition `rogue_vendor_inventory_seed(world_seed,vendor_id,day_cycle)` (xor of world_seed with folded FNV hash of vendor id & day cycle) for reproducible future inventory roll algorithm. Test `test_vendor_phase2_inventory` validates template load, weight invariants, and seed determinism. Roadmap marks 2.1 & 2.2 Done; remaining 2.3–2.5 pending (roll constraints, material/recipe injection, distribution/uniqueness tests).
