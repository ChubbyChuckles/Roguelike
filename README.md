...
Foundation implemented and expanded: snapshot restoration hook (`restore` callback + `rogue_snapshot_restore`) plus `rollback_manager.c/.h` offering per‑system configurable ring of checkpoints storing either FULL copies or DELTA entries (range diff vs previous FULL) to reduce memory (bytes_saved_via_delta tracked). Public APIs: configure, capture (single/multi), step_back(N), latest, purge, partial multi‑system rollback (`rogue_rollback_partial`), auto rollback for participant (`rogue_rollback_auto_for_participant`), stats snapshot, and dump (version history). Delta path selects smaller of full or encoded diff. Stats now include systems/bytes rewound. Extended unit test `test_rollback_manager` covers: multi‑capture progression, direct rollback to latest & earlier versions, negative bounds, second system small-step deltas, and partial multi‑system rollback with independent step counts. Remaining: richer per-operation impact reporting & wiring automatic invocation from transaction abort logic.
