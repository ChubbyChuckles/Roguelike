...
Foundation implemented: snapshot restoration hook (`restore` callback + `rogue_snapshot_restore` wrapper) and new `rollback_manager.c/.h` providing per‑system configurable ring buffer of checkpointed snapshot versions with APIs: configure, capture, step_back(N), latest (alias N=0), purge, dump. Unit test `test_rollback_manager` exercises multi‑capture history, rollback to latest, multi‑step rewind, purge behavior, and out‑of‑range error path. Upcoming planned work: integrate automatic rollback on transaction prepare/commit failure, coordinated multi‑system partial rollback honoring dependency order, hash integrity validation + mismatch counters, delta‑based rewind optimization (apply stored deltas vs full snapshot), impact reporting (versions & bytes rewound), and stress/perf scenarios (rapid capture/rollback wrap cycles).
