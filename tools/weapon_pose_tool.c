...
/* Reworked tool (Phase 5A update):
    - Single weapon image (BMP) overlaid on 8-frame player slash animation frames extracted from a
   player sheet (PNG or BMP).
    - Player sheet contains 8 frames laid out horizontally for one direction (down/up/side). Left
   direction is derived by mirroring side-right in game.
    - This tool lets you tune per-frame dx, dy, angle, scale, pivot for that direction.
*/

#define FRAME_COUNT 8

typedef struct FramePose
{
    float dx, dy, angle, scale, pivot_x, pivot_y;
} FramePose;
...
{
    printf("weapon_pose_tool --weapon <id> --player-sheet <path_to_sheet_png> [--weapon-image "
           "<bmp>] [--frame-size N] [--direction <down|up|side>] [--out path]\n");
}
...
static SDL_Texture* load_png_wic(SDL_Renderer* r, const char* path, int* w, int* h)
{
    ...
    UINT pw = 0, ph = 0;
    conv->lpVtbl->GetSize(conv, &pw, &ph);
    if (pw == 0 || ph == 0)
    {
        conv->lpVtbl->Release(conv);
        frame->lpVtbl->Release(frame);
    ...
    size_t stride = pw * 4ULL;
    size_t total = stride * ph;
    ...
    hr = conv->lpVtbl->CopyPixels(conv, NULL, (UINT) stride, (UINT) total, pixels);
    if (FAILED(hr))
    {
        free(pixels);
        conv->lpVtbl->Release(conv);
    ...
    SDL_Surface* surf = SDL_CreateRGBSurfaceFrom(pixels, (int) pw, (int) ph, 32, (int) stride,
...
static int load_texture_auto(SDL_Renderer* r, const char* path, SDL_Texture** out_tex, int* w,
                             int* h)
{
    ...
    {
#ifdef _WIN32
        SDL_Texture* t = load_png_wic(r, path, w, h);
        if (!t)
        {
            fprintf(stderr, "WARN: png load failed %s\n", path);
            return 0;
        }
        *out_tex = t;
        return 1;
    ...
    else
    {
        ...
        if (!s)
        {
            fprintf(stderr, "WARN: bmp load fail %s (%s)\n", path, SDL_GetError());
            return 0;
        }
        SDL_Texture* t = SDL_CreateTextureFromSurface(r, s);
        if (!t)
        {
            fprintf(stderr, "WARN: texture create fail %s (%s)\n", path, SDL_GetError());
            SDL_FreeSurface(s);
            return 0;
        }
        *w = s->w;
...
static void init_default_poses(void)
{
    for (int i = 0; i < FRAME_COUNT; i++)
    {
        if (poses[i].scale == 0)
            poses[i].scale = 1.0f;
        if (poses[i].pivot_x == 0 && poses[i].pivot_y == 0)
        {
            poses[i].pivot_x = 0.5f;
            poses[i].pivot_y = 0.5f;
        }
    }
}
...
static void save_json(const char* path)
{
    ...
    if (!f)
    {
        fprintf(stderr, "ERR: save fail %s\n", path);
        return;
    }
    fprintf(f,
            "{\n  \"weapon_id\": \"%s\",\n  \"direction\": \"%s\",\n  \"frame_size\": %d,\n  "
    ...
    for (int i = 0; i < FRAME_COUNT; i++)
    {
        FramePose* p = &poses[i];
        fprintf(f,
                "    { \"dx\": %.4f, \"dy\": %.4f, \"angle\": %.4f, \"scale\": %.4f, \"pivot_x\": "
                "%.4f, \"pivot_y\": %.4f }%s\n",
                p->dx, p->dy, p->angle, p->scale, p->pivot_x, p->pivot_y,
                (i == FRAME_COUNT - 1) ? "" : ",");
    }
    fprintf(f, "  ]\n}\n");
    fclose(f);
...
static void handle_key(const SDL_Event* e)
{
    ...
    float move = shift ? 5.0f : 1.0f;
    float rot = shift ? 10.0f : 2.0f;
    float scl = 0.05f;
    ...
    case SDLK_7:
    case SDLK_8:
        frame_index = e->key.keysym.sym - SDLK_1;
        break;
    case SDLK_LEFT:
        if (ctrl)
        {
    ...
    case SDLK_x:
        ...
        break;
    case SDLK_s:
        save_json(out_path ? out_path : "pose_out.json");
        break;
    default:
        break;
    }
...
{
    SDL_SetMainReady();
    const char* player_sheet_path = NULL;
    out_path = NULL;
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--player-sheet") == 0 && i + 1 < argc)
        {
            player_sheet_path = argv[++i];
        }
        else if (strcmp(argv[i], "--out") == 0 && i + 1 < argc)
        {
            out_path = argv[++i];
        }
        else if (strcmp(argv[i], "--weapon-image") == 0 && i + 1 < argc)
        {
            weapon_image_path = argv[++i];
        }
    ...
    if (!player_sheet_path)
    {
        fprintf(stderr, "ERR: --player-sheet required.\n");
        usage();
        return 1;
    }
    ...
    SDL_Window* win = SDL_CreateWindow("Weapon Pose Tool", SDL_WINDOWPOS_CENTERED,
    ...
    if (!load_texture_auto(ren, player_sheet_path, &player_sheet_tex, &player_sheet_w,
                           &player_sheet_h))
    {
        fprintf(stderr, "ERR: failed to load player sheet %s\n", player_sheet_path);
        return 1;
    }
    if (player_sheet_w < frame_size * FRAME_COUNT)
    {
        fprintf(stderr, "WARN: sheet width (%d) < expected %d; frames may crop.\n", player_sheet_w,
                frame_size * FRAME_COUNT);
    }
    /* Load single weapon image */
    char inferred_path[256];
    if (!weapon_image_path)
    {
        snprintf(inferred_path, sizeof inferred_path, "../assets/weapons/weapon_%s.bmp", weapon_id);
        weapon_image_path = inferred_path;
    }
    ...
    if (!out_path)
    {
        ...
        snprintf(auto_out, sizeof auto_out, "../assets/weapons/weapon_%s_%s_pose.json", weapon_id,
                 direction_label);
        out_path = auto_out;
    }
    int running = 1;
    while (running)
    {
        ...
        SDL_Rect pdst = {origin_x - frame_size / 2, origin_y - frame_size / 2, frame_size,
                         frame_size};
        SDL_RenderCopy(ren, player_sheet_tex, &src, &pdst);
        /* Draw weapon overlay */
        FramePose* p = &poses[frame_index];
        SDL_FRect dst;
        dst.w = weapon_w * p->scale;
        dst.h = weapon_h * p->scale;
        dst.x = origin_x + p->dx - dst.w * p->pivot_x;
        dst.y = origin_y + p->dy - dst.h * p->pivot_y;
        SDL_FPoint center = {dst.w * p->pivot_x, dst.h * p->pivot_y};
        if (weapon_tex_single)
        {
            SDL_RenderCopyExF(ren, weapon_tex_single, NULL, &dst, p->angle, &center, SDL_FLIP_NONE);
        }
        /* Crosshair */
        SDL_SetRenderDrawColor(ren, 255, 255, 0, 255);
        SDL_RenderDrawLine(ren, origin_x - 10, origin_y, origin_x + 10, origin_y);
        SDL_RenderDrawLine(ren, origin_x, origin_y - 10, origin_y, origin_y + 10);
        /* HUD text minimal */
        char info[256];
        snprintf(info, sizeof info,
                 "%s dir=%s frame=%d dx=%.1f dy=%.1f ang=%.1f scale=%.2f piv=(%.2f,%.2f)%s",
                 weapon_id, direction_label, frame_index, p->dx, p->dy, p->angle, p->scale,
                 p->pivot_x, p->pivot_y, dirty_flag ? " *" : "");
