...
/* Combat Module Overview
   Files:
     ...
     combat_player.c      - Player combat state machine, guard/poise, reactions,
   parry/backstab/charge, obstruction hook combat_strike.c      - Strike evaluation, hit windows,
   damage composition, event emission Internal header: combat_internal.h (private helpers like
...
#include "features.h" /* capability macros */
...
typedef struct RoguePlayerCombat
{
    ...
    float pending_charge_damage_mult;
    /* Phase 6.5 Parry / Riposte state */
    int parry_active;
    float parry_window_ms;
    float parry_timer_ms;
    int riposte_ready;
    float riposte_window_ms;
...
/* Consumer APIs (Phase 2.7): snapshot recent events in chronological order (oldest->newest).
...
/* Crit layering mode (Phase 2.4): 0=crit multiplier applied pre-mitigation (default),
 * 1=post-mitigation */
extern int g_crit_layering_mode;
/* Deterministic crit testing: when >=0, overrides RNG-based crits. 0=force non-crit, 1=force crit.
 * -1=normal RNG. */
extern int g_force_crit_mode;
/* Execution trigger thresholds (tunable). If enemy health prior to hit <= EXEC_HEALTH_PCT of max
 * AND killing blow, or overkill exceeds OVERKILL_PCT of max health => execution. */
...
#define ROGUE_DEF_SOFTCAP_REDUCTION_THRESHOLD                                                      \
    0.65f /* fraction of combined physical reduction above which soft cap applies */
...
#define ROGUE_DEF_SOFTCAP_MIN_RAW                                                                  \
    100 /* skip soft cap for trivial hits (raw<100) to preserve low-hit behavior */

/* Apply mitigation (Phase 2 pipeline). Returns final damage >=1 unless health already <=0. */
int rogue_apply_mitigation_enemy(struct RogueEnemy* e, int raw, unsigned char dmg_type,
                                 int* out_overkill);
...
/* Begin guarding (directional block). Passing guard_dir (0..3 same as facing) caches facing;
 * returns 1 if guard started. */
int rogue_player_begin_guard(RoguePlayer* p, int guard_dir);
/* Update guard state each frame (dt_ms). Should be called from main loop. Returns chip damage
 * blocked this frame (for tests). */
int rogue_player_update_guard(RoguePlayer* p, float dt_ms);
/* Attempt to apply incoming enemy melee damage with guard / perfect guard logic. Returns final
 * applied damage to health. */
/* Apply incoming melee with optional poise damage component. poise_damage applies if not blocked
 * (or reduced if normal block). */
...
/* Apply a reaction given cumulative poise damage or direct thresholds. This sets reaction_type &
 * timers. */
...
#define ROGUE_GUARD_BLOCK_POISE_SCALE                                                              \
    0.40f /* percent of incoming poise damage applied on normal block */
...
/* Queue a branch (different archetype) to be consumed by the next started attack (distinct from
 * simple buffered next). */
...
/* Phase 5.5 test hook: allow unit tests to supply a direct line obstruction predicate.
    Function should return 1 if line is obstructed, 0 if clear, or any other value to defer to tile
   DDA. */
...
int rogue_player_register_incoming_attack_parry(struct RoguePlayer* p, struct RoguePlayerCombat* pc,
                                                float attack_dir_x, float attack_dir_y);
int rogue_player_try_riposte(struct RoguePlayer* p, struct RoguePlayerCombat* pc,
                             struct RogueEnemy* e);
...
int rogue_player_try_deflect_projectile(struct RoguePlayer* p, struct RoguePlayerCombat* pc,
                                        float proj_dir_x, float proj_dir_y,
                                        float* out_reflect_dir_x, float* out_reflect_dir_y);
...
int rogue_combat_add_damage_observer(RogueDamageObserverFn fn,
                                     void* user); /* returns observer id >=0 or -1 */
