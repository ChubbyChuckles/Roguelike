...
#include "../core/persistence/save_manager.h"
...
static int g_typewriter_enabled = 0;
static float g_chars_per_ms = 0.05f;
/* Phase 7 analytics */
...
{
    if (!mood || !*mood)
        return 0xFFFFFFFFu; /* neutral default */
    if (rd_strcasecmp(mood, "neutral") == 0)
        return 0xFFFFFFFFu; /* no tint */
    if (rd_strcasecmp(mood, "angry") == 0)
        return 0xFFFF6040u; /* fiery red/orange */
    if (rd_strcasecmp(mood, "excited") == 0)
        return 0xFFFFD040u; /* bright gold */
    if (rd_strcasecmp(mood, "happy") == 0)
        return 0xFF80E070u; /* soft green */
...
static int rd_debug_enabled(void)
{
    ...
    char* val = NULL;
    size_t len = 0;
    if (_dupenv_s(&val, &len, "ROGUE_DIALOGUE_DEBUG") != 0 || !val)
        return 0;
    int enabled = (*val == '1' || *val == 't' || *val == 'T' || *val == 'y' || *val == 'Y');
    free(val);
    return enabled;
#else
    const char* env = getenv("ROGUE_DIALOGUE_DEBUG");
    return (env && (*env == '1' || *env == 't' || *env == 'T' || *env == 'y' || *env == 'Y')) ? 1
                                                                                              : 0;
#endif
}
static void rd_validate_mood(char* mood)
{
    if (!mood || !*mood)
        return;
    const char* allowed[] = {"neutral", "angry", "excited", "happy"};
    for (size_t i = 0; i < sizeof(allowed) / sizeof(allowed[0]); ++i)
    {
        if (rd_strcasecmp(mood, allowed[i]) == 0)
            return;
    }
    if (rd_debug_enabled())
        ROGUE_LOG_WARN("Dialogue: mood '%s' not allowed (forcing neutral)", mood); /* safe copy */
#if defined(_MSC_VER)
    strncpy_s(mood, 64, "neutral", _TRUNCATE);
#else
    strncpy(mood, "neutral", 63);
    mood[63] = '\0';
#endif
}
int rogue_dialogue_load_script_from_json_file(const char* path)
{
    ...
    if (rd_debug_enabled())
    {
        ROGUE_LOG_INFO("Dialogue JSON bytes=%d path=%s", len, path);
    }
    int registered = 0;
    int lines_total = 0;
    int lines_kept = 0;
    ...
    { /* multi-script */
                ...
                if (lines_sec)
                {
                    ...
                    if (larr)
                    {
                        ...
                        if (larr_end)
                        {
                            ...
                            {
                                char* lobj = strchr(lc, '{');
                                if (!lobj || lobj >= larr_end)
                                    break;
                                char* lobj_end = NULL;
                                int ldepth = 0;
                                for (char* q = lobj; q < larr_end; ++q)
                                {
                                    if (*q == '{')
                                    {
                                        ldepth++;
                                ...
                                if (!lobj_end)
                                    break;
                                char lcopy[1024];
                                size_t llen = (size_t) (lobj_end - lobj + 1);
                                if (llen > sizeof lcopy - 1)
                                    llen = sizeof lcopy - 1;
                                memcpy(lcopy, lobj, llen);
                                lcopy[llen] = '\0';
                                ...
                                lines_total++;
                                if (!(speaker[0] && textv[0]))
                                {
                                    lines_skipped++;
                                    if (rd_debug_enabled())
                                        ROGUE_LOG_WARN("Dialogue script %d skip line %d (missing "
                                                       "speaker/text)",
                                                       sid, line_idx);
                                    lc = lobj_end + 1;
                                    line_idx++;
                                ...
                                {
                                    /* Path scheme: assets/avatar_icons/<race>/<name>_<mood>.png
                                     * (replaces older nested mood folder). */
                                    snprintf(avatar_path, sizeof avatar_path,
                                             "../assets/avatar_icons/%s/%s_%s.png", race, name,
                                             mood);
                                }
                                int sflag = (strcmp(side, "right") == 0);
                                int hflag =
                                    (mirror[0] == 'v' ||
                                     mirror[0] ==
                                         'V'); /* legacy 'v' value now treated as horizontal */
                                unsigned int tint = rd_mood_tint(mood);
                                char meta[320];
                                if (avatar_path[0])
                                    snprintf(meta, sizeof meta, "%s;S=%d;V=%d;TR=%u;TG=%u;TB=%u",
                                             avatar_path, sflag, hflag, (tint >> 16) & 255,
                                             (tint >> 8) & 255, tint & 255);
                                else
                                    meta[0] = '\0';
                                int n =
                                    avatar_path[0]
                                        ? snprintf(temp + out, sizeof(temp) - out, "%s%s@%s|%s\n",
                                                   out ? "" : "", speaker, meta, textv)
                                        : snprintf(temp + out, sizeof(temp) - out, "%s%s|%s\n",
                                                   out ? "" : "", speaker, textv);
                                if (n > 0)
                                {
                                    out += (size_t) n;
                                    lines_kept++;
                                    if (rd_debug_enabled())
                                        ROGUE_LOG_INFO("Dialogue script %d keep line %d "
                                                       "speaker='%s' text_len=%zu",
                                                       sid, line_idx, speaker, strlen(textv));
                                }
                                else
                                {
                                    lines_skipped++;
                            ...
                            if (out > 0 &&
                                rogue_dialogue_register_from_buffer(sid, temp, (int) out) == 0)
                            {
                                registered++;
                                if (rd_debug_enabled())
                                    ROGUE_LOG_INFO("Dialogue script %d registered lines=%d", sid,
                                                   lines_kept);
                            }
                            else if (rd_debug_enabled())
                            {
                                ROGUE_LOG_WARN("Dialogue script %d registration failed (out=%zu)",
                                               sid, out);
                            }
                        }
                    }
        ...
        if (rd_debug_enabled())
            ROGUE_LOG_INFO("Dialogue multi summary scripts=%d lines_total=%d kept=%d skipped=%d",
                           registered, lines_total, lines_kept, lines_skipped);
        free(buf);
        return (registered > 0) ? 0 : -5;
    }
    /* single */
    single = 1;
    int script_id = -1;
    ...
    {
        if (rd_debug_enabled())
            ROGUE_LOG_WARN("Dialogue single missing id");
        free(buf);
        return -6;
    }
    char* lines_sec = strstr(buf, "\"lines\"");
    if (!lines_sec)
    {
        free(buf);
        return -7;
    }
    char* arr = strchr(lines_sec, '[');
    if (!arr)
    {
        free(buf);
        return -8;
    }
    char* arr_end = strchr(arr, ']');
    if (!arr_end)
    {
        free(buf);
        return -9;
    ...
    while (cursor < arr_end)
    {
        ...
        if (!obj_end || obj_end > arr_end)
            break;
        char oc[1024];
        size_t olen = (size_t) (obj_end - obj + 1);
        if (olen > sizeof oc - 1)
            olen = sizeof oc - 1;
        memcpy(oc, obj, olen);
        oc[olen] = '\0';
        ...
        if (!(speaker[0] && textv[0]))
        {
            lines_skipped++;
            if (rd_debug_enabled())
                ROGUE_LOG_WARN("Dialogue single skip line %d (missing speaker/text)", line_idx);
            cursor = obj_end + 1;
            line_idx++;
            continue;
        }
        ...
        if (race[0] && name[0] && mood[0])
        {
            snprintf(avatar_path, sizeof avatar_path, "../assets/avatar_icons/%s/%s_%s.png", race,
                     name, mood);
        }
        int sflag = (strcmp(side, "right") == 0);
        int hflag = (mirror[0] == 'v' || mirror[0] == 'V');
        unsigned int tint = rd_mood_tint(mood);
        char meta[320];
        if (avatar_path[0])
            snprintf(meta, sizeof meta, "%s;S=%d;V=%d;TR=%u;TG=%u;TB=%u", avatar_path, sflag, hflag,
        ...
        if (n > 0)
        {
            ...
            lines_kept++;
            if (rd_debug_enabled())
                ROGUE_LOG_INFO("Dialogue single keep line %d speaker='%s' text_len=%zu", line_idx,
                               speaker, strlen(textv));
        }
        else
        {
            lines_skipped++;
    ...
    int r = (out > 0) ? rogue_dialogue_register_from_buffer(script_id, temp, (int) out) : -10;
    if (rd_debug_enabled())
        ROGUE_LOG_INFO("Dialogue single summary id=%d status=%d lines_total=%d kept=%d skipped=%d",
                       script_id, r, lines_total, lines_kept, lines_skipped);
    free(buf);
    return r;
}
...
void rogue_dialogue_set_player_name(const char* name)
{
    ...
    if (n > sizeof(g_player_name) - 1)
        n = sizeof(g_player_name) - 1;
    memcpy(g_player_name, name, n);
    g_player_name[n] = '\0';
}
        ...
        if (p[0] == '$' && p[1] == '{')
        {
            ...
            if (close)
            {
                ...
                if (strcmp(key, "player_name") == 0)
                    rep = g_player_name;
                else if (strcmp(key, "run_seed") == 0)
                {
                    snprintf(seed_buf, sizeof seed_buf, "%u", g_run_seed);
                    rep = seed_buf;
                }
                if (rep)
                {
...
int rogue_dialogue_current_text(char* buffer, size_t cap)
{
    ...
    if (!pb)
        return -2;
    const RogueDialogueScript* sc = rogue_dialogue_get(pb->script_id);
    if (!sc)
        return -3;
    if (pb->line_index < 0 || pb->line_index >= sc->line_count)
        return -4;
    ...
    if (ln->token_flags & ROGUE_DIALOGUE_LINE_HAS_TOKENS)
    {
        return (int) expand_tokens(text, buffer, cap);
    }
...
static void trim(char* s)
{
    ...
    int a = 0;
    while (a < len && (s[a] == ' ' || s[a] == '\t' || s[a] == '\r'))
        a++;
    int b = len - 1;
    while (b >= a && (s[b] == ' ' || s[b] == '\t' || s[b] == '\r'))
        b--;
    if (a > 0 || b < len - 1)
    {
        int n = b - a + 1;
...
static int parse_and_register(int id, const char* buffer, int length)
{
    if (find_script(id))
        return -2;
    if (g_script_count >= ROGUE_DIALOGUE_MAX_SCRIPTS)
        return -3;
    int line_count = 0;
    const char* p = buffer;
    ...
    char* str_cursor = blob + lines_bytes;
    size_t str_remaining = blob_size - lines_bytes;
    p = buffer;
    int idx = 0;
    while (p < end && idx < line_count)
    {
        ...
        int l = (int) (nl ? (nl - p) : (end - p));
        char temp[1024];
        if (l > (int) sizeof(temp) - 1)
            l = (int) sizeof(temp) - 1;
        memcpy(temp, p, (size_t) l);
        temp[l] = '\0';
        p = nl ? nl + 1 : end;
        int allspace = 1;
        for (int i = 0; i < l; i++)
        ...
        *bar = '\0';
        ...
        char* at = strchr(speaker, '@');
        if (at && at[1])
        {
            *at = '\0';
            char* avatar_path = at + 1;
            trim(speaker);
            trim(avatar_path);
            if (*avatar_path)
            {
                ...
                int v_flag = 0;
                int tr = -1, tg = -1, tb = -1;
                char* meta = strchr(avatar_path, ';');
                while (meta)
                {
                    if (strncmp(meta, ";S=", 3) == 0 && isdigit((unsigned char) meta[3]))
                        side_flag = (meta[3] == '1');
                    else if (strncmp(meta, ";V=", 3) == 0 && isdigit((unsigned char) meta[3]))
                        v_flag = (meta[3] == '1');
                    else if (strncmp(meta, ";TR=", 4) == 0)
                        tr = atoi(meta + 4);
                    else if (strncmp(meta, ";TG=", 4) == 0)
                        tg = atoi(meta + 4);
                    else if (strncmp(meta, ";TB=", 4) == 0)
                        tb = atoi(meta + 4);
                    char* next_meta = strchr(meta + 1, ';');
                    if (!next_meta)
                        break;
                    meta = next_meta;
                }
                char* semi = strchr(avatar_path, ';');
                if (semi)
                    *semi = '\0';
                if (*avatar_path)
                {
                    rogue_dialogue_avatar_register(speaker, avatar_path);
                }
                lines[idx]._reserved[0] = (unsigned char) (side_flag ? 1 : 0);
                lines[idx]._reserved[1] = (unsigned char) (v_flag ? 1 : 0);
                if (tr >= 0 && tr <= 255 && tg >= 0 && tg <= 255 && tb >= 0 && tb <= 255)
                {
                    lines[idx]._reserved[2] = (unsigned char) tr;
                    lines[idx]._reserved[3] = (unsigned char) tg;
                    lines[idx]._reserved[4] = (unsigned char) tb;
                    lines[idx]._reserved[5] = 255;
                }
            }
        }
        RogueDialogueEffect* eff_list = NULL;
        unsigned char eff_count =
            0; /* effect sections may be chained with additional '|' delimiters */
        char* next_section = strchr(text, '|');
        while (next_section)
        {
            *next_section = '\0';
            char* eff_section = next_section + 1; /* parse this section (comma separated effects) */
            char* cursor = eff_section;
            while (*cursor)
            {
                while (*cursor == ' ' || *cursor == ',')
                    cursor++;
                if (!*cursor)
                    break;
                if (strncmp(cursor, "SET_FLAG(", 9) == 0)
                {
                    cursor += 9;
                    char name[24];
                    int n = 0;
                    while (*cursor && *cursor != ')' && n < (int) sizeof(name) - 1)
                    {
                        name[n++] = *cursor++;
                    }
                    name[n] = '\0';
                    ...
                    if (eff_count < 4)
                    {
                        if (!eff_list)
                            eff_list =
                                (RogueDialogueEffect*) calloc(4, sizeof(RogueDialogueEffect));
                        eff_list[eff_count].kind = ROGUE_DIALOGUE_EFFECT_SET_FLAG;
                        for (int ci = 0; ci < (int) sizeof(eff_list[eff_count].name); ++ci)
                            eff_list[eff_count].name[ci] = 0;
                        for (int ci = 0;
                             name[ci] && ci < (int) sizeof(eff_list[eff_count].name) - 1; ++ci)
                            eff_list[eff_count].name[ci] = name[ci];
                        eff_count++;
                    }
                }
                else if (strncmp(cursor, "GIVE_ITEM(", 10) == 0)
                {
                    cursor += 10;
                    int idv = 0, qtyv = 0;
                    ...
                    if (eff_count < 4)
                    {
                        if (!eff_list)
                            eff_list =
                                (RogueDialogueEffect*) calloc(4, sizeof(RogueDialogueEffect));
                        eff_list[eff_count].kind = ROGUE_DIALOGUE_EFFECT_GIVE_ITEM;
                        eff_list[eff_count].a = (unsigned short) idv;
                        eff_list[eff_count].b = (unsigned short) qtyv;
                        eff_count++;
                    }
                }
                while (*cursor && *cursor != ',')
        ...
        int is_key = 0;
        const char* key_part = NULL;
        const char* fallback_part = NULL;
        if (text[0] == '[')
        {
            char* close_br = strchr(text, ']');
            if (close_br && close_br > text + 1)
            {
                *close_br = '\0';
                key_part = text + 1;
                fallback_part = close_br + 1;
                while (*fallback_part == ' ' || *fallback_part == '\t')
                    fallback_part++;
                is_key = 1;
            }
        }
        ...
        if (!is_key)
        {
            ...
            if (speaker_len + text_len + 2 > str_remaining)
            {
                if (eff_list)
                    free(eff_list);
                free(blob);
            ...
            char* speaker_copy = str_cursor;
            memcpy(speaker_copy, speaker, speaker_len + 1);
            str_cursor += speaker_len + 1;
            str_remaining -= speaker_len + 1;
            char* text_copy = str_cursor;
            memcpy(text_copy, text, text_len + 1);
            str_cursor += text_len + 1;
            str_remaining -= text_len + 1;
            unsigned int token_flags = 0u;
            if (strstr(text_copy, "${"))
                token_flags |= ROGUE_DIALOGUE_LINE_HAS_TOKENS;
            lines[idx].speaker_id = speaker_copy;
            lines[idx].text = text_copy;
            lines[idx].token_flags = token_flags;
            lines[idx].effects = eff_list;
            lines[idx].effect_count = eff_count;
            idx++;
        }
        else
        {
            ...
            if (!fallback_part || !*fallback_part)
                fallback_part = "";
            size_t key_len = strlen(key_part);
            size_t fb_len = strlen(fallback_part);
            if (speaker_len + key_len + fb_len + 3 > str_remaining)
            {
                if (eff_list)
                    free(eff_list);
                free(blob);
            ...
            str_cursor += speaker_len + 1;
            str_remaining -= speaker_len + 1;
            char* key_copy = str_cursor;
            memcpy(key_copy, key_part, key_len + 1);
            str_cursor += key_len + 1;
            str_remaining -= key_len + 1;
            char* fb_copy = str_cursor;
            memcpy(fb_copy, fallback_part, fb_len + 1);
            str_cursor += fb_len + 1;
            str_remaining -= fb_len + 1;
            unsigned int token_flags = ROGUE_DIALOGUE_LINE_IS_KEY;
            if (strstr(fb_copy, "${"))
                token_flags |= ROGUE_DIALOGUE_LINE_HAS_TOKENS;
            lines[idx].speaker_id = speaker_copy;
            lines[idx].text = key_copy;
            lines[idx].token_flags = token_flags;
            lines[idx].effects = eff_list;
            lines[idx].effect_count = eff_count;
            idx++;
    ...
    *dst = (RogueDialogueScript){id, line_count, lines, blob, (int) blob_size, 0ull};
    return 0;
}
...
int rogue_dialogue_register_from_buffer(int id, const char* buffer, int length)
{
    if (!buffer || length <= 0)
        return -1;
    return parse_and_register(id, buffer, length);
}
...
void rogue_dialogue_reset(void)
{
    for (int i = 0; i < g_script_count; i++)
    {
        if (g_scripts[i].lines)
        {
            for (int l = 0; l < g_scripts[i].line_count; l++)
            {
                if (g_scripts[i].lines[l].effects)
                    free(g_scripts[i].lines[l].effects);
            }
        }
        free(g_scripts[i]._blob);
        g_scripts[i]._blob = NULL;
        g_scripts[i].lines = NULL;
    }
    g_script_count = 0;
...
int rogue_dialogue_capture(RogueDialoguePersistState* out)
{
    ...
    if (!g_playback.active)
        return 0;
    out->active = 1;
    out->script_id = g_playback.script_id;
    out->line_index = g_playback.line_index;
    out->reveal_ms = g_playback.reveal_ms;
    return 1;
}
int rogue_dialogue_restore(const RogueDialoguePersistState* st)
{
    ...
    const RogueDialogueScript* sc = rogue_dialogue_get(st->script_id);
    if (!sc)
        return -2;
    if (st->line_index < 0 || st->line_index >= sc->line_count)
        return -3;
    g_playback = (RogueDialoguePlayback){1, st->script_id, st->line_index, st->reveal_ms, 1};
    return 0;
}

/* Save component (choose id 9 unused in save manager enum range) */
#define ROGUE_SAVE_COMP_DIALOGUE 9
static int dialogue_write_fn(FILE* f)
{
    RogueDialoguePersistState st;
    int cap = rogue_dialogue_capture(&st);
    (void) cap;
    return fwrite(&st, sizeof st, 1, f) == 1 ? 0 : -1;
}
static int dialogue_read_fn(FILE* f, size_t size)
{
    if (size != sizeof(RogueDialoguePersistState))
        return -1;
    RogueDialoguePersistState st;
    if (fread(&st, sizeof st, 1, f) != 1)
        return -2;
    return rogue_dialogue_restore(&st);
}
void rogue_dialogue_register_save_component(void)
{
    static RogueSaveComponent comp = {ROGUE_SAVE_COMP_DIALOGUE, dialogue_write_fn, dialogue_read_fn,
                                      "dialogue"};
...
{
    return g_playback.active ? &g_playback : NULL;
}

int rogue_dialogue_start(int script_id)
{
    const RogueDialogueScript* sc = rogue_dialogue_get(script_id);
    if (!sc)
        return -1;
...
void rogue_dialogue_log_current_line(void)
{
    if (!g_playback.active)
        return;
    const RogueDialogueScript* sc = rogue_dialogue_get(g_playback.script_id);
    if (!sc)
        return;
    if (g_playback.line_index < 0 || g_playback.line_index >= sc->line_count)
        return;
    ...
    if (ln->token_flags & ROGUE_DIALOGUE_LINE_HAS_TOKENS)
    {
        expand_tokens(text, expanded, sizeof expanded);
        text = expanded;
    }
    ...
    if (script_index >= 0)
    {
        ...
        g_last_view_time_ms[script_index] = g_app.game_time_ms;
        /* FNV1a mix script id, line index, first 4 chars hash */
        unsigned int h = (unsigned int) sc->id;
        h ^= (unsigned int) g_playback.line_index;
        h *= 16777619u;
        if (text)
        {
...
int rogue_dialogue_advance(void)
{
    ...
    /* Phase 6 skip behavior: if typewriter active and current line not fully revealed yet, finish
     * reveal instead of advancing */
    if (g_typewriter_enabled)
    {
        ...
        if (ln->token_flags & ROGUE_DIALOGUE_LINE_HAS_TOKENS)
        {
            expand_tokens(text, tmp, sizeof tmp);
            text = tmp;
        }
        size_t full_len = strlen(text);
        float shown_chars = g_playback.reveal_ms * g_chars_per_ms;
        if (shown_chars + 0.5f < (float) full_len)
        { /* not fully revealed */
            /* jump reveal to end */
            g_playback.reveal_ms =
                (float) full_len / (g_chars_per_ms > 0 ? g_chars_per_ms : 0.001f);
            return 2; /* indicate reveal completed, no line advance */
        }
    }
    if (g_playback.line_index + 1 < sc->line_count)
    {
        g_playback.line_index++;
        g_playback.reveal_ms = 0.0f;
        rogue_dialogue_log_current_line();
        return 1;
    }
    else
...
void rogue_dialogue_update(double dt_ms)
{
    if (!g_playback.active)
        return;
    g_playback.reveal_ms += (float) dt_ms;
}

int rogue_dialogue_render_ui(struct RogueUIContext* ui)
{
    if (!ui || !g_playback.active)
        return 0;
    const RogueDialogueScript* sc = rogue_dialogue_get(g_playback.script_id);
    if (!sc)
        return 0;
    ...
    unsigned long long mask_bit =
        (g_playback.line_index < 64) ? (1ull << g_playback.line_index) : 0ull;
    if (mask_bit && !(sc->_executed_mask & mask_bit))
    {
        ((RogueDialogueScript*) sc)->_executed_mask |=
            mask_bit; /* cast away const for internal mutation */
        for (int ei = 0; ei < ln->effect_count; ++ei)
        {
            const RogueDialogueEffect* ef = &ln->effects[ei];
            if (ef->kind == ROGUE_DIALOGUE_EFFECT_SET_FLAG)
            {
                if (g_flag_count < 64)
                {
                    g_flags[g_flag_count++] = ef->name;
                }
            }
            else if (ef->kind == ROGUE_DIALOGUE_EFFECT_GIVE_ITEM)
            {
                if (g_item_count < 64)
                {
                    g_items[g_item_count].item_id = ef->a;
                    g_items[g_item_count].qty += (int) ef->b;
                    g_item_count++;
                }
            }
        }
    }
    /* Basic panel layout bottom of screen; using arbitrary coordinates (future: compute from
     * viewport) */
    float panel_w = 420.0f, panel_h = 110.0f;
    float x = 16.0f, y = 360.0f; /* assumptions */
    uint32_t bg = 0x202020C0u;
    uint32_t fg = 0xFFFFFFFFu;
    uint32_t speaker_col = 0x80FFD040u;
    ...
    if (g_typewriter_enabled)
    {
        size_t full_len = strlen(text);
        float shown_chars = g_playback.reveal_ms * g_chars_per_ms;
        size_t shown = (size_t) (shown_chars + 0.01f);
        if (shown > full_len)
            shown = full_len;
        memcpy(temp_tw, text, shown);
        temp_tw[shown] = '\0';
        draw_text = temp_tw;
    }
    rogue_ui_text(ui, (RogueUIRect){x + 12, y + 34, panel_w - 24, 48}, draw_text, fg);
    rogue_ui_text(ui, (RogueUIRect){x + panel_w - 80, y + panel_h - 22, 68, 16}, "[Enter]",
                  0xA0A0A0FFu);
    return 1;
}
...
void rogue_dialogue_render_runtime(void)
{
    ...
    if (ln->token_flags & ROGUE_DIALOGUE_LINE_HAS_TOKENS)
    {
        expand_tokens(full_text, expanded, sizeof expanded);
        full_text = expanded;
    }
    char temp_tw[512];
    ...
    if (g_typewriter_enabled)
    {
        ...
        float shown_chars = g_playback.reveal_ms * g_chars_per_ms;
        size_t shown = (size_t) (shown_chars + 0.5f);
        if (shown > full_len)
            shown = full_len;
        memcpy(temp_tw, full_text, shown);
        temp_tw[shown] = '\0';
        draw_text = temp_tw;
    }
    extern struct RogueAppState g_app;
    ...
    {
        int raw_h = av_peek->h;
        if (raw_h > ROGUE_AVATAR_MAX_H)
            raw_h = ROGUE_AVATAR_MAX_H;
        avatar_display_h = raw_h;                   /* no scale yet; panel height decides scaling */
        int needed_panel_h = avatar_display_h + 40; /* 20px top + 20px bottom padding approx */
        if (needed_panel_h > panel_h)
            panel_h = needed_panel_h;
        /* Keep panel from covering excessive screen (cap at ~60% viewport height) */
        int max_panel_h = (int) (vh * 0.60f);
        if (panel_h > max_panel_h)
        {
            panel_h = max_panel_h;
            if (avatar_display_h > panel_h - 40)
                avatar_display_h = panel_h - 40;
        }
    }
    int x = (vw - panel_w) / 2;
    int y = vh - panel_h - 30;
    ...
    else if (g_style.enable_gradient)
    {
        ...
        unsigned char r1 = (unsigned char) ((c1 >> 16) & 0xFF),
                      g1 = (unsigned char) ((c1 >> 8) & 0xFF), b1 = (unsigned char) (c1 & 0xFF),
                      a1 = (unsigned char) ((c1 >> 24) & 0xFF);
        for (int row = 0; row < panel_h; ++row)
        {
            ...
            if (ai > 255)
                ai = 255;
            SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, (Uint8) ri, (Uint8) gi, (Uint8) bi,
                                   (Uint8) ai);
    ...
    if (g_style.glow_strength > 0)
    {
        ...
        for (int i = 1; i <= layers; i++)
        {
            ...
            SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, (gc >> 16) & 255, (gc >> 8) & 255,
                                   gc & 255, a);
            SDL_Rect gr = {br.x - i, br.y - i, br.w + 2 * i, br.h + 2 * i};
            SDL_RenderDrawRect(g_internal_sdl_renderer_ref, &gr);
        }
    }
    ...
    if (g_style.corner_ornaments)
    {
        ...
        SDL_Rect o2 = {x + panel_w - 12, y + 4, 8, 8};
        SDL_Rect o3 = {x + 4, y + panel_h - 12, 8, 8};
        SDL_Rect o4 = {x + panel_w - 12, y + panel_h - 12, 8, 8};
        SDL_RenderFillRect(g_internal_sdl_renderer_ref, &o1);
        SDL_RenderFillRect(g_internal_sdl_renderer_ref, &o2);
        SDL_RenderFillRect(g_internal_sdl_renderer_ref, &o3);
    ...
    if (g_style.vignette)
    {
        ...
        {
            Uint8 alpha = (Uint8) (8);
            SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, 0, 0, 0, alpha);
            SDL_Rect vrect = {x + i, y + i, panel_w - 2 * i, panel_h - 2 * i};
            SDL_RenderDrawRect(g_internal_sdl_renderer_ref, &vrect);
        }
    }
    int text_left = x + 14;
    RogueTexture* av = av_peek;                             /* reuse earlier lookup */
    int avatar_side = (ln->_reserved[0] == 1) ? 1 : 0;      /* 0 left, 1 right */
    int avatar_h_mirror = (ln->_reserved[1] & 0x1) ? 1 : 0; /* horizontal mirror only */
    if (av && av->handle)
    {
        int aw = av->w, ah = av->h;
        ...
        float scale_h_cap =
            (ah > ROGUE_AVATAR_MAX_H) ? (float) ROGUE_AVATAR_MAX_H / (float) ah : 1.0f;
        int max_panel_available_h = panel_h - 40;
        if (max_panel_available_h < 20)
            max_panel_available_h = 20; /* safety */
        float scale_panel_fit =
            (ah > max_panel_available_h) ? (float) max_panel_available_h / (float) ah : 1.0f;
        float scale = scale_w;
        if (scale_panel_fit < scale)
            scale = scale_panel_fit;
        if (scale_h_cap < scale)
            scale = scale_h_cap; /* choose smallest to satisfy all */
        int dw = (int) (aw * scale);
        int dh = (int) (ah * scale);
        if (dw > ROGUE_AVATAR_MAX_W)
        {
            float adj = (float) ROGUE_AVATAR_MAX_W / (float) dw;
            dw = ROGUE_AVATAR_MAX_W;
            dh = (int) (dh * adj);
        }
        if (dh > max_panel_available_h)
        {
            float adj = (float) max_panel_available_h / (float) dh;
            dh = max_panel_available_h;
            dw = (int) (dw * adj);
        }
        SDL_Rect dst;
        if (avatar_side == 0)
        {
            dst.x = x + 12;
            text_left = dst.x + dw + 20;
        }
        else
        {
            dst.x = x + panel_w - dw - 12;
        }
        dst.y = y + panel_h - dh - 12;
        dst.w = dw;
        dst.h = dh;
        /* Mood tint application if reserved[2..4] populated */
        Uint8 tr = ln->_reserved[2], tg = ln->_reserved[3], tb = ln->_reserved[4];
        Uint8 ta = ln->_reserved[5] ? ln->_reserved[5] : 255;
        int has_tint = (ta != 0) && (tr || tg || tb) && !(tr == 255 && tg == 255 && tb == 255);
        Uint8 old_r = 255, old_g = 255, old_b = 255, old_a = 255;
        if (has_tint)
        {
            SDL_GetRenderDrawColor(g_internal_sdl_renderer_ref, &old_r, &old_g, &old_b, &old_a);
            SDL_SetTextureColorMod(av->handle, tr, tg, tb);
        }
        if (avatar_h_mirror)
        {
            SDL_RendererFlip flip = SDL_FLIP_HORIZONTAL;
            SDL_RenderCopyEx(g_internal_sdl_renderer_ref, av->handle, NULL, &dst, 0, NULL, flip);
        }
        else
        {
            SDL_RenderCopy(g_internal_sdl_renderer_ref, av->handle, NULL, &dst);
        }
        if (has_tint)
        {
            SDL_SetTextureColorMod(av->handle, 255, 255, 255);
        }
        if (avatar_side == 1)
        { /* keep text_left default when avatar on right */
        }
    }
    else
    {
        ...
        int max_avail_h = panel_h - 40;
        if (max_avail_h < 72)
            max_avail_h = 72;
        if (max_avail_h > ROGUE_AVATAR_MAX_H)
            max_avail_h = ROGUE_AVATAR_MAX_H;
        int fh = max_avail_h;
        int fw = fh;
        if (fw > ROGUE_AVATAR_MAX_W)
            fw = ROGUE_AVATAR_MAX_W; /* square-ish, limited by width cap */
        SDL_Rect dst;
        if (avatar_side == 0)
        {
            dst.x = x + 12;
            text_left = dst.x + fw + 20;
        }
        else
        {
            dst.x = x + panel_w - fw - 12;
        }
        dst.y = y + panel_h - fh - 12;
        dst.w = fw;
        dst.h = fh;
        SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, tr, tg, tb, 220);
    ...
    rogue_font_draw_text(text_left, y + 10, ln->speaker_id ? ln->speaker_id : "?", 1,
                         (RogueColor){(sc_col >> 16) & 255, (sc_col >> 8) & 255, sc_col & 255,
                                      (sc_col >> 24) & 255});
    /* Ensure text does not overlap avatar area: if avatar on left we already bumped text_left, if
     * on right limit width */
    int right_limit = x + panel_w - 14;
    if (av && av->handle && avatar_side == 1)
    { /* avatar on right -> shrink right limit */
        int aw = av->w;
        int ah = av->h;
        if (aw > 154)
            aw = 154;
        ...
        if (ah > panel_h - 40)
        {
            scale_w = (float) (panel_h - 40) / (float) ah;
        }
        int dw = (int) (aw * scale_w);
        right_limit -= (dw + 20);
        if (right_limit < text_left + 40)
            right_limit = text_left + 40;
    }
    /* Word wrapping with fixed glyph width assumption (6px) */
    int interior_w = right_limit - text_left;
    if (interior_w < 40)
        interior_w = 40;
    int char_w = 6;
    int max_chars_line = interior_w / char_w;
    if (max_chars_line < 8)
        max_chars_line = 8;
    const char* p = draw_text;
    char linebuf[256];
    int line_chars = 0;
    int line_idx = 0;
    int max_lines = 4;
    /* First pass to count wrapped lines for vertical centering */
    const char* pcount = draw_text;
    int temp_line_chars = 0;
    int counted_lines = 0;
    while (*pcount && counted_lines < max_lines)
    {
        while (*pcount == ' ')
            pcount++;
        if (!*pcount)
            break;
        const char* word = pcount;
        int wlen = 0;
        while (word[wlen] && word[wlen] != ' ' && word[wlen] != '\n')
            wlen++;
        int needed = (temp_line_chars == 0 ? 0 : 1) + wlen;
        if (temp_line_chars > 0 && temp_line_chars + needed > max_chars_line)
        {
            counted_lines++;
            temp_line_chars = 0;
        ...
        if (temp_line_chars == 0)
            temp_line_chars = wlen;
        else
            temp_line_chars += 1 + wlen;
        pcount += wlen;
        if (*pcount == '\n')
        {
            counted_lines++;
    ...
    if (temp_line_chars > 0 && counted_lines < max_lines)
        counted_lines++;
    if (counted_lines == 0)
        counted_lines = 1;
    int total_text_height = counted_lines * 20;
    ...
    while (*p && line_idx < max_lines)
    {
        ...
        while (word[wlen] && word[wlen] != ' ' && word[wlen] != '\n')
            wlen++;
        int needed = (line_chars == 0 ? 0 : 1) + wlen;
        if (line_chars > 0 && needed + line_chars > max_chars_line)
        {
            linebuf[line_chars] = '\0';
            if (draw_shadow)
                rogue_font_draw_text(text_left + 1, base_y + line_idx * 20 + 1, linebuf, 1,
                                     (RogueColor){(sh_col >> 16) & 255, (sh_col >> 8) & 255,
                                                  sh_col & 255, (sh_col >> 24) & 255});
            rogue_font_draw_text(text_left, base_y + line_idx * 20, linebuf, 1,
                                 (RogueColor){(text_col >> 16) & 255, (text_col >> 8) & 255,
                                              text_col & 255, (text_col >> 24) & 255});
            line_idx++;
            line_chars = 0;
        ...
        {
            int copy = wlen;
            if (copy > (int) sizeof(linebuf) - 1)
                copy = (int) sizeof(linebuf) - 1;
            memcpy(linebuf, word, copy);
            line_chars = copy;
            linebuf[line_chars] = '\0';
        }
        else
        {
            if (line_chars + 1 < (int) sizeof(linebuf) - 1)
            {
                linebuf[line_chars++] = ' ';
            }
            int copy = wlen;
            if (line_chars + copy > (int) sizeof(linebuf) - 1)
                copy = (int) sizeof(linebuf) - 1 - line_chars;
            if (copy > 0)
            {
                memcpy(linebuf + line_chars, word, copy);
                line_chars += copy;
                linebuf[line_chars] = '\0';
            }
        }
    ...
    if (line_chars > 0 && line_idx < max_lines)
    {
        linebuf[line_chars] = '\0';
        if (draw_shadow)
            rogue_font_draw_text(text_left + 1, base_y + line_idx * 20 + 1, linebuf, 1,
    ...
    if (g_typewriter_enabled)
    {
        ...
        if (shown_chars < (float) full_len && g_style.show_caret)
        {
            ...
            if (caret_phase == 0)
            {
                int cx = text_left + ((int) shown_chars % max_chars_line) * char_w;
                int cy = base_y + (line_idx < max_lines ? line_idx : (max_lines - 1)) * 20;
                SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, 255, 255, 255, 200);
                SDL_Rect crr = {cx, cy, 6, 2};
                SDL_RenderFillRect(g_internal_sdl_renderer_ref, &crr);
            }
        }
    }
    ...
    if (g_style.show_blink_prompt)
    {
        ...
        if (phase == 0)
        {
            rogue_font_draw_text(x + panel_w - 70, y + panel_h - 24, "[E]", 1,
                                 (RogueColor){200, 200, 200, 255});
        }
    }
    else
    {
        rogue_font_draw_text(x + panel_w - 70, y + panel_h - 24, "[E]", 1,
                             (RogueColor){190, 190, 190, 255});
    }
#endif
}
...
int rogue_dialogue_effect_item(int index, int* out_item_id, int* out_qty)
{
    if (index < 0 || index >= g_item_count)
        return 0;
    if (out_item_id)
        *out_item_id = g_items[index].item_id;
    if (out_qty)
        *out_qty = g_items[index].qty;
    return 1;
}

void rogue_dialogue_typewriter_enable(int enabled, float chars_per_ms)
{
    g_typewriter_enabled = enabled ? 1 : 0;
    if (chars_per_ms > 0)
        g_chars_per_ms = chars_per_ms;
}

int rogue_dialogue_analytics_get(int script_id, int* out_lines_viewed, double* out_last_view_ms,
                                 unsigned int* out_digest)
{
    int idx = find_script_index(script_id);
    if (idx < 0)
        return -1;
    if (out_lines_viewed)
        *out_lines_viewed = g_lines_viewed[idx];
    if (out_last_view_ms)
        *out_last_view_ms = g_last_view_time_ms[idx];
    if (out_digest)
        *out_digest = g_digest_accum;
    return 0;
}
