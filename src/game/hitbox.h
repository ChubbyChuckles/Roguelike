...
typedef struct RogueHitboxCapsule
{
    float ax, ay;
    float bx, by;
    float radius;
} RogueHitboxCapsule;
typedef struct RogueHitboxArc
{
    float ox, oy;
    float radius;
    float angle_start;
...
/* Builders */
static inline RogueHitbox rogue_hitbox_make_capsule(float ax, float ay, float bx, float by,
                                                    float radius)
{
    RogueHitbox h;
    h.type = ROGUE_HITBOX_CAPSULE;
    h.u.capsule.ax = ax;
    h.u.capsule.ay = ay;
    h.u.capsule.bx = bx;
    h.u.capsule.by = by;
    h.u.capsule.radius = radius;
    return h;
}
static inline RogueHitbox rogue_hitbox_make_arc(float ox, float oy, float radius, float a0,
                                                float a1, float inner_r)
{
    RogueHitbox h;
    h.type = ROGUE_HITBOX_ARC;
    h.u.arc.ox = ox;
    h.u.arc.oy = oy;
    h.u.arc.radius = radius;
    h.u.arc.angle_start = a0;
    h.u.arc.angle_end = a1;
...
{
    RogueHitbox h;
    h.type = ROGUE_HITBOX_PROJECTILE_SPAWN;
    h.u.proj.projectile_count = count;
    h.u.proj.origin_x = ox;
    h.u.proj.origin_y = oy;
    h.u.proj.base_speed = speed;
    h.u.proj.spread_radians = spread;
    h.u.proj.angle_center = center;
    return h;
}

/* Mutators for chain */
static inline void rogue_hitbox_chain_add_point(RogueHitbox* h, float x, float y)
{
    if (!h || h->type != ROGUE_HITBOX_CHAIN)
        return;
    if (h->u.chain.count < ROGUE_HITBOX_CHAIN_MAX_POINTS)
    {
        h->u.chain.px[h->u.chain.count] = x;
        h->u.chain.py[h->u.chain.count] = y;
        h->u.chain.count++;
    }
}
...
/* Utility: compute per-projectile firing angle i (0..count-1) evenly distributed across spread
   centered at angle_center. If count==1, returns center. */
