...
#define ROGUE_DIALOGUE_LINE_IS_KEY 0x2u /* Phase 5: text field stores localization key */
    ...
    /* Retrieve current line expanded into buffer (tokens replaced). Returns length written, or <0
     * on error/inactive. */
    int rogue_dialogue_current_text(char* buffer, size_t cap);

    /* Phase 5 Localization */
    int rogue_dialogue_locale_register(const char* locale, const char* key,
    ...
    /* Register a script from an in-memory buffer (used by unit tests). Buffer need not be
     * null-terminated. */
    ...
    int rogue_dialogue_capture(
        RogueDialoguePersistState* out); /* returns 1 if captured, 0 if no active dialogue */
    int rogue_dialogue_restore(
        const RogueDialoguePersistState* st); /* returns 0 on success, negative on invalid */
    void rogue_dialogue_register_save_component(
        void); /* registers dialogue save component with save manager */

    /* Phase 6 Typewriter & Skip */
    void
    rogue_dialogue_typewriter_enable(int enabled,
                                     float chars_per_ms); /* enable/disable typewriter reveal */

    /* Runtime (SDL) immediate render helper used by game loop (draws panel directly with SDL &
     * font). */
    ...
    /* Register or update an avatar image for a speaker (path loaded immediately). Returns 0 on
     * success. */
