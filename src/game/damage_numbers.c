...
void rogue_add_damage_number_ex(float x, float y, int amount, int from_player, int crit){
    if(amount==0) return;
    /* Simple spatial batching: if a recent number within small radius (<0.4 tiles) and same ownership, accumulate. */
    for(int i=0;i<g_app.dmg_number_count;i++){
        float dx = g_app.dmg_numbers[i].x - x; float dy = g_app.dmg_numbers[i].y - y;
        float dist2 = dx*dx + dy*dy;
        if(dist2 < 0.16f && g_app.dmg_numbers[i].from_player==from_player && g_app.dmg_numbers[i].crit==crit){
            g_app.dmg_numbers[i].amount += amount; /* refresh life slightly */
            float extend = 120.0f; g_app.dmg_numbers[i].life_ms += extend; if(g_app.dmg_numbers[i].life_ms > g_app.dmg_numbers[i].total_ms) g_app.dmg_numbers[i].life_ms = g_app.dmg_numbers[i].total_ms;
            return;
        }
    }
    if(g_app.dmg_number_count < (int)(sizeof(g_app.dmg_numbers)/sizeof(g_app.dmg_numbers[0]))){
        ...
        g_app.dmg_numbers[i].spawn_ms = (float)g_app.game_time_ms; g_app.dmg_numbers[i].alpha = 1.0f;
    }
}
...
void rogue_damage_numbers_update(float dt_seconds){
    ...
    for(int i=0;i<g_app.dmg_number_count;){
        ...
        float alpha=1.0f;
        if(age < 80.0f) alpha = age/80.0f; else {
            if(g_app.dmg_numbers[i].life_ms < 150.0f){ float t = g_app.dmg_numbers[i].life_ms/150.0f; alpha = t*t; }
        }
...
void rogue_damage_numbers_render(void){
    ...
    for(int i=0;i<g_app.dmg_number_count;i++){
    int alpha = (int)(255 * (g_app.dmg_numbers[i].alpha<0?0:(g_app.dmg_numbers[i].alpha>1?1:g_app.dmg_numbers[i].alpha)));
