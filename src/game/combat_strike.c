...
static int _clampi(int v,int lo,int hi){ if(v<lo) return lo; if(v>hi) return hi; return v; }
static int _effective_phys_resist_local(int p){
    if(p<=0) return 0; if(p>90) p=90;
    float pf=(float)p; float efff=0.0f;
    if(pf <= 50.0f){ efff = pf; }
    else { float over = pf - 50.0f; efff = 50.0f + over * 0.50f; }
    if(efff < 0.0f) efff = 0.0f; if(efff > 75.0f) efff = 75.0f;
    return (int)floorf(efff + 0.5f);
}
static int _rogue_apply_mitig_with_override_armor(const RogueEnemy* e, int raw, unsigned char dmg_type, int override_armor, int *out_overkill){
    if(!e || !e->alive) return 0;
    int dmg = raw; if(dmg<0) dmg=0;
    if(dmg_type != ROGUE_DMG_TRUE){
        if(dmg_type == ROGUE_DMG_PHYSICAL){
            int armor = override_armor; if(armor>0){ if(armor >= dmg) dmg = (dmg>1?1:dmg); else dmg -= armor; }
            int pr_raw = _clampi(e->resist_physical,0,90); int pr = _effective_phys_resist_local(pr_raw); if(pr>0){ int reduce=(dmg*pr)/100; dmg -= reduce; }
            if(raw >= ROGUE_DEF_SOFTCAP_MIN_RAW){
                float armor_frac = 0.0f; if(override_armor>0){ armor_frac = (float)override_armor / (float)(raw + override_armor); if(armor_frac>0.90f) armor_frac=0.90f; }
                float total_frac = armor_frac + (float)pr / 100.0f; if(total_frac > 0.0f){
                    if(total_frac > ROGUE_DEF_SOFTCAP_REDUCTION_THRESHOLD){
                        ...
                        if(target > dmg) { /* keep lower dmg (more mitigation) */ }
                        else dmg = target;
            ...
            int resist=0; switch(dmg_type){
                ...
                default: break; }
            resist = _clampi(resist,0,90); if(resist>0){ int reduce=(dmg*resist)/100; dmg -= reduce; }
        }
    }
    if(dmg < 1) dmg = 1;
...
int rogue_combat_player_strike(RoguePlayerCombat* pc, RoguePlayer* player, RogueEnemy enemies[], int enemy_count){
    ...
    for(int si=0; si<sweep_count; ++si){ int i = sweep_indices[si]; if(i<0 || i>=enemy_count) continue; if(!enemies[i].alive) continue; /* Only apply friendly-fire filter when enemy has explicit non-zero team id matching player */ if(enemies[i].team_id != 0 && enemies[i].team_id == player->team_id) continue; float ex = enemies[i].base.pos.x; float ey = enemies[i].base.pos.y; int effective_strength = player->strength + rogue_buffs_get_total(0); int base = 1 + effective_strength/5; float scaled = (float)base; if(def){ scaled = def->base_damage + (float)effective_strength * def->str_scale + (float)player->dexterity * def->dex_scale + (float)player->intelligence * def->int_scale; if(scaled<1.0f) scaled=1.0f; }
            ...
            float raw_total = (float)dmg; float dex_bonus = player->dexterity * 0.0035f; if(dex_bonus>0.55f) dex_bonus=0.55f; float crit_from_rating = rogue_rating_effective_percent(ROGUE_RATING_CRIT, player->crit_rating)*0.01f; float crit_chance = 0.05f + dex_bonus + (float)player->crit_chance * 0.01f + crit_from_rating; if(crit_chance>0.80f) crit_chance=0.80f; int is_crit = 0; if(pc->force_crit_next_strike){ is_crit=1; pc->force_crit_next_strike=0; } else { extern int g_force_crit_mode; if(g_force_crit_mode>=0){ is_crit = g_force_crit_mode?1:0; } else { is_crit = (((float)rand()/(float)RAND_MAX) < crit_chance)?1:0; } } float crit_mult = 1.0f; if(is_crit){ crit_mult = 1.0f + ((float)player->crit_damage * 0.01f); if(crit_mult > 5.0f) crit_mult = 5.0f; }
            int health_before = enemies[i].health; int final_dmg = 0; int overkill_accum = 0;
            if(part_phys > 0.01f){ int comp_raw = (int)floorf(part_phys + 0.5f); if(comp_raw<1) comp_raw=1; if(is_crit && g_crit_layering_mode==0){ float cval=(float)comp_raw*crit_mult; comp_raw=(int)floorf(cval+0.5f); if(comp_raw<1) comp_raw=1; } int eff_armor = enemies[i].armor; int pen_flat = player->pen_flat; if(pen_flat>0){ eff_armor -= pen_flat; if(eff_armor<0) eff_armor=0; } int pen_pct = player->pen_percent; if(pen_pct>100) pen_pct=100; if(pen_pct>0){ int reduce=(enemies[i].armor * pen_pct)/100; eff_armor -= reduce; if(eff_armor<0) eff_armor=0; } int local_overkill=0; int mitig=_rogue_apply_mitig_with_override_armor(&enemies[i],comp_raw,ROGUE_DMG_PHYSICAL,eff_armor,&local_overkill); if(is_crit && g_crit_layering_mode==1){ float cval2=(float)mitig*crit_mult; mitig=(int)floorf(cval2+0.5f); if(mitig<1) mitig=1; } enemies[i].health -= mitig; final_dmg += mitig; overkill_accum += local_overkill; rogue_add_damage_number_ex(ex, ey - 0.25f, mitig, 1, is_crit); }
            if(part_fire > 0.01f){ int comp_raw = (int)floorf(part_fire + 0.5f); if(comp_raw<1) comp_raw=1; if(is_crit && g_crit_layering_mode==0){ float cval=(float)comp_raw*crit_mult; comp_raw=(int)floorf(cval+0.5f); if(comp_raw<1) comp_raw=1; } int local_overkill=0; int mitig=rogue_apply_mitigation_enemy(&enemies[i],comp_raw,ROGUE_DMG_FIRE,&local_overkill); if(is_crit && g_crit_layering_mode==1){ float cval2=(float)mitig*crit_mult; mitig=(int)floorf(cval2+0.5f); if(mitig<1) mitig=1; } enemies[i].health -= mitig; final_dmg += mitig; overkill_accum += local_overkill; rogue_add_damage_number_ex(ex, ey - 0.25f, mitig, 1, is_crit); }
            if(part_frost > 0.01f){ int comp_raw = (int)floorf(part_frost + 0.5f); if(comp_raw<1) comp_raw=1; if(is_crit && g_crit_layering_mode==0){ float cval=(float)comp_raw*crit_mult; comp_raw=(int)floorf(cval+0.5f); if(comp_raw<1) comp_raw=1; } int local_overkill=0; int mitig=rogue_apply_mitigation_enemy(&enemies[i],comp_raw,ROGUE_DMG_FROST,&local_overkill); if(is_crit && g_crit_layering_mode==1){ float cval2=(float)mitig*crit_mult; mitig=(int)floorf(cval2+0.5f); if(mitig<1) mitig=1; } enemies[i].health -= mitig; final_dmg += mitig; overkill_accum += local_overkill; rogue_add_damage_number_ex(ex, ey - 0.25f, mitig, 1, is_crit); }
            if(part_arc > 0.01f){ int comp_raw = (int)floorf(part_arc + 0.5f); if(comp_raw<1) comp_raw=1; if(is_crit && g_crit_layering_mode==0){ float cval=(float)comp_raw*crit_mult; comp_raw=(int)floorf(cval+0.5f); if(comp_raw<1) comp_raw=1; } int local_overkill=0; int mitig=rogue_apply_mitigation_enemy(&enemies[i],comp_raw,ROGUE_DMG_ARCANE,&local_overkill); if(is_crit && g_crit_layering_mode==1){ float cval2=(float)mitig*crit_mult; mitig=(int)floorf(cval2+0.5f); if(mitig<1) mitig=1; } enemies[i].health -= mitig; final_dmg += mitig; overkill_accum += local_overkill; rogue_add_damage_number_ex(ex, ey - 0.25f, mitig, 1, is_crit); }
            ...
            /* Emit a single composite damage event per hit (sum of all components). Use the attack's declared damage type for now. */
            rogue_damage_event_record((unsigned short)(def?def->id:0), (unsigned char)(def?def->damage_type:ROGUE_DMG_PHYSICAL), (unsigned char)is_crit, (int)floorf(raw_total + 0.5f), final_dmg, overkill, (unsigned char)execution);
            ...
            /* Keep targets stationary during forced-attack test harness runs to allow repeated strikes without drift. */
