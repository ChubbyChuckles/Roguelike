 ...
 *   Separation keeps frequently modified strike logic out of hot include path for faster incremental builds.
 */
#if defined(_MSC_VER)
...
int rogue_damage_events_snapshot(RogueDamageEvent* out, int max_events){
    ...
    for(int i=0;i<count;i++) out[i] = g_damage_events[(start + i) % ROGUE_DAMAGE_EVENT_CAP];
    return count;
}
