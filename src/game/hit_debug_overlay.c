...
void rogue_hit_debug_render(void){
    ...
    if(df->pixel_mask_valid && df->mask_bits){
        float world_x = df->mask_player_x + df->mask_pose_dx - (float)df->mask_origin_x * df->mask_scale;
        float world_y = df->mask_player_y + df->mask_pose_dy - (float)df->mask_origin_y * df->mask_scale;
        int base_x = (int)(world_x * tsz - g_app.cam_x);
        int base_y = (int)(world_y * tsz - g_app.cam_y);
        SDL_Rect aabb = { base_x, base_y, (int)(df->mask_w * df->mask_scale * tsz), (int)(df->mask_h * df->mask_scale * tsz) };
        ...
        for(int y=0;y<df->mask_h;y+=step){ const uint32_t* row = df->mask_bits + (size_t)y * df->mask_pitch_words; for(int x=0;x<df->mask_w;x+=step){ uint32_t w = row[x>>5]; if(w & (1u<<(x&31))){ int sx = base_x + (int)(x * df->mask_scale * tsz); int sy = base_y + (int)(y * df->mask_scale * tsz); SDL_SetRenderDrawColor(g_app.renderer,120,120,255,160); SDL_RenderDrawPoint(g_app.renderer,sx,sy); } } }
    ...
    char buf[256]; snprintf(buf,sizeof(buf),"HITDBG frame=%d used=%c hits=%d cap=%d pix=%d misP=%d misC=%d len=%.2f\nPX=(%.2f,%.2f) mask[%d x %d] origin(%d,%d) pose(%.2f,%.2f) scale=%.2f",\
        ...
        g_app.player.base.pos.x, g_app.player.base.pos.y, df->mask_w, df->mask_h, df->mask_origin_x, df->mask_origin_y, df->mask_pose_dx, df->mask_pose_dy, df->mask_scale);
    draw_text(8,8,buf);
#endif
}
