...
extern "C"
{
    ...
    int rogue_weapon_pose_ensure_dir(int weapon_id,
                                     int dir_group); /* ensure specific direction JSON loaded */
    const RogueWeaponPoseFrame* rogue_weapon_pose_get_dir(int weapon_id, int dir_group,
                                                          int frame_index);
    /* Helper to mirror dx when facing left */
    static inline float rogue_weapon_pose_effective_dx(const RogueWeaponPoseFrame* f,
                                                       int facing_left)
    {
        return facing_left ? -f->dx : f->dx;
    }
