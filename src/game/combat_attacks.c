...
static const RogueAttackDef g_light_chain[] = {
    {0,"light_1",ROGUE_WEAPON_LIGHT,0,110,70,120,14,10,5,0,0.34f,0.06f,0.00f,1,{{0,70,ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF|ROGUE_CANCEL_ON_BLOCK,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF|ROGUE_CANCEL_ON_BLOCK), 0.50f, 1.0f, 0.0f},
    {1,"light_2",ROGUE_WEAPON_LIGHT,1,95, 65,115,12,12,6,0,0.36f,0.07f,0.00f,1,{{0,65,ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.45f, 1.2f, 0.0f},
    /* light_3: split into two sub windows for multi-hit demonstration */
    {2,"light_3",ROGUE_WEAPON_LIGHT,2,105,75,140,16,15,8,0,0.39f,0.08f,0.00f,2,{{0,36,ROGUE_CANCEL_ON_HIT,0.40f,0.0f,0.0f,-1,-1},{36,75,ROGUE_CANCEL_ON_HIT,0.90f,0.05f,0.02f,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.40f, 1.5f, 0.0f},
};
...
static const RogueAttackDef g_thrust_chain[] = {
    ...
    {1,"thrust_2",ROGUE_WEAPON_THRUST,1,125,60,115,14,14,8,0,0.15f,0.35f,0.00f,1,{{0,60,0,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.45f, 0.9f, 0.0f},
};
static const RogueAttackDef g_ranged_chain[] = {
    {0,"ranged_1",ROGUE_WEAPON_RANGED,0,140,40,100,10,0,4,0,0.05f,0.30f,0.00f,1,{{0,40,0,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.35f, 0.0f, 0.6f},
    {1,"ranged_2",ROGUE_WEAPON_RANGED,1,150,50,110,12,0,5,0,0.05f,0.34f,0.00f,1,{{0,50,0,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.35f, 0.0f, 0.7f},
};
static const RogueAttackDef g_spell_chain[] = {
    {0,"spell_1",ROGUE_WEAPON_SPELL_FOCUS,0,160,60,140,16,0,9,3,0.00f,0.00f,0.40f,1,{{0,60,0,1.0f,0,0,-1,-1}}, 0.0f, (ROGUE_CANCEL_ON_HIT|ROGUE_CANCEL_ON_WHIFF), 0.40f, 0.0f, 1.8f},
};
...
typedef struct FrameCacheEntry { const RogueAttackDef* def; unsigned char wi; short sf, ef; unsigned char valid; } FrameCacheEntry;
static FrameCacheEntry g_frame_cache[64]; /* supports up to 64 total windows across all authored attacks */
static int g_frame_cache_initialized = 0;

static void rogue_attack_build_frame_cache(void){
    if(g_frame_cache_initialized) return;
    ...
    for(int arch=0; arch<ROGUE_WEAPON_ARCHETYPE_COUNT; ++arch){
        ...
        for(int ci=0; ci<len; ++ci){
            ...
            if(!def) continue;
            for(int wi=0; wi<def->num_windows && cursor < (int)(sizeof(g_frame_cache)/sizeof(g_frame_cache[0])); ++wi){
                ...
                g_frame_cache[cursor].def = def; g_frame_cache[cursor].wi = (unsigned char)wi; g_frame_cache[cursor].sf = (short)sf; g_frame_cache[cursor].ef = (short)ef; g_frame_cache[cursor].valid = 1; cursor++;
            }
        }
    }
}
...
static FrameCacheEntry* rogue_attack_find_frame_cache(const RogueAttackDef* def, int wi){
    for(size_t i=0;i<sizeof(g_frame_cache)/sizeof(g_frame_cache[0]); ++i){
        if(g_frame_cache[i].valid && g_frame_cache[i].def == def && g_frame_cache[i].wi == (unsigned char)wi) return &g_frame_cache[i];
    }
    return NULL;
}
...
int rogue_attack_window_frame_span(const RogueAttackDef* def, int window_index, int* out_start_frame, int* out_end_frame){
    ...
    if(!g_frame_cache_initialized) rogue_attack_build_frame_cache();
    FrameCacheEntry* e = rogue_attack_find_frame_cache(def, window_index);
    if(!e){ /* fallback compute on the fly (should not normally happen unless cache overflow) */
        const RogueAttackWindow* w = &def->windows[window_index];
        float active = (def->active_ms>0? def->active_ms : 1.0f);
        int sf = (int)floorf( (w->start_ms / active) * 8.0f );
        int ef = (int)floorf( ((w->end_ms> w->start_ms? w->end_ms: w->start_ms+0.01f) / active) * 8.0f ) - 1;
        if(sf<0) sf=0; if(sf>7) sf=7; if(ef<sf) ef=sf; if(ef>7) ef=7;
        if(out_start_frame) *out_start_frame = sf; if(out_end_frame) *out_end_frame = ef; return 1;
    }
    if(out_start_frame) *out_start_frame = e->sf; if(out_end_frame) *out_end_frame = e->ef; return 1;
}
