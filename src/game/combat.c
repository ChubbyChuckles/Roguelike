...
int rogue_player_apply_incoming_melee(RoguePlayer* p, float raw_damage, float attack_dir_x, float attack_dir_y, int poise_damage, int *out_blocked, int *out_perfect){
    ...
    /* Phase 6.5: parry active check (supersedes guard) - perfect negation and enables riposte */
    if(p){
        extern RoguePlayerCombat g_combat_for_parry_temp; /* optional external reference, else will be unused */
    }
...
int rogue_combat_try_backstab(RoguePlayer* p, RoguePlayerCombat* pc, RogueEnemy* e){
    if(!p||!pc||!e) return 0; if(!e->alive) return 0; if(pc->backstab_cooldown_ms>0) return 0;
    /* Require player behind enemy: vector enemy->player forms dot with enemy facing dir <= -0.70 (roughly 134 deg) */
    float ex=e->base.pos.x, ey=e->base.pos.y; float px=p->base.pos.x, py=p->base.pos.y;
...
int rogue_player_register_incoming_attack_parry(RoguePlayer* p, RoguePlayerCombat* pc, float attack_dir_x, float attack_dir_y){
    if(!p||!pc) return 0; if(!pc->parry_active) return 0; /* no active parry */
    /* Direction check similar to guard: must be roughly facing attack (dot >= 0.10 for leniency) */
    float fdx=0,fdy=0; switch(p->facing){ case 0:fdy=1;break; case 1:fdx=-1;break; case 2:fdx=1;break; case 3:fdy=-1;break; }
    float alen = sqrtf(attack_dir_x*attack_dir_x + attack_dir_y*attack_dir_y); if(alen>0.0001f){ attack_dir_x/=alen; attack_dir_y/=alen; }
    float dot = fdx*attack_dir_x + fdy*attack_dir_y; if(dot < 0.10f) return 0;
    /* Successful parry: grant short i-frames & mark riposte window */
