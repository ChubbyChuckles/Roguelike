...
int rogue_player_update_guard(RoguePlayer* p, float dt_ms){ if(!p) return 0; int chip=0; extern struct RogueStatCache g_player_stat_cache; float rec_mult = 1.0f + (g_player_stat_cache.guard_recovery_pct/100.0f); if(rec_mult < 0.10f) rec_mult = 0.10f; /* prevent negative / zero */ if(rec_mult>3.0f) rec_mult=3.0f; if(p->guarding){ p->guard_active_time_ms += dt_ms; /* drain is inversely affected by recovery modifier (better recovery => less drain) */ float drain_mult = 1.0f - (g_player_stat_cache.guard_recovery_pct/150.0f); if(drain_mult < 0.25f) drain_mult = 0.25f; p->guard_meter -= dt_ms * ROGUE_GUARD_METER_DRAIN_HOLD_PER_MS * drain_mult; if(p->guard_meter <= 0.0f){ p->guard_meter=0.0f; p->guarding=0; } } else { p->guard_meter += dt_ms * ROGUE_GUARD_METER_RECOVER_PER_MS * rec_mult; if(p->guard_meter > p->guard_meter_max) p->guard_meter = p->guard_meter_max; }
...
int rogue_player_apply_incoming_melee(RoguePlayer* p, float raw_damage, float attack_dir_x, float attack_dir_y, int poise_damage, int *out_blocked, int *out_perfect){
    ...
    p->poise_regen_delay_ms = ROGUE_POISE_REGEN_DELAY_AFTER_HIT;
     /* Phase 7.2 damage conversion (physical -> elemental) implementation.
         Since this function currently receives only a single raw_damage assumed physical, we internally partition it.
         Ordering: passive block -> guard block -> conversion -> (future elemental mitigation when player resist fields added).
         We expose converted components via local variables for potential future telemetry (unused now). */
     float remain_phys = raw_damage; if(remain_phys < 0) remain_phys = 0;
     int c_fire = g_player_stat_cache.phys_conv_fire_pct; if(c_fire<0) c_fire=0;
     int c_frost = g_player_stat_cache.phys_conv_frost_pct; if(c_frost<0) c_frost=0;
     int c_arc = g_player_stat_cache.phys_conv_arcane_pct; if(c_arc<0) c_arc=0;
