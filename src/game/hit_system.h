...
extern "C"
{
    ...
    typedef struct RogueHitDebugFrame
    {
        ...
        int capsule_hits[32];
        int capsule_hit_count; /* raw capsule results */
        int pixel_hits[32];
        int pixel_hit_count;       /* raw pixel results */
        ...
        int pixel_mask_valid;
        int mask_w, mask_h;
        int mask_origin_x, mask_origin_y;
        unsigned int mask_pitch_words;
        const uint32_t* mask_bits;
        float mask_player_x, mask_player_y;
        float mask_pose_dx, mask_pose_dy;
        float mask_scale_x;
        float mask_scale_y; /* independent axis scale */
        int frame_id;
    } RogueHitDebugFrame;
    ...
    void rogue_hit_debug_store(const RogueCapsule* c, int* indices, float (*normals)[2],
                               int hit_count, int frame_id);
    ...
    void rogue_hit_debug_store_dual(const RogueCapsule* c, int* capsule_indices, int capsule_count,
                                    ...
                                    int pixel_used, int mismatch_pixel_only,
                                    int mismatch_capsule_only, int frame_id, int mask_w, int mask_h,
                                    int mask_origin_x, int mask_origin_y, float player_x,
                                    float player_y, float pose_dx, float pose_dy, float scale_x,
                                    float scale_y);
    ...
    typedef struct RogueHitboxTuning
    {
        ...
        float pursue_offset_x,
            pursue_offset_y; /* enemy AI target point relative to player (Shift+1..4 adjust) */
        /* Per facing (0=down,1=left,2=right,3=up) pixel mask offset + scale overrides for
