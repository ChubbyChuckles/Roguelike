...
/* Tactical Cover Seek (stub): sets in-cover flag true immediately */
typedef struct TacticalCoverSeek { const char* out_flag_key; } TacticalCoverSeek;
static RogueBTStatus tick_tactical_cover_seek(RogueBTNode* node,RogueBlackboard* bb,float dt){ (void)dt; TacticalCoverSeek* d=(TacticalCoverSeek*)node->user_data; rogue_bb_set_bool(bb,d->out_flag_key,true); return ROGUE_BT_SUCCESS; }
RogueBTNode* rogue_bt_tactical_cover_seek(const char* name,const char* bb_out_in_cover_flag_key){ RogueBTNode* n=rogue_bt_node_create(name,0,tick_tactical_cover_seek); if(!n) return NULL; TacticalCoverSeek* d=(TacticalCoverSeek*)calloc(1,sizeof(TacticalCoverSeek)); d->out_flag_key=bb_out_in_cover_flag_key; n->user_data=d; return n; }
