...
extern "C"
{
    ...
    struct RoguePerceptionAgent;
    struct RogueAITraceBuffer;

    /* 10.1 Behavior tree visualization: writes ASCII tree lines into out (null terminated). Returns
     * bytes written (excl null) or -1. */
    ...
    typedef enum RogueAIDebugPrimKind
    {
        ROGUE_AI_DEBUG_PRIM_FOV_CONE = 1,
        ROGUE_AI_DEBUG_PRIM_LOS_RAY = 2
    } RogueAIDebugPrimKind;
    typedef struct RogueAIDebugPrimitive
    {
        float ax, ay, bx, by;
        uint8_t kind;
    } RogueAIDebugPrimitive;
    /* Collect debug primitives for a single agent relative to player (player_x,y). Returns count
     * written. */
    int rogue_ai_perception_collect_debug(const struct RoguePerceptionAgent* agent, float player_x,
                                          float player_y, RogueAIDebugPrimitive* out, int max,
                                          float fov_deg, float vision_dist);
    ...
    /* 10.5 Determinism verifier: builds two trees via factory, ticks each 'ticks' times and hashes
       per-tick serialized path. Returns 1 if hashes match (deterministic), 0 otherwise. out_hash
       optional aggregate hash. */
