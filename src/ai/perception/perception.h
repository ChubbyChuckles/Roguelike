...
/* Override blocking predicate (tx,ty tile coordinates). Pass NULL to restore default using rogue_nav_is_blocked. */
...
/* Vision cone test. Returns 1 if target inside FOV angle (deg) & within max_dist AND LOS clear. Optionally outputs distance. */
...
/* Tick one agent perception against target (player) state. Updates threat, last seen position & TTL. */
...
/* Process hearing events for one agent (called each frame after emission, before decay). Returns number of events that contributed. */
...
/* Broadcast alert from source agent to others within radius. Raises their threat to at least baseline_threat and seeds last seen pos. */
