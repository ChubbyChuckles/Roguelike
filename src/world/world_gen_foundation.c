...
{
    RogueChunkCoord cc;
    cc.cx = (tx >= 0 ? tx : tx - (ROGUE_WORLD_CHUNK_SIZE - 1)) / ROGUE_WORLD_CHUNK_SIZE;
    cc.cy = (ty >= 0 ? ty : ty - (ROGUE_WORLD_CHUNK_SIZE - 1)) / ROGUE_WORLD_CHUNK_SIZE;
    return cc;
}
...
{
    if (out_tx)
        *out_tx = cc.cx * ROGUE_WORLD_CHUNK_SIZE;
    if (out_ty)
        *out_ty = cc.cy * ROGUE_WORLD_CHUNK_SIZE;
}
...
/* ---- RNG channels ---- */
static unsigned int rng_step(unsigned int st)
{
    unsigned int x = st;
    x ^= x << 13;
    x ^= x >> 17;
...
void rogue_worldgen_context_init(RogueWorldGenContext* ctx, const RogueWorldGenConfig* cfg)
{
    ...
    if (!ctx->macro_rng.state)
        ctx->macro_rng.state = 1u;
    if (!ctx->biome_rng.state)
        ctx->biome_rng.state = 1u;
    if (!ctx->micro_rng.state)
        ctx->micro_rng.state = 1u;
}
...
unsigned int rogue_worldgen_rand_u32(RogueRngChannel* ch)
{
    if (!ch)
        return 0;
    ch->state = rng_step(ch->state);
    return ch->state;
}
double rogue_worldgen_rand_norm(RogueRngChannel* ch)
{
    return (double) (rogue_worldgen_rand_u32(ch) & 0xffffffu) / (double) 0xffffffu;
}
...
unsigned long long rogue_world_hash_tilemap(const RogueTileMap* map)
{
    ...
    const unsigned char* t = map->tiles;
    int count = map->width * map->height;
    int nonzero = 0;
    for (int i = 0; i < count; i++)
    {
        h ^= (unsigned long long) t[i];
        h *= 1099511628211ULL;
        if (t[i])
            nonzero = 1;
    }
    if (!nonzero)
    { /* all zero tiles: mix a constant so hash != 0 sentinel after avalanche */
        h ^= 0xABCDEF1234567890ULL;
        h *= 1099511628211ULL;
    }
    /* Mix dimensions & a fixed tag to reduce collisions across different sizes */
    h ^= (unsigned long long) map->width;
    h *= 1099511628211ULL;
    h ^= (unsigned long long) map->height;
    h *= 1099511628211ULL;
