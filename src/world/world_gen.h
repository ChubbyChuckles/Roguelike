...
typedef struct RogueWorldGenConfig
{
    ...
    unsigned int
        biome_seed_offset;     /* extra offset applied when seeding biome channel to decorrelate */
...
typedef struct RogueChunkCoord
{
    int cx;
    int cy;
} RogueChunkCoord;
...
typedef struct RogueRngChannel
{
    unsigned int state;
} RogueRngChannel;
 ...
 * Populates out_map tiles (water / land biome tiles + rivers) and returns biome histogram &
 * continent count if provided. Returns false on allocation failure or invalid args. Deterministic
 * for (cfg->seed).
 */
...
typedef struct RogueStructurePlacement
{
    ...
    int y; /* top-left tile */
    int w;
    int h;                          /* final placed size (post-rotation) */
...
typedef struct RogueDungeonEdge
{
    int a;
    int b;
    int loop;
} RogueDungeonEdge; /* loop=1 if extra loop connection */
...
/* Carve dungeon into tilemap region starting at (ox,oy) with given bounds (w,h). Returns carved
 * floor count. */
...
/* Place keys and locked doors to enforce progression ordering; returns number of locked doors
 * placed */
...
/* Advance weather by ticks; may transition patterns when duration elapses; returns new pattern
 * index if changed else -1 */
...
/* Initialize streaming manager with a base world config template (width/height ignored; chunk size
 * fixed). */
...
/* Advance streaming system one tick; processes up to budget queued generations. Returns number
 * generated/loaded. */
...
/* Export simple biome frequency heatmap (writes width*height bytes into out, each = biome id).
 ...
 * On hot reload, existing registries are swapped atomically if validation succeeds (all-or-nothing)
 * to avoid partial state updates during generation. Tests exercise invalid pack rejection and
 * migration.
 */
...
/* Callback signature for migrating a raw text descriptor (in-place) from old_version ->
 * target_version. Returns 1 on success, 0 on failure (aborts load).
 */
 ...
 * If allow_hot_reload != 0 and a previous pack exists, validated registries replace the active ones
 * atomically. out_meta may be NULL. Returns ROGUE_PACK_LOAD_OK or error code.
 */
...
/* CLI-oriented validation helper: run validation on a path and print errors to stdout/stderr
 * (returns 0/1). */
...
/* Enable/disable SIMD & parallel paths (0=off, non-zero=on). Parallel currently affects macro
 * continent sampling. */
...
typedef struct RogueWorldGenBenchmark
{
    double scalar_ms;
    double simd_ms;
    double speedup;
    int samples;
} RogueWorldGenBenchmark;
int rogue_worldgen_run_noise_benchmark(int width, int height, RogueWorldGenBenchmark* out_bench);

/* Convenience: sample a random walkable spawn point from a generated tilemap (excludes water, lava,
 * walls, mountains). Returns 1 and writes tile coords to out_tx/out_ty on success, 0 if no suitable
 * tile found.
 */
