 ...
 * Provides transient arena allocator, optional SIMD acceleration for value noise/fbm, and a benchmark harness.
 * Parallel pass is stubbed with single-thread execution (future extension will dispatch tasks). Deterministic results preserved.
 */
...
/* -------- Arena -------- */
struct RogueWorldGenArena { unsigned char* base; size_t cap; size_t off; };
RogueWorldGenArena* rogue_worldgen_arena_create(size_t capacity_bytes){ if(capacity_bytes==0) capacity_bytes=1; RogueWorldGenArena* a=(RogueWorldGenArena*)malloc(sizeof(*a)); if(!a) return NULL; a->base=(unsigned char*)malloc(capacity_bytes); if(!a->base){ free(a); return NULL; } a->cap=capacity_bytes; a->off=0; return a; }
void rogue_worldgen_arena_destroy(RogueWorldGenArena* a){ if(!a) return; if(a->base) free(a->base); free(a); }
void* rogue_worldgen_arena_alloc(RogueWorldGenArena* a, size_t sz, size_t align){ if(!a||!a->base) return NULL; if(align<1) align=1; uintptr_t cur=(uintptr_t)(a->base + a->off); uintptr_t aligned = (cur + (align-1)) & ~(uintptr_t)(align-1); size_t new_off = (size_t)(aligned - (uintptr_t)a->base) + sz; if(new_off > a->cap) return NULL; a->off = new_off; return (void*)aligned; }
void rogue_worldgen_arena_reset(RogueWorldGenArena* a){ if(a) a->off=0; }
...
static double fbm_simd_grid(int w,int h,int oct,double lac,double gain){ double sum=0.0; double xs[4], ys[4], vs[4]; int idx=0; for(int y=0;y<h;y++) for(int x=0;x<w;x++){ xs[idx]=(double)x*0.01; ys[idx]=(double)y*0.01; idx++; if(idx==4){ value_noise4(xs,ys,vs); for(int i=0;i<4;i++){ /* approximate fbm by repeated value_noise to keep path deterministic */ double acc=0, amp=1, freq=1; for(int o=0;o<oct;o++){ acc += value_noise(xs[i]*freq, ys[i]*freq)*amp; freq*=lac; amp*=gain; } double v= acc; sum += v; } idx=0; } }
 /* tail */ if(idx>0){ for(int i=0;i<idx;i++){ double acc=0, amp=1, freq=1; for(int o=0;o<oct;o++){ acc += value_noise(xs[i]*freq, ys[i]*freq)*amp; freq*=lac; amp*=gain; } sum += acc; } }
