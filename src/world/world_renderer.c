...
void rogue_world_render_tiles(void)
{
    ...
    int first_tx = (int) (g_app.cam_x / tsz);
    if (first_tx < 0)
        first_tx = 0;
    int first_ty = (int) (g_app.cam_y / tsz);
    if (first_ty < 0)
        first_ty = 0;
    ...
    if (g_app.tileset_loaded)
    {
        for (int y = first_ty; y < last_ty; y++)
        {
            ...
            while (x < last_tx)
            {
                ...
                while (x + run < last_tx)
                {
                    const RogueSprite* spr2 =
                        g_app.tile_sprite_lut_ready
                            ? g_app.tile_sprite_lut[y * g_app.world_map.width + (x + run)]
                            : NULL;
                    if (spr2 != spr)
                        break;
                    run++;
                ...
                for (int i = 0; i < run; i++)
                {
                    SDL_Rect dst = {(int) ((x + i) * tsz - g_app.cam_x),
                                    (int) (y * tsz - g_app.cam_y), tsz * scale, tsz * scale};
                    SDL_RenderCopy(g_app.renderer, spr->tex->handle, &src, &dst);
                }
#else
                for (int i = 0; i < run; i++)
                    rogue_sprite_draw(spr, (int) ((x + i) * tsz - g_app.cam_x),
                                      (int) (y * tsz - g_app.cam_y), scale);
#endif
                x += run;
            }
        }
    ...
    else
    {
        ...
        for (int y = first_ty; y < last_ty; y++)
        {
            ...
            {
                unsigned char t = g_app.world_map.tiles[y * g_app.world_map.width + x];
                RogueColor c = {(unsigned char) (t * 20), (unsigned char) (t * 15),
                                (unsigned char) (t * 10), 255};
                SDL_SetRenderDrawColor(g_app.renderer, c.r, c.g, c.b, c.a);
                SDL_Rect r = {(int) (x * tsz - g_app.cam_x), (int) (y * tsz - g_app.cam_y),
                              tsz * scale, tsz * scale};
...
void rogue_world_render_items(void)
{
    ...
    for (int i = 0; i < g_app.item_instance_cap; i++)
        if (g_app.item_instances[i].active)
        {
            ...
            if (it->hidden_filter)
                continue; /* 12.2 hide filtered items */
            RogueRarityColor c = rogue_rarity_color((RogueItemRarity) it->rarity);
            SDL_SetRenderDrawColor(g_app.renderer, c.r, c.g, c.b, c.a);
            SDL_Rect rect_ri = {(int) (it->x * ts - g_app.cam_x), (int) (it->y * ts - g_app.cam_y),
                                ts / 2, ts / 2};
