...
void wg_generate_base(RogueTileMap* map, const RogueWorldGenConfig* cfg)
{
    if (cfg->advanced_terrain)
    {
        ...
        {
            free(elev_field_cache);
            free(moist_field_cache);
        ...
        for (int y = 0; y < map->height; y++)
            for (int x = 0; x < map->width; x++)
            {
                ...
                if (elev < water_level)
                {
                    t = (rng_norm() < 0.05) ? ROGUE_TILE_RIVER : ROGUE_TILE_WATER;
                }
                else
                {
                    ...
                    if (e < 0.04)
                    {
                        t = (moist > 0.60) ? ROGUE_TILE_SWAMP : ROGUE_TILE_GRASS;
                    }
                    else if (e < 0.16)
                    {
                        t = (moist > 0.55) ? ROGUE_TILE_FOREST : ROGUE_TILE_GRASS;
                    }
                    else if (e < 0.30)
                    {
                        t = (moist > 0.70) ? ROGUE_TILE_FOREST : ROGUE_TILE_MOUNTAIN;
                    }
                    else if (e < 0.48)
                    {
                        t = (moist < 0.35) ? ROGUE_TILE_SNOW : ROGUE_TILE_MOUNTAIN;
                    }
                    else
                    {
                        t = (moist < 0.45) ? ROGUE_TILE_SNOW : ROGUE_TILE_MOUNTAIN;
                    }
                }
    ...
    {
        seeds[i].x = rng_range(0, map->width - 1);
        seeds[i].y = rng_range(0, map->height - 1);
        seeds[i].base = pick_biome();
    }
    for (int y = 0; y < map->height; y++)
        for (int x = 0; x < map->width; x++)
        {
            ...
            for (int i = 0; i < nseeds; i++)
            {
                double dx = (double) (x - seeds[i].x);
                double dy = (double) (y - seeds[i].y);
