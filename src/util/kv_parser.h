...
typedef struct RogueKVEntry { const char* key; const char* value; int line; } RogueKVEntry;
typedef struct RogueKVError { int line; const char* message; } RogueKVError;
...
/* Load entire file into memory (returns 0 on failure). On success, kv->data points to malloc'd buffer. */
