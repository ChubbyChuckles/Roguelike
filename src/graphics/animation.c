...
/* Minimal JSON parsing for Aseprite frame list: we look for substrings "frame":{..."x":N,"y":M,"w":W,"h":H} and "duration":D */
...
static int parse_int_after(const char* start, const char* key, int* out){
    ...
    if(!p) return 0; p += strlen(key);
    while(*p==' '||*p=='"'||*p==':' ) p++;
    *out = atoi(p); return 1;
}

bool rogue_animation_load(RogueAnimation* anim, const char* png_path, const char* json_path, int fw, int fh){
    memset(anim,0,sizeof *anim);
    if(!rogue_texture_load(&anim->texture, png_path)) return false;
    if(json_path){
    FILE* f = NULL;
#if defined(_MSC_VER)
...
const RogueAnimFrame* rogue_animation_sample(const RogueAnimation* anim, int elapsed_ms){
    if(!anim->frame_count) return NULL;
    if(anim->total_duration_ms <= 0) return &anim->frames[0];
