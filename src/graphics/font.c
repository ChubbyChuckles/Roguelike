...
#ifdef ROGUE_HAVE_SDL
    if(!g_internal_sdl_renderer_ref || !text) return;
    if(scale < 1) scale = 1;
        ...
        for(int row=0; row<gh; ++row){
            ...
            for(int col=0; col<gw; ++col){
                if(bits & (1 << (gw-1-col))){
                    ...
                    SDL_SetRenderDrawColor(g_internal_sdl_renderer_ref, color.r, color.g, color.b, color.a);
...
#else
    (void)x; (void)y; (void)text; (void)scale; (void)color;
#endif
}
