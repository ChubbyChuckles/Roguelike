...
void rogue_tile_normalize_path(char* p){ while(*p){ if(*p=='\\') *p='/'; p++; } }
...
void rogue_tile_sprite_define(RogueTileType type, const char* path, int col, int row){
    if(!g_tile_sprites.initialized || type<0 || type>=ROGUE_TILE_MAX || !path) return;
...
bool rogue_tile_sprites_load_config(const char* path){
    if(!g_tile_sprites.initialized) return false;
    FILE* f = NULL;
#if defined(_MSC_VER)
    fopen_s(&f, path, "rb");
#else
    ...
    char line[1024]; int lineno=0; int added=0;
    while(fgets(line, sizeof line, f)){
        ...
        while(1){
            ...
            segment +=4; while(*segment==' '||*segment=='\t') segment++; if(*segment==',') segment++; while(*segment==' '||*segment=='\t') segment++;
            char name_tok[64]; int name_len=0; while(segment[name_len] && segment[name_len]!=',' && segment[name_len]!='\n' && name_len<63) name_len++; if(segment[name_len] != ','){ break; }
            memcpy(name_tok, segment, name_len); name_tok[name_len]='\0'; segment += name_len + 1; while(*segment==' '||*segment=='\t') segment++;
            char sheet_path[256]; int path_len=0; while(segment[path_len] && segment[path_len]!=',' && segment[path_len]!='\n' && path_len<255) path_len++; if(segment[path_len] != ','){ break; }
            memcpy(sheet_path, segment, path_len); sheet_path[path_len]='\0'; segment += path_len + 1; while(*segment==' '||*segment=='\t') segment++;
            int col = atoi(segment); while(*segment && *segment!=',' && *segment!='\n') segment++; if(*segment!=',') break; segment++; while(*segment==' '||*segment=='\t') segment++;
            int row = atoi(segment); while(*segment && *segment!=' ' && *segment!='\t' && *segment!='\n') segment++;
            rogue_tile_normalize_path(sheet_path);
            if(name_len>0 && path_len>0){ RogueTileType t = name_to_type(name_tok); rogue_tile_sprite_define(t, sheet_path, col, row); added++; }
            char* next = strstr(segment, "TILE"); if(!next) break; else segment = next;
        }
    }
    fclose(f);
...
bool rogue_tile_sprites_finalize(void){
    if(g_tile_sprites.finalized) return true;
    int loaded_any = 0; int failed = 0;
        ...
        for(int i=0;i<b->count;i++){
            ...
            if(!v->loaded){
                if(!rogue_texture_load(&v->texture, v->path)){
                    ROGUE_LOG_WARN("tile texture load fail: %s (tile=%d variant=%d)", v->path, t, i); failed++; continue; }
                v->sprite.tex = &v->texture; v->sprite.sx = v->col * g_tile_sprites.tile_size; v->sprite.sy = v->row * g_tile_sprites.tile_size; v->sprite.sw = g_tile_sprites.tile_size; v->sprite.sh = g_tile_sprites.tile_size; v->loaded = 1; loaded_any++;
            }
        }
    }
    if(loaded_any>0 && failed>0){ ROGUE_LOG_INFO("Tile sprites finalize: %d variants loaded, %d failed (partial success)", loaded_any, failed); }
    else if(loaded_any>0){ ROGUE_LOG_INFO("Tile sprites finalize: %d variants loaded (all successful)", loaded_any); }
    else { ROGUE_LOG_WARN("Tile sprites finalize: all %d variants failed to load", failed); }
...
void rogue_tile_sprites_shutdown(void){
    for(int t=0;t<ROGUE_TILE_MAX;t++){
        TileBucket* b=&g_tile_sprites.buckets[t];
        for(int i=0;i<b->count;i++) if(b->variants[i].loaded){ rogue_texture_destroy(&b->variants[i].texture); b->variants[i].loaded=0; }
