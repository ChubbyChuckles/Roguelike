...
typedef struct RogueMinimapPing { float x,y; float life_ms; int active; int rarity; } RogueMinimapPing;
...
int rogue_minimap_ping_loot(float x, float y, int rarity){
    ...
    for(int i=0;i<ROGUE_MINIMAP_PING_CAP;i++) if(!g_pings[i].active){
        g_pings[i].x=x; g_pings[i].y=y; g_pings[i].life_ms=0; g_pings[i].active=1; g_pings[i].rarity=rarity; return i;
    }
    return -1; /* full */
}
...
void rogue_minimap_pings_update(float dt_ms){
    for(int i=0;i<ROGUE_MINIMAP_PING_CAP;i++) if(g_pings[i].active){
        g_pings[i].life_ms += dt_ms; if(g_pings[i].life_ms >= g_ping_lifetime_ms) g_pings[i].active=0;
    }
}

int rogue_minimap_pings_active_count(void){ int c=0; for(int i=0;i<ROGUE_MINIMAP_PING_CAP;i++) if(g_pings[i].active) c++; return c; }
...
#ifdef ROGUE_HAVE_SDL
    if(!g_app.renderer) return; if(!g_app.world_map.width||!g_app.world_map.height) return;
    for(int i=0;i<ROGUE_MINIMAP_PING_CAP;i++) if(g_pings[i].active){
        ...
        float t = g_pings[i].life_ms / g_ping_lifetime_ms; if(t>1) t=1; float fade = (t<0.8f)?1.0f:(1.0f - (t-0.8f)/0.2f); if(fade<0) fade=0; 
        SDL_SetRenderDrawColor(g_app.renderer, c.r, c.g, c.b, (Uint8)(200*fade));
        SDL_Rect r={px-1,py-1,3,3}; SDL_RenderFillRect(g_app.renderer,&r); g_app.frame_draw_calls++;
    }
#else
    (void)mm_x_off; (void)mm_y_off; (void)mm_w; (void)mm_h;
#endif
}
