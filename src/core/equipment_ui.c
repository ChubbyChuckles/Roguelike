...
#include "core/equipment_stats.h" /* for runeword/set already aggregated; we only read cache */
...
static void compute_primary_deltas(int inst_index, int compare_slot, int* d_str,int* d_dex,int* d_vit,int* d_int,int* d_armor,int* d_res_phys){
    if(d_str)*d_str=0; if(d_dex)*d_dex=0; if(d_vit)*d_vit=0; if(d_int)*d_int=0; if(d_armor)*d_armor=0; if(d_res_phys)*d_res_phys=0;
    if(compare_slot<0) return; int equipped=rogue_equip_get((enum RogueEquipSlot)compare_slot); if(equipped<0) return;
    /* We simulate equipping candidate by temporarily swapping and recomputing stat cache. For determinism we snapshot+restore. */
    extern RoguePlayer g_exposed_player_for_stats; /* used by optimizer & tests */
    int original_inst = equipped;
    int cur_str=g_player_stat_cache.total_strength, cur_dex=g_player_stat_cache.total_dexterity, cur_vit=g_player_stat_cache.total_vitality, cur_int=g_player_stat_cache.total_intelligence; int cur_phys=g_player_stat_cache.resist_physical; int cur_armor=g_player_stat_cache.affix_armor_flat; /* armor flat aggregated */
    /* Equip candidate */
    rogue_equip_try((enum RogueEquipSlot)compare_slot, inst_index);
    rogue_stat_cache_mark_dirty(); rogue_equipment_apply_stat_bonuses(&g_exposed_player_for_stats); rogue_stat_cache_force_update(&g_exposed_player_for_stats);
    int cand_str=g_player_stat_cache.total_strength, cand_dex=g_player_stat_cache.total_dexterity, cand_vit=g_player_stat_cache.total_vitality, cand_int=g_player_stat_cache.total_intelligence; int cand_phys=g_player_stat_cache.resist_physical; int cand_armor=g_player_stat_cache.affix_armor_flat;
    /* Revert */
    rogue_equip_try((enum RogueEquipSlot)compare_slot, original_inst);
    rogue_stat_cache_mark_dirty(); rogue_equipment_apply_stat_bonuses(&g_exposed_player_for_stats); rogue_stat_cache_force_update(&g_exposed_player_for_stats);
    if(d_str)*d_str=cand_str-cur_str; if(d_dex)*d_dex=cand_dex-cur_dex; if(d_vit)*d_vit=cand_vit-cur_vit; if(d_int)*d_int=cand_int-cur_int; if(d_res_phys)*d_res_phys=cand_phys-cur_phys; if(d_armor)*d_armor=cand_armor-cur_armor;
}

char* rogue_equipment_compare_deltas(int inst_index, int compare_slot, char* buf, size_t cap){ if(!buf||cap<4) return NULL; buf[0]='\0'; if(compare_slot<0) return buf; int equipped=rogue_equip_get((enum RogueEquipSlot)compare_slot); if(equipped<0) return buf; int cand_min=rogue_item_instance_damage_min(inst_index); int cand_max=rogue_item_instance_damage_max(inst_index); int cur_min=rogue_item_instance_damage_min(equipped); int cur_max=rogue_item_instance_damage_max(equipped); int dmin=cand_min-cur_min; int dmax=cand_max-cur_max; int d_str,d_dex,d_vit,d_int,d_arm,d_rphys; compute_primary_deltas(inst_index,compare_slot,&d_str,&d_dex,&d_vit,&d_int,&d_arm,&d_rphys); snprintf(buf,cap,"Delta Damage: %+d-%+d\nStr:%+d Dex:%+d Vit:%+d Int:%+d Armor:%+d PhysRes:%+d\n", dmin,dmax,d_str,d_dex,d_vit,d_int,d_arm,d_rphys); return buf; }
...
char* rogue_item_tooltip_build_layered(int inst_index, int compare_slot, char* buf, size_t cap){ if(!buf||cap<8) return NULL; char base[512]; rogue_item_tooltip_build(inst_index, base, sizeof base); /* base builder */ size_t off=0; int n=snprintf(buf+off,cap-off,"%s", base); if(n<0||n>= (int)(cap-off)) return buf; off+=n; const RogueItemInstance* it=rogue_item_instance_at(inst_index); const RogueItemDef* d = it? rogue_item_def_at(it->def_index):NULL; if(d && d->base_armor>0){ n=snprintf(buf+off,cap-off,"Implicit: +%d Armor\n", d->base_armor); if(n>0 && n < (int)(cap-off)) off+=n; }
    ...
    /* Runeword indicator: we approximate by hashing def id into a faux pattern label if runeword stats present (cache layer). */
    if(d){ /* If runeword layer contributed any primary stat for this item, show generic runeword line. */
        if(g_player_stat_cache.runeword_strength||g_player_stat_cache.runeword_dexterity||g_player_stat_cache.runeword_vitality||g_player_stat_cache.runeword_intelligence){ n=snprintf(buf+off,cap-off,"Runeword: active\n"); if(n>0 && n < (int)(cap-off)) off+=n; }
    }
    if(compare_slot>=0){ char delta[256]; rogue_equipment_compare_deltas(inst_index, compare_slot, delta, sizeof delta); n=snprintf(buf+off,cap-off,"%s", delta); if(n>0 && n < (int)(cap-off)) off+=n; }
    if(off<cap) buf[off]='\0'; return buf; }
...
char* rogue_equipment_gem_inventory_panel(char* buf, size_t cap){ if(!buf||cap<8) return NULL; size_t off=0; int n=snprintf(buf+off,cap-off,"[Gem Inventory]\n"); if(n<0||n>=(int)(cap-off)) return buf; off+=n; if(g_socket_sel_inst>=0){ n=snprintf(buf+off,cap-off,"Selected: inst=%d socket=%d\n", g_socket_sel_inst,g_socket_sel_index); if(n>0&&n<(int)(cap-off)) off+=n; }
    /* Stub: list first 5 gem item defs encountered among equipped sockets. */
    int listed=0; for(int slot=0;slot<ROGUE_EQUIP__COUNT && listed<5; ++slot){ int inst=rogue_equip_get((enum RogueEquipSlot)slot); if(inst<0) continue; const RogueItemInstance* it=rogue_item_instance_at(inst); if(!it) continue; for(int s=0;s<it->socket_count && listed<5; ++s){ int gem=rogue_item_instance_get_socket(inst,s); if(gem>=0){ n=snprintf(buf+off,cap-off,"GemDef:%d (slot %d)\n", gem, s); if(n<0||n>=(int)(cap-off)) break; off+=n; listed++; } } }
    if(off<cap) buf[off]='\0'; return buf; }

float rogue_equipment_softcap_saturation(float value, float cap, float softness){ if(cap<=0.f||softness<=0.f) return 0.f; float adjusted = rogue_soft_cap_apply(value, cap, softness); return fminf(1.f, adjusted / cap); }
