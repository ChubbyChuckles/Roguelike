...
static int apply_quality_scale(int base, int quality){ if(quality<=0) return base; if(quality>20) quality=20; /* linear 0..20 -> up to +10% */ int scaled = (int)(base * (1.0f + (quality * 0.005f))); if(scaled<base) return base; return scaled; }
int rogue_item_instance_damage_min(int inst_index){ const RogueItemInstance* it = rogue_item_instance_at(inst_index); if(!it) return 0; const RogueItemDef* d = rogue_item_def_at(it->def_index); int base = d? d->base_damage_min:0; base = apply_quality_scale(base,it->quality); int val = base + affix_damage_bonus(it); if(it->fractured) val = (int)(val * 0.6f); return val; }
int rogue_item_instance_damage_max(int inst_index){ const RogueItemInstance* it = rogue_item_instance_at(inst_index); if(!it) return 0; const RogueItemDef* d = rogue_item_def_at(it->def_index); int base = d? d->base_damage_max:0; base = apply_quality_scale(base,it->quality); int val = base + affix_damage_bonus(it); if(it->fractured) val = (int)(val * 0.6f); return val; }
