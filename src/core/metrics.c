...
void rogue_metrics_reset(void){
    ...
    g_app.session_items_dropped = 0; g_app.session_items_picked = 0;
    for(int i=0;i<5;i++){ g_app.session_rarity_drops[i]=0; }
}
...
void rogue_metrics_record_drop(int rarity){ g_app.session_items_dropped++; if(rarity>=0 && rarity<5) g_app.session_rarity_drops[rarity]++; }
void rogue_metrics_record_pickup(int rarity){ g_app.session_items_picked++; (void)rarity; }
void rogue_metrics_rates(double* out_items_per_hour, double out_rarity_per_hour[5]){
    double elapsed = rogue_metrics_session_elapsed(); if(elapsed <= 0.0) elapsed = 1.0;
    double hours = elapsed / 3600.0;
    if(out_items_per_hour) *out_items_per_hour = g_app.session_items_dropped / hours;
    if(out_rarity_per_hour){ for(int i=0;i<5;i++){ out_rarity_per_hour[i] = g_app.session_rarity_drops[i] / hours; } }
}
