...
static inline void rogue_loot_log_set_level(int lvl){ if(lvl<0) lvl=0; if(lvl>2) lvl=2; g_rogue_loot_log_level = lvl; }
...
#define ROGUE_LOOT_LOG_DEBUG(fmt, ...) do { if(g_rogue_loot_log_level >= 2) { ROGUE_LOG_DEBUG(fmt, ##__VA_ARGS__); } } while(0)
#define ROGUE_LOOT_LOG_INFO(fmt, ...)  do { if(g_rogue_loot_log_level >= 1) { ROGUE_LOG_INFO(fmt, ##__VA_ARGS__); } } while(0)
