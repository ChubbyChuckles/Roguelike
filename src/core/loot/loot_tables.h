...
/* Extended variant supplying rarities (optional). If out_rarities provided entries will be filled
 * with rolled rarity or -1 for default. */
...
static inline unsigned int rogue_rng_next(unsigned int* s)
{
    *s = (*s * 1664525u) + 1013904223u;
    return *s;
}
static inline int rogue_rng_range(unsigned int* s, int hi_exclusive)
{
    return (int) (rogue_rng_next(s) % (unsigned) hi_exclusive);
}
