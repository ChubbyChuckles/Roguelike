...
void rogue_loot_analytics_set_baseline_counts(const int counts[5]); /* derives baseline fractions from expected relative counts */
void rogue_loot_analytics_set_baseline_fractions(const float fracs[5]); /* alternative explicit fractions (not required to sum to 1; auto-normalized) */
void rogue_loot_analytics_set_drift_threshold(float rel_fraction); /* default 0.5 (50% relative deviation) */
...
void rogue_loot_analytics_record_pos(int def_index, int rarity, double t_seconds, int x, int y); /* records event + position (also updates baseline metrics) */
int  rogue_loot_analytics_heat_at(int x, int y); /* returns count (out of bounds -> 0) */
int  rogue_loot_analytics_export_heatmap_csv(char* buf, int cap); /* minimal CSV export: first line header, subsequent rows y=0..H-1 */
