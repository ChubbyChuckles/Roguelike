                        ...
                        unsigned int* rng_state, RogueGeneratedItem* out)
{
    ...
    int rarity = rar_arr[0];
    if (rarity < 0)
    {
        const RogueItemDef* d = rogue_item_def_at(out->def_index);
        rarity = d ? d->rarity : 0;
    }
    /* Apply rarity floor from context enemy level (simple mapping: every 10 levels raise floor by 1
     * up to 2) */
    if (ctx)
    {
        int level_floor = ctx->enemy_level / 10;
        if (level_floor > 2)
            level_floor = 2;
        if (level_floor > 0)
        {
            if (rarity < level_floor)
                rarity = level_floor;
        }
    }
    /* Apply min floor global */
    int global_floor = rogue_rarity_get_min_floor();
    if (global_floor >= 0 && rarity < global_floor)
        rarity = global_floor;
    out->rarity = rarity;
    /* Spawn instance */
    int inst = rogue_items_spawn(out->def_index, qty_arr[0], 0.0f, 0.0f);
    if (inst >= 0)
    {
        out->inst_index = inst;
        unsigned int affix_seed = local ^ 0xA5A5A5A5u;
        ...
        if (it)
        {
            ...
            if (rarity >= 2)
            {
                ...
                {
                    want_prefix = ((affix_seed) & 1) == 0;
                    want_suffix = !want_prefix;
                }
            }
            int prefix_index = -1, suffix_index = -1;
            int prefix_value = 0, suffix_value = 0;
            if (want_prefix || want_suffix)
            {
                rogue_loot_perf_affix_roll_begin();
            }
            if (want_prefix)
            {
                prefix_index = rogue_generation_gated_affix_roll(ROGUE_AFFIX_PREFIX, rarity,
                                                                 &affix_seed, base_def, -1, -1);
                if (prefix_index >= 0)
                {
                    float qscalar = g_quality_scalar_min;
                    if (ctx)
                    {
                        float luck = (float) ctx->player_luck;
                        float t = luck / (5.0f + luck);
                        qscalar = g_quality_scalar_min +
                                  (g_quality_scalar_max - g_quality_scalar_min) * t;
                    }
                    prefix_value =
                        rogue_affix_roll_value_scaled(prefix_index, &affix_seed, qscalar);
                }
            }
            if (want_suffix)
            {
                suffix_index = rogue_generation_gated_affix_roll(
                    ROGUE_AFFIX_SUFFIX, rarity, &affix_seed, base_def, prefix_index, -1);
                if (suffix_index >= 0)
                {
                    float qscalar = g_quality_scalar_min;
                    if (ctx)
                    ...
                    suffix_value =
                        rogue_affix_roll_value_scaled(suffix_index, &affix_seed, qscalar);
                }
            }
            if (want_prefix || want_suffix)
            {
                rogue_loot_perf_affix_roll_end();
            }
            it->rarity = rarity;
