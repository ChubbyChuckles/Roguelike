...
static void rtrim(char* s){ size_t n=strlen(s); while(n>0){ char c=s[n-1]; if(c=='\r'||c=='\n'||c==' '||c=='\t'){ s[--n]='\0'; } else break; } }
...
int rogue_item_defs_convert_tsv_to_csv(const char* tsv_path, const char* out_csv_path){
    if(!tsv_path||!out_csv_path) return -1;
    FILE* fin=NULL; FILE* fout=NULL; int converted=0;
#if defined(_MSC_VER)
