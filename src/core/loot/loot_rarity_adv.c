...
static int rarity_effective_epic_threshold(void){
    int base = g_pity_epic_threshold; if(base<=0) return base;
    if(!g_pity_accel_enabled) return base;
    /* Simple acceleration: once counter passes half the base threshold, reduce remaining requirement by 25% */
    if(g_pity_counter > base/2){ int reduced = (base * 3) / 4; if(reduced < 1) reduced = 1; return reduced; }
    return base;
}
static int rarity_effective_legendary_threshold(void){
    int base = g_pity_legendary_threshold; if(base<=0) return base;
    if(!g_pity_accel_enabled) return base;
    if(g_pity_counter > base/2){ int reduced = (base * 3) / 4; if(reduced < 1) reduced = 1; return reduced; }
    return base;
}
...
int  rogue_rarity_pity_get_effective_legendary(void){ return rarity_effective_legendary_threshold(); }
...
int rogue_rarity_apply_pity(int rolled, int rmin, int rmax){
    ...
    if(eff_leg>0 && g_pity_counter >= eff_leg && rmax>=4){ target = 4; g_pity_counter=0; }
    else if(eff_epic>0 && g_pity_counter >= eff_epic && rmax>=3){ target = 3; g_pity_counter=0; }
