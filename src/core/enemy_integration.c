...
int rogue_enemy_integration_build_display_info(const struct RogueEnemy* enemy, int player_level, RogueEnemyDisplayInfo* out_info) {
    if(!enemy || !out_info) return 0;
    ...
    for(int i = 0; i < enemy->modifier_count && i < 8; i++) {
        ...
        snprintf(out_info->modifier_tags[i], sizeof(out_info->modifier_tags[i]), "M%d", enemy->modifier_ids[i]);
    }
    
    /* Color coding */
    rogue_enemy_integration_get_type_color(enemy, &out_info->color_r, &out_info->color_g, &out_info->color_b);
...
int rogue_enemy_integration_update_hud_target(const struct RogueEnemy* target_enemy, int player_level) {
...
void rogue_enemy_integration_get_type_color(const struct RogueEnemy* enemy, unsigned char* out_r, unsigned char* out_g, unsigned char* out_b) {
    if(!enemy || !out_r || !out_g || !out_b) return;
    ...
    *out_r = 255; *out_g = 255; *out_b = 255;
    ...
    if(enemy->boss_flag) {
        /* Boss - red/orange */
        *out_r = 255; *out_g = 140; *out_b = 0;
    } else if(enemy->elite_flag) {
        /* Elite - yellow */
        *out_r = 255; *out_g = 215; *out_b = 0;
    } else if(enemy->support_flag) {
        /* Support - cyan */
        *out_r = 0; *out_g = 191; *out_b = 255;
    }
...
const char* rogue_enemy_integration_get_modifier_telegraph(int modifier_id) {
    ...
    switch(modifier_id) {
        case 1: return "speed_aura";
        case 2: return "defense_aura";
        case 3: return "rage_aura";
        default: return "modifier_aura";
    }
}
...
int rogue_enemy_integration_solve_spawn_positions(const RogueRoomEncounterInfo* encounter_info, 
    ...
    if(!encounter_info || !room_dims || !out_solution) return 0;
    ...
    if(estimated_unit_count > 16) estimated_unit_count = 16;
    if(estimated_unit_count < 1) estimated_unit_count = 1;
    ...
    if(has_boss && spawn_index < 16) {
        ...
        if(rogue_enemy_integration_validate_spawn_position(center_x, center_y, room_dims, out_solution, spawn_index)) {
    ...
    for(int unit = (has_boss ? 1 : 0); unit < estimated_unit_count && spawn_index < 16; unit++) {
        ...
        for(int attempt = 0; attempt < max_attempts && !placed; attempt++) {
            ...
            float padding = 1.0f;
            float x = room_dims->min_x + padding + (rand() % 1000) / 1000.0f * (room_dims->max_x - room_dims->min_x - 2*padding);
            float y = room_dims->min_y + padding + (rand() % 1000) / 1000.0f * (room_dims->max_y - room_dims->min_y - 2*padding);
            
            if(rogue_enemy_integration_validate_spawn_position(x, y, room_dims, out_solution, spawn_index)) {
...
int rogue_enemy_integration_validate_spawn_position(float x, float y, const RogueRoomDimensions* room_dims, 
                                                    const RogueSpawnSolution* existing_positions, int check_count) {
    if(!room_dims) return 0;
    
    /* Check room bounds */
    if(x < room_dims->min_x || x > room_dims->max_x || y < room_dims->min_y || y > room_dims->max_y) {
        return 0;
    }
...
int rogue_enemy_integration_register_navmesh_handles(const RogueSpawnSolution* solution, 
                                                     const struct RogueEnemy* enemies, int enemy_count) {
    if(!solution || !enemies) return 0;
...
int rogue_enemy_integration_find_nearest_enemy(const RogueEnemyRegistry* registry, 
                                               ...
                                               float max_distance, 
                                               int* out_enemy_id) {
    if(!registry || !position || !out_enemy_id) return 0;
...
int rogue_enemy_integration_register_enemy(RogueEnemyRegistry* registry,
                                           int room_id,
                                           int encounter_id, 
                                           float position[2],
                                           const RogueEnemyDisplayInfo* display_info) {
    if(!registry || !position || !display_info) return -1;
    if(registry->count >= MAX_REGISTERED_ENEMIES) return -1;
...
void rogue_enemy_integration_update_enemy_position(RogueEnemyRegistry* registry,
                                                   int enemy_id,
                                                   float position[2]) {
    if(!registry || !position) return;
...
int rogue_enemy_integration_get_enemy_display_info(const RogueEnemyRegistry* registry,
    ...
    if(!registry || !out_display_info) return 0;
...
void rogue_enemy_integration_mark_enemy_dead(RogueEnemyRegistry* registry,
                                             int enemy_id) {
    if(!registry) return;
...
int rogue_enemy_integration_get_combat_stats(const RogueEnemyRegistry* registry,
                                             int enemy_id,
                                             RogueEnemyCombatStats* out_stats) {
    if(!registry || !out_stats) return 0;
    ...
    for(int i = 0; i < registry->count; i++) {
        ...
        if(entry->enemy_id == enemy_id && entry->is_alive) {
            ...
            out_stats->current_health = entry->display_info.final_stats.hp;  /* Full health initially */
...
int rogue_enemy_integration_apply_damage(RogueEnemyRegistry* registry,
                                         int enemy_id,
                                         float damage,
                                         int damage_type) {
    if(!registry || damage < 0.0f) return 0;
    ...
    for(int i = 0; i < registry->count; i++) {
        ...
        if(entry->enemy_id == enemy_id && entry->is_alive) {
            ...
            switch(damage_type) {
                case 1: effective_damage *= (1.0f - 0.0f); break; /* fire resist */
                case 2: effective_damage *= (1.0f - 0.0f); break; /* cold resist */
                case 3: effective_damage *= (1.0f - 0.0f); break; /* lightning resist */
                case 4: effective_damage *= (1.0f - 0.0f); break; /* poison resist */
                default: /* physical damage, no resist applied */ break;
            }
