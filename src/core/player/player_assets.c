...
static int state_name_to_index(const char* s)
{
    if (strcmp(s, "idle") == 0)
        return 0;
    if (strcmp(s, "walk") == 0)
...
static void load_player_sheet_paths(const char* path)
{
    ...
    if (!f)
    {
        ...
        for (size_t i = 0; i < sizeof(prefixes) / sizeof(prefixes[0]) && !f; i++)
        {
            ...
            if (f)
            {
                ROGUE_LOG_INFO("Opened player sheet config via fallback path: %s", attempt);
                break;
            }
        }
    }
    if (!f)
    {
        ROGUE_LOG_WARN("player sheet config open failed: %s", path);
        return;
    }
    char line[512];
    while (fgets(line, sizeof line, f))
    {
        ...
        path_tok[pi] = '\0';
        for (int k = (int) strlen(st_tok) - 1;
             k >= 0 && (st_tok[k] == '\r' || st_tok[k] == ' ' || st_tok[k] == '\t'); k--)
            st_tok[k] = '\0';
        for (int k = (int) strlen(dir_tok) - 1;
             k >= 0 && (dir_tok[k] == '\r' || dir_tok[k] == ' ' || dir_tok[k] == '\t'); k--)
            dir_tok[k] = '\0';
        for (int k = (int) strlen(path_tok) - 1;
             k >= 0 && (path_tok[k] == '\r' || path_tok[k] == ' ' || path_tok[k] == '\t'); k--)
            path_tok[k] = '\0';
        ...
        if (sidx < 0 || didx < 0)
            continue;
#if defined(_MSC_VER)
        strncpy_s(g_app.player_sheet_path[sidx][didx], sizeof g_app.player_sheet_path[sidx][didx],
                  path_tok, _TRUNCATE);
#else
        strncpy(g_app.player_sheet_path[sidx][didx], path_tok,
                sizeof g_app.player_sheet_path[sidx][didx] - 1);
        g_app.player_sheet_path[sidx][didx][sizeof g_app.player_sheet_path[sidx][didx] - 1] = '\0';
#endif
        if (strcmp(dir_tok, "side") == 0)
        {
#if defined(_MSC_VER)
            strncpy_s(g_app.player_sheet_path[sidx][2], sizeof g_app.player_sheet_path[sidx][2],
                      path_tok, _TRUNCATE);
#else
            strncpy(g_app.player_sheet_path[sidx][2], path_tok,
                    sizeof g_app.player_sheet_path[sidx][2] - 1);
    ...
    if (loaded > 0)
    {
        g_app.player_sheet_paths_loaded = 1;
        ROGUE_LOG_INFO("player sheet config loaded %d entries", loaded);
    }
}
...
static void load_player_anim_config(const char* path)
{
    ...
    while (fgets(line, sizeof line, f))
    {
        ...
        int s = -1;
        if (strcmp(act, "idle") == 0)
            s = 0;
        else if (strcmp(act, "walk") == 0)
            s = 1;
        ...
        if (tcount == 0)
            continue;
        for (int i = 0; i < tcount && i < 8; i++)
            g_app.player_frame_time_ms[s][d][i] = times[i];
    }
    fclose(f);
}
        ...
        for (int s = 0; s < 4; s++)
            for (int d = 0; d < 4; d++)
                if (!g_app.player_sheet_path[s][d][0])
                {
                    ...
                    snprintf(defp, sizeof defp, "assets/character/%s_%s.png", state_names[s],
                             use_dir);
#if defined(_MSC_VER)
                    strncpy_s(g_app.player_sheet_path[s][d], sizeof g_app.player_sheet_path[s][d],
                              defp, _TRUNCATE);
#else
                    strncpy(g_app.player_sheet_path[s][d], defp,
                            sizeof g_app.player_sheet_path[s][d] - 1);
    ...
    for (int s = 0; s < 4; s++)
        for (int d = 0; d < 4; d++)
        {
            ...
            if (rogue_texture_load(&g_app.player_tex[s][d], path))
            {
                ...
                {
                    ROGUE_LOG_INFO("Auto-adjust player frame size from %d to %d (sheet: %s)",
                                   g_app.player_frame_size, texh, path);
                    g_app.player_frame_size = texh;
                }
                ...
                {
                    ROGUE_LOG_WARN(
                        "Player sheet width %d < frame_size %d; forcing single frame: %s", texw,
                        g_app.player_frame_size, path);
                    frames = 1;
                }
                ...
                for (int f = 0; f < frames; f++)
                {
                    ...
                    if (remaining < g_app.player_frame_size)
                        remaining = (remaining > 0) ? remaining : g_app.player_frame_size;
                    g_app.player_frames[s][d][f].sw =
                        (remaining > g_app.player_frame_size) ? g_app.player_frame_size : remaining;
                    g_app.player_frames[s][d][f].sh =
                        (texh < g_app.player_frame_size) ? texh : g_app.player_frame_size;
                    int base = (s == 0) ? 160 : (s == 2 ? 90 : 120);
                    g_app.player_frame_time_ms[s][d][f] = base;
                }
                for (int f = g_app.player_frame_count[s][d]; f < 8; f++)
                {
                    g_app.player_frames[s][d][f].tex = &g_app.player_tex[s][d];
...
{
    (void) raw_dt_ms;
    (void) frame_dt_ms; /* frame_dt_ms currently unused after refactor */
    ...
    int kills = rogue_combat_player_strike(&g_app.player_combat, &g_app.player, g_app.enemies,
                                           g_app.enemy_count);
    if (kills > 0)
    {
        g_app.total_kills += kills;
        g_app.player.xp += kills * (3 + g_app.player.level);
    }

    /* Advance animation frames */
    int state = g_app.player_state;
    if (g_app.player_combat.phase == ROGUE_ATTACK_WINDUP ||
        g_app.player_combat.phase == ROGUE_ATTACK_STRIKE ||
        g_app.player_combat.phase == ROGUE_ATTACK_RECOVER)
        state = 3;
    int dir = g_app.player.facing;
    int sheet_dir = (dir == 1 || dir == 2) ? 1 : dir; /* map left/right to side index */
    int frame_count = g_app.player_frame_count[state][sheet_dir];
    if (frame_count <= 0)
        frame_count = 1;
    if (frame_count > 8)
        frame_count = 8;
    ...
    {
        frame_time = (float) g_app.player_frame_time_ms[state][sheet_dir][g_app.player.anim_frame];
        if (frame_time <= 0)
            frame_time = 120.0f;
        if (g_app.player.anim_time >= frame_time)
        {
            g_app.player.anim_time -= frame_time;
            next_frame = (g_app.player.anim_frame + 1) % frame_count;
        }
    }
