...
void rogue_enemy_system_update(float dt_ms){
    ...
    for(int i=0;i<ROGUE_MAX_ENEMIES;i++) if(g_app.enemies[i].alive){
        ...
        /* Phase 3.3: basic stagger timer decrement & passive poise regen when not staggered */
        if(e->staggered){
            e->stagger_timer_ms -= dt_ms; if(e->stagger_timer_ms <= 0.0f){ e->staggered=0; e->stagger_timer_ms=0.0f; e->poise = e->poise_max * 0.50f; }
        } else {
            if(e->poise < e->poise_max){ e->poise += dt_ms * 0.04f; if(e->poise > e->poise_max) e->poise = e->poise_max; }
        }
