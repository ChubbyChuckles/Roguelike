...
void rogue_skills_shutdown(void){
    ...
    for(int i=0;i<g_skill_count_internal;i++){
        if(g_skill_defs_internal){
            if(g_skill_defs_internal[i].name){ free((char*)g_skill_defs_internal[i].name); g_skill_defs_internal[i].name=NULL; }
...
/* Data-driven loader: supports legacy CSV (.cfg) and JSON (.json) formats.
   CSV columns: name,icon,max_rank,base_cd,cd_red,is_passive,tags,synergy_id,synergy_per_rank,mana,ap,max_charges,charge_ms,cast_ms,input_buf,min_weave,early_cancel_pct,cast_type,combo_builder,combo_spender,effect_spec_id
   JSON shape: [ { "name":..., "icon":..., "max_rank":N, ... } ] matching RogueSkillDef fields by key.
*/
static int rogue__ends_with(const char* s,const char* suf){ if(!s||!suf) return 0; size_t ls=strlen(s), lf=strlen(suf); if(lf>ls) return 0; return _stricmp? (int)(_stricmp(s+ls-lf,suf)==0) : (int)(strcmp(s+ls-lf,suf)==0); }

/* Minimal JSON value skipper / parser (expects well-formed input generated by our tools). */
static const char* rogue__skip_ws(const char* s){ while(*s && (unsigned char)*s<=32) ++s; return s; }
static const char* rogue__parse_string(const char* s, char* out, int out_sz){ s=rogue__skip_ws(s); if(*s!='"') return NULL; ++s; int i=0; while(*s && *s!='"'){ if(*s=='\\' && s[1]) s++; if(i+1<out_sz) out[i++]=*s; ++s; } if(*s!='"') return NULL; out[i]='\0'; return s+1; }
static const char* rogue__parse_number(const char* s, double* out){ s=rogue__skip_ws(s); char* end=NULL; double v=strtod(s,&end); if(end==s) return NULL; *out=v; return end; }
...
static int rogue__json_load(const char* buf){
    ...
    if(*s!='{') return loaded; ++s; RogueSkillDef def; memset(&def,0,sizeof def); def.id=-1; def.max_rank=1; def.synergy_id=-1; def.skill_strength=0; int done_obj=0; while(!done_obj){ s=rogue__skip_ws(s); if(*s=='}'){ ++s; break; }
                ...
                if(*s=='"'){ const char* vs=rogue__parse_string(s,sval,sizeof sval); if(!vs) return loaded; s=rogue__skip_ws(vs);
                    ...
                    else if(strcmp(key,"tags")==0) def.tags=(int)num;
                    else if(strcmp(key,"synergy_id")==0) def.synergy_id=(int)num;
                    else if(strcmp(key,"synergy_value_per_rank")==0) def.synergy_value_per_rank=(int)num;
                    else if(strcmp(key,"resource_cost_mana")==0) def.resource_cost_mana=(int)num;
                    else if(strcmp(key,"action_point_cost")==0) def.action_point_cost=(int)num;
                    else if(strcmp(key,"max_charges")==0) def.max_charges=(int)num;
                    else if(strcmp(key,"charge_recharge_ms")==0) def.charge_recharge_ms=(float)num;
                    else if(strcmp(key,"cast_time_ms")==0) def.cast_time_ms=(float)num;
                    else if(strcmp(key,"input_buffer_ms")==0) def.input_buffer_ms=(unsigned short)num;
                    else if(strcmp(key,"min_weave_ms")==0) def.min_weave_ms=(unsigned short)num;
                    else if(strcmp(key,"early_cancel_min_pct")==0) def.early_cancel_min_pct=(unsigned char)num;
                    else if(strcmp(key,"cast_type")==0) def.cast_type=(unsigned char)num;
                    else if(strcmp(key,"combo_builder")==0) def.combo_builder=(unsigned char)num;
                    else if(strcmp(key,"combo_spender")==0) def.combo_spender=(unsigned char)num;
                    else if(strcmp(key,"effect_spec_id")==0) def.effect_spec_id=(int)num;
                }
            ...
            int new_id = rogue_skill_register(&def); if(new_id>=0){
                ...
                if(def.icon && g_app.skill_icon_textures && new_id < g_skill_count_internal){
                    ...
                    if(!rogue_texture_load(&g_app.skill_icon_textures[new_id], attempt)){
                        ...
                        if(rogue_file_search_project(fname,icon_res,sizeof icon_res) && rogue_texture_load(&g_app.skill_icon_textures[new_id], icon_res)){
...
int rogue_skills_load_from_cfg(const char* path){
    ...
    f = fopen(path, "rb");
#endif
    char resolved[640]; resolved[0]='\0';
    if(!f){
        const char* slash1=strrchr(path,'/'); const char* slash2=strrchr(path,'\\'); const char* last=slash2?((slash1&&slash1>slash2)?slash1:slash2):(slash1?slash1:NULL); const char* base=last?last+1:path;
    ...
    if(strstr(path, ".json")!=NULL){
        fseek(f,0,SEEK_END); long sz=ftell(f); if(sz<0){ fclose(f); return 0; } fseek(f,0,SEEK_SET); char* buf=(char*)malloc((size_t)sz+1); if(!buf){ fclose(f); return 0; } size_t rd=fread(buf,1,(size_t)sz,f); buf[rd]='\0'; fclose(f); int loaded_json = rogue__json_load(buf); free(buf); return loaded_json; }
    char line[1024]; int loaded=0; while(fgets(line,sizeof line,f)){
        if(line[0]=='#' || line[0]=='\n' || line[0]=='\r') continue;
        /* tokenize by comma */
        char* parts[24]; int pc=0; char* p=line; while(pc<24){ char* comma=strchr(p,','); if(comma){ *comma='\0'; parts[pc++]=p; p=comma+1; } else { /* trim newline */ char* nl=strchr(p,'\n'); if(nl) *nl='\0'; parts[pc++]=p; break; } }
        if(pc<3) continue; /* need at least name/icon/max_rank */
        RogueSkillDef def; memset(&def,0,sizeof def); def.id=-1;
    /* duplicate strings safely */
#if defined(_MSC_VER)
    size_t nlen = strlen(parts[0]); char* ndup = (char*)malloc(nlen+1); if(ndup){ memcpy(ndup,parts[0],nlen+1);} else { continue; }
    size_t ilen = strlen(parts[1]); char* idup = (char*)malloc(ilen+1); if(idup){ memcpy(idup,parts[1],ilen+1);} else { free(ndup); continue; }
    def.name = ndup; def.icon = idup;
#else
    def.name = strdup(parts[0]); def.icon = strdup(parts[1]);
#endif
        int idx=2;
    ...
    if(new_id>=0){
            ...
            if(def.icon && g_app.skill_icon_textures){
                char attempt[512];
                if(strncmp(def.icon,"assets/",7)==0 || strncmp(def.icon,"../assets/",10)==0) snprintf(attempt,sizeof attempt,"%s",def.icon);
                else snprintf(attempt,sizeof attempt,"assets/%s",def.icon);
                if(rogue_texture_load(&g_app.skill_icon_textures[new_id], attempt)){
                    ROGUE_LOG_INFO("Skill icon loaded id=%d name=%s path=%s", new_id, def.name, attempt);
                } else {
                    const char* s1=strrchr(attempt,'/'); const char* s2=strrchr(attempt,'\\'); const char* last=s2?((s1&&s1>s2)?s1:s2):(s1?s1:NULL); const char* fname=last?last+1:attempt; char icon_res[640];
                    if(rogue_file_search_project(fname, icon_res, sizeof icon_res)){
                        ...
                        if(rogue_texture_load(&g_app.skill_icon_textures[new_id], icon_res)){
                            ROGUE_LOG_INFO("Skill icon loaded via fallback id=%d name=%s path=%s", new_id, def.name, icon_res);
                        } else {
                            ROGUE_LOG_WARN("Skill icon failed after fallback id=%d name=%s path=%s", new_id, def.name, icon_res);
                        }
                    } else {
                        ROGUE_LOG_WARN("Skill icon missing (no fallback match) id=%d name=%s original=%s", new_id, def.name, attempt);
                    }
                }
            }
#endif
