 ...
 * Features:
 *  - 13.1 Build slot arrays (ids + counts) with stack aggregation (already counts) and capacity clamp
 ...
 *  - 13.5 Persistence of sort mode (stored in g_app persistent field, serialized in player component tail)
 */
...
/* Apply a UI drag swap (already performed on arrays) to underlying inventory model: no-op because inventory core is aggregated counts.
 * Provided for future per-instance inventories; currently returns 0. */
int rogue_inventory_ui_apply_swap(int from_slot, int to_slot, int* ids, int* counts, int slot_capacity);
...
/* Drop helper: spawns a ground item instance at player position (quantity=1) and decrements inventory count.
 * Returns inst index or <0 on failure. */
