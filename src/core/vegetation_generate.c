...
void rogue_vegetation_generate(float tree_cover_target, unsigned int seed){
    if(tree_cover_target < 0.0f) tree_cover_target = 0.0f; if(tree_cover_target>0.70f) tree_cover_target=0.70f;
    ...
    if(grass_count==0){ if(forest_count==0) return; use_forest_as_grass = 1; }
    ...
    if(force_single_tree){ int gx=w/2, gy=h/2; int idx; pick_weighted(1,&idx); if(idx>=0){ RogueVegetationInstance* inst=&g_instances[g_instance_count++]; inst->x=gx+0.5f; inst->y=gy+0.5f; inst->def_index=(unsigned short)idx; inst->is_tree=1; inst->variant=0; inst->growth=255; } }
    while(!force_single_tree && placed_canopy_tiles < desired_tree_canopy_tiles && attempts < max_attempts){
        ...
        if(density < 0.48f && g_instance_count>0) continue;
    ...
    if(!force_single_tree && g_instance_count==0){
        for(int gy=2; gy<h-2 && g_instance_count==0; gy++) for(int gx=2; gx<w-2 && g_instance_count==0; gx++) if(g_app.world_map.tiles[gy*w+gx]==ROGUE_TILE_GRASS){ int idx; pick_weighted(1,&idx); if(idx>=0){ RogueVegetationInstance* inst=&g_instances[g_instance_count++]; inst->x=gx+0.5f; inst->y=gy+0.5f; inst->def_index=(unsigned short)idx; inst->is_tree=1; inst->variant=0; inst->growth=255; break; } }
    }
    ...
    if(rogue_vegetation_plant_count()==0){
        for(int gy=2; gy<h-2 && rogue_vegetation_plant_count()==0; gy++) for(int gx=2; gx<w-2 && rogue_vegetation_plant_count()==0; gx++){ unsigned char t=g_app.world_map.tiles[gy*w+gx]; if(!(t==ROGUE_TILE_GRASS || (use_forest_as_grass && t==ROGUE_TILE_FOREST))) continue; int idx; pick_weighted(0,&idx); if(idx>=0 && g_instance_count<ROGUE_MAX_VEG_INSTANCES){ RogueVegetationInstance* inst=&g_instances[g_instance_count++]; inst->x=gx+0.5f; inst->y=gy+0.5f; inst->def_index=(unsigned short)idx; inst->is_tree=0; inst->variant=0; inst->growth=200; break; } }
    }
}
