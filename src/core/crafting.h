...
/* Returns 0=common,1=uncommon,2=rare,3=epic,4=legendary for material def, or -1 if def_index invalid/not material. */
int rogue_material_tier(int def_index);

/* Crafting Recipes */
typedef struct RogueCraftIngredient { int def_index; int quantity; } RogueCraftIngredient;
...
/* Optional removal callback; if NULL uses add with negative which we currently do not support -> then fails. */
...
/* Upgrade path: given a base item definition index and recipe specifying rarity delta, returns new rarity clamped. */
 ...
 * Returns 0 success, <0 on failure. Affix regeneration done via callback to core item instance API. */
