...
#define strncpy_s_safe(dest, dest_size, src, count) strncpy(dest, src, (count < dest_size) ? count : dest_size-1)
#endif
...
bool rogue_worldgen_enemy_bridge_update(RogueWorldGenEnemyBridge* bridge, float delta_time) {
    if (!bridge || !bridge->initialized || !bridge->enabled) return false;
    ...
    if (seasonal->auto_season_progression) {
        ...
        if (elapsed >= seasonal->season_duration_us) {
            seasonal->current_season = (RogueSeasonType)((seasonal->current_season + 1) % SEASON_COUNT);
            seasonal->season_start_time_us = start_time;
    ...
    for (int i = 0; i < 64; i++) {
        SpawnDensityControl* density = &bridge->density_controls[i];
        if (start_time - density->last_density_update_us > 1000000) { /* Update every second */
    ...
    /* Process migrations */
    EnemyMigrationSystem* migration = &bridge->migration_system;
    if (migration->migration_enabled && 
        start_time - migration->last_migration_check_us > 5000000) { /* Check every 5 seconds */
    ...
    bridge->metrics.avg_processing_time_us = 
        (bridge->metrics.avg_processing_time_us * bridge->metrics.performance_samples + processing_time) /
...
bool rogue_worldgen_enemy_bridge_load_biome_encounters(RogueWorldGenEnemyBridge* bridge, 
    ...
    while (fgets(line, sizeof(line), file) && manager->encounter_count < MAX_BIOME_ENCOUNTERS) {
        ...
        if (sscanf_s(line, "%u,%f,%u,%u,%f,%d,%u",
                     &entry->enemy_id,
                     &entry->spawn_weight,
                     &entry->min_level,
                     &entry->max_level,
                     &entry->difficulty_modifier,
                     (int*)&entry->is_boss,
                     &entry->required_reputation) == 7) {
...
bool rogue_worldgen_enemy_bridge_set_region_scaling(RogueWorldGenEnemyBridge* bridge,
                                                   ...
                                                   uint32_t base_level) {
    if (!bridge || !bridge->initialized || region_id >= 64) return false;
...
uint32_t rogue_worldgen_enemy_bridge_get_scaled_enemy_level(RogueWorldGenEnemyBridge* bridge,
    ...
    if (!bridge || !bridge->initialized || region_id >= 64) return base_enemy_level;
    ...
    uint32_t scaled_level = (uint32_t)((float)base_enemy_level * level_multiplier * (1.0f + variance));
...
bool rogue_worldgen_enemy_bridge_set_season(RogueWorldGenEnemyBridge* bridge, 
    ...
    if (!bridge || !bridge->initialized || season >= SEASON_COUNT) return false;
...
bool rogue_worldgen_enemy_bridge_add_seasonal_variation(RogueWorldGenEnemyBridge* bridge,
    ...
    SeasonalVariation* variation = &bridge->seasonal_system.variations[bridge->seasonal_system.variation_count];
...
uint32_t rogue_worldgen_enemy_bridge_get_pack_size(RogueWorldGenEnemyBridge* bridge,
    ...
    if (!bridge || !bridge->initialized || region_id >= 64) return base_pack_size;
    
    RegionPackScaling* pack_scaling = &bridge->pack_scaling[region_id];
    
    float size_multiplier = pack_scaling->danger_rating;
    float variance = random_float(-pack_scaling->pack_size_variance, pack_scaling->pack_size_variance);
    uint32_t calculated_size = (uint32_t)((float)base_pack_size * size_multiplier * (1.0f + variance));
...
bool rogue_worldgen_enemy_bridge_add_biome_modifier(RogueWorldGenEnemyBridge* bridge,
                                                   ...
                                                   float chance,
                                                   float magnitude) {
    if (!bridge || !bridge->initialized || biome_type >= MAX_BIOME_TYPES) return false;
    
    BiomeModifierSystem* mod_system = &bridge->modifier_systems[biome_type];
    if (mod_system->modifier_count >= MAX_ENEMY_MODIFIERS) return false;
...
uint32_t rogue_worldgen_enemy_bridge_apply_environmental_modifiers(RogueWorldGenEnemyBridge* bridge,
                                                                  RogueBiomeType biome_type,
                                                                  uint32_t enemy_id) {
    if (!bridge || !bridge->initialized || biome_type >= MAX_BIOME_TYPES) return 0;
    
    /* Suppress unused parameter warning */
    (void)enemy_id;
    
    BiomeModifierSystem* mod_system = &bridge->modifier_systems[biome_type];
    if (!mod_system->modifiers_enabled) return 0;
...
bool rogue_worldgen_enemy_bridge_update_enemy_count(RogueWorldGenEnemyBridge* bridge,
    ...
    float density_ratio = (float)density->current_enemy_count / (float)density->max_concurrent_enemies;
    density->respawn_rate_modifier = 1.0f - (density_ratio * 0.5f); /* Slower respawn when crowded */
...
bool rogue_worldgen_enemy_bridge_add_migration_route(RogueWorldGenEnemyBridge* bridge,
    ...
    EnemyMigrationRoute* route = &bridge->migration_system.routes[bridge->migration_system.route_count];
...
bool rogue_worldgen_enemy_bridge_process_migrations(RogueWorldGenEnemyBridge* bridge) {
    ...
    for (uint32_t i = 0; i < bridge->migration_system.route_count; i++) {
        ...
        if (density_ratio >= route->migration_trigger_threshold) {
            /* Process migration */
            uint32_t migration_count = (uint32_t)(source_density->current_enemy_count * route->migration_rate);
            if (migration_count > 0) {
                /* Move enemies from source to destination */
                rogue_worldgen_enemy_bridge_update_enemy_count(bridge, route->source_region_id, -(int32_t)migration_count);
                rogue_worldgen_enemy_bridge_update_enemy_count(bridge, route->destination_region_id, (int32_t)migration_count);
...
WorldGenEnemyBridgeMetrics rogue_worldgen_enemy_bridge_get_metrics(const RogueWorldGenEnemyBridge* bridge) {
