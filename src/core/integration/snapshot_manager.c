...
int rogue_snapshot_register(const RogueSnapshotDesc* desc){ if(!desc||!desc->capture) return -1; if(g_reg_count>=SNAPSHOT_CAP) return -1; for(uint32_t i=0;i<g_reg_count;i++) if(g_descs[i].system_id==desc->system_id) return -2; g_descs[g_reg_count]=*desc; g_snaps[g_reg_count][0]=(RogueSystemSnapshot){ desc->system_id, desc->name, 0,0,0,NULL,0 }; g_snaps[g_reg_count][1]=(RogueSystemSnapshot){ desc->system_id, desc->name, 0,0,0,NULL,0 }; g_head[g_reg_count]=0; g_reg_count++; g_stats.registered_systems=g_reg_count; return 0; }
...
int rogue_snapshot_capture(int system_id){ int idx=index_of(system_id); if(idx<0) return -1; RogueSnapshotDesc* d=&g_descs[idx]; void* data=NULL; size_t size=0; uint32_t ver=0; int rc=d->capture(d->user,&data,&size,&ver); if(rc!=0){ g_stats.total_capture_failures++; return -2; } if(d->max_size && size>d->max_size){ free(data); return -3; } RogueSystemSnapshot* cur=&g_snaps[idx][g_head[idx]]; if(ver<=cur->version && cur->data){ free(data); return -4; } // choose next slot to preserve previous snapshot for external holders
 int next = g_head[idx]^1; RogueSystemSnapshot* s=&g_snaps[idx][next]; if(s->data) { free(s->data); s->data=NULL; }
 s->data=data; s->size=size; s->version=ver; s->hash=fnv1a64(data,size); s->timestamp=++g_capture_counter; g_head[idx]= (uint8_t)next; g_stats.total_captures++; g_stats.total_bytes_stored += size; return 0; }

const RogueSystemSnapshot* rogue_snapshot_get(int system_id){ int idx=index_of(system_id); if(idx<0) return NULL; return &g_snaps[idx][g_head[idx]]; }
