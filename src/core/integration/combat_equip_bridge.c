...
int rogue_combat_equip_bridge_init(RogueCombatEquipBridge* bridge) {
    ...
    BRIDGE_LOG(bridge, "INFO", "Combat-Equipment Bridge initialized successfully with real-time stat integration");
    return 1;
}
...
int rogue_combat_equip_bridge_update_stats(RogueCombatEquipBridge* bridge, struct RoguePlayer* player) {
    ...
    for (int slot = 0; slot < ROGUE_EQUIP_SLOT_COUNT; slot++) {
        ...
        if (item_instance < 0) continue; /* No item equipped in this slot */
    ...
    BRIDGE_LOG(bridge, "INFO", "Equipment stats updated: dmg_mult=%.2f, crit_chance=+%.1f%%, calc_time=%.2f us",
               bridge->cached_stats.damage_multiplier, bridge->cached_stats.crit_chance_bonus, calc_time);
    
    return 1;
}

int rogue_combat_equip_bridge_get_combat_stats(RogueCombatEquipBridge* bridge, RogueCombatEquipmentStats* out_stats) {
...
int rogue_combat_equip_bridge_apply_stats_to_combat(RogueCombatEquipBridge* bridge, struct RoguePlayerCombat* combat) {
    ...
    BRIDGE_LOG(bridge, "INFO", "Applied equipment stats to combat: dmg_mult=%.2f, attack_speed_mult=%.2f",
...
int rogue_combat_equip_bridge_on_damage_taken(RogueCombatEquipBridge* bridge, 
    ...
    uint16_t durability_damage = (uint16_t)((damage_amount / 20) + 1); /* 1 durability per 20 damage + 1 base */
    
    /* Apply durability damage to armor pieces */
    uint8_t armor_slots[] = {ROGUE_EQUIP_ARMOR_CHEST, ROGUE_EQUIP_ARMOR_HEAD, 
                            ROGUE_EQUIP_ARMOR_LEGS, ROGUE_EQUIP_ARMOR_HANDS, ROGUE_EQUIP_ARMOR_FEET};
    
    for (int i = 0; i < 5; i++) {
        uint8_t slot = armor_slots[i];
        int item_instance = rogue_equip_get(slot);
        if (item_instance < 0) continue;
        
        /* Create durability event */
        RogueEquipmentDurabilityEvent* event = &bridge->durability_events[bridge->durability_event_count];
        event->slot = slot;
        ...
        BRIDGE_LOG(bridge, "INFO", "Durability damage to slot %u: -%u points, %u remaining", 
                   slot, durability_damage, event->remaining_durability);
...
int rogue_combat_equip_bridge_update_set_bonuses(RogueCombatEquipBridge* bridge, struct RoguePlayer* player) {
    if (!bridge || !bridge->initialized || !player) {
        return 0;
    }
...
int rogue_combat_equip_bridge_update_weight_impact(RogueCombatEquipBridge* bridge, struct RoguePlayer* player) {
    if (!bridge || !bridge->initialized || !player) {
        return 0;
    }
    ...
    if (bridge->weight_impact.encumbered) {
        ...
        bridge->weight_impact.attack_speed_modifier = 1.0f - (bridge->weight_impact.weight_penalty * 0.3f);
        bridge->weight_impact.dodge_speed_modifier = 1.0f - (bridge->weight_impact.weight_penalty * 0.5f);
        bridge->weight_impact.stamina_drain_multiplier = 1.0f + (bridge->weight_impact.weight_penalty * 0.8f);
    } else {
    ...
    BRIDGE_LOG(bridge, "INFO", "Weight impact updated: total=%.1f, encumbered=%s, attack_speed=%.2f",
               total_weight, bridge->weight_impact.encumbered ? "YES" : "NO",
               bridge->weight_impact.attack_speed_modifier);
    
    return 1;
}

int rogue_combat_equip_bridge_get_weight_impact(RogueCombatEquipBridge* bridge, RogueEquipmentWeightImpact* out_impact) {
...
int rogue_combat_equip_bridge_apply_weight_to_combat(RogueCombatEquipBridge* bridge, struct RoguePlayerCombat* combat) {
    if (!bridge || !bridge->initialized || !combat) {
        return 0;
    }
    ...
    BRIDGE_LOG(bridge, "INFO", "Applied weight impact to combat: attack_speed=%.2f, dodge_speed=%.2f, stamina_mult=%.2f",
               bridge->weight_impact.attack_speed_modifier,
...
int rogue_combat_equip_bridge_on_equipment_upgraded(RogueCombatEquipBridge* bridge,
    ...
    BRIDGE_LOG(bridge, "INFO", "Equipment upgraded in slot %u: %u -> %u, invalidating combat stat cache", 
               slot, old_item_id, new_item_id);
    
    /* Publish equipment change event */
    RogueEventPayload payload = {0};
    memcpy(payload.raw_data, &slot, sizeof(uint8_t));
    rogue_event_publish(ROGUE_COMBAT_EQUIP_EVENT_UPGRADE_NOTIFICATION,
...
int rogue_combat_equip_bridge_on_equipment_enchanted(RogueCombatEquipBridge* bridge,
    ...
    BRIDGE_LOG(bridge, "INFO", "Equipment enchanted in slot %u with enchant %u, invalidating combat stat cache", 
               slot, enchant_id);
    
    RogueEventPayload payload = {0};
    memcpy(payload.raw_data, &enchant_id, sizeof(uint32_t));
    rogue_event_publish(ROGUE_COMBAT_EQUIP_EVENT_ENCHANT_APPLIED,
                        &payload, ROGUE_EVENT_PRIORITY_NORMAL,
                        0, "CombatEquipBridge");
...
int rogue_combat_equip_bridge_on_equipment_socketed(RogueCombatEquipBridge* bridge,
    ...
    BRIDGE_LOG(bridge, "INFO", "Equipment socketed in slot %u with gem %u, invalidating combat stat cache", 
               slot, gem_id);
...
int rogue_combat_equip_bridge_get_metrics(RogueCombatEquipBridge* bridge, RogueCombatEquipBridgeMetrics* out_metrics) {
...
int rogue_combat_equip_bridge_check_performance_thresholds(RogueCombatEquipBridge* bridge) {
    ...
    if (bridge->metrics.peak_stat_calc_time_us > 1000.0f) { /* 1ms threshold */
        BRIDGE_LOG(bridge, "WARN", "Stat calculation time exceeded threshold: %.2f us", 
                   bridge->metrics.peak_stat_calc_time_us);
        warnings++;
    }
    
    if (bridge->metrics.cache_misses > bridge->metrics.cache_hits * 0.2f) { /* 20% miss rate threshold */
