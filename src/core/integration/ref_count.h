...
extern "C"
{
    ...
    typedef struct RogueWeakRef
    {
        struct RogueRcHeader* hdr;
    } RogueWeakRef;

    // Create weak ref from strong object (increment weak count). Returns handle with hdr==NULL if
    // obj NULL.
    RogueWeakRef rogue_rc_weak_from(void* obj);
    // Acquire strong ref from weak (returns NULL if object already destroyed). On success returns
    // strong pointer (retained).
    ...
    // enumerate(obj, out_array, max) should write up to max child strong object pointers into
    // out_array, returning count.
    ...
    // Returns number of bytes written (truncated if buffer too small). If buf NULL returns required
    // size.
