...
static void record_acq(RogueLockStats* st, RogueLockPriority pri, uint64_t wait_ns, int contended)
{
    st->acquisitions++;
    st->priority_acq[pri]++;
    st->wait_time_ns += wait_ns;
    if (contended)
        st->contended_acquisitions++;
}
...
static int check_order_push(int order_id)
{
    ...
    if (tls_lock_top < 32)
    {
        tls_lock_stack[tls_lock_top++] = order_id;
    }
    return 0;
}
static void check_order_pop(int order_id)
{
    if (tls_lock_top > 0)
    {
        ...
        if (tls_lock_stack[tls_lock_top - 1] == order_id)
            tls_lock_top--;
        else
        {
            // search and compact (rare path)
            for (int i = 0; i < tls_lock_top; i++)
                if (tls_lock_stack[i] == order_id)
                {
                    for (int j = i + 1; j < tls_lock_top; j++)
                        tls_lock_stack[j - 1] = tls_lock_stack[j];
                    tls_lock_top--;
                    break;
                }
        }
...
{
    RogueMutex* m = (RogueMutex*) calloc(1, sizeof(RogueMutex));
    if (!m)
        return NULL;
    m->order_id = order_id;
...
#endif
    if (g_lock_stats.mutex_count < LOCK_MAX)
        g_lock_stats.mutexes[g_lock_stats.mutex_count++] = m;
    return m;
}
...
int rogue_mutex_acquire(RogueMutex* m, RogueLockPriority pri, int timeout_ms)
{
    ...
    int contended = 0;
    int pushed = 0;
    if (check_order_push(m->order_id) != 0)
    {
        m->stats.failed_deadlocks++;
        g_lock_stats.total_deadlock_prev++;
        return -1;
    }
    pushed = 1;
#ifdef _WIN32
    ...
    if (timeout_ms == 0)
    {
        ...
        {
            m->stats.failed_timeouts++;
            g_lock_stats.total_timeout++;
            if (pushed)
    ...
    else
    {
        if (pthread_mutex_trylock(&m->mtx) != 0)
        {
            ...
            { // timed wait via loop sleep
                int waited = 0;
                while (pthread_mutex_trylock(&m->mtx) != 0)
                {
...
#endif
    uint64_t end = now_ns();
    record_acq(&m->stats, pri, end - start, contended);
    g_lock_stats.total_acq++;
    if (contended)
        g_lock_stats.total_contention++;
    return 0;
}
...
{
    RogueRwLock* l = (RogueRwLock*) calloc(1, sizeof(RogueRwLock));
    if (!l)
        return NULL;
    l->order_id = order_id;
    l->name = name ? name : "rwlock";
#ifdef _WIN32
    InitializeSRWLock(&l->lock);
#else
    pthread_rwlock_init(&l->rw, NULL);
#endif
    if (g_lock_stats.rwlock_count < LOCK_MAX)
        g_lock_stats.rwlocks[g_lock_stats.rwlock_count++] = l;
    return l;
}
...
int rogue_rwlock_acquire_read(RogueRwLock* l, RogueLockPriority pri, int timeout_ms)
{
    ...
    uint64_t end = now_ns();
    record_acq(&l->read_stats, pri, end - start, contended);
    g_lock_stats.total_acq++;
    if (contended)
        g_lock_stats.total_contention++;
    return 0;
...
int rogue_rwlock_acquire_write(RogueRwLock* l, RogueLockPriority pri, int timeout_ms)
{
    ...
    uint64_t start = now_ns();
    int contended = 0;
    if (check_order_push(l->order_id) != 0)
    {
        l->write_stats.failed_deadlocks++;
        g_lock_stats.total_deadlock_prev++;
        return -1;
    }
    if (timeout_ms == 0)
    ...
    uint64_t end = now_ns();
    record_acq(&l->write_stats, pri, end - start, contended);
    g_lock_stats.total_acq++;
    if (contended)
        g_lock_stats.total_contention++;
    return 0;
...
void rogue_rwlock_get_stats(const RogueRwLock* l, RogueLockStats* read_out,
                            RogueLockStats* write_out)
{
    if (!l)
        return;
    if (read_out)
        *read_out = l->read_stats;
    if (write_out)
        *write_out = l->write_stats;
}
...
void rogue_lock_global_stats(RogueGlobalLockStats* out)
{
    if (!out)
        return;
    out->mutex_count = g_lock_stats.mutex_count;
    out->rwlock_count = g_lock_stats.rwlock_count;
    out->total_acquisitions = g_lock_stats.total_acq;
    out->total_contentions = g_lock_stats.total_contention;
    out->total_deadlock_preventions = g_lock_stats.total_deadlock_prev;
    out->total_timeouts = g_lock_stats.total_timeout;
}

void rogue_lock_dump(void* fptr)
{
    FILE* f = fptr ? (FILE*) fptr : stdout;
    fprintf(
        f,
        "[locks] mutexes=%u rwlocks=%u acq=%llu contention=%llu timeouts=%llu deadlock_prev=%llu\n",
        g_lock_stats.mutex_count, g_lock_stats.rwlock_count,
        (unsigned long long) g_lock_stats.total_acq,
        (unsigned long long) g_lock_stats.total_contention,
        (unsigned long long) g_lock_stats.total_timeout,
        (unsigned long long) g_lock_stats.total_deadlock_prev);
    for (uint32_t i = 0; i < g_lock_stats.mutex_count; i++)
    {
        RogueMutex* m = g_lock_stats.mutexes[i];
        fprintf(f, " mutex '%s' acq=%llu contended=%llu timeouts=%llu wait_ns=%llu\n", m->name,
                (unsigned long long) m->stats.acquisitions,
                (unsigned long long) m->stats.contended_acquisitions,
                (unsigned long long) m->stats.failed_timeouts,
                (unsigned long long) m->stats.wait_time_ns);
    }
    for (uint32_t i = 0; i < g_lock_stats.rwlock_count; i++)
    {
        RogueRwLock* l = g_lock_stats.rwlocks[i];
        fprintf(f,
                " rwlock '%s' r_acq=%llu w_acq=%llu r_cont=%llu w_cont=%llu r_timeout=%llu "
                "w_timeout=%llu\n",
                l->name, (unsigned long long) l->read_stats.acquisitions,
                (unsigned long long) l->write_stats.acquisitions,
                (unsigned long long) l->read_stats.contended_acquisitions,
                (unsigned long long) l->write_stats.contended_acquisitions,
                (unsigned long long) l->read_stats.failed_timeouts,
                (unsigned long long) l->write_stats.failed_timeouts);
    }
}
