...
#define ROGUE_CACHE_LEVELS 3
    ...
    // Insert/update entry. If level_hint outside range choose appropriate level (L1 for small <=256
    // bytes, else L2 else L3). Returns 0 success, -1 on allocation/other failure.
    int rogue_cache_put(uint64_t key, const void* data, size_t size, uint32_t version,
                        int level_hint);
    ...
    // Iterate across all live entries (for debugging/export). The callback may return false to
    // stop.
    typedef bool (*RogueCacheIterFn)(uint64_t key, const void* data, size_t size, uint32_t version,
                                     int level, void* ud);
