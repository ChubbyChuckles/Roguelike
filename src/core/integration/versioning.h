...
extern "C"
{
    ...
    // On failure during a multi-step migration chain, original data is restored (rollback) and
    // error returned.
    ...
    // Perform migration of an instance from source_version to target_version (or to current if
    // target_version==0). data_ptr points to a buffer pointer holding the instance; may be
    // reallocated. size_ptr updated if size changes. Progress optional. Returns 0 on success,
    // non-zero on error (unknown type, missing path, migration failure).
    int rogue_version_migrate(const char* type_name, uint32_t source_version,
                              uint32_t target_version, void** data_ptr, size_t* size_ptr,
                              RogueMigrationProgress* progress_out);
