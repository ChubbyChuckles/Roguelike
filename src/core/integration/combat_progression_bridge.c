...
void rogue_combat_progression_bridge_shutdown(RogueCombatProgressionBridge* bridge)
{
    ...
    BRIDGE_LOG(bridge, "INFO",
               "Shutting down Combat-Progression Bridge. Stats: XP events=%u, Skills tracked=%u, "
               "Achievements=%u",
                                             ...
                                             uint32_t enemy_difficulty, uint32_t combat_event_id)
{
    ...
    rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_XP_AWARDED, &payload, ROGUE_EVENT_PRIORITY_NORMAL,
    ...
    BRIDGE_LOG(bridge, "INFO",
               "Awarded %u XP (base=%u, bonus=%u, mult=%.2f) from %s for combat event %u", total_xp,
               base_xp, bonus_xp, difficulty_multiplier,
               (source == ROGUE_XP_SOURCE_DAMAGE_DEALT)       ? "damage"
               : (source == ROGUE_XP_SOURCE_ENEMY_DEFEATED)   ? "defeat"
               : (source == ROGUE_XP_SOURCE_DIFFICULTY_BONUS) ? "difficulty"
                                                              : "efficiency",
               combat_event_id);
                                                      ...
                                                      uint32_t combat_context_id)
{
    ...
    skill_usage->mastery_threshold_reached =
        (skill_usage->mastery_points_gained >= mastery_threshold);
    ...
    if (skill_usage->mastery_threshold_reached)
    {
        ...
        BRIDGE_LOG(bridge, "INFO", "Skill %u reached mastery level %u!", skill_id,
                   skill_usage->skill_level);
    }
    ...
    rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_SKILL_USED, &payload, ROGUE_EVENT_PRIORITY_NORMAL,
                        0, "CombatProgressionBridge");
...
int rogue_combat_progression_bridge_activate_passive_skill(RogueCombatProgressionBridge* bridge,
                                                           uint16_t skill_id, uint8_t effect_type,
                                                           float magnitude, uint32_t duration_ms)
{
    ...
    RoguePassiveSkillEffect* passive =
        &bridge->active_passive_effects[bridge->active_passives_count];
    ...
    RogueEventPayload payload = {0};
    memcpy(payload.raw_data, &skill_id, sizeof(uint16_t));
    rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_PASSIVE_EFFECT_APPLIED, &payload,
                        ROGUE_EVENT_PRIORITY_NORMAL, 0, "CombatProgressionBridge");
                                                       ...
                                                       uint32_t event_data)
{
         ...
         i++)
    {
        ...
        case 0x03: /* XP gained threshold */
            should_trigger =
                (bridge->total_xp_awarded_session >= achievement_definitions[i].required_value);
            progress_value = bridge->total_xp_awarded_session;
            break;
        case 0x04: /* Skill usage threshold */
            should_trigger =
                (bridge->total_skill_activations >= achievement_definitions[i].required_value);
        ...
        if (should_trigger && progress_value >= achievement_definitions[i].required_value)
        {
            ...
            if (!already_unlocked)
            {
                ...
                rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_ACHIEVEMENT_UNLOCKED, &payload,
                                    ROGUE_EVENT_PRIORITY_HIGH, 0, "CombatProgressionBridge");
...
int rogue_combat_progression_bridge_get_recent_achievements(
    RogueCombatProgressionBridge* bridge, RogueCombatAchievement* out_achievements,
    uint8_t max_count)
{
                                                      ...
                                                      uint32_t tactical_complexity)
{
    ...
    {
        float avg_damage_pref =
            (float) bridge->playstyle_data_points[0] / bridge->playstyle_data_points[3];
        float avg_risk =
            (float) bridge->playstyle_data_points[1] / bridge->playstyle_data_points[3];
        float avg_tactical =
            (float) bridge->playstyle_data_points[2] / bridge->playstyle_data_points[3];
        ...
        else if (damage_type_preference > 80.0f)
        { /* High magical preference */
        ...
        rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_PLAYSTYLE_DETECTED, &payload,
                            ROGUE_EVENT_PRIORITY_LOW, 0, "CombatProgressionBridge");

        const char* style_names[] = {"Aggressive", "Defensive", "Balanced", "Tactical", "Magical"};
        BRIDGE_LOG(
            bridge, "INFO", "🎯 Playstyle detected: %s (%.1f%% confidence) after %u sessions",
            style_names[detected_style], confidence * 100.0f, bridge->playstyle_data_points[3]);
    }
...
rogue_combat_progression_bridge_get_detected_playstyle(RogueCombatProgressionBridge* bridge,
                                                              ...
                                                              uint32_t damage_taken)
{
    ...
    float current_resource_eff =
        (damage_taken == 0) ? 100.0f
    ...
    metrics->damage_per_mana_efficiency =
        (metrics->damage_per_mana_efficiency + current_dpm) / 2.0f;
    metrics->time_to_kill_avg_ms = (metrics->time_to_kill_avg_ms + combat_duration_ms) / 2.0f;
    metrics->resource_usage_efficiency =
        (metrics->resource_usage_efficiency + current_resource_eff) / 2.0f;
    ...
    float time_score =
        fmaxf(0.0f, 100.0f - (metrics->time_to_kill_avg_ms / 1000.0f)); /* Faster = better */

    metrics->overall_efficiency_score =
        (dps_score * 0.4f + resource_score * 0.4f + time_score * 0.2f);
    ...
    for (size_t i = 0; i < sizeof(milestone_thresholds) / sizeof(milestone_thresholds[0]); i++)
    {
            ...
            bridge->efficiency_milestone_count <= i)
        {
            ...
            rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_EFFICIENCY_MILESTONE, &payload,
                                ROGUE_EVENT_PRIORITY_NORMAL, 0, "CombatProgressionBridge");
...
int rogue_combat_progression_bridge_apply_death_penalty(RogueCombatProgressionBridge* bridge,
                                                        uint32_t player_level, uint32_t current_xp)
{
    if (!bridge || !bridge->initialized)
    {
        return 0;
    ...
    penalty->xp_penalty_amount =
        (uint32_t) fmaxf(current_xp * (penalty->xp_penalty_percentage / 100.0f), 100.0f);
    ...
    rogue_event_publish(ROGUE_COMBAT_PROG_EVENT_DEATH_PENALTY_APPLIED, &payload,
                        ROGUE_EVENT_PRIORITY_HIGH, 0, "CombatProgressionBridge");

    BRIDGE_LOG(bridge, "WARN",
               "💀 Death penalty applied: -%u XP (%.1f%%), -%u%% durability, -%u currency",
...
int rogue_combat_progression_bridge_get_death_penalty_info(
    RogueCombatProgressionBridge* bridge, RogueCombatDeathPenalty* out_penalty_info)
{
...
void rogue_combat_progression_bridge_set_debug_mode(RogueCombatProgressionBridge* bridge,
                                                    bool enabled)
{
    if (bridge)
    {
...
void rogue_combat_progression_bridge_get_metrics(RogueCombatProgressionBridge* bridge,
                                                 char* out_buffer, size_t buffer_size)
{
    ...
    snprintf(out_buffer, buffer_size,
             ...
             "  Errors: %u\n",
             bridge->metrics.xp_distributions_processed, bridge->total_xp_awarded_session,
             bridge->metrics.skill_usage_events_processed, bridge->total_skill_activations,
             bridge->metrics.passive_effects_applied, bridge->active_passives_count,
             bridge->metrics.achievements_triggered, bridge->achievements_unlocked_session,
             bridge->metrics.playstyle_analyses_performed,
             (bridge->playstyle_profile.detected_style < ROGUE_PLAYSTYLE_COUNT)
                 ? (const char*[]){"Aggressive", "Defensive", "Balanced", "Tactical",
                                   "Magical"}[bridge->playstyle_profile.detected_style]
                 : "Unknown",
             bridge->metrics.efficiency_calculations_performed,
             bridge->efficiency_metrics.overall_efficiency_score,
             bridge->metrics.death_penalties_applied, bridge->metrics.avg_processing_time_ms,
             bridge->error_count);
}
