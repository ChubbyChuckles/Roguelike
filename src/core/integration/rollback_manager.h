...
extern "C"
{
    ...
    // For simplicity delta entries only reference the immediate prior FULL entry (no chains of
    // deltas).
    ...
    // Retrieve pointer to internal immutable array of rollback events (chronological
    // oldest->newest). Returns 0 on success; out_events set to NULL if no events; count set to
    ...
    int rogue_rollback_partial(const int* system_ids, const uint32_t* steps,
                               size_t count); // per-system step counts
    ...
    // Transaction manager integration: map participant -> system snapshot id for auto rollback on
    // abort.
