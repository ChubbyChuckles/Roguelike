...
// Debug logging macro
#define BRIDGE_LOG(bridge, level, ...) do { \
...
int rogue_enemy_ai_bridge_init(RogueEnemyAIBridge* bridge, struct RogueEventBus* event_bus) {
    ...
    BRIDGE_LOG(bridge, "INFO", "Enemy-AI Bridge initialized successfully with event bus integration");
    return 1;
}
...
int rogue_enemy_ai_bridge_sync_state(RogueEnemyAIBridge* bridge, struct RogueEnemy* enemy) {
    ...
    if (enemy->ai_bt_enabled && enemy->ai_bt_state) {
        ...
        // blackboard_set_float(enemy->ai_bt_state, "health_ratio", (float)enemy->health / enemy->max_health);
        // blackboard_set_float(enemy->ai_bt_state, "position_x", enemy->base.x);
        // blackboard_set_float(enemy->ai_bt_state, "position_y", enemy->base.y);
...
int rogue_enemy_ai_bridge_apply_modifier_hook(RogueEnemyAIBridge* bridge, struct RogueEnemy* enemy, uint32_t modifier_id) {
    if (!bridge || !bridge->initialized || !enemy) {
        return 0;
    }
    
    BRIDGE_LOG(bridge, "INFO", "Applying modifier %u to enemy AI ID %u", modifier_id, enemy->encounter_id);
...
RogueEnemyAIIntensity rogue_enemy_ai_bridge_scale_intensity(RogueEnemyAIBridge* bridge, struct RogueEnemy* enemy, uint32_t difficulty_level) {
    if (!bridge || !bridge->initialized || !enemy) {
...
uint32_t rogue_enemy_ai_bridge_create_group(RogueEnemyAIBridge* bridge, uint32_t* enemy_ids, uint8_t enemy_count, uint32_t formation_pattern) {
...
int rogue_enemy_ai_bridge_update_group_coordination(RogueEnemyAIBridge* bridge, uint32_t group_id) {
    ...
    if (group->shared_blackboard) {
        ...
        // blackboard_set_int(group->shared_blackboard, "formation_pattern", group->formation_pattern);
        // blackboard_set_int(group->shared_blackboard, "member_count", group->member_count);
    }
...
void rogue_enemy_ai_bridge_update_metrics(RogueEnemyAIBridge* bridge, float dt) {
    ...
    if (bridge->last_metrics_update >= bridge->metrics_update_interval) {
        ...
        for (int i = 0; i < 5; i++) {
            total_tick_time += bridge->metrics.intensity_metrics[i].average_tick_time_us * bridge->metrics.intensity_metrics[i].total_ticks;
        ...
        bridge->metrics.blackboard_memory_bytes = bridge->metrics.total_ai_agents * 256; // Est. 256 bytes per blackboard
        bridge->metrics.behavior_tree_memory_bytes = bridge->metrics.active_behavior_trees * 512; // Est. 512 bytes per tree
        bridge->metrics.group_coordination_memory_bytes = bridge->active_group_count * 128; // Est. 128 bytes per group
...
int rogue_enemy_ai_bridge_get_debug_status(const RogueEnemyAIBridge* bridge, char* buffer, size_t buffer_size) {
    ...
    return snprintf(buffer, buffer_size,
        ...
        bridge->successful_syncs,
        bridge->failed_syncs,
        bridge->metrics.average_tick_time_us,
        bridge->metrics.peak_tick_time_us,
        bridge->metrics.performance_warnings,
        bridge->metrics.performance_errors);
}
