...
#define ROGUE_BRIDGE_MAX_NAME_LENGTH 64
    ...
    void rogue_integration_bridge_set_auto_process(RogueIntegrationBridge* bridge,
                                                   bool auto_process);
    void rogue_integration_bridge_set_debug_mode(RogueIntegrationBridge* bridge, bool debug_mode);
    void rogue_integration_bridge_set_max_events_per_frame(RogueIntegrationBridge* bridge,
                                                           int max_events);

    // System registration and management
    bool rogue_integration_bridge_register_system(RogueIntegrationBridge* bridge,
                                                  RogueBridgeSystemType system_type,
                                                  const char* system_name);
    bool rogue_integration_bridge_unregister_system(RogueIntegrationBridge* bridge,
                                                    RogueBridgeSystemType system_type);
    bool rogue_integration_bridge_is_system_active(RogueIntegrationBridge* bridge,
                                                   RogueBridgeSystemType system_type);
    RogueSystemBridge* rogue_integration_bridge_get_system(RogueIntegrationBridge* bridge,
                                                           RogueBridgeSystemType system_type);

    // Event listener management
    uint32_t rogue_integration_bridge_add_listener(RogueIntegrationBridge* bridge,
                                                   RogueBridgeEventType event_type,
                                                   RogueBridgeSystemType system_type,
                                                   RogueBridgeListenerCallback callback,
                                                   void* user_data, const char* listener_name);
    bool rogue_integration_bridge_remove_listener(RogueIntegrationBridge* bridge,
                                                  uint32_t listener_id);
    bool rogue_integration_bridge_enable_listener(RogueIntegrationBridge* bridge,
                                                  uint32_t listener_id, bool enabled);
    int rogue_integration_bridge_get_listeners_for_event(RogueIntegrationBridge* bridge,
                                                         RogueBridgeEventType event_type,
                                                         RogueBridgeListener** listeners,
                                                         int max_listeners);

    // Event creation and posting
    uint32_t rogue_integration_bridge_create_event(RogueIntegrationBridge* bridge,
                                                   RogueBridgeEventType event_type,
                                                   RogueBridgeSystemType source_system,
                                                   RogueBridgeSystemType target_system,
                                                   RogueBridgePriority priority, const void* data,
    ...
    bool rogue_integration_bridge_post_event_immediate(
        RogueIntegrationBridge* bridge, RogueBridgeEventType event_type,
        RogueBridgeSystemType source_system, RogueBridgeSystemType target_system,
        RogueBridgePriority priority, const void* data, size_t data_size, const char* description);

    // Event processing
    int rogue_integration_bridge_process_events(RogueIntegrationBridge* bridge, int max_events);
    bool rogue_integration_bridge_process_single_event(RogueIntegrationBridge* bridge,
                                                       const RogueBridgeEvent* event);
    ...
    bool rogue_integration_bridge_enemy_spawn(RogueIntegrationBridge* bridge,
                                              const RogueEnemySpawnData* spawn_data);
    bool rogue_integration_bridge_enemy_death(RogueIntegrationBridge* bridge,
                                              const RogueEnemyDeathData* death_data);
    bool
    rogue_integration_bridge_enemy_modifier_applied(RogueIntegrationBridge* bridge,
                                                    const RogueEnemyModifierData* modifier_data);

    // Enemy-AI Bridge Listener Helpers
    uint32_t rogue_integration_bridge_add_enemy_spawn_listener(RogueIntegrationBridge* bridge,
                                                               RogueBridgeListenerCallback callback,
                                                               void* user_data);
    uint32_t rogue_integration_bridge_add_enemy_death_listener(RogueIntegrationBridge* bridge,
                                                               RogueBridgeListenerCallback callback,
                                                               void* user_data);
    uint32_t rogue_integration_bridge_add_enemy_modifier_listener(
        RogueIntegrationBridge* bridge, RogueBridgeListenerCallback callback, void* user_data);
    ...
    bool rogue_integration_bridge_equipment_changed(RogueIntegrationBridge* bridge,
                                                    const RogueEquipmentChangeData* equipment_data);
    bool rogue_integration_bridge_combat_damage(RogueIntegrationBridge* bridge,
                                                const RogueCombatDamageData* damage_data);
    ...
    bool rogue_integration_bridge_export_event_log(RogueIntegrationBridge* bridge,
                                                   const char* filename);

    // Performance monitoring
    void rogue_integration_bridge_get_performance_stats(RogueIntegrationBridge* bridge,
