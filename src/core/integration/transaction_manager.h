...
typedef struct RogueTxParticipantDesc {
    ...
    const char* name;
    void* user;
    int (*on_prepare)(void* user, int tx_id, char* errbuf, size_t errcap, uint32_t* out_version);
...
// Commit (runs 2PC). Returns 0 success, <0 on error/abort (still transitions state appropriately).
