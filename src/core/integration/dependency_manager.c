...
RogueDependencyManager* rogue_dependency_manager_create(void)
{
    RogueDependencyManager* manager =
        (RogueDependencyManager*) calloc(1, sizeof(RogueDependencyManager));
    if (!manager)
    {
        return NULL;
    }
...
bool rogue_dependency_manager_add_file(RogueDependencyManager* manager, const char* file_path,
                                       RogueFileType file_type, int priority)
{
    ...
    if (manager->debug_mode)
    {
        printf("[DEBUG] Added file: %s (type: %s, priority: %d)\n", file_path,
               rogue_dependency_manager_get_file_type_name(file_type), priority);
    }
...
RogueDependencyNode* rogue_dependency_manager_find_node(RogueDependencyManager* manager,
...
bool rogue_dependency_manager_update_file_info(RogueDependencyManager* manager,
                                               const char* file_path, uint64_t last_modified,
                                               const char* checksum)
{
    if (!manager || !file_path || !checksum)
        return false;
...
bool rogue_dependency_manager_add_dependency(RogueDependencyManager* manager,
                                             const char* source_file, const char* target_file,
                                             const char* reference_key, RogueDependencyType type,
                                             int priority, const char* description)
{
    ...
    {
        snprintf(dep->description, ROGUE_DEP_MAX_NAME_LENGTH, "%s->%s:%s", source_file, target_file,
                 reference_key);
    }
...
bool rogue_dependency_manager_remove_dependency(RogueDependencyManager* manager,
                                                const char* source_file, const char* target_file,
...
int rogue_dependency_manager_get_dependencies(RogueDependencyManager* manager,
                                              const char* source_file,
                                              RogueDependency* dependencies, int max_dependencies)
{
    ...
    int copy_count = (source_node->dependency_count < max_dependencies)
                         ? source_node->dependency_count
                         : max_dependencies;
...
bool rogue_dependency_manager_resolve_all(RogueDependencyManager* manager)
{
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        for (int j = 0; j < node->dependency_count; j++)
        {
            ...
            RogueDependencyNode* target_node =
                rogue_dependency_manager_find_node(manager, dep->target_file);
    ...
    if (manager->debug_mode)
    {
        printf("[DEBUG] Dependency resolution complete: %s (%llu ms)\n",
               all_resolved ? "SUCCESS" : "FAILED",
               (unsigned long long) manager->last_resolve_time_ms);
    }
...
rogue_dependency_manager_get_dependency_status(RogueDependencyManager* manager,
                                               const char* source_file, const char* target_file,
                                               const char* reference_key)
{
    if (!manager || !source_file || !target_file || !reference_key)
...
static bool dfs_detect_cycle(RogueDependencyManager* manager, RogueDependencyNode* node,
                             int* time_counter)
{
    node->visited = true;
    node->in_path = true;
    ...
    {
        RogueDependency* dep = &node->dependencies[i];
        if (dep->type == ROGUE_DEP_TYPE_CIRCULAR_BREAK)
            continue; // Skip circular breakers
        ...
        if (target_node->in_path)
        {
            ...
            if (manager->graph.cycle_count < 16)
            {
                snprintf(manager->graph.cycles[manager->graph.cycle_count], 512,
                         "Cycle: %s -> %s (via %s)", dep->source_file, dep->target_file,
                         dep->reference_key);
                manager->graph.cycle_count++;
            }
...
int rogue_dependency_manager_get_cycles(RogueDependencyManager* manager, char cycles[][512],
                                        int max_cycles)
{
    if (!manager || !cycles || max_cycles <= 0)
        return 0;

    int copy_count =
        (manager->graph.cycle_count < max_cycles) ? manager->graph.cycle_count : max_cycles;
...
bool rogue_dependency_manager_has_circular_dependency(RogueDependencyManager* manager,
                                                      const char* file_path)
{
    if (!manager || !file_path)
        return false;
...
static void topological_sort_visit(RogueDependencyManager* manager, RogueDependencyNode* node,
                                   char result[][ROGUE_DEP_MAX_PATH_LENGTH], int* result_count,
                                   bool* visited)
{
...
bool rogue_dependency_manager_generate_load_order(RogueDependencyManager* manager,
                                                  RogueLoadOrder* load_order)
{
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        {
            topological_sort_visit(manager, &manager->graph.nodes[i], temp_result, &result_count,
                                   visited);
        }
    }
...
bool rogue_dependency_manager_get_dependency_aware_order(
    RogueDependencyManager* manager, const char** file_paths, int file_count,
    ...
    *ordered_count = 0;
    ...
    for (int i = 0; i < full_order.file_count && *ordered_count < file_count; i++)
    {
        for (int j = 0; j < file_count; j++)
        {
            ...
            {
                strncpy(ordered_files[*ordered_count], full_order.files[i],
                        ROGUE_DEP_MAX_PATH_LENGTH - 1);
...
bool rogue_dependency_manager_analyze_impact(RogueDependencyManager* manager,
                                             const char* changed_file,
                                             RogueImpactAnalysis* analysis)
{
    if (!manager || !changed_file || !analysis)
        return false;
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        for (int j = 0; j < node->dependency_count; j++)
        {
            ...
            if (strcmp(dep->target_file, changed_file) == 0)
            {
                ...
                {
                    strncpy(analysis->reload_files[analysis->reload_count], node->file_path,
                            ROGUE_DEP_MAX_PATH_LENGTH - 1);
                    analysis->reload_files[analysis->reload_count][ROGUE_DEP_MAX_PATH_LENGTH - 1] =
                        '\0';
                ...
                if (analysis->affected_count < ROGUE_DEP_MAX_IMPACT_SYSTEMS)
                {
                    const char* system_name =
                        rogue_dependency_manager_get_file_type_name(node->file_type);
                    ...
                    if (!already_listed)
                    {
                        strncpy(analysis->affected_systems[analysis->affected_count], system_name,
                                ROGUE_DEP_MAX_NAME_LENGTH - 1);
                        analysis->affected_systems[analysis->affected_count]
                                                  [ROGUE_DEP_MAX_NAME_LENGTH - 1] = '\0';
...
int rogue_dependency_manager_get_affected_files(RogueDependencyManager* manager,
                                                const char* changed_file,
                                                char affected_files[][ROGUE_DEP_MAX_PATH_LENGTH],
                                                int max_files)
{
...
int rogue_dependency_manager_get_dependent_systems(RogueDependencyManager* manager,
                                                   const char* file_path,
                                                   char systems[][ROGUE_DEP_MAX_NAME_LENGTH],
                                                   int max_systems)
{
    ...
    int copy_count =
        (analysis.affected_count < max_systems) ? analysis.affected_count : max_systems;
    for (int i = 0; i < copy_count; i++)
    {
...
bool rogue_dependency_manager_validate_file_dependencies(RogueDependencyManager* manager,
                                                         const char* file_path)
{
    if (!manager || !file_path)
        return false;
...
int rogue_dependency_manager_get_unresolved_dependencies(RogueDependencyManager* manager,
                                                         RogueDependency* unresolved,
                                                         int max_unresolved)
{
...
int rogue_dependency_manager_get_missing_dependencies(RogueDependencyManager* manager,
                                                      RogueDependency* missing, int max_missing)
{
...
bool rogue_dependency_manager_add_weak_dependency(RogueDependencyManager* manager,
                                                  const char* source_file, const char* target_file,
                                                  const char* reference_key,
...
bool rogue_dependency_manager_is_weak_dependency(RogueDependencyManager* manager,
                                                 const char* source_file, const char* target_file,
                                                 const char* reference_key)
{
    if (!manager || !source_file || !target_file || !reference_key)
...
int rogue_dependency_manager_get_weak_dependencies(RogueDependencyManager* manager,
                                                   const char* file_path,
                                                   RogueDependency* weak_deps, int max_weak)
{
...
bool rogue_dependency_manager_export_graphviz(RogueDependencyManager* manager,
                                              const char* output_path)
{
    if (!manager || !output_path)
        return false;
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        if (!filename)
            filename = strrchr(node->file_path, '\\');
        if (!filename)
            filename = node->file_path;
        else
            filename++;

        fprintf(f, "  \"%s\" [label=\"%s\\n(%s)\" style=%s];\n", node->file_path, filename,
                rogue_dependency_manager_get_file_type_name(node->file_type),
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        for (int j = 0; j < node->dependency_count; j++)
        {
            ...
            switch (dep->status)
            {
            case ROGUE_DEP_STATUS_RESOLVED:
                color = "green";
                break;
            case ROGUE_DEP_STATUS_MISSING:
                color = "red";
                break;
            case ROGUE_DEP_STATUS_CIRCULAR:
                color = "orange";
                style = "dashed";
                break;
            case ROGUE_DEP_STATUS_UNRESOLVED:
                color = "gray";
                break;
            default:
                break;
            ...
            fprintf(f, "  \"%s\" -> \"%s\" [label=\"%s\" color=%s style=%s];\n", dep->source_file,
                    dep->target_file, dep->reference_key, color, style);
        }
    }
...
void rogue_dependency_manager_print_graph(RogueDependencyManager* manager)
{
    ...
    printf("Files: %d, Dependencies: %d, Valid: %s\n", manager->graph.node_count,
           manager->total_dependencies, manager->graph.is_valid ? "YES" : "NO");
    ...
    for (int i = 0; i < manager->graph.node_count; i++)
    {
        ...
        printf("  %s (%s, priority %d, deps: %d)\n", node->file_path,
               rogue_dependency_manager_get_file_type_name(node->file_type), node->priority,
               node->dependency_count);
...
void rogue_dependency_manager_print_load_order(RogueDependencyManager* manager,
                                               const RogueLoadOrder* load_order)
{
    if (!manager || !load_order)
        return;
...
void rogue_dependency_manager_print_impact_analysis(RogueDependencyManager* manager,
                                                    const RogueImpactAnalysis* analysis)
{
    if (!manager || !analysis)
        return;
...
void rogue_dependency_manager_get_statistics(RogueDependencyManager* manager, int* total_deps,
                                             int* resolved_deps, int* failed_deps,
                                             int* circular_deps)
{
    if (!manager)
        return;
    ...
    if (total_deps)
        *total_deps = manager->total_dependencies;
    if (resolved_deps)
        *resolved_deps = manager->resolved_dependencies;
    if (failed_deps)
        *failed_deps = manager->failed_resolutions;
    if (circular_deps)
        *circular_deps = manager->circular_dependencies;
}

uint64_t rogue_dependency_manager_get_average_resolve_time(RogueDependencyManager* manager)
{
    if (!manager || manager->resolve_count == 0)
        return 0;
...
RogueFileType rogue_dependency_manager_get_file_type_from_path(const char* file_path)
{
    ...
    if (strstr(filename, "items"))
        return ROGUE_FILE_TYPE_ITEMS;
    if (strstr(filename, "affixes"))
        return ROGUE_FILE_TYPE_AFFIXES;
    if (strstr(filename, "enemies"))
        return ROGUE_FILE_TYPE_ENEMIES;
    if (strstr(filename, "encounters"))
        return ROGUE_FILE_TYPE_ENCOUNTERS;
    if (strstr(filename, "loot_tables"))
        return ROGUE_FILE_TYPE_LOOT_TABLES;
    if (strstr(filename, "biomes"))
        return ROGUE_FILE_TYPE_BIOMES;
    if (strstr(filename, "skills"))
        return ROGUE_FILE_TYPE_SKILLS;
...
const char* rogue_dependency_manager_get_file_type_name(RogueFileType file_type)
{
    switch (file_type)
    {
    case ROGUE_FILE_TYPE_ITEMS:
        return "Items";
    case ROGUE_FILE_TYPE_AFFIXES:
        return "Affixes";
    case ROGUE_FILE_TYPE_ENEMIES:
        return "Enemies";
    case ROGUE_FILE_TYPE_ENCOUNTERS:
        return "Encounters";
    case ROGUE_FILE_TYPE_LOOT_TABLES:
        return "LootTables";
    case ROGUE_FILE_TYPE_BIOMES:
        return "Biomes";
    case ROGUE_FILE_TYPE_SKILLS:
        return "Skills";
    case ROGUE_FILE_TYPE_OTHER:
        return "Other";
    default:
        return "Unknown";
    }
...
const char* rogue_dependency_manager_get_dependency_type_name(RogueDependencyType dep_type)
{
    switch (dep_type)
    {
    case ROGUE_DEP_TYPE_STRONG:
        return "Strong";
    case ROGUE_DEP_TYPE_WEAK:
        return "Weak";
    case ROGUE_DEP_TYPE_CIRCULAR_BREAK:
...
{
    if (!reference_key || strlen(reference_key) == 0 ||
        strlen(reference_key) >= ROGUE_DEP_MAX_NAME_LENGTH)
    {
        return false;
    }
