...
static void add_validation_error(RogueSchemaValidationResult* result, RogueSchemaErrorType type,
                                        ...
                                        const char* field_path, RogueSchemaValidationResult* result)
{
    ...
    if (rules->constraints.string.has_min_length && len < rules->constraints.string.min_length)
    {
        char msg[256];
        snprintf(msg, sizeof(msg), "String too short (got %zu, min %u)", len,
                 rules->constraints.string.min_length);
        add_validation_error(result, ROGUE_SCHEMA_ERROR_STRING_TOO_SHORT, field_path, msg);
        return false;
    }

    if (rules->constraints.string.has_max_length && len > rules->constraints.string.max_length)
    {
        char msg[256];
        snprintf(msg, sizeof(msg), "String too long (got %zu, max %u)", len,
                 rules->constraints.string.max_length);
                                         ...
                                         RogueSchemaValidationResult* result)
{
    ...
    {
        char msg[256];
        snprintf(msg, sizeof(msg), "Integer too small (got %lld, min %lld)", (long long) val,
                 (long long) rules->constraints.integer.min_value);
    ...
    {
        char msg[256];
        snprintf(msg, sizeof(msg), "Integer too large (got %lld, max %lld)", (long long) val,
                 (long long) rules->constraints.integer.max_value);
...
bool rogue_schema_register(RogueSchemaRegistry* registry, const RogueSchema* schema)
{
    ...
    if (registry->schema_count >= registry->schema_capacity)
    {
        ...
        RogueSchema* new_schemas = realloc(registry->schemas, new_capacity * sizeof(RogueSchema));
        if (!new_schemas)
        {
                                 ...
                                 const char* field_path, RogueSchemaValidationResult* result)
{
    ...
    case ROGUE_SCHEMA_TYPE_ARRAY:
        if (value->type == JSON_ARRAY)
        {
            ...
            {
                char msg[256];
                snprintf(msg, sizeof(msg), "Array too short (got %zu, min %u)", count,
                         field->validation.constraints.array.min_items);
            ...
            {
                char msg[256];
                snprintf(msg, sizeof(msg), "Array too long (got %zu, max %u)", count,
                         field->validation.constraints.array.max_items);
    ...
    case ROGUE_SCHEMA_TYPE_ENUM:
        if (value->type == JSON_STRING && field->enum_values)
        {
            ...
            if (!found)
            {
                ...
                add_validation_error(result, ROGUE_SCHEMA_ERROR_ENUM_VALUE_INVALID, field_path,
                                     msg);
                valid = false;
            }
        }
    ...
    if (valid && field->validation.custom_validator)
    {
        if (!field->validation.custom_validator(value, field, field->validation.validation_context))
        {
            ...
            add_validation_error(result, ROGUE_SCHEMA_ERROR_CUSTOM_VALIDATION_FAILED, field_path,
                                 msg);
            valid = false;
        }
    }
                                ...
                                RogueSchemaValidationResult* result)
{
    ...
    {
        add_validation_error(result, ROGUE_SCHEMA_ERROR_INVALID_TYPE, "",
...
const char* rogue_schema_error_to_string(RogueSchemaErrorType error_type)
{
    switch (error_type)
    {
    case ROGUE_SCHEMA_ERROR_NONE:
        return "No error";
    case ROGUE_SCHEMA_ERROR_INVALID_TYPE:
        return "Invalid type";
    ...
    case ROGUE_SCHEMA_ERROR_VALUE_TOO_LARGE:
        return "Value too large";
    case ROGUE_SCHEMA_ERROR_STRING_TOO_SHORT:
        return "String too short";
    case ROGUE_SCHEMA_ERROR_STRING_TOO_LONG:
        return "String too long";
    case ROGUE_SCHEMA_ERROR_PATTERN_MISMATCH:
        return "Pattern mismatch";
    case ROGUE_SCHEMA_ERROR_ENUM_VALUE_INVALID:
        return "Invalid enum value";
    case ROGUE_SCHEMA_ERROR_ARRAY_TOO_SHORT:
        return "Array too short";
    case ROGUE_SCHEMA_ERROR_ARRAY_TOO_LONG:
        return "Array too long";
    case ROGUE_SCHEMA_ERROR_CUSTOM_VALIDATION_FAILED:
        return "Custom validation failed";
    case ROGUE_SCHEMA_ERROR_CIRCULAR_REFERENCE:
        return "Circular reference";
    case ROGUE_SCHEMA_ERROR_SCHEMA_NOT_FOUND:
...
const char* rogue_schema_type_to_string(RogueSchemaType type)
{
    ...
    case ROGUE_SCHEMA_TYPE_ENUM:
        return "enum";
    case ROGUE_SCHEMA_TYPE_REFERENCE:
        return "reference";
    default:
        return "unknown";
    }
...
RogueSchemaField* rogue_schema_add_field(RogueSchema* schema, const char* name,
...
void rogue_schema_field_set_range(RogueSchemaField* field, int64_t min, int64_t max)
{
    ...
    field->validation_flags |=
        ROGUE_SCHEMA_VALIDATION_MIN_VALUE | ROGUE_SCHEMA_VALIDATION_MAX_VALUE;
...
void rogue_schema_field_set_string_length(RogueSchemaField* field, uint32_t min, uint32_t max)
{
    ...
    field->validation_flags |=
        ROGUE_SCHEMA_VALIDATION_MIN_LENGTH | ROGUE_SCHEMA_VALIDATION_MAX_LENGTH;
...
bool rogue_schema_validate_dependencies(const RogueSchemaRegistry* registry,
                                        const RogueSchema* schema)
{
    (void) registry;
    (void) schema;
...
bool rogue_schema_export_json_schema(const RogueSchema* schema, char* output_buffer,
                                     size_t buffer_size)
{
    (void) schema;
    (void) output_buffer;
...
bool rogue_schema_detect_circular_references(const RogueSchemaRegistry* registry,
                                             const RogueSchema* schema)
{
    (void) registry;
    (void) schema;
