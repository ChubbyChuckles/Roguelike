...
static void rb_stats_init(void)
{
    static int inited = 0;
    if (!inited)
    {
        memset(&g_rb_stats, 0, sizeof(g_rb_stats));
        inited = 1;
    }
}
...
{
    if (system_id < 0 || system_id >= ROGUE_MAX_SYSTEMS)
        return -1;
    if (capacity == 0 || capacity > ROGUE_MAX_ROLLBACK_SNAPSHOTS)
        return -2;
...
static int rb_capture_locked(int system_id)
{
    ...
    RollbackEntry* prev_full = NULL;
    int idx_latest = (int) r->head - 1;
    if (idx_latest < 0)
        idx_latest = (int) r->capacity - 1;
    if (r->count > 0)
    {
        RollbackEntry* last = &r->entries[idx_latest];
        if (last->type == RB_ENTRY_FULL)
            prev_full = last;
        else if (last->type == RB_ENTRY_DELTA)
        { // find its base
            for (uint32_t k = 0; k < r->count; k++)
            {
                int pos = (int) r->head - 1 - (int) k;
                while (pos < 0)
                    pos += (int) r->capacity;
                RollbackEntry* cand = &r->entries[pos];
                if (cand->type == RB_ENTRY_FULL)
                {
                    prev_full = cand;
                    break;
                }
    ...
    unsigned char* store_buf = NULL;
    size_t store_size = 0;
    RollbackEntryType type = RB_ENTRY_FULL;
    ...
    if (prev_full && prev_full->size == snap->size && prev_full->size > 0)
    {
        ...
        const unsigned char* a = prev_full->data;
        const unsigned char* b =
            (const unsigned char*) snap->data; // prev_full->data holds full snapshot
        // naive diff: store (uint32_t count, then pairs
        // (offset(uint32_t),length(uint32_t),bytes...)) if savings > header
        size_t max_changes = 0;
        for (size_t i = 0; i < full_size; i++)
            if (a[i] != b[i])
            {
                max_changes++;
                while (i + 1 < full_size && a[i + 1] != b[i + 1])
                    i++;
            }
        size_t est_header = 8 + max_changes * (8); // rough
        unsigned char* diff = malloc(full_size + est_header);
        if (!diff)
            return -3;
        unsigned char* p = diff;
        uint32_t range_count = 0;
        p += 8; // reserve space (range_count, full_size)
        for (size_t i = 0; i < full_size; i++)
            if (a[i] != b[i])
            {
                size_t start = i;
                ...
                memcpy(p, &start, sizeof(uint32_t));
                p += 4;
        ...
        memcpy(diff, &range_count, 4);
        memcpy(diff + 4, &full_size, 4);
        size_t diff_size = (size_t) (p - diff);
        if (diff_size < full_size)
        {
            store_buf = diff;
            store_size = diff_size;
            type = RB_ENTRY_DELTA;
            base_version = prev_full->version;
            delta_bytes = diff_size;
            g_rb_stats.delta_entries++;
            g_rb_stats.bytes_delta += diff_size;
            g_rb_stats.bytes_saved_via_delta += (full_size > diff_size ? full_size - diff_size : 0);
        }
        else
        {
            free(diff);
    ...
    if (!store_buf)
    {
        store_buf = malloc(full_size);
        if (!store_buf)
            return -3;
        memcpy(store_buf, snap->data, full_size);
        store_size = full_size;
        type = RB_ENTRY_FULL;
        g_rb_stats.bytes_full += full_size;
    }
...
int rogue_rollback_capture_multi(const int* system_ids, size_t count)
{
    ...
    {
        int rc = rogue_rollback_capture(system_ids[i]);
        if (rc)
            return rc;
    }
    return 0;
...
int rogue_rollback_step_back(int system_id, uint32_t steps)
{
    ...
    if (e->type == RB_ENTRY_DELTA)
    { // find base FULL entry
        RollbackEntry* base = NULL;
        for (uint32_t k = 0; k < r->count; k++)
        {
            int pos = (int) r->head - 1 - (int) k;
            while (pos < 0)
                pos += (int) r->capacity;
            RollbackEntry* cand = &r->entries[pos];
            if (cand->type == RB_ENTRY_FULL && cand->version == e->base_version)
            {
                base = cand;
                break;
            }
        ...
        if (!base)
            return -6;
        full_buf = malloc(e->size);
        if (!full_buf)
            return -7;
        memcpy(full_buf, base->data, base->size);
        // parse delta
        unsigned char* p = e->data;
        uint32_t range_count = 0;
        memcpy(&range_count, p, 4);
        p += 4;
        uint32_t full_size = 0;
        memcpy(&full_size, p, 4);
        p += 4;
        if (full_size != e->size)
        {
            free(full_buf);
        ...
        for (uint32_t ri = 0; ri < range_count; ri++)
        {
            ...
            memcpy(&len, p, 4);
            p += 4;
            if (off + len > full_size)
            {
                free(full_buf);
                return -9;
            }
            memcpy(full_buf + off, p, len);
            p += len;
        }
    }
    const void* restore_data = (e->type == RB_ENTRY_FULL) ? e->data : full_buf;
    RogueSystemSnapshot tmp = {system_id, NULL, e->version, e->hash, e->size, (void*) restore_data,
                               0};
    ...
    if (rc == 0)
    {
        ...
        {
            g_rb_stats.validation_failures++;
        }
        // log event
        RogueRollbackEvent* ev = &g_events[g_event_head];
        ev->timestamp = ++g_event_seq;
        ev->system_id = system_id;
        ev->to_version = e->version;
        ev->from_version = cur ? cur->version : 0;
        ev->bytes_rewound = e->size;
        ev->was_delta = (uint8_t) (e->type == RB_ENTRY_DELTA);
        ev->auto_triggered = 0;
        g_event_head = (g_event_head + 1) % ROGUE_ROLLBACK_EVENT_CAP;
        if (g_event_count < ROGUE_ROLLBACK_EVENT_CAP)
            g_event_count++;
    }
    if (full_buf)
        free(full_buf);
...
int rogue_rollback_latest(int system_id) { return rogue_rollback_step_back(system_id, 0); }
...
int rogue_rollback_dump(void* fptr)
{
    ...
    for (int i = 0; i < ROGUE_MAX_SYSTEMS; i++)
    {
        ...
        if (!r->configured)
            continue;
        fprintf(f, "System %d: cap=%u count=%u head=%u versions:", i, r->capacity, r->count,
                r->head);
        for (uint32_t k = 0; k < r->count; k++)
        {
            int32_t pos = (int32_t) r->head - 1 - (int32_t) k;
            while (pos < 0)
                pos += r->capacity;
            RollbackEntry* e = &r->entries[pos];
            fprintf(f, " %u", e->version);
        }
        fprintf(f, "\n");
    }
    return 0;
...
int rogue_rollback_partial(const int* system_ids, const uint32_t* steps, size_t count)
{
    if (!system_ids || !steps)
        return -1;
    for (size_t i = 0; i < count; i++)
    {
        int rc = rogue_rollback_step_back(system_ids[i], steps[i]);
        if (rc)
            return rc;
        g_rb_stats.partial_rollbacks++;
    }
    return 0;
}
...
{
    if (participant_id < 0 || participant_id >= (int) (sizeof(g_participant_system_map) /
                                                       sizeof(g_participant_system_map[0])))
        return -1;
    int sys = g_participant_system_map[participant_id];
    if (sys <= 0)
        return -2; // 0 reserved / unmapped
    // manually reproduce step_back logic for logging auto flag
    RollbackRing* r = &g_rb[sys];
    if (!r->configured || r->count == 0)
        return -3;
    int32_t idx = (int32_t) r->head - 1;
    if (idx < 0)
        idx += r->capacity;
    RollbackEntry* e = &r->entries[idx];
    unsigned char* full_buf = NULL;
    ...
    if (rc == 0)
    {
        ...
        g_rb_stats.systems_rewound++;
        g_rb_stats.bytes_rewound += e->size;
        g_rb_stats.auto_rollbacks++;
        const RogueSystemSnapshot* cur = rogue_snapshot_get(sys);
        if (cur && cur->hash != e->hash)
            g_rb_stats.validation_failures++;
        RogueRollbackEvent* ev = &g_events[g_event_head];
        ev->timestamp = ++g_event_seq;
...
int rogue_rollback_map_participant(int participant_id, int system_id)
{
    if (participant_id < 0 || participant_id >= (int) (sizeof(g_participant_system_map) /
                                                       sizeof(g_participant_system_map[0])))
        return -1;
    g_participant_system_map[participant_id] = system_id;
    return 0;
}
...
int rogue_rollback_events_get(const RogueRollbackEvent** out_events, size_t* count)
{
    ...
    if (g_event_count == 0)
    {
        ...
        *count = 0;
        return 0;
    } // produce contiguous logical order oldest→newest (simple if ring wrapped we need temp order)
    // For simplicity, return physical array start; consumer must handle wrap (documented). We'll
    // reorder only if wrapped into a static temp.
    *out_events = g_events;
    *count = g_event_count;
    return 0;
}
