...
void rogue_validation_tick(uint64_t current_tick);  // call each frame with monotonically increasing tick
int rogue_validation_run_now(int force_all); // force_all=1 ignores incremental hash skip
void rogue_validation_trigger(void); // mark pending run next tick (even if interval not reached)
