...
// *inout_count provides capacity on input, actual count on success. Returns 0 on success, <0 on error (cycle detected -2, capacity -3).
...
// Get pointer to immutable array of records in chronological order (oldest→newest). count returns number of valid records.
int rogue_snapshot_replay_log_get(const RogueSnapshotDeltaRecord** out_records, size_t* out_count);
// Reapply recorded deltas from start_index (0 oldest) count entries to advance system states; validates hashes.
