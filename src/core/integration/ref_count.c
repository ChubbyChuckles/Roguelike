...
{
    uint32_t type_id;
    RogueRcEdgeEnumFn fn;
} s_edge_enum[RC_MAX_EDGE_ENUM];
#ifdef _MSC_VER
...
{
    while (InterlockedCompareExchange(&s_live_lock, 1, 0) != 0)
    { /* spin */
    }
}
...
static void live_lock(void)
{
    while (atomic_flag_test_and_set_explicit(&s_live_lock, memory_order_acquire))
    {
    }
}
...
static RogueRcHeader* hdr_from_obj(void* obj)
{
    if (!obj)
        return NULL;
    return ((RogueRcHeader*) obj) - 1;
}
static const RogueRcHeader* hdr_from_cobj(const void* obj)
{
    if (!obj)
        return NULL;
    return ((const RogueRcHeader*) obj) - 1;
}
...
void* rogue_rc_alloc(size_t size, uint32_t type_id, void (*dtor)(void*))
{
    ...
    live_lock();
    if (live_now > s_peak_live)
        s_peak_live = live_now;
    live_unlock();
#else
    ...
    while (live_now > peak && !atomic_compare_exchange_weak(&s_peak_live, &peak, live_now))
    {
        peak = atomic_load(&s_peak_live);
    }
#endif
...
static void live_remove(RogueRcHeader* h)
{
    if (!h)
        return;
    if (h->prev)
...
#else
        if (atomic_load_explicit(&h->weak, memory_order_acquire) == 0)
        {
...
void* rogue_rc_weak_acquire(RogueWeakRef weak)
{
    ...
    for (;;)
    {
        ...
        if (s == 0)
            return NULL;
        if (InterlockedCompareExchange(&h->strong, s + 1, s) == s)
            return (void*) (h + 1);
    }
#else
    ...
    while (s)
    {
        if (atomic_compare_exchange_weak(&h->strong, &s, s + 1))
            return (void*) (h + 1);
    }
    return NULL;
#endif
...
void rogue_rc_dump_leaks(void* fptr)
{
    ...
    while (cur)
    {
        ...
        fprintf(f, " id=%llu type=%u strong=%ld weak=%ld\n", (unsigned long long) cur->id,
                cur->type_id, cur->strong, cur->weak);
#else
        fprintf(f, " id=%llu type=%u strong=%u weak=%u\n", (unsigned long long) cur->id,
                cur->type_id, atomic_load(&cur->strong), atomic_load(&cur->weak));
#endif
        cur = cur->next;
    }
    live_unlock();
...
void rogue_rc_iterate(RogueRcIterFn fn, void* ud)
{
    ...
    while (cur)
    {
        RogueRcHeader* next = cur->next; // allow callback to release
        if (!fn((void*) (cur + 1), cur->type_id, cur->id, ud))
        {
            live_unlock();
            return;
        }
...
static RogueRcEdgeEnumFn find_enum(uint32_t type)
{
    ...
    {
        if (s_edge_enum[i].type_id == type)
            return s_edge_enum[i].fn;
    }
    return NULL;
}

size_t rogue_rc_generate_dot(char* buf, size_t max)
{ // 2-pass length compute
    ...
    while (cur)
    {
        ...
        needed += snprintf(line, sizeof(line), " n%llu [label=\"t%u s%u w%u\"];\n",
                           (unsigned long long) cur->id, cur->type_id, atomic_load(&cur->strong),
                           atomic_load(&cur->weak));
#endif
        RogueRcEdgeEnumFn efn = find_enum(cur->type_id);
        if (efn)
        {
            void* children[32];
            size_t c = efn((void*) (cur + 1), children, 32);
            for (size_t i = 0; i < c; i++)
            {
                RogueRcHeader* ch = hdr_from_obj(children[i]);
                if (ch)
                    needed += snprintf(line, sizeof(line), " n%llu -> n%llu;\n",
...
#define APPEND_STR(s)                                                                              \
    ...
    while (cur)
    {
        ...
        int l = snprintf(line, sizeof(line), " n%llu [label=\"t%u s%u w%u\"];\n",
                         ...
                         atomic_load(&cur->weak));
#endif
        if (written + (size_t) l < max)
        {
            memcpy(buf + written, line, l);
            written += l;
        }
        RogueRcEdgeEnumFn efn = find_enum(cur->type_id);
        if (efn)
        {
            ...
            for (size_t i = 0; i < c; i++)
            {
                ...
                if (ch)
                {
                    int l2 = snprintf(line, sizeof(line), " n%llu -> n%llu;\n",
                                      (unsigned long long) cur->id, (unsigned long long) ch->id);
                    if (written + (size_t) l2 < max)
                    {
                        memcpy(buf + written, line, l2);
                        written += l2;
                    }
                }
            }
...
bool rogue_rc_validate(void)
{
    ...
    live_unlock();
#ifdef _MSC_VER
    if (count != (uint64_t) s_live_objects)
        ok = false;
#else
    if (count != atomic_load(&s_live_objects))
        ok = false;
#endif
    return ok;
}
