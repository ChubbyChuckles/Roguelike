...
/* Cross-platform monotonic time (nanoseconds). Uses clock_gettime if available, else clock() fallback (low resolution). */
static unsigned long long rogue_time_now_ns(void){
#if defined(_WIN32) && (defined(_MSC_VER))
    static int inited=0; static double freq_inv=0.0; static LARGE_INTEGER f; LARGE_INTEGER c; if(!inited){ QueryPerformanceFrequency(&f); freq_inv = 1000000000.0 / (double)f.QuadPart; inited=1; } QueryPerformanceCounter(&c); return (unsigned long long)((double)c.QuadPart * freq_inv);
#elif defined(CLOCK_MONOTONIC_RAW)
    struct timespec ts; clock_gettime(CLOCK_MONOTONIC_RAW,&ts); return (unsigned long long)ts.tv_sec*1000000000ull + (unsigned long long)ts.tv_nsec;
...
static int sum_weights_scalar(const int* w, int n){ unsigned long long t0=rogue_time_now_ns(); int total=0; for(int i=0;i<n;i++){ total += w[i]; } g_perf.m.affix_roll_scalar_sums++; g_perf.m.weight_sum_time_ns += (rogue_time_now_ns()-t0); return total; }
...
static int sum_weights_simd(const int* w, int n){
    ...
    for(;i<n;i++) total += w[i];
    g_perf.m.affix_roll_simd_sums++; g_perf.m.weight_sum_time_ns += (rogue_time_now_ns()-t0); return total;
#else
...
void rogue_loot_perf_affix_roll_end(void){ if(g_perf.affix_roll_t0){ unsigned long long dt = rogue_time_now_ns() - g_perf.affix_roll_t0; g_perf.m.affix_roll_time_ns += dt; g_perf.affix_roll_t0=0; } }
