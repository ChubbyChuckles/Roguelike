...
void rogue_player_render(void){
    ...
    if(spr->sw && spr->tex && spr->tex->handle){
        ...
        if(g_app.levelup_aura_timer_ms>0.0f){
            ...
            unsigned char cr=(unsigned char)(120 + 90*pulse); unsigned char cg=(unsigned char)(80 + 120*pulse); unsigned char cb=255; unsigned char ca=(unsigned char)(120*tnorm + 60);
            SDL_SetRenderDrawColor(g_app.renderer,cr,cg,cb,ca);
            for(int dy=-radius; dy<=radius; ++dy){ int dx_lim=(int)sqrt(radius*radius - dy*dy); SDL_RenderDrawLine(g_app.renderer,cx-dx_lim,cy+dy,cx+dx_lim,cy+dy); }
        }
