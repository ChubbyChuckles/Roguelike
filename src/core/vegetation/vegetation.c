...
static int parse_line(char* line, int is_tree)
{
    ...
    */
    ...
    /* Normalize path: configs currently store '../assets/...' but loader already tries ../ prefix
       variants. Strip a single leading '../' so base path becomes 'assets/...', enabling
       '../assets' fallback to succeed from build dir. */
    if (strncmp(d->image, "../assets/", 10) == 0)
    {
        size_t len = strlen(d->image);
        memmove(d->image, d->image + 3,
                len - 2); /* shift left 3 chars ('../') incl. null terminator */
    }
    /* tx1 */
    tok = strtok_s(NULL, ",\r\n", &context);
    if (!tok)
    ...
    { /* Could be ty2 */
        ...
        if (look3)
        { /* Treat look & look2 as tx2,ty2 and look3 as rarity */
...
{
    FILE* f = NULL;
    int which = open_with_fallback(plants_cfg, &f);
    if (f)
    {
        char line[512];
    ...
    f = NULL;
    which = open_with_fallback(trees_cfg, &f);
    if (f)
    {
        char line[512];
        while (fgets(line, sizeof line, f))
...
void rogue_vegetation_generate(float tree_cover_target, unsigned int seed)
{
    ...
    if (tree_cover_target > 0.70f)
        tree_cover_target = 0.70f; /* safety */
    ...
    if (grass_count == 0)
    {
        ...
        use_forest_as_grass =
            1; /* treat forest as base when no grass exists (e.g., heavily forested seeds) */
    }
           ...
           attempts < max_attempts)
    {
        ...
        if (density < 0.48f && g_instance_count > 0)
            continue; /* allow first tree anywhere */
        ...
        inst->variant = 0;
        inst->growth = 255;
        placed_canopy_tiles += (int) (3.14159f * d->canopy_radius * d->canopy_radius *
                                      0.55f); /* approximate fill factor */
    }
...
/* Trunk-only collision for entities with viewpoint perspective rules:
   - From below: allow overlap until entity center y surpasses tree center y - trunk_allow_front
   (50% sprite height overlap rule approximated)
   - From sides: allow approach so long as within horizontal trunk fraction (about 25% each side of
   center) before blocking.
   - From above (behind tree): block earlier when entity within back distance (approx 2 tiles above
   base). We approximate trunk as an axis-aligned rectangle centered at tree.x with width = sprite_w
   * trunk_frac_w and height = sprite_h * trunk_frac_h starting at base downward. Because we only
   have tile metrics here, derive sprite dimensions from definition tile rectangle. */
int rogue_vegetation_entity_blocking(float ox, float oy, float nx, float ny)
{
    ...
    /* New simplified trunk logic:
         ...
         We intentionally ignore large directional rules that previously produced wide invisible
       walls. Blocking region per tree: horizontal: |x - base_x| < trunk_r  (small) vertical  :
       base_y - trunk_h_top <= y <= base_y + trunk_h_bottom A tiny "approach from behind" cushion
       extends 0.15 tiles above the top to prevent popping through from above. This keeps collision
       localized to visible roots while still stopping passage through trunk.
    */
    ...
    for (int i = 0; i < g_instance_count; i++)
        if (g_instances[i].is_tree)
        {
            ...
            float base_x = inst->x;
            float base_y = inst->y;
            /* Radius scales a little with width but capped very small so player can slide around
             * sides. */
            ...
            float trunk_top =
                base_y -
                0.30f; /* block starts closer to base to let player step further under canopy */
            ...
            float y = py;
            if (adx > trunk_r)
                continue; /* horizontally outside */
            if (y >= trunk_top && y <= trunk_bottom)
                return 1; /* inside trunk band */
