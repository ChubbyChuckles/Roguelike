...
void rogue_skills_shutdown(void)
{
    ...
    for (int i = 0; i < g_skill_count_internal; i++)
    {
        if (g_skill_defs_internal && g_skill_defs_internal[i].name)
        {
            free((char*) g_skill_defs_internal[i].name);
            g_skill_defs_internal[i].name = NULL;
        }
        if (g_skill_defs_internal && g_skill_defs_internal[i].icon)
        {
...
int rogue_skill_register(const RogueSkillDef* def)
{
    ...
    *d = *def;
    ...
    if (nt)
    {
        ...
        g_app.skill_icon_textures[g_skill_count_internal - 1].handle = NULL;
        g_app.skill_icon_textures[g_skill_count_internal - 1].w = 0;
        g_app.skill_icon_textures[g_skill_count_internal - 1].h = 0;
    }
#endif
    return d->id;
}
...
static char* rogue__strdup(const char* s)
{
    if (!s)
        return NULL;
    size_t n = strlen(s);
...
int rogue_skills_load_from_cfg(const char* path)
{
    ...
    {
        ROGUE_LOG_ERROR("Failed to open skills cfg: %s", path);
        return 0;
    }
#else
    f = fopen(path, "rb");
    ...
    while (fgets(line, sizeof line, f))
    {
        ...
        if (colc < 1)
        {
            ROGUE_LOG_WARN("skills.cfg line %d: empty/invalid", lineno);
            continue;
        }
        ROGUE_LOG_DEBUG("skills.cfg line %d: name=%s icon=%s", lineno, cols[0],
                        (colc > 1) ? cols[1] : "<none>");
        RogueSkillDef def;
        ...
        if (colc > 1)
            def.icon = rogue__strdup(cols[1]);
        if (colc > 2)
            def.max_rank = atoi(cols[2]);
        else
            def.max_rank = 1;
        if (colc > 3)
            def.base_cooldown_ms = (float) atof(cols[3]);
        if (colc > 4)
            def.cooldown_reduction_ms_per_rank = (float) atof(cols[4]);
        if (colc > 5)
            def.is_passive = atoi(cols[5]);
        if (colc > 6)
            def.tags = atoi(cols[6]);
        if (colc > 7)
            def.synergy_id = atoi(cols[7]);
        if (colc > 8)
            def.synergy_value_per_rank = atoi(cols[8]);
        if (colc > 9)
            def.resource_cost_mana = atoi(cols[9]);
        if (colc > 10)
            def.action_point_cost = atoi(cols[10]);
        if (colc > 11)
            def.max_charges = atoi(cols[11]);
        if (colc > 12)
            def.charge_recharge_ms = (float) atof(cols[12]);
        if (colc > 13)
            def.cast_time_ms = (float) atof(cols[13]);
        if (colc > 14)
            def.input_buffer_ms = (unsigned short) atoi(cols[14]);
        if (colc > 15)
            def.min_weave_ms = (unsigned short) atoi(cols[15]);
        if (colc > 16)
            def.early_cancel_min_pct = (unsigned char) atoi(cols[16]);
        if (colc > 17)
            def.cast_type = (unsigned char) atoi(cols[17]);
        if (colc > 18)
            def.combo_builder = (unsigned char) atoi(cols[18]);
        if (colc > 19)
            def.combo_spender = (unsigned char) atoi(cols[19]);
        if (colc > 20)
            def.effect_spec_id = atoi(cols[20]);
        /* Register (assigns id) */
        ...
        if (rid >= 0)
        {
            ...
            if (def.icon && g_app.skill_icon_textures)
            {
                ...
                {
                    snprintf(full, sizeof full, "%s", def.icon);
                }
                else
                {
                    snprintf(full, sizeof full, "assets/%s", def.icon);
                }
                if (rogue_texture_load(&g_app.skill_icon_textures[rid], full))
                {
                    ROGUE_LOG_INFO("Skill icon loaded (id=%d name=%s path=%s)", rid, def.name,
                                   full);
                }
                else
                {
                    ROGUE_LOG_WARN("Skill icon FAILED to load (id=%d name=%s path=%s)", rid,
                                   def.name, full);
                }
            }
#endif
        ...
        {
            ROGUE_LOG_ERROR("Failed to register skill '%s' (line %d)",
                            def.name ? def.name : "<null>", lineno);
            free((char*) def.name);
            free((char*) def.icon);
        }
    }
