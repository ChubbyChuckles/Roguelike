...
static PassiveJournalEntry* g_journal=NULL; static int g_journal_count=0; static int g_journal_cap=0;
...
static void journal_append(int node_id, unsigned int ts){
    if(g_journal_count==g_journal_cap){ int new_cap = g_journal_cap? g_journal_cap*2:32; PassiveJournalEntry* nre=(PassiveJournalEntry*)realloc(g_journal,sizeof(PassiveJournalEntry)*(size_t)new_cap); if(!nre) return; g_journal=nre; g_journal_cap=new_cap; }
    PassiveJournalEntry e; e.node_id=node_id; e.ts=ts; g_journal[g_journal_count++]=e;
    /* hash update */
    unsigned long long h = g_journal_hash; unsigned char* p=(unsigned char*)&e; for(size_t i=0;i<sizeof e;i++){ h ^= p[i]; h *= 1099511628211ull; } g_journal_hash=h;
}
...
int rogue_progression_passives_load_dsl(const char* text){ if(!g_initialized||!text) return -1; /* reset previous effects */
    g_effect_count=0; if(g_node_effects) memset(g_node_effects,0,sizeof(PassiveNodeEffects)*(size_t)g_node_count);
    const char* p=text; int line=0; char linebuf[256];
    while(*p){ int len=0; while(p[len] && p[len]!='\n' && len<255){ linebuf[len]=p[len]; len++; } linebuf[len]='\0'; if(p[len]=='\n') p += len+1; else p += len; line++;
        /* trim */ char* s=linebuf; while(isspace((unsigned char)*s)) s++; if(*s=='#' || *s=='\0') continue; /* comment/blank */
        /* tokenize */ char* ctx=NULL; char* tok=ROGUE_STRTOK(s," \t",&ctx); if(!tok) continue; int node_id=atoi(tok); if(node_id<0||node_id>=g_node_count) continue; PassiveNodeEffects* pne=&g_node_effects[node_id]; pne->offset=g_effect_count; pne->count=0; char* eff;
        for(;;){
