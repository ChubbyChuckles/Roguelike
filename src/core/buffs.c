...
int rogue_buffs_apply(RogueBuffType type, int magnitude, double duration_ms, double now_ms, RogueBuffStackRule rule, int snapshot){
    if(magnitude<=0 || duration_ms<=0) return 0; if(now_ms<0) now_ms=0; if(rule<0||rule>ROGUE_BUFF_STACK_ADD) rule=ROGUE_BUFF_STACK_ADD;
    /* dampening */
    for(int i=0;i<ROGUE_MAX_ACTIVE_BUFFS;i++) if(g_buffs[i].active && g_buffs[i].type==type){ if(now_ms - g_buffs[i].last_apply_ms < g_min_reapply_interval_ms) return 0; break; }
    for(int i=0;i<ROGUE_MAX_ACTIVE_BUFFS;i++) if(g_buffs[i].active && g_buffs[i].type==type){
        RogueBuff* b=&g_buffs[i]; b->last_apply_ms=now_ms;
        switch(rule){
            case ROGUE_BUFF_STACK_UNIQUE: return 0; /* ignore */
            case ROGUE_BUFF_STACK_REFRESH: if(magnitude > b->magnitude) b->magnitude=magnitude; b->end_ms = now_ms + duration_ms; return 1;
            case ROGUE_BUFF_STACK_EXTEND: b->end_ms += duration_ms; if(b->end_ms < now_ms + duration_ms) b->end_ms = now_ms + duration_ms; if(b->magnitude < magnitude) b->magnitude=magnitude; return 1;
            case ROGUE_BUFF_STACK_ADD: default: b->magnitude += magnitude; if(b->magnitude>999) b->magnitude=999; if(now_ms + duration_ms > b->end_ms) b->end_ms = now_ms + duration_ms; return 1; }
    }
    for(int i=0;i<ROGUE_MAX_ACTIVE_BUFFS;i++) if(!g_buffs[i].active){
        g_buffs[i].active=1; g_buffs[i].type=type; g_buffs[i].magnitude=magnitude; g_buffs[i].end_ms = now_ms + duration_ms; g_buffs[i].snapshot = snapshot?1:0; g_buffs[i].stack_rule=rule; g_buffs[i].last_apply_ms=now_ms; return 1;
    }
    return 0;
}

int rogue_buffs_strength_bonus(void){ int total=0; for(int i=0;i<ROGUE_MAX_ACTIVE_BUFFS;i++) if(g_buffs[i].active && g_buffs[i].type==ROGUE_BUFF_STAT_STRENGTH) total += g_buffs[i].magnitude; return total; }
void rogue_buffs_set_dampening(double min_interval_ms){ if(min_interval_ms<0) min_interval_ms=0; g_min_reapply_interval_ms=min_interval_ms; }
