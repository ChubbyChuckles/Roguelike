...
#ifdef ROGUE_HAVE_SDL
typedef struct WeaponOverlayItem { SDL_Texture* tex; float x,y,w,h; float pivot_x,pivot_y; float angle; int flip; unsigned char r,g,b,a; } WeaponOverlayItem;
static WeaponOverlayItem g_weapon_overlays[32]; static int g_weapon_overlay_count=0;
#endif
...
void rogue_scene_drawlist_flush(void){
    ...
    for(int i=0;i<g_weapon_overlay_count;i++){
        WeaponOverlayItem* w=&g_weapon_overlays[i]; if(!w->tex) continue; SDL_FRect dst={w->x,w->y,w->w,w->h}; SDL_FPoint pivot={ w->w*w->pivot_x, w->h*w->pivot_y }; SDL_SetTextureColorMod(w->tex,w->r,w->g,w->b); if(w->a!=255) SDL_SetTextureAlphaMod(w->tex,w->a); SDL_RenderCopyExF(g_app.renderer,w->tex,NULL,&dst,(double)w->angle,&pivot,w->flip? SDL_FLIP_HORIZONTAL: SDL_FLIP_NONE); SDL_SetTextureColorMod(w->tex,255,255,255); if(w->a!=255) SDL_SetTextureAlphaMod(w->tex,255); }
#else
...
#ifdef ROGUE_HAVE_SDL
    if(!sdl_texture) return; if(g_weapon_overlay_count >= (int)(sizeof(g_weapon_overlays)/sizeof(g_weapon_overlays[0]))) return;
    WeaponOverlayItem* it=&g_weapon_overlays[g_weapon_overlay_count++]; it->tex=(SDL_Texture*)sdl_texture; it->x=x; it->y=y; it->w=w; it->h=h; it->pivot_x=pivot_x; it->pivot_y=pivot_y; it->angle=angle_deg; it->flip=flip; it->r=r; it->g=g; it->b=b; it->a=a;
#else
    (void)sdl_texture; (void)x; (void)y; (void)w; (void)h; (void)pivot_x; (void)pivot_y; (void)angle_deg; (void)flip; (void)r; (void)g; (void)b; (void)a;
#endif
}
