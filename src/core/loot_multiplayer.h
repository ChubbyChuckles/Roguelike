...
/* Begin a need/greed session for an active instance. player_ids length = count (<=8). Returns 0 or error (<0). */
int rogue_loot_need_greed_begin(int inst_index, const int* player_ids, int count);
/* Submit a choice for a participant: need_flag=1 => NEED, need_flag=0 => GREED, pass_flag=1 => PASS. Returns roll (0-999) or code. */
int rogue_loot_need_greed_choose(int inst_index, int player_id, int need_flag, int pass_flag);
/* Resolve the session if all participants have chosen (or force). Returns winner player id or -1 if unresolved/error. Assigns ownership. */
...
/* Anti-dup safeguards: returns 1 if instance is locked (active need/greed session not yet resolved). */
