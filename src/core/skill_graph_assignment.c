...
{
    if (!maze || !out_ids || maze->node_count <= 0)
        return 0;
        ...
        for (int n = 0; n < maze->node_count && filled < maze->node_count; ++n)
        {
            ...
            for (int tries = 0; tries < skill_count; ++tries)
            {
                ...
                if (trg == 0)
                    ...
                    ok = 1;
                else if (trg > maze->rings && ring == maze->rings)
                    ok = 1;
                else if (pass >= 1)
                {
            ...
            if (out_ids[n] < 0 && skill_count > 0)
            {
                out_ids[n] = (n + cursor) % count_safe;
                filled++;
            }
        }
    }
    for (int n = 0; n < maze->node_count; n++)
    {
        if (out_ids[n] < 0)
            out_ids[n] = n % count_safe;
    }
    return filled;
}
