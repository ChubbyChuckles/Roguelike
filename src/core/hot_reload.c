    ...
    for (int i = 0; i < system->watcher_count; i++) {
        ...
        while (ptr < buffer + length) {
            ...
            if (event->len > 0) {
                ...
                if (event->mask & IN_CREATE) change_type = ROGUE_RELOAD_CHANGE_CREATED;
                else if (event->mask & IN_MODIFY) change_type = ROGUE_RELOAD_CHANGE_MODIFIED;
                else if (event->mask & IN_DELETE) change_type = ROGUE_RELOAD_CHANGE_DELETED;
                else if (event->mask & IN_MOVE) change_type = ROGUE_RELOAD_CHANGE_RENAMED;
                else continue;
                ...
                if (watcher->event_count < ROGUE_HOT_RELOAD_MAX_FILES) {
                    ...
                    strncpy(reload_event->file_path, full_path, sizeof(reload_event->file_path) - 1);
                    ...
                    ROGUE_LOG_DEBUG("File change detected: %s (%s)", 
                                   full_path, rogue_reload_change_type_to_string(change_type));
                }
            }
...
bool rogue_hot_reload_compute_file_hash(const char* file_path, char* hash_out, size_t hash_size) {
    if (!file_path || !hash_out || hash_size == 0) return false;
...
bool rogue_hot_reload_has_file_changed(const RogueFileInfo* old_info, const char* file_path) {
    if (!old_info || !file_path) return false;
...
bool rogue_hot_reload_update_file_info(RogueFileInfo* info, const char* file_path) {
    if (!info || !file_path) return false;
...
void rogue_hot_reload_set_change_detection_mode(RogueHotReloadSystem* system, bool use_hash, bool use_timestamp, bool use_size) {
    ...
    ROGUE_LOG_INFO("Change detection mode updated: hash=%s, timestamp=%s, size=%s",
                   use_hash ? "enabled" : "disabled",
                   use_timestamp ? "enabled" : "disabled", 
...
bool rogue_hot_reload_validate_file(RogueHotReloadSystem* system, const char* file_path) {
    if (!system || !file_path) return false;
...
bool rogue_hot_reload_add_file_to_transaction(RogueHotReloadSystem* system, uint64_t transaction_id, const char* file_path) {
    ...
    ROGUE_LOG_DEBUG("Added file to transaction %llu: %s", (unsigned long long)transaction_id, file_path);
    return true;
}
...
bool rogue_hot_reload_commit_transaction(RogueHotReloadSystem* system, uint64_t transaction_id) {
    ...
    for (int i = 0; i < system->transaction_count; i++) {
        if (system->transactions[i].transaction_id == transaction_id) {
            ...
            if (success) {
                system->reloads_successful++;
                ROGUE_LOG_INFO("Transaction %llu committed successfully", (unsigned long long)transaction_id);
            } else {
                ...
                ROGUE_LOG_ERROR("Transaction %llu committed with errors", (unsigned long long)transaction_id);
            }
...
bool rogue_hot_reload_set_rollback_directory(RogueHotReloadSystem* system, const char* directory) {
    if (!system || !directory) return false;
...
bool rogue_hot_reload_backup_file(RogueHotReloadSystem* system, const char* file_path) {
    ...
    snprintf(backup_path, sizeof(backup_path), "%s/%s.backup", system->rollback_directory, filename);
...
bool rogue_hot_reload_send_notification(RogueHotReloadSystem* system, const char* target_system, const char* config_file, RogueReloadChangeType change_type, void* user_data) {
    if (!system || !target_system || !config_file) return false;
    ...
    ROGUE_LOG_DEBUG("Queued notification for %s: %s (%s)", 
                   target_system, config_file, rogue_reload_change_type_to_string(change_type));
    return true;
}

void rogue_hot_reload_process_notifications(RogueHotReloadSystem* system) {
    if (!system || !system->notifier) return;
...
void rogue_hot_reload_acknowledge_notification(RogueHotReloadSystem* system, int notification_id) {
    if (!system || notification_id < 0 || notification_id >= system->notification_count) return;
...
const char* rogue_reload_status_to_string(RogueReloadStatus status) {
    switch (status) {
        case ROGUE_RELOAD_STATUS_INACTIVE: return "INACTIVE";
        case ROGUE_RELOAD_STATUS_WATCHING: return "WATCHING";
        case ROGUE_RELOAD_STATUS_VALIDATING: return "VALIDATING";
        case ROGUE_RELOAD_STATUS_APPLYING: return "APPLYING";
        case ROGUE_RELOAD_STATUS_ROLLING_BACK: return "ROLLING_BACK";
        case ROGUE_RELOAD_STATUS_ERROR: return "ERROR";
...
const char* rogue_reload_change_type_to_string(RogueReloadChangeType type) {
    switch (type) {
        case ROGUE_RELOAD_CHANGE_CREATED: return "CREATED";
        case ROGUE_RELOAD_CHANGE_MODIFIED: return "MODIFIED";
        case ROGUE_RELOAD_CHANGE_DELETED: return "DELETED";
        case ROGUE_RELOAD_CHANGE_RENAMED: return "RENAMED";
        default: return "UNKNOWN";
    }
}
...
const char* rogue_reload_priority_to_string(RogueReloadPriority priority) {
    switch (priority) {
        case ROGUE_RELOAD_PRIORITY_LOW: return "LOW";
        case ROGUE_RELOAD_PRIORITY_NORMAL: return "NORMAL";
        case ROGUE_RELOAD_PRIORITY_HIGH: return "HIGH";
        case ROGUE_RELOAD_PRIORITY_CRITICAL: return "CRITICAL";
        default: return "UNKNOWN";
    }
}
...
const char* rogue_reload_stage_to_string(RogueReloadStage stage) {
    switch (stage) {
        case ROGUE_RELOAD_STAGE_DETECT: return "DETECT";
        case ROGUE_RELOAD_STAGE_VALIDATE: return "VALIDATE";
        case ROGUE_RELOAD_STAGE_STAGE: return "STAGE";
        case ROGUE_RELOAD_STAGE_APPLY: return "APPLY";
        case ROGUE_RELOAD_STAGE_NOTIFY: return "NOTIFY";
        default: return "UNKNOWN";
    }
}
...
void rogue_hot_reload_get_statistics(const RogueHotReloadSystem* system, uint64_t* files_watched, uint64_t* changes_detected, uint64_t* reloads_successful, uint64_t* reloads_failed, uint64_t* rollbacks_performed) {
    ...
    if (files_watched) *files_watched = system->files_watched;
    if (changes_detected) *changes_detected = system->changes_detected;
    if (reloads_successful) *reloads_successful = system->reloads_successful;
    if (reloads_failed) *reloads_failed = system->reloads_failed;
    if (rollbacks_performed) *rollbacks_performed = system->rollbacks_performed;
}
...
bool rogue_hot_reload_add_dependency(RogueHotReloadSystem* system, const char* config_file, const char* dependency, RogueReloadPriority priority, bool is_weak) {
    (void)system; (void)config_file; (void)dependency; (void)priority; (void)is_weak;
    return true;
}

bool rogue_hot_reload_remove_dependency(RogueHotReloadSystem* system, const char* config_file, const char* dependency) {
    (void)system; (void)config_file; (void)dependency;
    return true;
}
