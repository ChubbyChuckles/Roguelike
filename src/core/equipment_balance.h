...
void rogue_equipment_analytics_record_proc_trigger(int magnitude); /* magnitude optional (unused for now) */
void rogue_equipment_analytics_record_dr_source(float reduction_pct); /* 0..100 (percent reduction) */
...
int  rogue_equipment_analytics_export_json(char* buf, int cap); /* {"proc_oversaturation":0/1,"dr_chain":0/1} */
...
typedef struct RogueBalanceParams {
    ...
    float dr_chain_floor;       /* minimum post-DR damage fraction; lower triggers flag (default 0.2) */
} RogueBalanceParams;
