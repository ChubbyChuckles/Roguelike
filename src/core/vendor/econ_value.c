...
{
    static const int mult[5] = {1, 3, 9, 27, 81};
...
int rogue_econ_item_value(int def_index, int rarity, int affix_power_raw, float durability_fraction)
{
    ...
    if (rarity > 4)
        rarity = 4;
    if (affix_power_raw < 0)
        affix_power_raw = 0;
    if (durability_fraction < 0.0f)
        durability_fraction = 0.0f;
    if (durability_fraction > 1.0f)
        durability_fraction = 1.0f;
    ...
    float norm_affix = 0.0f;
    if (affix_power_raw > 0)
    {
        norm_affix = (float) affix_power_raw / ((float) affix_power_raw + 20.0f);
    }
    float durability_scalar = 0.4f + 0.6f * durability_fraction;
    double value = (double) base_value * slot_factor * (double) rarity_mult * (1.0 + norm_affix) *
                   durability_scalar;
    if (value < 1.0)
        value = 1.0;
