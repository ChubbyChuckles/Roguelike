...
{
    if (category < 0 || category >= ROGUE_SINK__COUNT || amount <= 0)
        return;
    long long v = (long long) g_sink_totals[category] + amount;
    if (v > 2000000000LL)
        v = 2000000000LL;
    g_sink_totals[category] = (int) v;
}
int rogue_vendor_sinks_total(int category)
{
    if (category < 0 || category >= ROGUE_SINK__COUNT)
        return 0;
    return g_sink_totals[category];
}
int rogue_vendor_sinks_grand_total(void)
{
    long long s = 0;
    for (int i = 0; i < ROGUE_SINK__COUNT; i++)
    {
        s += g_sink_totals[i];
    }
    if (s > 2000000000LL)
...
static int scale_fee(int base, int player_level)
{
    if (base <= 0)
        return 0;
    if (player_level < 1)
        player_level = 1;
    double mult = 1.0 + 0.015 * (double) (player_level > 1 ? player_level : 1);
    double v = (double) base * mult;
                                      ...
                                      RogueSpendGoldFn spend_gold_cb, int* out_gold_fee)
{
    if (out_gold_fee)
        *out_gold_fee = 0;
    if (inst_index < 0 || (!reroll_prefix && !reroll_suffix) || !spend_gold_cb)
        return -1;
    if (reroll_prefix && reroll_suffix && catalyst_count_available <= 0)
        return -3; /* require catalyst for dual */
    ...
    if (!it)
        return -1;
    rarity = it->rarity;
    has_prefix = it->prefix_index >= 0;
    has_suffix = it->suffix_index >= 0;
    ...
    if (!reroll_prefix && !reroll_suffix)
        return -2;
    int base_fee = 40 + rarity * 60;
    if (reroll_prefix && reroll_suffix)
        base_fee = (int) (base_fee * 1.75);
    int gold_fee = scale_fee(base_fee, player_level);
    if (spend_gold_cb(gold_fee) != 0)
        return -4; /* insufficient */
    if (reroll_prefix && reroll_suffix)
    {
        if (consume_catalyst_cb && consume_catalyst_cb() != 0)
            return -3;
    }
    int enchant_cost = 0;
    int reroll_rc =
        rogue_item_instance_enchant(inst_index, reroll_prefix, reroll_suffix, &enchant_cost);
    if (reroll_rc < 0)
        return -5;
    rogue_vendor_sinks_add(ROGUE_SINK_UPGRADE, gold_fee);
    if (out_gold_fee)
        *out_gold_fee = gold_fee;
    return 0;
}
                                   ...
                                   int* out_gold_fee)
{
    ...
    if (out_gold_fee)
        *out_gold_fee = 0;
    if (from_material_index < 0 || to_material_index < 0 || count_in <= 0 || !consume_source_cb ||
        !grant_target_cb || !spend_gold_cb)
        return -1;
    ...
    if (count_in < rate)
        return -2;
    int out = count_in / rate;
    if (out <= 0)
        return -2;
    int base_fee = (from->base_value > 0 ? from->base_value : 10) *
                   out; /* gold fee scales with source value */
    int gold_fee = scale_fee(base_fee, player_level);
    if (spend_gold_cb(gold_fee) != 0)
        return -4;
    if (consume_source_cb(count_in) != 0)
        return -3;
    if (grant_target_cb(out) != 0)
        return -5;
