...
int rogue_econ_add_gold(int amount)
{
    long long v = (long long) g_gold + amount;
    if (v < 0)
        v = 0;
    if (v > 2000000000LL)
        v = 2000000000LL;
...
int rogue_econ_buy_price(const RogueVendorItem* v)
{
    if (!v)
        return 0;
    int base = v->price;
    double disc = 1.0 - (double) g_reputation * 0.002;
    if (disc < 0.5)
        disc = 0.5;
    int p = (int) (base * disc + 0.5);
    if (p < 1)
        p = 1;
    return p;
}
...
int rogue_econ_try_buy(const RogueVendorItem* v)
{
    if (!v)
        return -1;
    int cost = rogue_econ_buy_price(v);
    if (g_gold < cost)
        return -2;
...
int rogue_econ_sell(const RogueVendorItem* v)
{
    if (!v)
        return 0;
    int credit = rogue_econ_sell_value(v);
    rogue_econ_add_gold(credit);
    return credit;
}
