...
static float g_demand_score[ROGUE_VENDOR_DEMAND_CATEGORIES];   /* short-term demand (recent sales vs
                                                                  buybacks) */
static float g_scarcity_score[ROGUE_VENDOR_DEMAND_CATEGORIES]; /* longer-term net outflow (positive
                                                                  => scarcity) */
...
static void ensure_init(void)
{
    ...
    {
        g_demand_score[i] = 0.0f;
        g_scarcity_score[i] = 0.0f;
    }
    g_inited = 1;
}
...
{
    ensure_init();
    if (category < 0 || category >= ROGUE_VENDOR_DEMAND_CATEGORIES)
        return;
...
float rogue_vendor_pricing_get_demand_scalar(int category)
{
    ensure_init();
    if (category < 0 || category >= ROGUE_VENDOR_DEMAND_CATEGORIES)
        return 1.0f; /* Map score to scalar: tanh mapped into [0.85,1.15] */
    float s = g_demand_score[category];
    float t = tanhf(s * 0.15f);
    return 1.0f + t * 0.15f;
}
float rogue_vendor_pricing_get_scarcity_scalar(int category)
{
    ...
    if (category < 0 || category >= ROGUE_VENDOR_DEMAND_CATEGORIES)
        return 1.0f;
    /* Map long-term scarcity score into roughly [0.9,1.2]. Positive score => scarcity => higher
     * price. */
    float s = g_scarcity_score[category];
    /* Soft compression: scale then clamp */
    float scaled = s * 0.01f;
    if (scaled > 0.2f)
        scaled = 0.2f;
    if (scaled < -0.1f)
        scaled = -0.1f;
...
int rogue_vendor_compute_price(int vendor_def_index, int item_def_index, int rarity, int category,
                               int is_vendor_selling, int condition_pct, int rep_tier_index,
                               float negotiation_discount_pct)
{
    ...
    if (rep_tier_index >= 0)
    {
        ...
        if (rt)
        {
            if (is_vendor_selling)
            {
                price *= (100 - rt->buy_discount_pct) / 100.0f;
            }
            else
            {
                price *= (100 + rt->sell_bonus_pct) / 100.0f;
            }
        }
    }
    ...
    if (is_vendor_selling && negotiation_discount_pct > 0.0f)
    {
        ...
        price *= (100.0f - negotiation_discount_pct) / 100.0f;
    }
    ...
    /* Adaptive exploit scalar (anti rapid flip) applies only when vendor buying items from player?
     * We apply when vendor sells to player to damp exploitation; if flipping detected we raise
     * vendor sell price slightly. */
    ...
    if (price < 1.0f)
        price = 1.0f;
    if (price > 1000000.0f)
        price = 1000000.0f; /* safety upper bound */
...
uint32_t rogue_vendor_price_modifiers_hash(void)
{
    ...
    for (int i = 0; i < ROGUE_VENDOR_DEMAND_CATEGORIES; i++)
    {
        /* Mix demand */
        uint32_t bits;
        memcpy(&bits, &g_demand_score[i], sizeof bits);
        h ^= bits;
        h *= 16777619u;
        memcpy(&bits, &g_scarcity_score[i], sizeof bits);
        h ^= bits;
        h *= 16777619u;
    }
    /* Final avalanche */
