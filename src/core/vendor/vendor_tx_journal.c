...
static unsigned int g_accum_hash = 0x811C9DC5u;
...
int rogue_vendor_tx_journal_append(unsigned int timestamp_ms, int vendor_def_index, int action_code,
                                   unsigned long long item_guid, unsigned int price, int rep_delta,
                                   int discount_pct)
{
    if (g_entry_count >= ROGUE_VENDOR_TX_JOURNAL_CAP)
        return -1;
    RogueVendorTxEntry e;
    e.op_id = (unsigned int) g_entry_count;
    e.timestamp_ms = timestamp_ms;
    e.vendor_id = (unsigned int) (vendor_def_index < 0 ? 0u : (unsigned int) vendor_def_index);
    e.action_code = (unsigned int) action_code;
    e.item_guid_low = (unsigned int) (item_guid & 0xFFFFFFFFu);
    e.price = price;
    e.rep_delta =
        (unsigned int) (rep_delta < 0 ? (unsigned int) (-rep_delta) : (unsigned int) rep_delta);
    e.discount_pct = (unsigned int) (discount_pct < 0     ? 0
                                     : discount_pct > 100 ? 100
                                                          : discount_pct);
...
void rogue_vendor_tx_journal_record(int vendor_def_index, unsigned long long item_guid,
                                    int action_code, int price, int rep_delta, int discount_pct)
{ /* timestamp not integrated with real clock yet; using op_id*10 as synthetic placeholder */
    unsigned int ts = (unsigned int) (g_entry_count * 10u);
    rogue_vendor_tx_journal_append(ts, vendor_def_index, action_code, item_guid,
