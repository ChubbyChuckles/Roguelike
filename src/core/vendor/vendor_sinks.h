...
enum RogueVendorSinkCategory { ROGUE_SINK_REPAIR=0, ROGUE_SINK_UPGRADE, ROGUE_SINK_TRADEIN, ROGUE_SINK_FEES, ROGUE_SINK__COUNT };
...
/* Upgrade service: reroll a single affix (prefix or suffix) using catalyst + gold fee.
   Parameters:
     ...
     catalyst_count_available: how many catalyst materials player possesses (will consume 1 on success if needed)
     consume_catalyst_cb: callback to spend one catalyst (must return 0 on success)
     spend_gold_cb: callback to spend computed gold fee (0 success, non-zero insufficient funds)
   Returns:
     0 on success, negative on error:
       -1 invalid params
       -2 nothing selected to reroll or affix missing
       -3 insufficient catalyst
       -4 gold spend declined
       -5 reroll operation failed (downstream)
*/
...
/* Material trade-in: convert N low-tier materials into 1 higher-tier (unfavorable rate). 
   Parameters:
     ...
     to_material_index: target material registry index (tier T+1 expected but not enforced strictly)
     count_in: number of source materials offered
     player_level: for fee scaling (adds gold fee proportional)
     consume_source_cb(count): consumes source materials (must return 0 on success)
     grant_target_cb(count): grants target materials (return 0 on success)
