...
static void copy_str_local(char* dst, size_t cap, const char* src)
{
    if (cap == 0)
        return;
    if (!src)
...
int rogue_vendor_inventory_templates_load(void)
{
    g_template_count = 0;
    char path[256];
    if (!rogue_find_asset_path("vendors/inventory_templates.json", path, sizeof path))
        return 0;
    char* buf = NULL;
    if (!read_entire_file_local(path, &buf))
        return 0;
    const char* section = strstr(buf, "\"inventory_templates\"");
    if (!section)
    {
        free(buf);
        return 0;
    ...
    while (g_template_count < ROGUE_MAX_VENDOR_INV_TEMPLATES)
    {
        ...
        tmp[obj_len] = '\0';
        char arche[32];
        if (!json_find_string_local(tmp, "\"archetype\"", arche, sizeof arche))
        {
            p = end + 1;
            continue;
        }
        RogueVendorInventoryTemplate* t = &g_templates[g_template_count];
        memset(t, 0, sizeof *t);
        copy_str_local(t->archetype, sizeof t->archetype, arche);
...
const RogueVendorInventoryTemplate* rogue_vendor_inventory_template_find(const char* archetype)
{
    ...
    for (int i = 0; i < g_template_count; i++)
        if (strcmp(g_templates[i].archetype, archetype) == 0)
                                         ...
                                         int day_cycle)
{
    ...
    if (vendor_id)
    {
        h = rogue_fnv1a64(vendor_id, strlen(vendor_id), h);
    }
