...
extern "C"
{
    ...
    enum
    {
        ROGUE_CRAFT_STATION_FORGE = 0,
        ROGUE_CRAFT_STATION_ALCHEMY = 1,
        ROGUE_CRAFT_STATION_WORKBENCH = 2,
        ROGUE_CRAFT_STATION_ALTAR = 3,
        ROGUE_CRAFT_STATION__COUNT
    };
    ...
    typedef struct RogueCraftJob
    {
        ...
        int state;        /* 0=waiting,1=active,2=done(pending output),3=canceled,4=delivered */
    } RogueCraftJob;
     ...
     *  -1 invalid args, -2 recipe missing, -3 skill requirement unmet, -4 insufficient materials,
     * -5 inventory consume failure */
    int rogue_craft_queue_enqueue(const RogueCraftRecipe* recipe, int recipe_index,
                                  int current_skill, RogueInvGetFn inv_get,
                                  RogueInvConsumeFn inv_consume);
    ...
    /* Cancel a job by id.
     * Waiting (state 0): full refund.
     * Active  (state 1): 50% refund (floor) of each input.
     * Done/Pending Output (state 2) or Delivered (state 4) cannot be canceled.
     * Canceled (state 3) is terminal.
