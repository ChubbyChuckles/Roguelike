...
int rogue_inventory_consume(int def_index, int quantity){
    if(def_index < 0 || def_index >= ROGUE_ITEM_DEF_CAP || quantity <= 0) return 0;
    int have = g_counts[def_index]; if(have <= 0) return 0; int remove = quantity; if(remove > have) remove = have; g_counts[def_index] = have - remove; if(g_counts[def_index]==0) g_distinct--; return remove; }

void rogue_inventory_serialize(FILE* f){
    if(!f) return; for(int i=0;i<ROGUE_ITEM_DEF_CAP;i++){ if(g_counts[i]>0){ fprintf(f,"INV%d=%d\n", i, g_counts[i]); } }
}
int rogue_inventory_try_parse_kv(const char* key, const char* val){
    if(strncmp(key,"INV",3)!=0) return 0; int idx = atoi(key+3); if(idx<0 || idx>=ROGUE_ITEM_DEF_CAP) return 0; int q = atoi(val); if(q<0) q=0; if(g_counts[idx]==0 && q>0) g_distinct++; g_counts[idx]=q; return 1; }
