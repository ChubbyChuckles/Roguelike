...
/* Delta block magic (Phase 1.6) written into inventory save component when there are changed
 * entries. */
#define ROGUE_INV_DELTA_MAGIC 0x31445649u /* "IVD1" little-endian */

    typedef int (*RogueInventoryCapHandler)(
        int def_index, uint64_t add_qty); /* Phase 1.7: called when unique cap would be exceeded;
                                             return 0 to retry pickup after performing mitigation
                                             (e.g., salvage/remove), else negative to abort. */
    ...
    /* Label metadata (bitmask of ROGUE_INV_LABEL_*). Setting labels on a non-existent entry has no
     * effect (returns -1). */
    ...
    /* Phase 1.6: enumerate dirty (changed since last snapshot) entries. Returns count copied
     * (<=cap). Quantities array receives current absolute quantity (0 means removed). Passing NULL
     * resets tracking without enumeration. */
