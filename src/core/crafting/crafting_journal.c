...
void rogue_craft_journal_reset(void){ g_entry_count=0; g_accum_hash=0x811C9DC5u; }
...
int rogue_craft_journal_append(unsigned int item_guid, unsigned int pre_budget, unsigned int post_budget, unsigned int rng_stream_id, unsigned int outcome_hash){
    if(g_entry_count>=ROGUE_CRAFT_JOURNAL_CAP) return -1;
    RogueCraftJournalEntry e; e.op_id = (unsigned int)g_entry_count; e.item_guid=item_guid; e.pre_budget=pre_budget; e.post_budget=post_budget; e.rng_stream_id=rng_stream_id; e.outcome_hash=outcome_hash;
...
const RogueCraftJournalEntry* rogue_craft_journal_entry(int index){ if(index<0||index>=g_entry_count) return NULL; return &g_entries[index]; }
