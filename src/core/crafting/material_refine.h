...
#define ROGUE_MATERIAL_QUALITY_MAX 100
    ...
    /* Refinement operation: convert `consume_count` units from `from_quality` bucket into
     * higher-quality units at `to_quality`. Rules:
     ...
     *  - Critical success chance (5%): produced increased by +50% (rounded) and 20% of produced
     * (rounded) escalates one extra quality tier (to_quality+1) if < max.
     ...
     *   -1 invalid args, -2 insufficient source, -3 no output (all lost to failure), -4 material
     * index invalid. On success writes produced units at target quality to *out_produced (excluding
     * any overflowed extra tier units) and sets *out_crit flag (1 if critical success path taken).
     */
