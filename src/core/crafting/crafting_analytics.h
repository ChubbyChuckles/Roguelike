...
void rogue_craft_analytics_on_node_harvest(int node_def_index,int material_def_index,int qty,int rare_flag,unsigned int now_ms);

/* Craft outcome event: crafted item with resulting material quality bias (0..100) and success/failure flag (e.g., enhancement success) */
...
/* Material flow: acquisition & consumption tracking (def indexes refer to item defs mapped to materials). */
...
float rogue_craft_analytics_enhance_risk_variance(void); /* difference between expected risk mean and observed failure mean */
