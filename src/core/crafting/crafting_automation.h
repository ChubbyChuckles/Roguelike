...
#include <stddef.h>
    ...
    /* 9.1 Craft planner
     * Computes aggregated input requirements for a target recipe * batch_qty. If recursive is
     * non‑zero, attempts to expand inputs that themselves are craftable by other recipes (one level
     * per recursion depth), up to max_depth (cycle safe). Returns count of populated entries (<=
     * max_out). Deterministic ordering by ascending item_def.
     */
     ...
     * sum_over_materials( scarcity_ratio(material) * node_weight_fraction ). scarcity_ratio =
     * missing/(have+1). Returns count written (<= max_out). If no deficits, returns 0.
     */
    ...
    /* Generate at most max_out suggestions. Heuristic: for each material whose average quality <
     * avg_threshold and with >= min_bucket units at from_quality (default bucket 0..40 scanned),
     * suggest a refine upward by delta_q. Returns count. Deterministic ordering by material_def
     * ascending then from_quality.
     */
     ...
     * Scores expected value of salvaging an item vs crafting an upgrade recipe (cost includes input
     * material base values). Returns 1 if crafting upgrade is recommended, 0 otherwise. Negative on
     * error. Outputs raw values if pointers non‑NULL.
     */
     ...
     * Returns material_def with highest scarcity ratio among materials appearing in any recipe
     * inputs. Returns -1 if none missing. */
