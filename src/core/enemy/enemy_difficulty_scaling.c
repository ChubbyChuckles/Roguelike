...
int rogue_enemy_difficulty_load_params_file(const char* path)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        char key[64];
        float value = 0.f;
        if (sscanf(p, "%63[^=]=%f", key, &value) == 2)
        {
            if (strcmp(key, "d_def") == 0)
                g_params.d_def = value;
            else if (strcmp(key, "d_dmg") == 0)
                g_params.d_dmg = value;
            else if (strcmp(key, "cap_def") == 0)
                g_params.cap_def = value;
            else if (strcmp(key, "cap_dmg") == 0)
                g_params.cap_dmg = value;
            else if (strcmp(key, "u_def") == 0)
                g_params.u_def = value;
            else if (strcmp(key, "u_dmg") == 0)
                g_params.u_dmg = value;
            else if (strcmp(key, "u_cap_def") == 0)
                g_params.u_cap_def = value;
            else if (strcmp(key, "u_cap_dmg") == 0)
                g_params.u_cap_dmg = value;
            else if (strcmp(key, "ramp_soft") == 0)
                g_params.ramp_soft = value;
            else if (strcmp(key, "dominance_threshold") == 0)
                g_params.dominance_threshold = (int) value;
            else if (strcmp(key, "trivial_threshold") == 0)
                g_params.trivial_threshold = (int) value;
            else if (strcmp(key, "reward_trivial_scalar") == 0)
                g_params.reward_trivial_scalar = value;
        }
    }
    fclose(f);
    return 0;
...
RogueEnemyBaseStats rogue_enemy_base_stats(int enemy_level)
{
    RogueEnemyBaseStats s = {rogue_enemy_base_hp(enemy_level), rogue_enemy_base_damage(enemy_level),
                             rogue_enemy_base_defense(enemy_level)};
    return s;
}
                                                          ...
                                                          float* out_hp_mult, float* out_dmg_mult)
{
    if (player_level < 1 || enemy_level < 1)
        return -1;
    if (!out_hp_mult || !out_dmg_mult)
        return -2;
    int dL = player_level - enemy_level;
    ...
    { /* player over-level -> penalties */
        float down_hp = dL * p->d_def;
        float down_dmg = dL * p->d_dmg;
        if (down_hp > p->cap_def)
            down_hp = p->cap_def;
        if (down_dmg > p->cap_dmg)
            down_dmg = p->cap_dmg;
        hp_mult = 1.f - down_hp;
        if (hp_mult < 0.05f)
            hp_mult = 0.05f; /* never 0 */
        dmg_mult = 1.f - down_dmg;
        if (dmg_mult < 0.05f)
            dmg_mult = 0.05f;
    }
    else
    { /* dL < 0 => enemy over-level -> buffs */
        int adL = -dL;
        float up_hp = adL * p->u_def - p->ramp_soft;
        if (up_hp < 0.f)
            up_hp = 0.f;
        if (up_hp > p->u_cap_def)
            up_hp = p->u_cap_def;
        float up_dmg = adL * p->u_dmg - p->ramp_soft;
        if (up_dmg < 0.f)
            up_dmg = 0.f;
        if (up_dmg > p->u_cap_dmg)
            up_dmg = p->u_cap_dmg;
        hp_mult = 1.f + up_hp;
        dmg_mult = 1.f + up_dmg;
    }
    *out_hp_mult = hp_mult;
    *out_dmg_mult = dmg_mult;
    return 0;
}
                                    ...
                                    RogueEnemyFinalStats* out)
{
    if (!out)
        return -3;
    RogueEnemyBaseStats base = rogue_enemy_base_stats(enemy_level);
    const RogueEnemyTierDesc* tier = rogue_enemy_tier_get(tier_id);
    if (!tier)
        return -2;
    float rel_hp_mult = 1.f, rel_dmg_mult = 1.f;
    if (rogue_enemy_difficulty_internal__relative_multipliers(player_level, enemy_level,
                                                              &rel_hp_mult, &rel_dmg_mult) != 0)
        return -1;
    ...
    float adapt = rogue_enemy_adaptive_scalar();
    if (adapt < 0.01f)
        adapt = 0.01f;
    final_hp *= adapt;
    final_dmg *= adapt;
    final_def *= adapt;
    out->hp = final_hp;
    out->damage = final_dmg;
    out->defense = final_def;
...
{
    (void) modifier_complexity_score;
    (void) adaptive_state_scalar;
    const RogueEnemyDifficultyParams* p = &g_params;
    int dL = player_level - enemy_level;
    if (dL >= p->trivial_threshold)
        return p->reward_trivial_scalar;
    if (dL <= 0)
        return 1.f; /* equal or under-level => full rewards */
    /* Linear interpolation between dominance_threshold (full) and trivial_threshold (min) */
    if (dL <= p->dominance_threshold)
        return 1.f;
    float span = (float) (p->trivial_threshold - p->dominance_threshold);
    if (span <= 0.f)
        return 1.f;
    float t = (float) (dL - p->dominance_threshold) / span;
    if (t < 0.f)
        t = 0.f;
    if (t > 1.f)
        t = 1.f;
    return 1.f - t * (1.f - p->reward_trivial_scalar);
}
...
{
    int biome_id;
    RogueEnemyDifficultyParams params;
} BiomeParamEntry;
                                                 ...
                                                 const RogueEnemyDifficultyParams* params)
{
    ...
    for (int i = 0; i < g_biome_param_count; i++)
        if (g_biome_params[i].biome_id == biome_id)
        {
            g_biome_params[i].params = *params;
            return 0;
        }
    if (g_biome_param_count >= MAX_BIOME_PARAMS)
        return -1;
    g_biome_params[g_biome_param_count].biome_id = biome_id;
    g_biome_params[g_biome_param_count].params = *params;
    g_biome_param_count++;
    return 0;
}
...
const RogueEnemyDifficultyParams* rogue_enemy_difficulty_params_for_biome(int biome_id)
{
    for (int i = 0; i < g_biome_param_count; i++)
        if (g_biome_params[i].biome_id == biome_id)
            return &g_biome_params[i].params;
    return NULL;
}
...
static const RogueEnemyDifficultyParams* _select_params(int biome_id)
{
    const RogueEnemyDifficultyParams* b =
        (biome_id >= 0) ? rogue_enemy_difficulty_params_for_biome(biome_id) : NULL;
    return b ? b : &g_params;
}
...
RogueEnemyDeltaLSeverity rogue_enemy_difficulty_classify_delta(int player_level, int enemy_level)
{
    ...
    int dL = player_level - enemy_level;
    if (dL == 0)
        return ROGUE_DLVL_EQUAL;
    if (dL > 0)
    {
        if (dL >= p->trivial_threshold)
            return ROGUE_DLVL_TRIVIAL;
        if (dL >= p->dominance_threshold)
            return ROGUE_DLVL_DOMINANCE;
        if (dL >= 5)
            return ROGUE_DLVL_MAJOR; /* heuristic */
        return ROGUE_DLVL_MINOR;
    }
    /* Enemy higher level */
    if (-dL >= 8)
        return ROGUE_DLVL_MAJOR;
    if (-dL >= 4)
        return ROGUE_DLVL_MODERATE;
    return ROGUE_DLVL_MINOR;
}
                                                   ...
                                                   RogueEnemyDerivedAttributes* out)
{
    ...
    if (phys > 0.60f)
        phys = 0.60f;
    float elem = phys * 0.92f;
    if (elem > 0.55f)
        elem = 0.55f;
    out->crit_chance = crit;
    out->phys_resist = phys;
    out->elem_resist = elem;
    return 0;
}
                                          ...
                                          int biome_id, RogueEnemyFinalStats* out)
{
    ...
    if (rogue_enemy_difficulty_internal__relative_multipliers(player_level, enemy_level,
                                                              &rel_hp_mult, &rel_dmg_mult) != 0)
        return -1;
    const RogueEnemyDifficultyParams* p = _select_params(biome_id);
    (void) p; /* future biome-specific adjustments could apply here */
    float tier_hp = base.hp * tier->mult.hp_budget;
    float tier_dmg = base.damage * tier->mult.dps_budget;
...
{
    (void) player_level;
    (void) biome_id; /* reserved for future adaptive & biome-specific ordering */
    const RogueEnemyTierDesc* tier = rogue_enemy_tier_get(tier_id);
    if (!tier)
        return -2;
    return rogue_enemy_difficulty_internal__attrib_curves(enemy_level, tier->mult.hp_budget,
                                       ...
                                       float player_dps)
{
    ...
    if (rogue_enemy_compute_final_stats_biome(player_level, enemy_level, tier_id, biome_id, &fs) !=
        0)
        return -2.f;
