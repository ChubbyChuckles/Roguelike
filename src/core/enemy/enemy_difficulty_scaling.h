/* enemy_difficulty_scaling.h - Enemy Difficulty System Phase 1 (baseline scaling + relative level
 * differential) Roadmap Coverage: 1.1 - 1.7
 */
...
#include "core/enemy/enemy_difficulty.h"
    ...
    typedef struct RogueEnemyBaseStats
    {
        float hp;
        float damage;
        float defense;
    ...
    typedef struct RogueEnemyFinalStats
    {
        float hp;
        float damage;
        float defense;
        ...
        float dmg_mult;
        float def_mult;
    } RogueEnemyFinalStats;

    /* Derived combat attributes (Phase 1.3) */
    typedef struct RogueEnemyDerivedAttributes
    {
        float crit_chance;
        float phys_resist;
        float elem_resist;
    } RogueEnemyDerivedAttributes;
    ...
    int rogue_enemy_difficulty_register_biome_params(
        int biome_id, const RogueEnemyDifficultyParams* params); /* returns 0 on success */
    const RogueEnemyDifficultyParams*
    rogue_enemy_difficulty_params_for_biome(int biome_id); /* NULL if none */

    /* ΔL severity categorization (Phase 1.6 UI hook placeholder) */
    typedef enum RogueEnemyDeltaLSeverity
    {
        ROGUE_DLVL_EQUAL = 0,
        ROGUE_DLVL_MINOR,
        ROGUE_DLVL_MODERATE,
        ROGUE_DLVL_MAJOR,
        ROGUE_DLVL_DOMINANCE,
        ROGUE_DLVL_TRIVIAL
    } RogueEnemyDeltaLSeverity;
    RogueEnemyDeltaLSeverity rogue_enemy_difficulty_classify_delta(int player_level,
                                                                   int enemy_level);

    /* TTK estimation helper for analytics & tests: simplistic model using provided player_dps
     * estimate. */
    float rogue_enemy_estimate_ttk_seconds(int player_level, int enemy_level, int tier_id,
                                           int biome_id, float player_dps);
