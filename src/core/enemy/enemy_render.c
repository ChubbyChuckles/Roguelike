    ...
    for (int i = 0; i < ROGUE_MAX_ENEMIES; i++)
        if (g_app.enemies[i].alive)
        {
            ...
            if (spr && spr->tex && spr->tex->handle && spr->sw)
            {
                ...
                {
                    ma = (Uint8) (e->death_fade < 0
                                      ? 0
                                      : (e->death_fade > 1.0f ? 255 : (e->death_fade * 255.0f)));
                }
                int dst_x = ex - spr->sw / 2;
                int dst_y = ey - spr->sh / 2;
                int y_base = ey + spr->sh / 2;
                int flip_flag = (e->facing == 1) ? 1 : 0;
                rogue_scene_drawlist_push_sprite(spr, dst_x, dst_y, y_base, flip_flag, mr, mg, mb,
                                                 ma);
            }
            else
            {
                unsigned char r = (unsigned char) e->tint_r, g = (unsigned char) e->tint_g,
                              b = (unsigned char) e->tint_b, a = 255;
                if (e->ai_state == ROGUE_ENEMY_AI_DEAD)
                    a = (unsigned char) (e->death_fade * 255.0f);
                SDL_SetRenderDrawColor(g_app.renderer, r, g, b, a);
                SDL_Rect er = {ex - 4, ey - 4, 8, 8};
                SDL_RenderFillRect(g_app.renderer, &er);
            }
            int maxhp = e->max_health > 0 ? e->max_health : 1;
            float ratio = (e->health > 0) ? (float) e->health / (float) maxhp : 0.0f;
            if (ratio < 0)
                ratio = 0;
            if (ratio > 1)
                ratio = 1;
            int barw = 20, barh = 3;
            int bx = ex - barw / 2;
            int by = ey - (spr ? spr->sh / 2 : 6) - 6;
            SDL_SetRenderDrawColor(g_app.renderer, 25, 8, 8, 200);
            SDL_Rect bg = {bx - 1, by - 1, barw + 2, barh + 2};
            SDL_RenderFillRect(g_app.renderer, &bg);
            SDL_SetRenderDrawColor(g_app.renderer, 120, 0, 0, 255);
            SDL_Rect fg1 = {bx, by, (int) (barw * ratio), barh};
            SDL_RenderFillRect(g_app.renderer, &fg1);
            SDL_SetRenderDrawColor(g_app.renderer, 220, 30, 30, 255);
            SDL_Rect fg2 = {bx, by, (int) (barw * ratio * 0.55f), barh};
            SDL_RenderFillRect(g_app.renderer, &fg2);
