...
typedef enum RogueEncounterType { ROGUE_ENCOUNTER_SWARM=0, ROGUE_ENCOUNTER_MIXED, ROGUE_ENCOUNTER_CHAMPION_PACK, ROGUE_ENCOUNTER_BOSS_ROOM } RogueEncounterType;

typedef struct RogueEncounterTemplate {
    int id; char name[48]; RogueEncounterType type; int min_count; int max_count;
    int boss; int support_min; int support_max; int elite_spacing; float elite_chance;
...
/* Composition result */
typedef struct RogueEncounterUnit { int enemy_type_id; int level; int is_elite; } RogueEncounterUnit;

typedef struct RogueEncounterComposition {
    int template_id;
    int unit_count; RogueEncounterUnit units[64];
    int elite_count; int boss_present; int support_count;
