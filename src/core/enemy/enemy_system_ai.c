...
void rogue_enemy_ai_update(float dt_ms){
    ...
    if(p_dist2 < 1.00f && g_app.player.health>0 && e->attack_cooldown_ms<=0){ int dmg=(int)(1 + g_app.difficulty_scalar * 0.6); if(dmg<1) dmg=1; float ech=(float)e->crit_chance * 0.01f; if(ech>0.35f) ech=0.35f; int ecrit = (((float)rand()/(float)RAND_MAX) < ech)?1:0; if(ecrit){ float emult = 1.0f + (float)e->crit_damage * 0.01f; if(emult>3.0f) emult=3.0f; dmg = (int)floorf(dmg * emult + 0.5f); } int blocked=0, perfect=0; float adx=-pdx; float ady=-pdy; int final = rogue_player_apply_incoming_melee(&g_app.player, (float)dmg, adx, ady, 8, &blocked, &perfect); if(final>0){ g_app.player.health -= final; if(g_app.player.health<0) g_app.player.health=0; } if(blocked){ if(final>0) rogue_add_damage_number_ex(g_app.player.base.pos.x, g_app.player.base.pos.y - 0.2f, final, 0, 0); } else { rogue_add_damage_number_ex(g_app.player.base.pos.x, g_app.player.base.pos.y - 0.2f, final, 0, ecrit); } e->hurt_timer=200.0f; g_app.time_since_player_hit_ms=0.0f; float base_cd = 1050.0f + (float)(rand()%700); if(prof){ base_cd *= prof->cooldown_mult; } e->attack_cooldown_ms = base_cd; }
