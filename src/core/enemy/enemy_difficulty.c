...
static const char* g_archetype_names[ROGUE_ENEMY_ARCHETYPE__COUNT] = {
    "Melee","Ranged","Caster","EliteSupport","Boss"
};
 ...
 *  - MiniBoss: 3.2x HP (noticeable endurance), 2.2x DPS (threat), +130% control & mobility for pattern variety.
 *  - Boss: 8x HP baseline, 3.2x DPS; control +180%, mobility +170% for arena dynamics.
 *  - Nemesis: Slightly above Boss (HP 8.5x) but *less* DPS than pure boss (3.0x) reserving space for adaptive scaling.
 */
...
const RogueEnemyTierDesc* rogue_enemy_tier_get_by_index(int index){
    if(index < 0 || index >= g_tier_count) return NULL;
    return &g_tiers[index];
}

const RogueEnemyTierDesc* rogue_enemy_tier_get(int id){
    for(int i=0;i<g_tier_count;i++) if(g_tiers[i].id == id) return &g_tiers[i];
    return NULL;
}
...
const char* rogue_enemy_archetype_name(int archetype){
    if(archetype < 0 || archetype >= ROGUE_ENEMY_ARCHETYPE__COUNT) return NULL;
...
static int _validate_unique_ids(void){
    for(int i=0;i<g_tier_count;i++){
        for(int j=i+1;j<g_tier_count;j++){
            if(g_tiers[i].id == g_tiers[j].id) return -1;
        }
    }
    return 0;
}
static int _validate_monotonic(void){
    /* Expect non-decreasing hp & dps from one tier to the next (Nemesis slight DPS drop is intentional: allow adaptive headroom).
       We'll enforce: hp monotonic strictly increasing; dps mostly increasing with at most one allowed dip (Nemesis). */
    float prev_hp = -1.f;
    int dps_drop_allowed = 1;
    float prev_dps = -1.f;
    for(int i=0;i<g_tier_count;i++){
        const RogueEnemyTierDesc* t=&g_tiers[i];
        if(t->mult.hp_budget <= prev_hp) return -2; /* strict */
        prev_hp = t->mult.hp_budget;
        if(t->mult.dps_budget < prev_dps){
            if(dps_drop_allowed){
                dps_drop_allowed = 0; /* consume allowance */
            } else return -3; /* multiple dips */
        } else {
