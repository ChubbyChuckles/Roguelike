...
{
    if (!g_player_stats_path[0])
        return "player_stats.cfg";
    return g_player_stats_path;
}
const char* rogue__gen_params_path(void)
{
    if (!g_gen_params_path[0])
        return "gen_params.cfg";
    return g_gen_params_path;
}
...
#else
        strncpy(g_player_stats_path, player_stats_path, sizeof g_player_stats_path - 1);
        g_player_stats_path[sizeof g_player_stats_path - 1] = '\0';
#endif
    }
    if (gen_params_path)
    {
...
#else
        strncpy(g_gen_params_path, gen_params_path, sizeof g_gen_params_path - 1);
        g_gen_params_path[sizeof g_gen_params_path - 1] = '\0';
#endif
    }
}
