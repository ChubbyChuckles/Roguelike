...
typedef struct ApiEntry { const char* name; const char* desc; } ApiEntry;
...
static const ApiEntry k_entries[] = {
    ...
    {"rogue_loot_security_verify_files", "Verify config files vs last snapshot (tamper detection)."},
    ...
    {"rogue_loot_security_set_server_mode", "Enable/disable server authoritative verification mode."},
    {"rogue_loot_server_verify", "Validate client-reported roll against recomputed hash."},
    {"rogue_loot_filter_refresh_instances", "Reapply loot filter predicates to existing ground items."},
    {"rogue_loot_tables_load_from_cfg", "Load loot table definitions from config file."},
    {"rogue_loot_tables_roll", "Perform weighted loot table roll producing item indices + quantities."},
};

int rogue_loot_generate_api_doc(char* buf, int cap){
    if(!buf || cap<=0) return -1;
    if(cap < 128) { buf[0]='\0'; return -1; }
    int written=0;
    ...
    for(size_t i=0;i<sizeof(k_entries)/sizeof(k_entries[0]); ++i){
        ...
        if(r<0) break;
        if(r >= cap-written){ /* truncate */
            written = cap-1; break;
        }
        written += r;
    }
    if(written >= cap) written = cap-1;
    buf[written] = '\0';
    return written;
}
