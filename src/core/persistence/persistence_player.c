...
void rogue_persistence_load_player_stats(void)
{
    ...
    while (fgets(line, sizeof line, f))
    {
        ...
        *eq = '\0';
        ...
        {
            g_player_stats_version = atoi(val);
            if (g_player_stats_version <= 0)
                g_player_stats_version = 1;
            continue;
        }
        if (strcmp(key, "LEVEL") == 0)
            g_app.player.level = atoi(val);
        else if (strcmp(key, "XP") == 0)
            g_app.player.xp = atoi(val);
        else if (strcmp(key, "XP_TO_NEXT") == 0)
            g_app.player.xp_to_next = atoi(val);
        else if (strcmp(key, "XP_TOTAL") == 0)
        {
            ...
            g_app.player.strength = atoi(val);
        else if (strcmp(key, "DEX") == 0)
            g_app.player.dexterity = atoi(val);
        else if (strcmp(key, "VIT") == 0)
            g_app.player.vitality = atoi(val);
        else if (strcmp(key, "INT") == 0)
            g_app.player.intelligence = atoi(val);
        else if (strcmp(key, "CRITC") == 0)
            g_app.player.crit_chance = atoi(val);
        else if (strcmp(key, "CRIT_R") == 0)
            g_app.player.crit_rating = atoi(val);
        else if (strcmp(key, "HASTE_R") == 0)
            g_app.player.haste_rating = atoi(val);
        else if (strcmp(key, "AVOID_R") == 0)
            g_app.player.avoidance_rating = atoi(val);
        else if (strcmp(key, "CRITD") == 0)
            g_app.player.crit_damage = atoi(val);
        else if (strcmp(key, "UNSPENT") == 0)
            g_app.unspent_stat_points = atoi(val);
        else if (strcmp(key, "HP") == 0)
            g_app.player.health = atoi(val);
        else if (strcmp(key, "MP") == 0)
            g_app.player.mana = atoi(val);
        else if (strcmp(key, "TALENTPTS") == 0)
            g_app.talent_points = atoi(val);
        ...
        else if (strncmp(key, "SKCD", 4) == 0)
        {
            ...
            if (st)
            {
                st->cooldown_end_ms = atof(val);
            }
        }
        else if (rogue_inventory_try_parse_kv(key, val))
        { /* handled */
        }
        else if (strncmp(key, "GI", 2) == 0)
        {
            int slot = atoi(key + 2);
            if (slot >= 0 && slot < g_app.item_instance_cap)
            {
                int fields[7];
                for (int k = 0; k < 7; k++)
                    fields[k] = -1;
                ...
                int sidx = fields[5];
                int sval = fields[6];
                if (def_index >= 0 && qty > 0)
                {
                    int inst = rogue_items_spawn(def_index, qty, 0.0f, 0.0f);
                    if (inst >= 0)
                    {
    ...
    if (g_app.item_instances)
    {
        for (int i = 0; i < g_app.item_instance_cap; i++)
            if (g_app.item_instances[i].active)
            {
                ...
                fprintf(f, "GI%d=%d,%d,%d,%d,%d,%d,%d\n", i, it->def_index, it->quantity,
                        it->rarity, it->prefix_index, it->prefix_value, it->suffix_index,
                        it->suffix_value);
            }
    }
    fclose(f);
