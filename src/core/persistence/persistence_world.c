...
{
    /* Defaults */
    g_app.gen_water_level = 0.34f;
    g_app.gen_noise_octaves = 6;
    g_app.gen_noise_gain = 0.48f;
    g_app.gen_noise_lacunarity = 2.05f;
    g_app.gen_river_sources = 10;
    g_app.gen_river_max_length = 1200;
    g_app.gen_cave_thresh = 0.60f;
    ...
    while (fgets(line, sizeof line, f))
    {
        ...
        *eq = '\0';
        ...
        {
            g_gen_params_version = atoi(val);
            if (g_gen_params_version <= 0)
                g_gen_params_version = 1;
            continue;
        }
        if (strcmp(key, "WATER_LEVEL") == 0)
            g_app.gen_water_level = (float) atof(val);
        else if (strcmp(key, "NOISE_OCTAVES") == 0)
            g_app.gen_noise_octaves = atoi(val);
        else if (strcmp(key, "NOISE_GAIN") == 0)
            g_app.gen_noise_gain = (float) atof(val);
        else if (strcmp(key, "NOISE_LACUNARITY") == 0)
            g_app.gen_noise_lacunarity = (float) atof(val);
        else if (strcmp(key, "RIVER_SOURCES") == 0)
            g_app.gen_river_sources = atoi(val);
        else if (strcmp(key, "RIVER_MAX_LENGTH") == 0)
            g_app.gen_river_max_length = atoi(val);
        else if (strcmp(key, "CAVE_THRESH") == 0)
            g_app.gen_cave_thresh = (float) atof(val);
    }
    fclose(f);
}
