...
static int parse_line(char* line){
    ...
    while(seg_cursor && *seg_cursor){
        ...
        if(*seg_cursor){
            char* item_cur = seg_cursor; char* f_item = lt_next_field(&item_cur); char* f_weight = lt_next_field(&item_cur); char* f_qmin = lt_next_field(&item_cur); char* f_qmax = lt_next_field(&item_cur);
