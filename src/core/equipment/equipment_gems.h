...
/* Phase 5.2 Gem definitions: Supports flat primary/resist/armor bonuses plus percent primary scaling and metadata for conditional/proc chance. */
...
/* Load gem defs from simple CSV: id,item_def_id,str,dex,vit,int,armor,res_phys,res_fire,res_cold,res_light,res_poison,res_status,pct_str,pct_dex,pct_vit,pct_int,proc_chance,conditional_flags */
...
/* High-level socket operations with economy & safety. Return 0 on success; negative codes mirror base socket API plus -6 insufficient funds. */
int rogue_item_instance_socket_insert_pay(int inst_index,int slot,int gem_def_index,int* out_cost);
