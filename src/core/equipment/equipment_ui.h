...
/* Builds full layered tooltip: base, implicit, affixes (from base builder), gems, set, runeword, comparative deltas.
   compare_slot: slot enum value to compare against (-1 to skip). */
...
/* Convenience: build a pseudo gem inventory panel listing socket selection + a few gem ids present in inventory (stubbed). */
...
/* Soft-cap saturation helper used in UI color coding tests: returns percent of cap after soft curve. */
