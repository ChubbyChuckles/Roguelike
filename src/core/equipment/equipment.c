...
static int category_for_slot(enum RogueEquipSlot slot)
{
    ...
    case ROGUE_EQUIP_OFFHAND:
        return ROGUE_ITEM_ARMOR; /* placeholder: treat shields/foci as armor category until distinct
                                  */
    ...
    case ROGUE_EQUIP_RING1:
    case ROGUE_EQUIP_RING2:
    case ROGUE_EQUIP_AMULET:
    case ROGUE_EQUIP_BELT:
    case ROGUE_EQUIP_CLOAK:
    case ROGUE_EQUIP_CHARM1:
    case ROGUE_EQUIP_CHARM2:
        return ROGUE_ITEM_ARMOR; /* reuse armor pipeline for now */
    default:
        return -1;
    }
}
...
int rogue_equip_item_is_two_handed(int inst_index)
{
    ...
    if (d->category != ROGUE_ITEM_WEAPON)
        return 0;
    return (d->flags & ROGUE_ITEM_FLAG_TWO_HANDED) ? 1 : 0;
}

static unsigned long long equip_mix64(unsigned long long h, unsigned long long v)
{
...
int rogue_equip_try(enum RogueEquipSlot slot, int inst_index)
{
    ...
    {
        if (rogue_equip_item_is_two_handed(inst_index))
        {
            if (g_slots[ROGUE_EQUIP_OFFHAND] >= 0)
            {
                g_slots[ROGUE_EQUIP_OFFHAND] = -1;
            }
        }
    }
    /* Equipping offhand while two-handed weapon equipped should fail */
    if (slot == ROGUE_EQUIP_OFFHAND)
    {
        int winst = g_slots[ROGUE_EQUIP_WEAPON];
        if (winst >= 0 && rogue_equip_item_is_two_handed(winst))
            return -4;
    }
    g_slots[slot] = inst_index;
    /* Phase 15.2: update item equip hash chain (equip event) */
    RogueItemInstance* mut = (RogueItemInstance*) it;
    mut->equip_hash_chain = equip_mix64(mut->equip_hash_chain, ((unsigned long long) slot << 56) ^
                                                                   mut->guid ^ 0xE11AFBULL);
...
int rogue_equip_unequip(enum RogueEquipSlot slot)
{
    ...
    if (prev >= 0)
    {
        const RogueItemInstance* it = rogue_item_instance_at(prev);
        if (it)
        {
            RogueItemInstance* mut = (RogueItemInstance*) it;
            mut->equip_hash_chain = equip_mix64(
                mut->equip_hash_chain, ((unsigned long long) slot << 56) ^ mut->guid ^ 0x51CED9ULL);
        }
        rogue_stat_cache_mark_dirty();
...
int rogue_equip_set_transmog(enum RogueEquipSlot slot, int def_index)
{
    ...
    if (def_index >= 0)
    {
        ...
        if (!d)
            return -3; /* Only allow same broad category for now */
        int cat = category_for_slot(slot);
        if (d->category != cat)
            return -4;
    }
    g_transmog_defs[slot] = def_index;
    return 0;
}
int rogue_equip_get_transmog(enum RogueEquipSlot slot)
{
    if (slot < 0 || slot >= ROGUE_EQUIP__COUNT)
        return -1;
    return g_transmog_defs[slot];
}
...
int rogue_equip_repair_slot(enum RogueEquipSlot slot)
{
    if (slot < 0 || slot >= ROGUE_EQUIP__COUNT)
        return -1;
    int inst = g_slots[slot];
    if (inst < 0)
        return -2;
    const RogueItemInstance* itc = rogue_item_instance_at(inst);
    if (!itc)
        return -3;
    if (itc->durability_max <= 0)
        return 1;
    int missing = itc->durability_max - itc->durability_cur;
    if (missing <= 0)
        return 1;
    ...
    rogue_item_instance_repair_full(inst);
    return 0;
}
int rogue_equip_repair_all(void)
{
    ...
    {
        int r = rogue_equip_repair_slot((enum RogueEquipSlot) s);
        if (r == 0)
            repaired++;
    }
    return repaired;
