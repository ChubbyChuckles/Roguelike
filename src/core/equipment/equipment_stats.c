...
void rogue_equipment_apply_stat_bonuses(RoguePlayer* p){
    ...
    if(p){
        /* Idempotent application: track previously applied equipment deltas so repeated calls
           do not compound base stats and drift the fingerprint (test_equipment_phase18_stress_combo).
           We compute against a reconstructed baseline each invocation. */
        static int prev_applied_str=0, prev_applied_dex=0, prev_applied_vit=0, prev_applied_int=0;
        int base_str = p->strength - prev_applied_str; if(base_str<0) base_str=0;
        int base_dex = p->dexterity - prev_applied_dex; if(base_dex<0) base_dex=0;
        int base_vit = p->vitality - prev_applied_vit; if(base_vit<0) base_vit=0;
        int base_int = p->intelligence - prev_applied_int; if(base_int<0) base_int=0;
        /* Use a temp copy so stat cache sees original base values, not mutated player fields */
        RoguePlayer temp = *p; temp.strength = base_str; temp.dexterity = base_dex; temp.vitality = base_vit; temp.intelligence = base_int;
        rogue_stat_cache_force_update(&temp); /* recompute layers on baseline */
        /* Apply fresh deltas to player (visible legacy behavior) */
        int applied_str = g_player_stat_cache.total_strength - g_player_stat_cache.base_strength; if(applied_str<0) applied_str=0;
        int applied_dex = g_player_stat_cache.total_dexterity - g_player_stat_cache.base_dexterity; if(applied_dex<0) applied_dex=0;
        int applied_vit = g_player_stat_cache.total_vitality - g_player_stat_cache.base_vitality; if(applied_vit<0) applied_vit=0;
        int applied_int = g_player_stat_cache.total_intelligence - g_player_stat_cache.base_intelligence; if(applied_int<0) applied_int=0;
        ...
        prev_applied_str = applied_str; prev_applied_dex = applied_dex; prev_applied_vit = applied_vit; prev_applied_int = applied_int;
