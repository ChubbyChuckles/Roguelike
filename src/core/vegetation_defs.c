...
static int parse_line(char* line, int is_tree){
    ...
    if(strncmp(d->image, "../assets/", 10)==0){ size_t len = strlen(d->image); memmove(d->image, d->image+3, len-2); }
    ...
    char* look2_ctx = context; char* look2 = strtok_s(NULL, ",\r\n", &look2_ctx); if(look2){ int v2 = atoi(look2); char* look3_ctx = look2_ctx; char* look3 = strtok_s(NULL, ",\r\n", &look3_ctx); if(look3){ d->tile_x2 = (unsigned short)v1; d->tile_y2 = (unsigned short)v2; d->rarity = (unsigned short)atoi(look3); context = look3_ctx; have_rect = 1; }}
    if(!have_rect){ d->tile_x2=d->tile_x; d->tile_y2=d->tile_y; d->rarity=(unsigned short)v1; context=save_ptr; }
    if(d->rarity==0) d->rarity=1;
    if(is_tree){ char* cr = strtok_s(NULL, ",\r\n", &context); if(!cr) return 0; d->canopy_radius = (unsigned char)atoi(cr); if(d->canopy_radius==0) d->canopy_radius=1; } else d->canopy_radius=0;
