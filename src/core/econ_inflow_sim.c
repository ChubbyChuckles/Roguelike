...
int rogue_econ_inflow_baseline(int kills_per_min, double hours,
    ...
    if(!out) return 0;
    if(kills_per_min < 0 || hours <= 0.0) return 0;
    if(avg_item_drops_per_kill < 0.0 || avg_material_drops_per_kill < 0.0) return 0;
    int total_defs = rogue_item_defs_count(); if(total_defs <= 0) return 0;
    if(rogue_econ_material_catalog_count() <= 0){ /* attempt lazy build */
        rogue_econ_material_catalog_build();
    }
    /* Simplistic baseline: average value per non-material item = arithmetic mean of base value over non-material defs. */
    double sum_item_base = 0.0; int item_count = 0;
    for(int i=0;i<total_defs;i++){
        const RogueItemDef* d = rogue_item_def_at(i); if(!d) continue;
        if(d->category == ROGUE_ITEM_MATERIAL) continue;
        if(d->base_value > 0){ sum_item_base += (double)d->base_value; item_count++; }
    }
    double avg_item_base = (item_count>0)? (sum_item_base / (double)item_count) : 1.0;
    /* Average material base value: sample catalog entries. */
    double sum_mat_base = 0.0; int mat_entries = rogue_econ_material_catalog_count();
    for(int i=0;i<mat_entries;i++){
        const RogueMaterialEntry* e = rogue_econ_material_catalog_get(i); if(!e) continue; if(e->base_value > 0) sum_mat_base += (double)e->base_value; }
