 ...
 * Ordering: Base -> Relative ΔL -> Tier/Modifiers -> Adaptive Scalar -> (future) Boss/Nemesis scripts.
 */
...
void rogue_enemy_adaptive_submit_kill(float ttk_seconds); /* Enemy killed with measured time-to-kill */
void rogue_enemy_adaptive_submit_player_damage(float damage_amount, float interval_seconds); /* aggregate damage intake */
