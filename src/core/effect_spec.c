...
void rogue_effect_reset(void){
    free(g_effect_specs); g_effect_specs=NULL; g_effect_spec_count=0; g_effect_spec_cap=0;
}
...
int rogue_effect_register(const RogueEffectSpec* spec){
    ...
    if(g_effect_spec_count == g_effect_spec_cap){
        ...
        if(!ns) return -1;
        g_effect_specs = ns; g_effect_spec_cap = nc;
    }
...
const RogueEffectSpec* rogue_effect_get(int id){
    if(id < 0 || id >= g_effect_spec_count) return NULL;
