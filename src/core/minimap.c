...
static void ensure_minimap_target(int w, int h){
#ifdef ROGUE_HAVE_SDL
    if(!g_app.renderer) return; if(w<=0||h<=0) return;
    ...
    if(!g_app.minimap_tex){
        ...
        if(!g_app.minimap_tex){ ROGUE_LOG_WARN("minimap texture create %dx%d failed: %s", w,h, SDL_GetError()); return; }
...
static void redraw_minimap_if_needed(int mm_w, int mm_h, int step){
    ...
    for(int y=0; y<g_app.world_map.height; y+=step){
        for(int x=0; x<g_app.world_map.width; x+=step){
            ...
            SDL_Rect r={mx,my,1,1}; SDL_RenderFillRect(g_app.renderer,&r);
        }
    }
...
void rogue_minimap_update_and_render(int mm_max_size){
    ...
    if(mm_scale_f < 0.5f) mm_scale_f = 0.5f;
    ...
    if(mm_w>mm_max) mm_w=mm_max; if(mm_h>mm_max) mm_h=mm_max;
