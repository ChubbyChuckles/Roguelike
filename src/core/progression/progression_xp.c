 ...
 * Late levels compression: log component dampens runaway growth keeping progress asymptotic but unbounded.
 * xp_to_next(level) = base + a*level + b*level^2 + c*level^2.1 + d*log(level+1)
...
unsigned int rogue_xp_to_next_for_level(int level){
    ...
    double val = BASE + A_lin*lv + B_quad*lv*lv + C_pow*pow(lv, P_exp) + D_log*log(lv+1.0);
    if(val < 1.0) val = 1.0;
    if(val > (double)0xFFFFFFFFu) val = (double)0xFFFFFFFFu;
    return (unsigned int)(val + 0.5);
}

unsigned long long rogue_xp_total_required_for_level(int level){
    if(level <= 1) return 0ULL;
    unsigned long long total = 0ULL;
    for(int l=1; l<level; ++l){ total += (unsigned long long)rogue_xp_to_next_for_level(l); }
    return total;
}

float rogue_xp_catchup_multiplier(int player_level, int median_level){
    if(median_level < 1) median_level = 1;
    if(player_level >= median_level) return 1.0f;
