...
{
    double acc = 0.0;
    for (int i = 0; i < g_spent; i++)
    {
        acc += NODE_BASE / (1.0 + NODE_CURV * (double) i);
    }
    return acc;
}
...
double rogue_perpetual_level_scalar(int level)
{
    ...
    double x = 1.0 - exp(-(double) level / LAMBDA);
    return pow(x, ALPHA);
}
...
{ /* target zero; positive = combat slower -> slight buff; negative = too fast -> nerf */
    double adj = g_global_coeff;
    double k = 0.05;                        /* gentle proportional */
    adj *= (1.0 + (-k * median_ttk_delta)); /* if delta positive (slower), we reduce nerf (negative
                                               effect), so invert sign */
    if (adj < 0.85)
        adj = 0.85;
    else if (adj > 1.15)
        adj = 1.15;
    g_global_coeff = adj;
    return g_global_coeff;
}

double rogue_perpetual_effective_power(int level)
{
    double base = rogue_perpetual_level_scalar(level);
    double nodes = rogue_perpetual_raw_power();
    return (base + nodes) * g_global_coeff;
}
