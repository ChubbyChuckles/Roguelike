...
extern "C"
{
    ...
    /* Build progression maze from config JSON; performs procedural augmentation (optional
     * branches). Returns 1 on success. */
    ...
    int rogue_progression_maze_node_unlockable(const RogueProgressionMaze* m, int node_id,
                                               int level, int str, int dex, int intel, int vit);

    /* Compute shortest point cost between two nodes (cost_points as edge weight of destination
     * node). Returns -1 if unreachable. */
    int rogue_progression_maze_shortest_cost(const RogueProgressionMaze* m, int from_node,
                                             int to_node);

    /* Simple orphan audit: returns number of non-root nodes (node_id>0) with adj_count==0 and not
     * optional. */
...
#define ROGUE_MAZE_FLAG_KEYSTONE 0x4u
    ...
    /* Ring expansion milestones (conceptual extra outer rings unlocked at milestone levels).
     * Returns additional ring layers available beyond the base maze->base.rings. */
    int rogue_progression_ring_expansions_unlocked(int player_level);

    /* Dynamically expand the maze by appending `extra_rings` outer ring layers (Phase 7.1 full
     * implementation). Returns number of new rings actually added (0 if none). New nodes are
     * procedurally placed in a circular band.
     */
    ...
    /* Populate out_layers array (capacity >= m->base.rings) with ring radii (average distance from
     * origin) and returns ring count; returns 0 on failure. */
    int rogue_progression_maze_layers(const RogueProgressionMaze* m, float* out_layers,
                                      int max_layers);
    /* Project node index to polar coordinates (radius,theta) relative to center (computed
     * centroid). Returns 1 on success. */
    int rogue_progression_maze_project(const RogueProgressionMaze* m, int node_id,
                                       float* out_radius, float* out_theta);
    /* Generate a coarse ASCII overview (concentric approximation). Writes up to `buf_size-1` chars
     * incl NUL. Returns chars written (excluding NUL) or -1. */
    int rogue_progression_maze_ascii_overview(const RogueProgressionMaze* m, char* buffer,
                                              int buf_size, int cols, int rows);
