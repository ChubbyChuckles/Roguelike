...
static unsigned short recompute_rank(MasteryEntry* e)
{
    ...
    while (xp >= (unsigned long long) thresh)
    {
        xp -= (unsigned long long) thresh;
        rank++;
        thresh *= RANK_GROWTH;
        if (rank > 2000)
            break;
...
int rogue_mastery_init(int max_skills, int enable_decay)
{
    ...
    g_decay_enabled = enable_decay ? 1 : 0;
    g_now = 0;
    g_ring_points_cache = -1;
    return 0;
}
...
int rogue_mastery_add_xp(int skill_id, unsigned int xp, unsigned int timestamp_ms)
{
    ...
    if (!g_entries)
    {
        if (rogue_mastery_init(skill_id + 1, g_decay_enabled) < 0)
            return -1;
    }
    if (ensure_cap(skill_id) < 0)
        return -1;
    MasteryEntry* e = &g_entries[skill_id];
    if (timestamp_ms > g_now)
        g_now = timestamp_ms; /* advance clock */
    /* Scale raw xp by mastery tier bonus? For acquisition we keep linear; tiers affect usage bonus
     * not accrual */
    e->xp += xp;
...
void rogue_mastery_update(unsigned int elapsed_ms)
{
    ...
    for (int i = 0; i < g_cap; i++)
    {
        ...
        {
            floor += threshold_for_rank_int(r);
        }
        double surplus = (double) e->xp - floor;
        if (surplus <= 0.0)
            continue;
        ...
        if (remove > surplus)
            remove = (unsigned long long) surplus;
        e->xp -= remove;
        e->dirty = 1;
        mark_ring_points_dirty();
        /* Advance anchor so we don't repeatedly decay same window overly; reset last_activity to
         * grace boundary start */
...
{
    if (skill_id < 0 || !g_entries || skill_id >= g_cap)
        return 0;
    MasteryEntry* e = &g_entries[skill_id];
    if (e->dirty)
        return recompute_rank(e);
    return e->rank_cache;
}
...
unsigned long long rogue_mastery_xp_to_next(int skill_id)
{
    ...
    if (r > 0)
    {
        double floor = 0.0;
        for (int i = 0; i < r; i++)
            floor += threshold_for_rank_int(i);
        if (have > (unsigned long long) floor)
            have -= (unsigned long long) floor;
        else
            have = 0;
    }
    if (have >= needed)
...
int rogue_mastery_minor_ring_points(void)
{
    if (!g_entries)
        return 0;
    if (g_ring_points_cache >= 0)
        return g_ring_points_cache;
    int count = 0;
    for (int i = 0; i < g_cap; i++)
    {
        if (g_entries[i].xp)
        {
            if (rogue_mastery_rank(i) >= RING_UNLOCK_RANK)
                count++;
        }
    }
    g_ring_points_cache = count;
    return count;
}
...
{
    if (xp <= 0.0)
        return (double) rogue_mastery_xp(skill_id);
    rogue_mastery_add_xp(skill_id, (unsigned int) (xp + 0.5), g_now);
    return (double) rogue_mastery_xp(skill_id);
}
...
{
    return threshold_for_rank_int(rank);
}
