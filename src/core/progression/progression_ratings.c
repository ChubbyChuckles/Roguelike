...
typedef struct RatingBandCurve
{
    float scale;
    float k;
} RatingBandCurve;
typedef struct RatingCurveSet
{
    RatingBandCurve bands[5];
    float hard_cap;
} RatingCurveSet;
...
static const RatingCurveSet g_curves[3] = {
    ...
    {{{0.055f, 200.f}, {0.034f, 360.f}, {0.020f, 700.f}, {0.011f, 1300.f}, {0.0075f, 2200.f}},
     65.f}};
...
static float curve_apply(enum RogueRatingType type, int rating)
{
    ...
    if (type < 0 || type > 2)
        type = 0;
    const RatingCurveSet* set = &g_curves[type];
    float total = 0.f;
    for (int b = 0; b < g_break_count; b++)
    {
        int start = g_breaks[b];
        if (rating < start)
            break;
        int next = (b + 1 < g_break_count) ? g_breaks[b + 1] : -1;
        int band_rating;
        if (next < 0)
            band_rating = rating - start;
        else
        {
            int span = next - start;
            band_rating = (rating - start >= span) ? span : (rating - start);
        }
        if (band_rating <= 0)
            continue;
        const RatingBandCurve* bc = &set->bands[b];
        float dr = (float) band_rating;
        float inc = (dr * bc->scale) / (1.f + dr / bc->k);
        total += inc;
    }
    if (total > set->hard_cap)
        total = set->hard_cap;
    return total;
}
...
{
    return curve_apply(type, rating);
}
float rogue_rating_apply_chain(enum RogueRatingType type, float base_flat_percent, int rating,
                               float mult_modifier_percent)
{
    float eff = base_flat_percent + curve_apply(type, rating);
    if (mult_modifier_percent != 0.f)
        eff *= (1.f + mult_modifier_percent / 100.f);
    return eff;
}
