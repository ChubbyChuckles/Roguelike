...
/* Initialize mastery system for up to max_skills (will grow dynamically if exceeded when max_skills>0).
 * enable_decay: 1 enables inactivity decay logic. */
int rogue_mastery_init(int max_skills, int enable_decay);
void rogue_mastery_shutdown(void);

/* Add mastery XP for a skill (skill usage event). `xp` is logical units (not yet scaled) and timestamp_ms
 * is the global progression clock (monotonic). Returns 0 on success, <0 on failure. */
int rogue_mastery_add_xp(int skill_id, unsigned int xp, unsigned int timestamp_ms);

/* Advance time (call each frame or tick) with elapsed milliseconds to process decay bookkeeping. */
...
/* Mastery bonus scalar (>=1.0) applied to allowed effect domains (damage, resource efficiency, buildup).
 * Tier mapping is intentionally coarse to ease balancing tweaks. */
