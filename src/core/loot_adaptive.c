...
void rogue_adaptive_reset(void){ memset(g_rarity_counts,0,sizeof g_rarity_counts); memset(g_category_counts,0,sizeof g_category_counts); for(int i=0;i<5;i++){ g_rarity_factors[i]=1.0f; } for(int i=0;i<ROGUE_ITEM__COUNT;i++){ g_category_factors[i]=1.0f; } }
