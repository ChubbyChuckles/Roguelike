...
static RogueEvent* create_event(RogueEventTypeId type_id, const RogueEventPayload* payload,
                               RogueEventPriority priority, uint32_t source_system_id, const char* source_name);
...
bool rogue_event_bus_init(const RogueEventBusConfig* config) {
    ...
    /* Register core event types (Phase 1.2) */
    ...
    ROGUE_LOG_INFO("Event bus '%s' initialized (Strategy: %d, Max Queue: %u, Threads: %u)",
                   config->name,
                   (int)config->processing_strategy,
...
bool rogue_event_publish(RogueEventTypeId type_id,
    ...
    return rogue_event_publish_with_deadline(type_id, payload, priority, 0,
...
bool rogue_event_publish_with_deadline(RogueEventTypeId type_id,
    ...
    ROGUE_LOG_DEBUG("Published event type %u from system %u (Queue depth: %u)",
                    type_id, source_system_id, g_event_bus.total_queue_size);
...
bool rogue_event_publish_batch(const RogueEvent* events, uint32_t event_count) {
    ...
    for (uint32_t i = 0; i < event_count; i++) {
        ...
        RogueEvent* event = create_event(source_event->type_id, &source_event->payload,
                                       source_event->priority, source_event->source_system_id,
    ...
    ROGUE_LOG_DEBUG("Published batch: %u/%u events queued successfully", 
                    successfully_queued, event_count);
...
uint32_t rogue_event_subscribe(RogueEventTypeId type_id,
    ...
    return rogue_event_subscribe_conditional(type_id, callback, user_data, NULL, subscriber_system_id);
}
...
uint32_t rogue_event_subscribe_rate_limited(RogueEventTypeId type_id,
                                           ...
                                           uint32_t subscriber_system_id) {
    uint32_t subscription_id = rogue_event_subscribe_conditional(type_id, callback, user_data,
                                                               NULL, subscriber_system_id);
    ...
    if (subscription_id > 0) {
        ...
        ROGUE_LOG_DEBUG("Set rate limit %u/sec for subscription %u", 
                        rate_limit_per_second, subscription_id);
    }
...
uint32_t rogue_event_process_sync(uint32_t max_events, uint32_t time_budget_us) {
    ...
    for (int priority = 0; priority < ROGUE_EVENT_PRIORITY_COUNT && processed_count < max_events; priority++) {
        while (g_event_bus.event_queue_heads[priority] && processed_count < max_events) {
            ...
            if (event->deadline_us > 0 && current_time > event->deadline_us) {
                ROGUE_LOG_WARN("Event type %u missed deadline by %llu microseconds",
                               event->type_id, 
            ...
            if (event_processed) {
                ...
                if (event->retry_count < event->max_retries) {
                    ...
                    ROGUE_LOG_DEBUG("Re-queued event type %u for retry (%u/%u)",
                                    event->type_id, event->retry_count, event->max_retries);
                } else {
                    ROGUE_LOG_WARN("Event type %u failed after %u retries", 
                                   event->type_id, event->max_retries);
                    free_event(event);
                    g_event_bus.stats.events_failed++;
                }
                continue;
    ...
    if (processed_count > 0) {
        ROGUE_LOG_DEBUG("Processed %u events in %llu microseconds",
                        processed_count,
...
bool rogue_event_register_type(RogueEventTypeId type_id, const char* type_name) {
    ...
    if (g_event_type_registered[type_id]) {
        ROGUE_LOG_WARN("Event type %u already registered as '%s'", 
                       type_id, g_event_type_names[type_id]);
        return true;
    }
    
#ifdef _MSC_VER
    strncpy_s(g_event_type_names[type_id], sizeof(g_event_type_names[type_id]), type_name, _TRUNCATE);
#else
...
bool rogue_event_bus_update_config(const RogueEventBusConfig* new_config) {
    ...
    g_event_bus.config.max_processing_time_per_frame_us = new_config->max_processing_time_per_frame_us;
...
static void record_event_for_replay(const RogueEvent* event) {
    ...
    *replay_event = *event;
    ...
    g_event_bus.replay_history_index = (g_event_bus.replay_history_index + 1) % 
                                      g_event_bus.config.replay_history_depth;
...
static void update_statistics_on_process(const RogueEvent* event, uint64_t processing_time_us) {
    ...
    if (g_event_bus.stats.events_processed > 0) {
        g_event_bus.stats.average_latency_us = 
            (g_event_bus.stats.average_latency_us * (double)(g_event_bus.stats.events_processed - 1) + (double)latency) /
