...
static RogueTexture* veg_get_texture(const char* path){
    ...
    if(g_sheet_tex_count >= (int)(sizeof g_sheet_textures/sizeof g_sheet_textures[0])) return NULL;
    ...
    g_sheet_tex_count++; return &slot->tex;
}

static void veg_queue_instance(RogueVegetationInstance* v){
    RogueVegetationDef* d=&g_defs[v->def_index]; RogueTexture* tex = veg_get_texture(d->image); if(!tex || !tex->handle) return;
    int tiles_w = (int)(d->tile_x2 - d->tile_x + 1); int tiles_h = (int)(d->tile_y2 - d->tile_y + 1);
    int sprite_px_w = tiles_w * g_app.tile_size; int sprite_px_h = tiles_h * g_app.tile_size;
    if(g_veg_sprite_pool_used >= ROGUE_MAX_VEG_INSTANCES) return;
    RogueSprite* temp=&g_veg_sprite_pool[g_veg_sprite_pool_used++]; memset(temp,0,sizeof *temp);
    ...
    int world_center_px_x = (int)(v->x * g_app.tile_size - g_app.cam_x); int world_center_px_y = (int)(v->y * g_app.tile_size - g_app.cam_y);
    int dst_x = world_center_px_x - sprite_px_w/2; int dst_y = world_center_px_y - sprite_px_h; int y_base = world_center_px_y;
...
#ifdef ROGUE_HAVE_SDL
    if(!g_app.renderer) return; g_veg_sprite_pool_used = 0; for(int i=0;i<g_instance_count;i++) veg_queue_instance(&g_instances[i]);
#endif
}
