...
void rogue_enemy_render(void){
    ...
    for(int i=0;i<ROGUE_MAX_ENEMIES;i++) if(g_app.enemies[i].alive){
        ...
        if(spr && spr->tex && spr->tex->handle && spr->sw){
            ...
            int dst_x = ex - spr->sw/2; int dst_y = ey - spr->sh/2; int y_base = ey + spr->sh/2; int flip_flag = (e->facing==1)?1:0;
            rogue_scene_drawlist_push_sprite(spr,dst_x,dst_y,y_base,flip_flag,mr,mg,mb,ma);
        } else {
