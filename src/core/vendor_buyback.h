...
int rogue_vendor_buyback_record(int vendor_def_index, unsigned long long item_guid, int item_def_index,
                                int rarity, int category, int condition_pct, int price, unsigned int now_ms);

/* Enumerate active buyback entries for a vendor; returns count written. */
int rogue_vendor_buyback_list(int vendor_def_index, RogueVendorBuybackEntry* out, int max_out, unsigned int now_ms);

/* Attempt to buy back an item (player purchasing it). Returns price to pay or -1 if unavailable. */
int rogue_vendor_buyback_purchase(int vendor_def_index, unsigned long long item_guid, unsigned int now_ms);
...
/* Anti-duplicate GUID lineage check: returns 1 if guid recently seen in buyback set (possible exploit), 0 otherwise. */
