...
bool json_array_add(RogueJsonValue* array, RogueJsonValue* item) {
    ...
    RogueJsonValue** new_items = realloc(array->data.array_value.items, 
                                       new_count * sizeof(RogueJsonValue*));
    if (!new_items) {
        return false;
    }
...
bool json_object_set(RogueJsonValue* object, const char* key, RogueJsonValue* value) {
    ...
    RogueJsonValue** new_values = realloc(object->data.object_value.values, 
                                        new_count * sizeof(RogueJsonValue*));
    if (!new_values) {
        free(new_keys);
        return false;
