                    ...
                    if(larr){
                        ...
                        if(larr_end){
                            ...
                            while(lc<larr_end){
                                ...
                                int sflag=(strcmp(side,"right")==0); int hflag=(mirror[0]=='v'||mirror[0]=='V'); /* legacy 'v' value now treated as horizontal */ unsigned int tint=rd_mood_tint(mood);
                                char meta[320]; if(avatar_path[0]) snprintf(meta,sizeof meta,"%s;S=%d;V=%d;TR=%u;TG=%u;TB=%u",avatar_path,sflag,hflag,(tint>>16)&255,(tint>>8)&255,tint&255); else meta[0]='\0';
...
void rogue_dialogue_render_runtime(void){
    ...
    if(avatar_h_mirror){ SDL_RendererFlip flip = SDL_FLIP_HORIZONTAL; SDL_RenderCopyEx(g_internal_sdl_renderer_ref,av->handle,NULL,&dst,0,NULL,flip); }
