...
typedef enum RogueUITimelinePolicy { ROGUE_UI_TIMELINE_REPLACE=0, ROGUE_UI_TIMELINE_IGNORE=1, ROGUE_UI_TIMELINE_APPEND=2 } RogueUITimelinePolicy;
...
void rogue_ui_timeline_play(struct RogueUIContext* ctx, uint32_t id_hash,
                             const RogueUITimelineKeyframe* kfs, int count,
                             float duration_ms, RogueUITimelinePolicy policy);

void rogue_ui_timeline_step(struct RogueUIContext* ctx, double dt_ms);
float rogue_ui_timeline_scale(const struct RogueUIContext* ctx, uint32_t id_hash, int* active_out);
float rogue_ui_timeline_alpha(const struct RogueUIContext* ctx, uint32_t id_hash, int* active_out);
